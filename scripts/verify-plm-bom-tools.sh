...
create_part() {
  ...
  curl -sS -X POST "$PLM_BASE_URL/api/v1/aml/apply" "${HEADERS[@]}" \
    -H 'content-type: application/json' \
    -d "{\"type\":\"$PLM_ITEM_TYPE\",\"action\":\"add\",\"properties\":{\"item_number\":\"$item_number\",\"name\":\"$name\"}}" \
...
delete_bom_child() {
  ...
  curl -sS -X DELETE "$PLM_BASE_URL/api/v1/bom/$parent_id/children/$child_id" "${HEADERS[@]}" >/dev/null || true
}
...
delete_substitute() {
  ...
  curl -sS -X DELETE "$PLM_BASE_URL/api/v1/bom/$bom_line_id/substitutes/$substitute_id" "${HEADERS[@]}" >/dev/null || true
}
...
python3 - <<'PY' "$resp_where" "$resp_compare" "$resp_subs" "$report_json" "$PARENT_A" "$PARENT_B" "$CHILD_X" "$CHILD_Y" "$CHILD_Z" "$SUB_PART" "$BOM_LINE_X" "$SUB_REL_ID"
import json,sys
where_raw, compare_raw, subs_raw, out = sys.argv[1:5]
parent_a, parent_b, child_x, child_y, child_z, sub_part, bom_line, sub_rel = sys.argv[5:13]
def load(raw):
    try:
...
python3 - <<'PY' "$report_json" "$report_md" "$API_BASE" "$PLM_BASE_URL" "$PARENT_A" "$PARENT_B" "$CHILD_X" "$CHILD_Y" "$CHILD_Z" "$SUB_PART" "$BOM_LINE_X" "$SUB_REL_ID"
import json,sys
...
lines = [
    ...
    f"- Substitute Relationship ID: {sub_rel}",
    "",
    "## Results",
...
if [[ "$PLM_CLEANUP" == "true" ]]; then
  delete_substitute "$BOM_LINE_X" "$SUB_REL_ID"
