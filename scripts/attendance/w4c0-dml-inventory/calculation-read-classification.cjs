...
function entry(relPath, enclosingSymbol, table, count, posture, role) {
  return Object.freeze({ relPath, enclosingSymbol, table, count, posture, role })
}
...
const ATTENDANCE_CALCULATION_READ_CLASSIFICATIONS = Object.freeze([
  ...
  entry('packages/core-backend/src/attendance/w4c3c-active-current.ts', '(module-scope)', 'attendance_record_calculations', 2, 'current', 'active_current_projection'),
  ...
  entry('packages/core-backend/src/services/AttendanceW4CalculationDetail.ts', 'readAuthoritativeTraceCalculation', 'attendance_record_calculations', 1, 'current', 'immutable_current_trace'),
