...
echo "🚀 开始部署 metasheet2..."

# 更新系统并安装必要软件
echo "📦 安装系统依赖..."
sudo apt-get update
sudo apt-get install -y docker.io docker-compose-plugin git curl openssl
...
# GitHub 凭据输入
echo "🔐 请输入 GitHub 凭据以访问私有镜像..."
read -p "GitHub username (for GHCR): " GH_USER
read -s -p "GHCR PAT (read:packages): " GHCR_PAT
echo
