...
PY
  ...
  PORT="${PORT}" NODE_ENV=development DATABASE_URL="${SMOKE_DATABASE_URL}" DISABLE_WORKFLOW=true DISABLE_EVENT_BUS=true SKIP_PLUGINS=true \
...
API_BASE="${API_BASE}" WEB_BASE="${WEB_BASE}" OUTPUT_PATH="${OUTPUT_DIR}/smoke-report.json" pnpm verify:smoke
