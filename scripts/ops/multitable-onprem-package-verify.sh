...
function verify_integration_plugin_runtime_dependencies() {
  ...
  local k3_sqlserver_smoke="${root}/plugins/plugin-integration-core/scripts/smoke-k3-sqlserver-executor.cjs"
  ...
  search_fixed_string '"smoke:k3-sqlserver-executor": "node scripts/smoke-k3-sqlserver-executor.cjs"' "$plugin_package_json" || die "plugin-integration-core package.json must expose the K3 SQL Server smoke command"
  search_fixed_string 'K3_MSSQL_LEGACY_TLS' "$k3_sqlserver_smoke" || die "K3 SQL Server smoke script must be packaged and expose legacy TLS knobs"
  search_fixed_string 'pnpm --filter @metasheet/core-backend smoke:sqlserver' "$c5_sqlserver_smoke_runbook" || die "C5 SQL Server smoke runbook must document the generic SQL Server smoke command"
  search_fixed_string 'pnpm --filter plugin-integration-core smoke:k3-sqlserver-executor' "$c5_sqlserver_smoke_runbook" || die "C5 SQL Server smoke runbook must document the K3 SQL Server smoke command"
  search_fixed_string 'No write boundary is crossed' "$c5_sqlserver_smoke_runbook" || die "C5 SQL Server smoke runbook must keep the no-write boundary"
...
function verify_generic_integration_workbench_contract() {
  ...
  local material_only_gate_design="${root}/docs/development/integration-k3wise-material-only-gate-checker-design-20260526.md"
  local material_only_gate_verification="${root}/docs/development/integration-k3wise-material-only-gate-checker-verification-20260526.md"
  local process_control_plane="${root}/docs/operations/integration-k3wise-material-only-process-control-plane-20260527.md"
  ...
  search_fixed_string 'material-only' "$live_gate_package" || die "live GATE package runbook must document the Material-only sub-acceptance lane (#1792)"
  search_fixed_string 'Material-only lane' "$process_control_plane" || die "process control plane must define the Material-only lane"
  search_fixed_string 'Full / customer-trial lane' "$process_control_plane" || die "process control plane must define the Full / customer-trial lane (distinct terminal states)"
  search_fixed_string 'Blocked-reason taxonomy' "$process_control_plane" || die "process control plane must define the blocked-reason taxonomy mapping"
  search_fixed_string 'independently confirms each gate artifact' "$process_control_plane" || die "process control plane must define the Reviewer responsibility"
  search_fixed_string 'does not import workflow-job-contract' "$process_control_plane" || die "process control plane must keep the no-workflow-job-contract / no-runtime boundary"
  search_fixed_string 'materialOnlySafety' "$gate_contract_checker" || die "GATE contract checker must validate the Material-only safety answers (scope/BOM/Save-only/autoSubmit/autoAudit/previewFields)"
  search_fixed_string '--init-template' "$install_txt" || die "INSTALL.txt must document the GATE contract template initializer"
  search_fixed_string 'README-CUSTOMER-HANDOFF.zh.md' "$install_txt" || die "INSTALL.txt must document the generated customer handoff README"
  ...
  search_fixed_string 'README-CUSTOMER-HANDOFF.zh.md' "$webapi_read_manifest" || die "WebAPI read/list manifest must mention the generated customer handoff README"
  ...
  search_fixed_string 'README-CUSTOMER-HANDOFF.zh.md' "$relationship_mapping_manifest" || die "relationship mapping manifest must mention the generated customer handoff README"
...
function verify_bridge_agent_tooling_contract() {
  ...
  search_fixed_string 'INVALID_FILTERS' "$readonly_script" || die "readonly Bridge Agent must reject invalid structured filters"
  search_fixed_string 'filter field is not allowlisted' "$readonly_script" || die "readonly Bridge Agent filters must be object-field allowlisted"
  search_fixed_string 'filter value must be a primitive equality value' "$readonly_script" || die "readonly Bridge Agent filters must stay primitive equality-only"
  search_fixed_string 'AddWithValue' "$readonly_script" || die "readonly Bridge Agent filters must bind values as SQL parameters"
  search_fixed_string 'filtersApplied' "$readonly_script" || die "readonly Bridge Agent query responses must expose whether structured filters were applied"
  search_fixed_string 'SELECT TOP $Limit' "$readonly_script" || die "readonly Bridge Agent must build bounded SELECT TOP queries"
  search_fixed_string ' WHERE ' "$readonly_script" || die "readonly Bridge Agent must build filtered SELECT queries without accepting raw SQL"
