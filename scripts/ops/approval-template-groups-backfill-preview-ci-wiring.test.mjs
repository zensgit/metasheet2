...
// W7 preview unit (`previewApprovalTemplateGroupBackfill`, `src/routes/approvals.ts`). CI
// two-point wiring contract (no DB): the preview suite proves the design's §5.2 "preview never
// writes" claim against real Postgres — meaningless without a DB. It needs BOTH (1) the
// vitest.config.ts exclude entry (so the no-DB job cannot skip-green it) AND (2) the
// plugin-tests.yml directory real-DB whole-file step (so a real-DB run actually names it).
// Removing either point makes the no-write proof silently never execute while exact-head CI stays
// green. This source-level guard (no DB) reddens if either point is dropped. It runs in the no-DB
// `test` job, so it gates every PR. Same per-file dedicated-guard convention as the sibling
// schema suite's own guard (`approval-template-groups-backfill-schema-ci-wiring.test.mjs`) — the
// phase 1 files' disclosed bash-`:?`-only residual (§13.6 P3-2) is NOT repeated here or there.
const __dirname = dirname(fileURLToPath(import.meta.url))
const repoRoot = join(__dirname, '..', '..')
const FILE = 'tests/integration/approval-template-groups-backfill-preview.db.test.ts'
...
test('vitest.config.ts excludes the A3 backfill-preview suite from the no-DB job', () => {
...
test('plugin-tests.yml runs the A3 backfill-preview suite as a whole file in the directory real-DB step (id: approval-real-db-integration)', () => {
...
test('the A3 backfill-preview suite file exists on disk', () => {
