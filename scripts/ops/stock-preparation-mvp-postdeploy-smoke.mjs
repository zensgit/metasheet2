...
// discipline).
//
// Values-free output discipline: this script prints ONLY statuses, booleans, counts, modes, and coarse
// enum/error codes. It never prints a drawing number, unit symbol, quantity, path, ERP identifier,
// exception message, hostname, or token. Internal MetaSheet handles (snapshotBatchId / mappingId /
// exceptionId hashes) are values-free by construction and may appear in the artifact checks.json.
//
...
async function main() {
  ...
  S.readinessTableCount = readiness1.body?.data?.evidence?.tableCount ?? -1
  ...
  S.ensureCreatedCount = ensure.body?.data?.evidence?.createdCount ?? -1
  ...
  S.optionsSyncedFieldCount = optionsSync.body?.data?.evidence?.syncedFieldCount ?? -1
  S.optionsSkippedFieldCount = optionsSync.body?.data?.evidence?.skippedFieldCount ?? -1
  ...
  S.planSnapshotStatus = planData.snapshotBatch?.snapshotStatus || ''
  S.planRunStatus = planData.syncRun?.status || ''
  S.planLineCount = planData.evidence?.mapping?.result?.lines ?? -1
  ...
  S.persistMode = persistData.mode || ''
  S.persistCreated = JSON.stringify(persistData.created || {})
  ...
  S.persistReplayMode = replayData.mode || ''
  ...
  S.batchCount = batchData.batchCount ?? -1
  S.batchIncomplete = ourBatch ? ourBatch.incomplete === true : null
  S.batchLineCount = ourBatch ? ourBatch.lineCount : -1
  ...
  S.diffAdded = diffData.changeCounts?.added ?? -1
  S.diffBlockingExceptions = diffData.blockingExceptionCount ?? -1
  ...
  S.diffRowCount = diffRowsData.rowCount ?? -1
  ...
  S.candidatesSync1Created = sync1Data.created?.mappings ?? -1
  ...
  S.mappingConfirmMode = confirmData.mode || ''
  ...
  S.mappingConfirmReplayMode = confirmReplay.body?.data?.mode || ''
  ...
  S.candidatesSync2Mode = sync2Data.mode || ''
  must('re-sync after confirm -> 200 skipped_existing (0 created, 2 skipped)',
    ...
    `http=${sync2.status} mode=${S.candidatesSync2Mode} skippedExisting=${sync2Data.skipped?.existing}`)
  ...
  S.unitCandidateRowCount = unitData.rowCount ?? -1
  ...
  S.unitConfirmMode = unitConfirmData.mode || ''
  ...
  S.unitConfirmReplayMode = unitConfirmReplay.body?.data?.mode || ''
  ...
  S.run1Status = run1Data.status || ''
  S.run1Ready = run1Data.ready === true
  S.run1UnresolvedBlocking = run1Data.unresolvedBlockingExceptionCount ?? -1
  ...
  S.exceptions1RowCount = excData1.rowCount ?? -1
  S.exceptions1UnresolvedBlocking = excData1.unresolvedBlockingCount ?? -1
  ...
  S.probeMixedBulkCode = mixedBulk.body?.error?.code || ''
  ...
  S.probeBadActionCode = badAction.body?.error?.code || ''
  ...
  S.resolveSingleMode = resolveSingle.body?.data?.mode || ''
  ...
  S.resolveBulkResolved = resolveBulkData.resolved ?? -1
  ...
  S.run2Status = run2Data.status || ''
  S.run2Ready = run2Data.ready === true
  S.run2UnresolvedBlocking = run2Data.unresolvedBlockingExceptionCount ?? -1
  ...
  must('generation re-run -> 200: resolutions survive create-only re-run (0 unresolved, 2 skipped)',
    ...
    `http=${run2.status} unresolved=${S.run2UnresolvedBlocking} skipped=${run2Data.skipped?.exceptions}`)
  ...
  S.exceptions2ResolvedCount = excData2.byStatus?.resolved ?? -1
  S.exceptions2UnresolvedBlocking = excData2.unresolvedBlockingCount ?? -1
  ...
  S.prepLineRowCount = prepLines.body?.data?.rowCount ?? -1
  ...
  S.probeBadPolicyCode = badPolicy.body?.error?.code || ''
  ...
  S.probeConfirmedByCode = bodyConfirmedBy.body?.error?.code || ''
  S.probeConfirmedByField = bodyConfirmedBy.body?.error?.details?.field || ''
  ...
  S.probeGhostBaseCode = ghostBase.body?.error?.code || ''
  ...
  S.probeGhostCurrentCode = ghostCurrent.body?.error?.code || ''
  ...
  for (const mappingId of [confirmedMappingId, ...createdCandidateIds]) {
    ...
    must('mapping retire -> 200 retired', retire.ok && retire.body?.data?.mode === 'retired', `http=${retire.status} mode=${retire.body?.data?.mode || '-'}`)
  }
  ...
  S.unitRetireMode = unitRetire.body?.data?.mode || ''
  ...
  const sentinels = [...fixture.sentinels, ...ENGINE_MESSAGE_SENTINELS]
