...
// batch DDL. CI two-point wiring contract (no DB): the schema suite proves the design-gate's
// changesRequired #4 FK fix (`atgbbl_link_fk` CASCADE) is load-bearing against real Postgres —
// meaningless without a DB. It needs BOTH (1) the vitest.config.ts exclude entry (so the no-DB
// job cannot skip-green it) AND (2) the plugin-tests.yml directory real-DB whole-file step (so a
// real-DB run actually names it). Removing either point makes the whole DDL-correctness proof
// silently never execute while exact-head CI stays green. This source-level guard (no DB) reddens
// if either point is dropped. It runs in the no-DB `test` job, so it gates every PR. Unlike the
// two phase 1 template-groups files (a disclosed residual — protected only by the step's bash
// `:?`, see the phase 2 design doc §13.6 citing P3-2), this file has its own dedicated guard.
const __dirname = dirname(fileURLToPath(import.meta.url))
const repoRoot = join(__dirname, '..', '..')
const FILE = 'tests/integration/approval-template-groups-backfill-schema.db.test.ts'
...
test('the A3 backfill-schema suite file exists on disk', () => {
