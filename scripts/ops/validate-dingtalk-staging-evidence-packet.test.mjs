...
function writeMobileSignoffOutput(signoffDir, overrides = {}) {
  ...
  writeFileSync(path.join(signoffDir, 'summary.md'), '# Mobile Signoff Summary\n', 'utf8')
...
function addMobileSignoffToPacket(packetDir, overrides = {}) {
  const destination = overrides.destination ?? 'mobile-signoff/01-mobile-signoff-compiled'
  const signoffDir = path.join(packetDir, destination)
  writeMobileSignoffOutput(signoffDir, overrides)

  const manifestPath = path.join(packetDir, 'manifest.json')
  const manifest = JSON.parse(readFileSync(manifestPath, 'utf8'))
  manifest.requireMobileSignoffPass = overrides.requireMobileSignoffPass ?? true
  manifest.includedMobileSignoff = [
    {
      source: 'output/dingtalk-public-form-mobile-signoff/142-compiled',
      destination,
      mobileSignoffStatus: {
        status: 'pass',
        summary: 'summary.json',
        redactedEvidence: 'mobile-signoff.redacted.json',
        strict: true,
        overallStatus: 'pass',
        requiredChecks: mobileSignoffRequiredCheckIds.length,
        ...overrides.mobileSignoffStatus,
      },
    },
  ]
...
test('validate-dingtalk-staging-evidence-packet accepts a final gated packet and writes report', () => {
  ...
  try {
    ...
    assert.equal(report.includedMobileSignoffCount, 0)
...
test('validate-dingtalk-staging-evidence-packet accepts final mobile signoff evidence', () => {
  const tmpDir = makeTmpDir()
  const packetDir = path.join(tmpDir, 'packet')
  const reportPath = path.join(tmpDir, 'publish-check.json')
  ...
  try {
    writePacket(packetDir)
    addMobileSignoffToPacket(packetDir)
...
test('validate-dingtalk-staging-evidence-packet rejects non-strict mobile signoff evidence', () => {
  const tmpDir = makeTmpDir()
  const packetDir = path.join(tmpDir, 'packet')
  ...
  try {
    ...
    assert.match(result.stderr, /includedMobileSignoff\[0\]\.mobileSignoffStatus\.strict is not true/)
    assert.match(result.stderr, /includedMobileSignoff\[0\]\/summary\.json status is not pass/)
    assert.match(result.stderr, /includedMobileSignoff\[0\]\/summary\.json errors is not empty/)
  } finally {
    rmSync(tmpDir, { recursive: true, force: true })
  }
})

test('validate-dingtalk-staging-evidence-packet rejects raw mobile signoff json in packet', () => {
  const tmpDir = makeTmpDir()
  const packetDir = path.join(tmpDir, 'packet')

  try {
    writePacket(packetDir)
    const signoffDir = addMobileSignoffToPacket(packetDir)
    writeFileSync(path.join(signoffDir, 'mobile-signoff.json'), '{"raw":true}\n', 'utf8')

    const result = runValidator(['--packet-dir', packetDir])

    assert.equal(result.status, 1)
    assert.match(result.stderr, /mobile-signoff\.json must not be included/)
  } finally {
    rmSync(tmpDir, { recursive: true, force: true })
  }
})

test('validate-dingtalk-staging-evidence-packet rejects missing required mobile signoff entry', () => {
  const tmpDir = makeTmpDir()
  const packetDir = path.join(tmpDir, 'packet')
  ...
  try {
    ...
    assert.equal(result.status, 1)
    assert.match(result.stderr, /manifest requireMobileSignoffPass is true but includedMobileSignoff is empty/)
  } finally {
    rmSync(tmpDir, { recursive: true, force: true })
  }
})

test('validate-dingtalk-staging-evidence-packet rejects unregistered mobile signoff entries', () => {
  const tmpDir = makeTmpDir()
  const packetDir = path.join(tmpDir, 'packet')
  ...
  try {
    writePacket(packetDir)
    addMobileSignoffToPacket(packetDir)
    mkdirSync(path.join(packetDir, 'mobile-signoff/99-old-signoff'), { recursive: true })
    writeFileSync(path.join(packetDir, 'mobile-signoff/99-old-signoff/stale.txt'), 'stale\n', 'utf8')

    const result = runValidator(['--packet-dir', packetDir])

    assert.equal(result.status, 1)
    assert.match(result.stderr, /mobile-signoff\/99-old-signoff is not registered in manifest includedMobileSignoff/)
  } finally {
    rmSync(tmpDir, { recursive: true, force: true })
  }
})
