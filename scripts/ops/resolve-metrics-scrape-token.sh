...
if [[ -z "${DEPLOY_KNOWN_HOSTS:-}" ]]; then
  warn_or_fail "DEPLOY_KNOWN_HOSTS is required to pin the deploy-host identity for the metrics token fallback"
fi
...
tmp_known_hosts="$(mktemp "${TMPDIR:-/tmp}/metrics-scrape-known-hosts.XXXXXX")"
