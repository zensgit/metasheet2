...
// W9 rollback unit (`rollbackApprovalTemplateGroupBackfillBatch`,
// `src/services/ApprovalTemplateGroupService.ts`). CI two-point wiring contract (no DB): the
// rollback suite proves the design's §4.2 (set-based optimistic-concurrency token match) and §4.3
// (created_new / remaining=0 archive precision) against real Postgres — meaningless without a DB.
// It needs BOTH (1) the vitest.config.ts exclude entry (so the no-DB job cannot skip-green it)
// AND (2) the plugin-tests.yml directory real-DB whole-file step (so a real-DB run actually names
// it). Removing either point makes those proofs silently never execute while exact-head CI stays
// green. This source-level guard (no DB) reddens if either point is dropped. It runs in the no-DB
// `test` job, so it gates every PR. Same per-file dedicated-guard convention as the three sibling
// backfill suites' own guards (schema / preview / execute).
const __dirname = dirname(fileURLToPath(import.meta.url))
const repoRoot = join(__dirname, '..', '..')
const FILE = 'tests/integration/approval-template-groups-backfill-rollback.db.test.ts'
...
test('vitest.config.ts excludes the A3 backfill-rollback suite from the no-DB job', () => {
...
test('plugin-tests.yml runs the A3 backfill-rollback suite as a whole file in the directory real-DB step (id: approval-real-db-integration)', () => {
...
test('the A3 backfill-rollback suite file exists on disk', () => {
