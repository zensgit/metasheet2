...
| `evidence-onsite-c4-c9-template.json` | Operator-facing on-site C4-C9 evidence worksheet. It starts as `PARTIAL`, carries the required evidence slots, and becomes `PASS` only after the live PoC run IDs, K3 response ids, writeback rows, replay proof, rollback proof, and customer confirmation are filled. |
