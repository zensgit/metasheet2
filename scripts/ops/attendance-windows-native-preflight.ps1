...
$resolvedSourceSha = Assert-WindowsNativeExactSourceSha `
...
$allowedAttendanceEnv = @(
  ...
  'ATTENDANCE_WINDOWS_NATIVE_EXPECTED_SOURCE_SHA'
)
...
Write-Host "  sourceSha: $resolvedSourceSha"
Write-Host "  node: $nodeVersion"
Write-Host "  pnpm: $pnpmVersion"
Write-Host "  postgres: $($databaseEndpoint.HostName):$($databaseEndpoint.Port)/metasheet_windows_qa"
if ($RequireRedis) {
  $redisEndpoint = Resolve-WindowsNativeRedisEndpoint -EnvValues $envValues
...
Write-Host '  boundary: Draft/HOLD internal synthetic QA only; no deployment/staging authorization'
