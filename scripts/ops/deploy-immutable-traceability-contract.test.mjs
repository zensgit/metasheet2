...
test('docker-build workflow deploys exact commit images and verifies served backend/web commits', () => {
  ...
  assertContains(raw, 'curl -fsS http://127.0.0.1:8900/health', 'backend version probe')
  assertContains(raw, 'curl -fsS http://127.0.0.1:8081/build-info.json', 'web version probe')
...
test('manual production deploy script rejects mutable tags and verifies served commits', () => {
  ...
  assertContains(raw, 'curl -fsS http://127.0.0.1:8900/health', 'manual deploy script')
  assertContains(raw, 'curl -fsS http://127.0.0.1:8081/build-info.json', 'manual deploy script')
