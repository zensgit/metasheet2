...
REMOTE_WORK_ROOT="${REMOTE_WORK_ROOT:-/home/mainuser/land-runs}"

die() {
  echo "[land-remote-git-slice] ERROR: $*" >&2
  exit 1
}
...
usage() {
  cat <<'EOF'
Usage:
  bash scripts/ops/land-remote-git-slice.sh [options]
...
Options:
  ...
  --submit-manifest <path>    Local submit manifest. Default: output/remote-git-slice-submissions/<slice>/submit/artifacts/manifest.json
  --verify                    Run remote land in verify mode
  --keep-remote-work-dir      Keep the remote temp work dir after fetching artifacts
  --local-output-dir <path>   Local artifact output dir. Default: output/remote-git-slice-landings/<slice>/<verify|landed>
  --remote-work-root <path>   Remote temp work root. Default: /home/mainuser/land-runs
...
if [[ -z "${SUBMIT_MANIFEST}" ]]; then
  SUBMIT_MANIFEST="${REPO_ROOT}/output/remote-git-slice-submissions/${SLICE_NAME}/submit/artifacts/manifest.json"
fi
...
[[ -f "${SUBMIT_MANIFEST}" ]] || die "Submit manifest not found: ${SUBMIT_MANIFEST}"
...
if [[ -z "${LOCAL_OUTPUT_DIR}" ]]; then
  LOCAL_OUTPUT_DIR="${REPO_ROOT}/output/remote-git-slice-landings/${SLICE_NAME}/${MODE_NAME}"
fi

SSH_TARGET="${USER_NAME}@${HOST}"
SSH_OPTS=(-i "${IDENTITY_FILE}" -o BatchMode=yes -o IdentitiesOnly=yes -o StrictHostKeyChecking=no)

LOCAL_TMP="$(mktemp -d "${TMPDIR:-/tmp}/remote-git-slice-land-XXXXXX")"
cleanup_local() {
  rm -rf "${LOCAL_TMP}"
}
...
cp "${REPO_ROOT}/scripts/ops/git-slice-land.mjs" "${SCRIPTS_DIR}/git-slice-land.mjs"
cp "${REPO_ROOT}/scripts/ops/git-slices.mjs" "${SCRIPTS_DIR}/git-slices.mjs"
cp "${SUBMIT_MANIFEST}" "${MANIFEST_DIR}/submit.json"
...
ARGS=(
  node "${WORK_DIR}/payload/scripts/ops/git-slice-land.mjs"
  --slice "${SLICE_NAME}"
  --submit-manifest "${WORK_DIR}/payload/manifests/submit.json"
...
scp "${SSH_OPTS[@]}" "${SSH_TARGET}:$(printf '%q' "${REMOTE_WORK_DIR}/artifacts/manifest.json")" "${LOCAL_OUTPUT_DIR}/artifacts/manifest.json" >/dev/null
scp "${SSH_OPTS[@]}" "${SSH_TARGET}:$(printf '%q' "${REMOTE_WORK_DIR}/artifacts/land-summary.md")" "${LOCAL_OUTPUT_DIR}/artifacts/land-summary.md" >/dev/null
scp "${SSH_OPTS[@]}" "${SSH_TARGET}:$(printf '%q' "${REMOTE_WORK_DIR}/artifacts/README.md")" "${LOCAL_OUTPUT_DIR}/artifacts/README.md" >/dev/null
scp "${SSH_OPTS[@]}" "${SSH_TARGET}:$(printf '%q' "${REMOTE_WORK_DIR}/artifacts/land-commands.sh")" "${LOCAL_OUTPUT_DIR}/artifacts/land-commands.sh" >/dev/null
ssh "${SSH_OPTS[@]}" "${SSH_TARGET}" "if [ -d $(printf '%q' "${REMOTE_WORK_DIR}/artifacts/patches") ]; then tar -C $(printf '%q' "${REMOTE_WORK_DIR}/artifacts") -czf $(printf '%q' "${REMOTE_WORK_DIR}/patches.tar.gz") patches; fi" >/dev/null
scp "${SSH_OPTS[@]}" "${SSH_TARGET}:$(printf '%q' "${REMOTE_WORK_DIR}/patches.tar.gz")" "${LOCAL_OUTPUT_DIR}/patches.tar.gz" >/dev/null 2>&1 || true
if [[ -f "${LOCAL_OUTPUT_DIR}/patches.tar.gz" ]]; then
  tar -xzf "${LOCAL_OUTPUT_DIR}/patches.tar.gz" -C "${LOCAL_OUTPUT_DIR}/artifacts"
  rm -f "${LOCAL_OUTPUT_DIR}/patches.tar.gz"
fi
