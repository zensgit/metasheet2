...
LAND_MANIFEST="${LAND_MANIFEST:-}"
VERIFY_MODE="${VERIFY_MODE:-0}"
JSON_OUTPUT="${JSON_OUTPUT:-0}"
KEEP_REMOTE_WORK_DIR="${KEEP_REMOTE_WORK_DIR:-0}"
LOCAL_OUTPUT_DIR="${LOCAL_OUTPUT_DIR:-}"
REMOTE_WORK_ROOT="${REMOTE_WORK_ROOT:-/home/mainuser/publish-runs}"

die() {
  echo "[publish-remote-git-slice] ERROR: $*" >&2
  exit 1
}
...
usage() {
  cat <<'EOF'
Usage:
  bash scripts/ops/publish-remote-git-slice.sh [options]
...
Options:
  ...
  --land-manifest <path>      Local land manifest. Default: output/remote-git-slice-landings/<slice>/landed/artifacts/manifest.json
  --verify                    Run remote publish in verify mode
  --keep-remote-work-dir      Keep the remote temp work dir after fetching artifacts
  --local-output-dir <path>   Local artifact output dir. Default: output/remote-git-slice-publishes/<slice>/<verify|publish>
  --remote-work-root <path>   Remote temp work root. Default: /home/mainuser/publish-runs
...
if [[ -z "${LAND_MANIFEST}" ]]; then
  LAND_MANIFEST="${REPO_ROOT}/output/remote-git-slice-landings/${SLICE_NAME}/landed/artifacts/manifest.json"
fi
...
[[ -f "${LAND_MANIFEST}" ]] || die "Land manifest not found: ${LAND_MANIFEST}"
...
if [[ -z "${LOCAL_OUTPUT_DIR}" ]]; then
  LOCAL_OUTPUT_DIR="${REPO_ROOT}/output/remote-git-slice-publishes/${SLICE_NAME}/${MODE_NAME}"
fi

SSH_TARGET="${USER_NAME}@${HOST}"
SSH_OPTS=(-i "${IDENTITY_FILE}" -o BatchMode=yes -o IdentitiesOnly=yes -o StrictHostKeyChecking=no)

LOCAL_TMP="$(mktemp -d "${TMPDIR:-/tmp}/remote-git-slice-publish-XXXXXX")"
cleanup_local() {
  rm -rf "${LOCAL_TMP}"
}
...
cp "${REPO_ROOT}/scripts/ops/git-slice-publish.mjs" "${SCRIPTS_DIR}/git-slice-publish.mjs"
cp "${REPO_ROOT}/scripts/ops/git-slices.mjs" "${SCRIPTS_DIR}/git-slices.mjs"
cp "${LAND_MANIFEST}" "${MANIFEST_DIR}/land.json"
...
ARGS=(
  node "${WORK_DIR}/payload/scripts/ops/git-slice-publish.mjs"
  --slice "${SLICE_NAME}"
  --land-manifest "${WORK_DIR}/payload/manifests/land.json"
  --repo-url "${REPO_URL}"
