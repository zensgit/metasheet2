...
test('dingtalk-p4-regression-gate writes an ops plan without executing checks', () => {
  ...
  try {
    ...
    assert.equal(summary.totals.skipped, 13)
    ...
    assert.ok(summary.checks.some((check) => check.id === 'ops-attendance-c5-real-dingtalk-preflight'))
    assert.ok(summary.checks.some((check) => check.id === 'ops-backend-docker-image-contract'))
...
test('dingtalk-p4-regression-gate all plan includes public form runtime and mobile signoff coverage', () => {
  ...
  try {
    ...
    assert.equal(summary.planOnly, true)
    assert.equal(summary.overallStatus, 'plan_only')
    assert.equal(summary.totals.total, 27)
