...
test('Time Machine D2a archive-catalog proof is exactly two-point wired', () => {
  ...
  const step = extractStepById(workflow, REAL_DB_STEP_IDS.multitable)
  assert.ok(
    step,
    `plugin-tests.yml must contain the exact multitable real-DB step id ${REAL_DB_STEP_IDS.multitable}`,
  )
