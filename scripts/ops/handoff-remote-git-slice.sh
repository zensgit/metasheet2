...
REMOTE_WORK_ROOT="${REMOTE_WORK_ROOT:-/home/mainuser/handoff-runs}"

die() {
  echo "[handoff-remote-git-slice] ERROR: $*" >&2
  exit 1
}
...
usage() {
  cat <<'EOF'
Usage:
  bash scripts/ops/handoff-remote-git-slice.sh [options]
...
Options:
  ...
  --source-branch <name>     Remote promoted source branch to hand off
  --source-report <path>     Local JSON report/manifest with branchName. Default: output/remote-git-slice-promotions/<slice>/promoted/report.json
  --verify                   Run remote handoff in verify mode
  --keep-remote-work-dir     Keep the remote temp work dir after fetching artifacts
  --local-output-dir <path>  Local artifact output dir. Default: output/remote-git-slice-handoffs/<slice>/<verify|handoff>
  --remote-work-root <path>  Remote temp work root. Default: /home/mainuser/handoff-runs
...
if [[ -z "${SOURCE_BRANCH}" ]]; then
  if [[ -z "${SOURCE_REPORT}" ]]; then
    SOURCE_REPORT="${REPO_ROOT}/output/remote-git-slice-promotions/${SLICE_NAME}/promoted/report.json"
  fi
...
if [[ -z "${LOCAL_OUTPUT_DIR}" ]]; then
  LOCAL_OUTPUT_DIR="${REPO_ROOT}/output/remote-git-slice-handoffs/${SLICE_NAME}/${MODE_NAME}"
fi
...
LOCAL_TMP="$(mktemp -d "${TMPDIR:-/tmp}/remote-git-slice-handoff-XXXXXX")"
cleanup_local() {
  rm -rf "${LOCAL_TMP}"
}
...
cp "${REPO_ROOT}/scripts/ops/git-slice-handoff.mjs" "${SCRIPTS_DIR}/git-slice-handoff.mjs"
...
(
  ...
  CMD=(
    node
    "${WORK_DIR}/payload/scripts/ops/git-slice-handoff.mjs"
    --slice "${SLICE_NAME}"
...
inner = report["report"]
print("Remote git slice handoff")
...
print(f"  source_head: {inner.get('sourceHead')}")
