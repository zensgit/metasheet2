...
const wiringFile = 'scripts/ops/elearning-assignment-lifecycle-ci-wiring.test.mjs'
const unitFiles = [
...
test('assignment-lifecycle units are whole-file canaries before Postgres', () => {
...
test('assignment-lifecycle real-DB suite is excluded from no-DB and wired post-migrate', () => {
...
test('real-DB suite refuses skip-shaped green and implementation files exist', () => {
  for (const file of dbFiles) {
...
test('workflow runs this wiring contract before dependency installation', () => {
  assert.match(
    workflow,
    /node --test scripts\/ops\/elearning-assignment-lifecycle-ci-wiring\.test\.mjs/,
  )
