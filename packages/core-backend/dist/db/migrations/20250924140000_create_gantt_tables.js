...
async function up(db) {
    ...
    await db.schema
        ...
        .addColumn('progress', 'integer', (col) => col.notNull().defaultTo(0).check((0, kysely_1.sql) `progress >= 0 AND progress <= 100`))
        .addColumn('parent_id', 'uuid', (col) => col.references('gantt_tasks.id').onDelete('cascade'))
        .addColumn('order_index', 'integer', (col) => col.notNull().defaultTo(0))
        .addColumn('is_milestone', 'boolean', (col) => col.notNull().defaultTo(false))
        .addColumn('priority', 'text', (col) => col.check((0, kysely_1.sql) `priority IN ('low', 'normal', 'high', 'critical')`))
        .addColumn('assigned_to', 'uuid')
        .addColumn('estimated_hours', (0, kysely_1.sql) `numeric(8,2)`)
        .addColumn('actual_hours', (0, kysely_1.sql) `numeric(8,2)`)
        .addColumn('status', 'text', (col) => col.notNull().defaultTo('not_started').check((0, kysely_1.sql) `status IN ('not_started', 'in_progress', 'completed', 'cancelled', 'on_hold')`))
    ...
    await db.schema
        ...
        .ifNotExists()
        .addColumn('id', 'uuid', (col) => col.primaryKey().defaultTo((0, kysely_1.sql) `gen_random_uuid()`))
        .addColumn('source_task_id', 'uuid', (col) => col.notNull().references('gantt_tasks.id').onDelete('cascade'))
        .addColumn('target_task_id', 'uuid', (col) => col.notNull().references('gantt_tasks.id').onDelete('cascade'))
        .addColumn('type', 'text', (col) => col.notNull().defaultTo('finish_to_start').check((0, kysely_1.sql) `type IN ('finish_to_start', 'start_to_start', 'finish_to_finish', 'start_to_finish')`))
    ...
    await db.schema
        ...
        .addColumn('view_id', 'uuid', (col) => col.notNull().references('views.id').onDelete('cascade'))
        .addColumn('name', 'text', (col) => col.notNull())
        .addColumn('type', 'text', (col) => col.notNull().check((0, kysely_1.sql) `type IN ('person', 'equipment', 'material')`))
        .addColumn('capacity', (0, kysely_1.sql) `numeric(8,2)`, (col) => col.notNull().defaultTo(100))
        .addColumn('cost_per_hour', (0, kysely_1.sql) `numeric(10,2)`)
        .addColumn('created_at', 'timestamptz', (col) => col.notNull().defaultTo((0, kysely_1.sql) `NOW()`))
        .addColumn('updated_at', 'timestamptz', (col) => col.notNull().defaultTo((0, kysely_1.sql) `NOW()`))
        .execute();
    ...
    await db.schema
        ...
        .ifNotExists()
        .addColumn('id', 'uuid', (col) => col.primaryKey().defaultTo((0, kysely_1.sql) `gen_random_uuid()`))
        .addColumn('task_id', 'uuid', (col) => col.notNull().references('gantt_tasks.id').onDelete('cascade'))
        .addColumn('resource_id', 'uuid', (col) => col.notNull().references('gantt_resources.id').onDelete('cascade'))
        .addColumn('allocation_percent', 'integer', (col) => col.notNull().defaultTo(100).check((0, kysely_1.sql) `allocation_percent >= 0 AND allocation_percent <= 100`))
        .addColumn('assigned_at', 'timestamptz', (col) => col.notNull().defaultTo((0, kysely_1.sql) `NOW()`))
        .execute();
...
async function down(db) {
    // Drop triggers
    await (0, kysely_1.sql) `DROP TRIGGER IF EXISTS update_gantt_resources_updated_at ON gantt_resources`.execute(db);
    await (0, kysely_1.sql) `DROP TRIGGER IF EXISTS update_gantt_tasks_updated_at ON gantt_tasks`.execute(db);
...
//# sourceMappingURL=20250924140000_create_gantt_tables.js.map