...
export async function up(db) {
    ...
    await db.schema
        ...
        .addColumn('progress', 'integer', (col) => col.notNull().defaultTo(0).check(sql `progress >= 0 AND progress <= 100`))
        ...
        .addColumn('priority', 'text', (col) => col.check(sql `priority IN ('low', 'normal', 'high', 'critical')`))
        .addColumn('assigned_to', 'uuid')
        .addColumn('estimated_hours', sql `numeric(8,2)`)
        .addColumn('actual_hours', sql `numeric(8,2)`)
        .addColumn('status', 'text', (col) => col.notNull().defaultTo('not_started').check(sql `status IN ('not_started', 'in_progress', 'completed', 'cancelled', 'on_hold')`))
    ...
    await db.schema
        ...
        .addColumn('type', 'text', (col) => col.notNull().defaultTo('finish_to_start').check(sql `type IN ('finish_to_start', 'start_to_start', 'finish_to_finish', 'start_to_finish')`))
    ...
    await db.schema
        ...
        .addColumn('view_id', 'uuid', (col) => col.notNull().references('views.id').onDelete('cascade'))
        .addColumn('name', 'text', (col) => col.notNull())
        .addColumn('type', 'text', (col) => col.notNull().check(sql `type IN ('person', 'equipment', 'material')`))
    ...
    await db.schema
        ...
        .addColumn('allocation_percent', 'integer', (col) => col.notNull().defaultTo(100).check(sql `allocation_percent >= 0 AND allocation_percent <= 100`))
