...
async function up(db) {
    ...
    await db.schema
        ...
        .addColumn('installed_at', 'timestamptz', col => col.notNull().defaultTo((0, kysely_1.sql) `NOW()`))
    ...
    await (0, kysely_1.sql) `CREATE INDEX IF NOT EXISTS idx_plugin_registry_status ON plugin_registry(status)`.execute(db);
    // Index on capabilities for capability-based queries (conditional to avoid conflict with migration 008)
    await (0, kysely_1.sql) `CREATE INDEX IF NOT EXISTS idx_plugin_registry_capabilities ON plugin_registry USING gin(capabilities)`.execute(db);
    ...
    await db.schema
        ...
        .addColumn('last_modified', 'timestamptz', col => col.notNull().defaultTo((0, kysely_1.sql) `NOW()`))
    ...
    await db.schema
        ...
        .addColumn('schema', 'jsonb')
        .addColumn('version', 'text', col => col.notNull())
        .addColumn('backup_timestamp', 'timestamptz', col => col.notNull().defaultTo((0, kysely_1.sql) `NOW()`))
        .execute();
    // Index on plugin_name for efficient lookups
    await (0, kysely_1.sql) `CREATE INDEX IF NOT EXISTS idx_plugin_config_backups_plugin_name ON plugin_config_backups(plugin_name)`.execute(db);
    ...
    await (0, kysely_1.sql) `CREATE INDEX IF NOT EXISTS idx_plugin_security_audit_plugin_name ON plugin_security_audit(plugin_name)`.execute(db);
    await (0, kysely_1.sql) `CREATE INDEX IF NOT EXISTS idx_plugin_security_audit_timestamp ON plugin_security_audit(timestamp)`.execute(db);
    await (0, kysely_1.sql) `CREATE INDEX IF NOT EXISTS idx_plugin_security_audit_severity ON plugin_security_audit(severity)`.execute(db);
    ...
    await (0, kysely_1.sql) `CREATE INDEX IF NOT EXISTS idx_plugin_resource_usage_plugin_name_timestamp ON plugin_resource_usage(plugin_name, timestamp)`.execute(db);
    ...
    await (0, kysely_1.sql) `CREATE INDEX IF NOT EXISTS idx_plugin_notification_history_plugin_name ON plugin_notification_history(plugin_name)`.execute(db);
    await (0, kysely_1.sql) `CREATE INDEX IF NOT EXISTS idx_plugin_notification_history_channel_status ON plugin_notification_history(channel, status)`.execute(db);
    ...
    await (0, kysely_1.sql) `CREATE UNIQUE INDEX IF NOT EXISTS idx_plugin_notification_subscriptions_user_channel ON plugin_notification_subscriptions(user_id, channel)`.execute(db);
    ...
    await (0, kysely_1.sql) `CREATE INDEX IF NOT EXISTS idx_plugin_job_queue_status_next_run ON plugin_job_queue(status, next_run_at)`.execute(db);
    await (0, kysely_1.sql) `CREATE INDEX IF NOT EXISTS idx_plugin_job_queue_queue_name ON plugin_job_queue(queue_name)`.execute(db);
    ...
    await (0, kysely_1.sql) `CREATE INDEX IF NOT EXISTS idx_plugin_scheduled_jobs_next_run ON plugin_scheduled_jobs(is_paused, next_run)`.execute(db);
    ...
    await (0, kysely_1.sql) `CREATE INDEX IF NOT EXISTS idx_plugin_file_storage_plugin_name ON plugin_file_storage(plugin_name)`.execute(db);
    ...
    await (0, kysely_1.sql) `CREATE INDEX IF NOT EXISTS idx_plugin_rate_limits_reset_time ON plugin_rate_limits(reset_time)`.execute(db);
    ...
    await db.schema
        ...
        .addColumn('created_at', 'timestamptz', col => col.notNull().defaultTo((0, kysely_1.sql) `NOW()`))
        .addColumn('accessed_at', 'timestamptz', col => col.notNull().defaultTo((0, kysely_1.sql) `NOW()`))
        .execute();
    // Indexes for cache operations
    await (0, kysely_1.sql) `CREATE INDEX IF NOT EXISTS idx_plugin_cache_plugin_name ON plugin_cache(plugin_name)`.execute(db);
    await (0, kysely_1.sql) `CREATE INDEX IF NOT EXISTS idx_plugin_cache_expires_at ON plugin_cache(expires_at)`.execute(db);
    await (0, kysely_1.sql) `CREATE INDEX IF NOT EXISTS idx_plugin_cache_tags ON plugin_cache USING gin(tags)`.execute(db);
    // Add foreign key constraints where appropriate
    await db.schema
        .alterTable('plugin_configs')
        .addForeignKeyConstraint('fk_plugin_configs_registry', ['plugin_name'], 'plugin_registry', ['name'])
        .onDelete('cascade')
        .execute();
    await db.schema
        .alterTable('plugin_config_backups')
        .addForeignKeyConstraint('fk_plugin_config_backups_registry', ['plugin_name'], 'plugin_registry', ['name'])
        .onDelete('cascade')
        .execute();
    await db.schema
        .alterTable('plugin_security_audit')
        .addForeignKeyConstraint('fk_plugin_security_audit_registry', ['plugin_name'], 'plugin_registry', ['name'])
        .onDelete('cascade')
        .execute();
    await db.schema
        .alterTable('plugin_scheduled_jobs')
        .addForeignKeyConstraint('fk_plugin_scheduled_jobs_registry', ['plugin_name'], 'plugin_registry', ['name'])
        .onDelete('cascade')
        .execute();
  ...
  `.execute(db);
    ...
    for (const table of tablesWithUpdatedAt) {
        await (0, kysely_1.sql) `DROP TRIGGER IF EXISTS update_${kysely_1.sql.raw(table)}_updated_at ON ${kysely_1.sql.raw(table)}`.execute(db);
        await (0, kysely_1.sql) `CREATE TRIGGER update_${kysely_1.sql.raw(table)}_updated_at BEFORE UPDATE ON ${kysely_1.sql.raw(table)} FOR EACH ROW EXECUTE FUNCTION update_updated_at_column()`.execute(db);
    }
}
...
//# sourceMappingURL=20250924180000_create_plugin_management_tables.js.map