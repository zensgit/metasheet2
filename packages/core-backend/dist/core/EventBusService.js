...
export class EventBusService extends EventEmitter {
    ...
    async subscribe(subscriberId, eventPattern, handler, options) {
        ...
        if (!eventPattern.includes('*')) {
            super.on(eventPattern, async (event) => {
    ...
    async processEvent(event) {
        ...
        for (const subscription of subscriptions) {
            try {
                ...
                const result = await this.executeWithTimeout(handler(processedEvent, context), timeout);
                // Log delivery
     ...
     */
    async handleLocalEvent(event, subscription, handler) {
        try {
            const context = {
                subscription,
                attempt: 1,
     ...
     */
    async validateEventSchema(eventName, payload) {
        const eventType = await this.getEventType(eventName);
     ...
     */
    async validatePluginPermission(pluginId, action, eventPattern) {
     ...
     */
    async findMatchingSubscriptions(eventName) {
        const subscriptions = [];
        // Check exact match
    ...
    async loadSubscriptions() {
        ...
        for (const sub of subs) {
            const subscription = {
                ...sub,
                handler_config: JSON.parse(sub.handler_config),
                filter_expression: sub.filter_expression ? JSON.parse(sub.filter_expression) : null
            };
    ...
    async loadPluginPermissions() {
        ...
        for (const perm of permissions) {
            this.pluginPermissions.set(perm.plugin_id, perm);
        }
    ...
    async getEventType(eventName) {
        ...
        return {
            ...type,
            payload_schema: type.payload_schema ? JSON.parse(type.payload_schema) : null,
            metadata_schema: type.metadata_schema ? JSON.parse(type.metadata_schema) : null
        };
    }
    /**
     ...
     */
    determineReplayType(criteria) {
        if (criteria.event_ids?.length)
            return 'single_event';
        if (criteria.time_range)
            return 'time_range';
        if (criteria.subscription_ids?.length)
            return 'subscription';
