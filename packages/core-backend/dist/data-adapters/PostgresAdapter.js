...
export class PostgresAdapter extends BaseDataAdapter {
    ...
    async select(table, options = {}) {
        ...
        if (options.orderBy?.length) {
            const orderClauses = options.orderBy.map(order => `${this.sanitizeIdentifier(order.column)} ${order.direction.toUpperCase()}`);
    ...
    async update(table, data, where) {
      ...
      RETURNING *
    `;
        return this.query(sql, [...values, ...whereClause.params]);
    }
    ...
    `;
        const [tablesResult, viewsResult] = await Promise.all([
            this.query(tablesQuery, [schema]),
            this.query(viewsQuery, [schema])
        ]);
        const tables = [];
    ...
    async getTableInfo(table, schema = 'public') {
      ...
      WHERE i.indisprimary
        ...
        AND n.nspname = $2
    `;
        const pkResult = await this.query(pkQuery, [table, schema]);
    ...
    `;
        const indexResult = await this.query(indexQuery, [table, schema]);
    ...
    `;
        const fkResult = await this.query(fkQuery, [table, schema]);
    ...
    `;
        const result = await this.query(query, [table, schema]);
    ...
    async tableExists(table, schema = 'public') {
        const query = `
      SELECT EXISTS (
        SELECT 1
