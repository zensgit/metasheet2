...
export async function userHasPermission(userId, code) {
    if (!pool)
        return false;
    try {
        ...
        if (direct.rows.length > 0)
            return true;
        // via roles
        const viaRole = await pool.query(`SELECT 1
       FROM user_roles ur
...
export async function listUserPermissions(userId) {
    ...
    if (!pool)
        return [];
    try {
        const { rows } = await pool.query(`SELECT DISTINCT permission_code AS code FROM (
...
export function invalidateUserPerms(userId) {
