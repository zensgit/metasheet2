...
export function filesRouter() {
    const r = Router();
    r.post('/api/files/upload', rbacGuard('files', 'write'), async (req, res) => {
    ...
    r.get('/api/files/:id', rbacGuard('files', 'read'), async (req, res) => {
        if (pool) {
            ...
            await auditLog({ actorId: req.user?.id, actorType: 'user', action: 'read', resourceType: 'file', resourceId: req.params.id });
