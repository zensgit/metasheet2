...
class LocalStorageProvider {
    ...
    async listFiles(prefix, options) {
        ...
        if (options?.filter) {
            ...
            files = files.filter(file => {
                if (filter.contentType && file.contentType !== filter.contentType)
                    return false;
    ...
    async createFolder(dirPath) {
        ...
        try {
            ...
            this.logger.error(`Failed to create folder ${dirPath}`, error);
            throw error;
        }
    }
    async deleteFolder(dirPath, recursive) {
        const fullPath = path.join(this.basePath, dirPath);
        try {
            if (recursive) {
            ...
            this.logger.error(`Failed to delete folder ${dirPath}`, error);
            throw error;
        }
    }
...
class S3StorageProvider {
    ...
    async download(fileId) {
        try {
            ...
            this.logger.error(`Failed to download file from S3: ${fileId}`, error);
            throw error;
        }
    }
    async delete(fileId) {
        try {
            ...
            this.logger.error(`Failed to delete file from S3: ${fileId}`, error);
            throw error;
        }
    }
    async exists(fileId) {
    ...
    async createFolder(dirPath) {
        ...
        try {
            ...
            this.logger.error(`Failed to create folder in S3: ${dirPath}`, error);
            throw error;
        }
    }
    async deleteFolder(dirPath, recursive) {
        ...
        try {
            ...
            this.logger.error(`Failed to delete folder from S3: ${dirPath}`, error);
            throw error;
        }
    }
    async getStorageUsage() {
        try {
            ...
            this.logger.error('Failed to get S3 storage usage', error);
            throw error;
        }
    }
    generateKey() {
...
class StorageServiceImpl extends eventemitter3_1.EventEmitter {
    ...
    async uploadMultiple(files) {
        try {
            const results = await Promise.all(files.map(({ file, options }) => this.upload(file, options)));
    ...
    async deleteFolder(path, recursive) {
        try {
     ...
     * 创建本地存储服务
     */
    static createLocalService(basePath, baseUrl) {
        return new StorageServiceImpl(new LocalStorageProvider(basePath, baseUrl));
    }
    /**
     * 创建 S3 存储服务
     */
    static createS3Service(s3Client, bucket, region) {
