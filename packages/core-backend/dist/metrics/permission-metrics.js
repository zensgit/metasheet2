...
export class PermissionMetrics {
     ...
     */
    incrementAuthFailure(reason, endpoint, method) {
     ...
     */
    incrementApiRequest(endpoint, method, status) {
     ...
     */
    incrementRbacDenial(resourceType, action, role, reason) {
     ...
     */
    recordPermissionCheckDuration(resourceType, action, durationMs) {
     ...
     */
    incrementTokenValidation(valid, reason) {
     ...
     */
    incrementDepartmentDenial(department, resourceType, action) {
    ...
    toPrometheusFormat() {
        ...
        const sortedMetrics = Array.from(this.metrics.entries()).sort(([a], [b]) => a.localeCompare(b));
    ...
    addHistogramValue(name, labels, value) {
