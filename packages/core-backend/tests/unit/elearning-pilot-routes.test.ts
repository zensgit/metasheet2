...
describe('elearning routes (independent content/assignment/watch/exam gates)', () => {
  ...
  test('exam answer routes accept a 10k CJK short answer and reject bodies over 8 MiB before service', async () => {
    ...
    expect(over.order).toEqual(['identity', 'org', 'rbac'])

    const anonymous = makeApp({ env: FLAG_EXAM_ON, viewer: null })
    const anonymousRes = await serve(anonymous.app)
      .post(`/api/elearning/exams/attempts/${ATTEMPT}/submit`)
      .set('content-type', 'application/json')
      .send('{malformed')
    expect(anonymousRes.status).toBe(401)
    expect(anonymousRes.body).toEqual({ error: 'unauthenticated' })
    expect(anonymous.order).toEqual(['identity'])
    expect(anonymous.readCalls).toBe(0)
    expect(anonymous.examSubmitCalls).toHaveLength(0)

    const denied = makeApp({ env: FLAG_EXAM_ON, hasRead: false })
    const deniedRes = await serve(denied.app)
      .put(`/api/elearning/exams/attempts/${ATTEMPT}/answers`)
      .set('content-type', 'application/json')
      .send('{malformed')
    expect(deniedRes.status).toBe(403)
    expect(deniedRes.body).toEqual({ error: 'Insufficient permissions' })
    expect(denied.order).toEqual(['identity', 'org', 'rbac'])
    expect(denied.readCalls).toBe(1)
    expect(denied.examSaveCalls).toHaveLength(0)
  })
