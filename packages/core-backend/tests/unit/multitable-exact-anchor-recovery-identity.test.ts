...
describe('L6-b exact-anchor recovery identity — mint/verify', () => {
  ...
  test('malformed claims fail closed (malformed_claims): empty required token-authority fields (not anchorSeq)', () => {
    // NIT-1 precision: empty checkpoint / liveSetHash are shape defects, NOT pre-contract and NOT anchorSeq.
    const emptyCkpt = mintExactAnchorRecoveryIdentity({ ...CLAIMS, checkpointId: '' })
    expect(verifyExactAnchorRecoveryIdentity(emptyCkpt, expected)).toMatchObject({ valid: false, reason: 'malformed_claims' })
    const noLive = mintExactAnchorRecoveryIdentity({ ...CLAIMS, liveSetHash: '' })
    expect(verifyExactAnchorRecoveryIdentity(noLive, expected)).toMatchObject({ valid: false, reason: 'malformed_claims' })
  })
  ...
  test('P1 token contract — mode is REQUIRED and closed-vocabulary: a pre-contract token (no mode) and a garbage mode both fail closed as pre_contract_token', () => {
    ...
    expect(verifyExactAnchorRecoveryIdentity(badMode, expected)).toMatchObject({ valid: false, reason: 'pre_contract_token' })
  })

  test('P1 token contract — authorizedScopeHash is REQUIRED: a token without the signed authorization basis fails closed as pre_contract_token', () => {
