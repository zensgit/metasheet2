...
/**
 * Boot-resolution regression gate.
 *
 * `attendance-admin.ts` module-scope-`require()`s files inside
 ...
 * resolves `require(<string literal>)` at runtime, and `docker-build.yml`
 * builds the image but never boots it.
 *
 ...
 * real compiled `attendance-admin.js` from that real layout. A regression to
 * any hard-coded relative-depth `require` reproduces the boot crash and reds
 * this test with `MODULE_NOT_FOUND`.
 *
 * The probe copies `plugins/plugin-attendance` (rather than symlinking it
 * in) and writes a real `packages/core-backend/package.json`, because the
 * resolver anchors the repo root on that named file — which
 * `Dockerfile.backend`'s `COPY packages ./packages` genuinely puts in the
 * image — and rejects a symlinked path component between the repo root and
 * the target file, so a symlinked plugin directory would not mirror
 * production either.
 */
describe('attendance-admin.ts plugin-attendance require — dist-layout boot', () => {
  it(
    ...
    () => {
      ...
      // the bug this test targets. Fail loudly and specifically here instead.
      ...
      // init and crash-loop the backend. The probe below writes that file into
      // the probe tree, so it must not be the probe's own invention: derive
      // the fact from the real `Dockerfile.backend` runner stage instead of
      // assuming it.
      ...
      try {
        ...
        // the bug this test targets, so the probe must not manufacture it.
        ...
        // component between the repo root and the target file.
        // `dereference: true` so the copy contains real files even if the
        // source tree itself holds links.
