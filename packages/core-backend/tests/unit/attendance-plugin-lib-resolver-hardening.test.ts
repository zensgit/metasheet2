...
/**
 * Hardening gate for `resolvePluginAttendanceLibPath` (rebuild phase 1).
 *
 * The resolver's predecessor returned the FIRST `existsSync` hit while walking
 * up from `__dirname`. Four attack classes succeeded against it, each with
 * EXECUTION proven by a marker file, not merely by which path was selected:
 *
 *   A1 shadow root         — a nearer `packages/core-backend/plugins/plugin-attendance/lib/…`
 *                            won from both the src and dist start dirs.
 *   A2 directory masquerade — the target replaced by a DIRECTORY containing
 *                            `index.js` still resolved and executed.
 *   A3a symlinked FILE     — a symlinked leaf pointing outside the repo executed.
 *   A3b symlinked PARENT   — a symlinked `lib/` component: the leaf still
 *                            `lstat`s as a REGULAR FILE, which is why an
 *                            `isFile()` check alone does not close this class.
 *   A4 path escape         — `relativeLibPath = '../../../escaped.cjs'` executed
 *                            a file outside the plugin root.
 *
 * Every case below asserts BOTH that the resolver refuses (with a diagnosable
 * `code`) AND — where a plant was written — that the plant never executed.
 * The first two tests are the negative control: with nothing planted, the
 * resolver must still resolve canonically from BOTH the `src/routes/` and the
 * `dist/src/routes/` start dirs, which is the dual-layout property this
 * resolver exists for (#4814 P1).
 */
...
describe('resolvePluginAttendanceLibPath hardening (rebuild phase 1)', () => {
  describe('negative control — nothing planted, both production layouts still resolve', () => {
    ...
    it('resolves to the SAME canonical path from the dist/src/routes/ start dir (#4814 P1 dual layout)', () => {
  ...
  describe('A1 — shadow root planted NEARER than the canonical root', () => {
    it('the canonical file still wins from BOTH start dirs and the shadow never executes', () => {
      ...
      // Prove the plant is real and would have been chosen by a first-hit walk:
      // it exists, and it sits strictly nearer both start dirs than the canonical file.
    ...
    it('a SECOND repo-root anchor is refused as ambiguous rather than silently preferred', () => {
      // A shadow root that also carries the named anchor is the strongest form
      // of A1: it is indistinguishable from the real root by name alone.
  ...
  describe('A2 — directory masquerade', () => {
  ...
  describe('A3a — symlinked FILE component', () => {
  ...
  describe('A3b — symlinked PARENT component (the isFile()-insufficiency proof)', () => {
    it('a symlinked lib/ directory is refused even though the LEAF lstats as a regular file', () => {
      ...
      // THE point of this case: the final component passes an isFile() check.
  ...
  describe('A4 / A5 — path escape and the closed file set', () => {
