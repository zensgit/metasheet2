 ...
 *    `setupMiddleware` has already run to completion — EXCEPT the thirteen
...
describe('the real app assembly (index.ts) registers this route behind the global gate and the attendance audit/security middleware', () => {
  ...
  it('A5 — pre-gate prefix: the 13 sites that run before the gate, inside setupMiddleware, are exactly this frozen list (the pre-authentication surface)', () => {
