...
import { buildRecoveryArchiveManifest } from '../../src/multitable/recovery-archive-manifest'
import { buildRecoveryArchiveManifestObjectEnvelope } from '../../src/multitable/recovery-archive-manifest-object-envelope'
...
describe('production archive worker authorization', () => {
  ...
  test('manual continuation uses canonical fresh actor/scope policy before every resumed section', async () => {
    ...
    // Structural prepared-resume fixture only; real MAC verification is exercised by the real-DB finalizer.
    const signed = buildRecoveryArchiveManifest({ archive_generation_id: generationId, workspace_id: identity.workspaceId,
      base_id: identity.baseId, sheet_id: identity.sheetId, anchor_operation_id: binding.anchorOperationId,
      anchor_seq: '1', checkpoint_id: binding.checkpointId, created_at: '2026-09-19T00:00:00.000Z',
      expires_at: '2026-09-20T00:00:00.000Z', source_vector_hash: 'c'.repeat(64) },
    RECOVERY_ARCHIVE_V1_SECTION_NAMES.map((name, index) => ({ name, rows: [], aead_algorithm: 'aes-256-gcm',
      key_id: binding.keyId, wrapped_dek_id: 'wrapped', dek_fingerprint: 'a'.repeat(64), nonce: Buffer.alloc(12, index).toString('hex') })),
    'd'.repeat(64))
    const manifestEnvelope = buildRecoveryArchiveManifestObjectEnvelope({ manifestJson: signed.manifestJson, wrappedDek: Buffer.from([1]) })
    const packageBody = {
      ...
      sections: RECOVERY_ARCHIVE_V1_SECTION_NAMES.map((sectionName, index) => ({ sectionName,
        ...
        ciphertext: 'AQ==', authTag: Buffer.alloc(16).toString('base64'), plaintextSha256: signed.manifest.sections[index]!.plaintext_sha256,
      })),
    }
    const payload = Buffer.from(JSON.stringify({ version: 2, ...packageBody,
      manifestEnvelope: Buffer.from(manifestEnvelope.envelopeBytes).toString('base64') }))
    const unsignedPayload = Buffer.from(JSON.stringify({ version: 1, ...packageBody }))
    let withManifest = true
    ...
    const query = async (text: string, params?: unknown[]) => {
      ...
      if (text.includes('FROM public.meta_recovery_archive_prepared_captures')) {
        const bytes = withManifest ? payload : unsignedPayload
        return { rows: hasPrepared ? [{ payload: bytes, payload_sha256: createHash('sha256').update(bytes).digest('hex') }] : [] }
      }
    ...
    upload.mockClear()
    withManifest = false
    await expect(run(input)).rejects.toThrow('RECOVERY_ARCHIVE_PREPARED_MANIFEST_REQUIRED')
