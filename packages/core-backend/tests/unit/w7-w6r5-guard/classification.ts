 ...
 * `plugins/plugin-attendance/lib/leave-offset-partial-absence-guard.cjs`: a
 * submit/approve 422 gate that does not build or read the frozen work-date
 * context. Classifying it `not_calculation_path` re-opens W7-R10's first
 * blind spot for that one file (both ban legs no longer apply to it). The
 * reason on the entry is the review record. The blind spot is NOT closed as
 * a class: a `not_calculation_path` entry is a reviewable assertion, not a
 * proof. The mitigation is the one W7-R10 prescribes — review the
 * classification diff and spot-check the entry for non-reachability from the
 * frozen-context build path. The guard cannot do that for you; it can only
 * force the entry to be written down. Every entry MUST therefore carry a
 * reason (the type below makes that structural, not a convention).
 *
...
export const ATTENDANCE_W7_NOT_CALCULATION_PATH_FILES_V1: readonly AttendanceW7NotCalculationPathEntryV1[] =
  Object.freeze([
    {
      relPath: 'plugins/plugin-attendance/lib/leave-offset-partial-absence-guard.cjs',
      reason:
        'Leave-offset submit and final-approval 422 gate only. It decides whether partial_unpaid_absence is rejected before any balance write. It does not build, read, or import the frozen work-date context, so the W6 aggregate bans do not apply to it.',
    },
  ] as const)
