 ...
 *
 * In scope: `plugins/plugin-attendance/index.cjs` (the file that owns most
 * attendance calculation writes) and `packages/core-backend/src/services/`
 * — a W7 consumer wired in either place must be caught by this gate.
 *
 * Governing document:
...
const CALCULATION_PATH_PREFIXES = [
  ...
  // A DIRECTORY prefix, not individually named files: any file under
  // `plugins/plugin-attendance/lib/` is in scope, since that is where a W7
  // winner-selection module would naturally be added. Any file that must be
  // exempt has to be named and justified below, not excluded by omission.
...
/**
 * The set of W6 module markers a calculation-path file must not reference,
 * derived from the actual `w6-*.ts` filenames rather than hand-listed, so a
 * future W6 module is in scope on the day it is created. Includes the
 * contract module (not types-only: it performs a module-scope `require()`
 * of the FSER lib and owns the frozen enum constants both service files
 * import), not just the aggregate.
 */
...
describe('W6-R5 repository inventory: no calculation-path file imports the W6 aggregate', () => {
  ...
  it('the plugin lib/ DIRECTORY is in scope: a HYPOTHETICAL new file there classifies as calculation-path', () => {
    // Asserts the directory prefix directly rather than merely writing it: a W7
    ...
    // And the specific files matched previously are still in scope.
    for (const file of [
  ...
  it('the W6 module markers are derived and include the contract module', () => {
