...
describe('AutomationService', () => {
  ...
  describe('update_field action', () => {
    it('runs update_record query and emits follow-up update event', async () => {
      ...
      // R13 Lane C: update_record now emits its revision inside ONE real transaction (mirrors D-1's
      // executeDeleteRecord), so the executor's `withTransaction` routes through
      // `AutomationService`'s hard-wired `poolManager.get().transaction(...)` instead of calling
      // `deps.queryFn` directly. Route that transaction through the SAME injected `query` mock (rather
      // than a real Postgres pool) so this stays a no-DB unit test — mirrors the
      // `multitable-button-routes.test.ts` `createMockPool()` pattern.
      ...
      // R13 Lane C: the same-txn revision insert actually ran (source=automation, action=update).
