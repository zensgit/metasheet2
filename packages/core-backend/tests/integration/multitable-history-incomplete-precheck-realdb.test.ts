...
let app: Express
const revertPreview = (sheet: string) => request(app).post(`/api/multitable/sheets/${sheet}/revert-preview`).send({ asOf: T1 })
const revertExecute = (sheet: string, previewIdentity: string) => request(app).post(`/api/multitable/sheets/${sheet}/revert-execute`).send({ asOf: T1, previewIdentity })
const resetPreview = (sheet: string) => request(app).post(`/api/multitable/sheets/${sheet}/reset-preview`).send({ asOf: T1 })
const resetExecute = (sheet: string, previewIdentity: string) => request(app).post(`/api/multitable/sheets/${sheet}/reset-execute`).send({ asOf: T1, previewIdentity, confirm: 'reset' })
...
async function sheetWriteState(sheet: string): Promise<{
  ...
  return { records, revisionCount, restoreRevisionCount, trashCount }
}

/** Drives all FOUR gated surfaces against a sheet expected to refuse, asserting the exact unified body and
 *  that no execute token exists anywhere in any response — then proves the DB byte-identical. */
async function expectAllFourRefuseWithZeroWrites(sheet: string): Promise<void> {
  ...
  const exRevert = await revertExecute(sheet, 'dummy-token-never-minted')
  expect(exRevert.status).toBe(409)
  expect(exRevert.body).toEqual(HISTORY_INCOMPLETE_BODY) // execute re-check fires BEFORE identity verification
  const pvReset = await resetPreview(sheet)
  expect(pvReset.status).toBe(409)
  expect(pvReset.body).toEqual(HISTORY_INCOMPLETE_BODY)
  const exReset = await resetExecute(sheet, 'dummy-token-never-minted')
...
describeIfDatabase('multitable D-1c §0.6 HISTORY_INCOMPLETE precheck (real DB)', () => {
  ...
  test('G-HI-3: formula-sheet healthy records (materialized derived keys absent from / stale in snapshots) → precheck passes, revert AND reset proceed', async () => {
    ...
    if (pvReset.body?.data?.previewIdentity) {
      ...
      expect(pvReset.body?.data?.summary).toEqual({ visibleRevertCount: 0, deleteCount: 0, visibleUndeleteCount: 0, conflictCount: 0 })
    }
  })
  ...
  test('G-HI-4: live record with ZERO revisions (uncaptured CREATE) → all four surfaces refuse, zero writes — the reset silent-delete hole is closed', async () => {
    await insertLive(SHEET, Z, { [NAME]: 'ghost' }, 1) // NO revision at all — invisible to reconstructRecordsAtT
    await expectAllFourRefuseWithZeroWrites(SHEET)
