 ...
 *          (v3: [approval-bridge, approval-projection, approval-trigger, dingtalk-todo-mirror,
 *          multitable-record-approval] — five, sorted, all pending), committed WITH the terminal
 ...
 *          outbox row (fanned to the 5 manifest-v3 completion consumers) and ZERO `approval.task_created`
 *          rows; spy silent.
...
describeIfDatabase('P1#2e — producer family 1: approval completion + task_created REPLACE (real DB)', () => {
  ...
  test('F1-G1 flag ON approve-to-terminal: one approval.approved outbox row fanned to 5; legacy emit SUPPRESSED', async () => {
    ...
    expect(await consumerStatuses(rows[0].id)).toEqual(['pending', 'pending', 'pending', 'pending', 'pending'])
