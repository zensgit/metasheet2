...
import {
  mintOperation,
  sealOperation,
  pruneSealedOperation,
  OperationLedger,
  OperationLedgerSchemaError,
...
describeIfDatabase('W0-1 L6-a — sealed operation-endpoint ledger (real DB)', () => {
  ...
  // H3 (owner hardening): with the flag OFF mint is INERT; with the flag ON AND the L6 schema PRESENT (the
  // migrated test DB) mint is ACTIVE. The reversed case — flag ON but schema INCOMPLETE — no longer degrades
  // to inert; it FAILS CLOSED (throws). That fail-closed path + its positive control are §H3 below.
  test('M1 mintOperation is INERT with the flag OFF; ACTIVE with the flag ON + full schema (H3 fail-closed: §H3)', async () => {
    delete process.env[FLAG]
  ...
  // ── §H1 (owner hardening): sealed endpoint DB-immutable to DELETE too, EXCEPT the retention path ─────
  // The INSERT/UPDATE guards (§E/§F2) do not stop a `DELETE FROM meta_record_history_operations …` from
  // dropping an already-frozen recovery anchor. H1 installs trg_mrho_reject_delete: an ordinary DELETE fails
  // closed at the DB level; the ONLY sanctioned deletion is the whole-operation retention path (§H2), which
  // sets the transaction-local retention GUC. Mutation: dropping trg_mrho_reject_delete makes the
  // ordinary-DELETE legs green (endpoint gone) — this test then reds.
  test('§H1 a SEALED operation endpoint is DB-immutable to DELETE — ordinary DELETE is rejected; the anchor never vanishes', async () => {
    const op = mkOpId()
    const R = mkRecord('h1del')
    await inTxn(async (run) => {
      await synthRevision(run, R, 1, '9101', op)
      await synthRevision(run, R, 2, '9102', op)
      await insertEndpoint(run, op, '9102', 2) // endpoint written LAST, count/max exact
    })
    expect(await endpointOf(op)).toEqual({ endpoint_seq: '9102', event_count: 2 })

    // an ordinary targeted DELETE (no retention GUC) is rejected fail-closed at the DB level
    await expect(
      q('DELETE FROM meta_record_history_operations WHERE sheet_id=$1 AND operation_id=$2', [SHEET, op]),
    ).rejects.toThrow(/immutable.*DELETE/)
    // even a broad sheet-level sweep is refused (the trigger is per-row, unconditional outside retention)
    await expect(
      q('DELETE FROM meta_record_history_operations WHERE sheet_id=$1', [SHEET]),
    ).rejects.toThrow(/immutable.*DELETE/)

    // the reject trigger blocks a deletion the deferred FK alone would NOT catch: in ONE txn delete the
    // events FIRST (FK deferred ⇒ not yet violated), then the endpoint — the trigger still refuses it. Without
    // the trigger this txn would COMMIT (events already gone ⇒ no FK violation), silently dropping the anchor.
    await expect(
      inTxn(async (run) => {
        await run('DELETE FROM meta_record_revisions WHERE sheet_id=$1 AND operation_id=$2', [SHEET, op])
        await run('DELETE FROM meta_record_history_operations WHERE sheet_id=$1 AND operation_id=$2', [SHEET, op])
      }),
    ).rejects.toThrow(/immutable.*DELETE/)

    // positive control: the sealed anchor is still present and byte-unchanged after every rejected delete
    // (each rejecting txn rolled back — events and endpoint both intact)
    expect(await endpointOf(op)).toEqual({ endpoint_seq: '9102', event_count: 2 })
    expect((await q('SELECT count(*)::int AS n FROM meta_record_revisions WHERE sheet_id=$1 AND operation_id=$2', [SHEET, op])).rows[0]).toMatchObject({ n: 2 })

    // the ONLY sanctioned deletion is the retention path (§H2) — it succeeds where ordinary DELETE cannot
    await pruneSealedOperation(q, SHEET, op)
    expect(await endpointOf(op)).toBeUndefined()
  })

  // ── §H2 (owner hardening): whole-operation retention — atomic prune of a sealed operation ────────────
  // Prunes a WHOLE sealed operation ATOMICALLY (endpoint + ALL its events across both event tables), never a
  // partial prune that would orphan an endpoint or violate its count/max invariant. A younger /
  // still-referenced operation survives untouched. Mutation: making the prune partial (e.g. skip the events
  // delete) reds — the DEFERRABLE FK fires at COMMIT of the prune (or the "events gone" assertion fails).
  test('§H2 retention prunes a whole sealed operation atomically (endpoint + all events); a younger operation survives', async () => {
    // TARGET op: 2 revisions + 1 marker + endpoint
    const target = mkOpId()
    const Rt = mkRecord('h2t')
    await inTxn(async (run) => {
      await synthRevision(run, Rt, 1, '9201', target)
      await synthRevision(run, Rt, 2, '9202', target)
      await synthMarker(run, Rt, 3, '9203', target)
      await insertEndpoint(run, target, '9203', 3)
    })
    // SURVIVOR op (younger, higher seq): 1 revision + endpoint — must be untouched by pruning TARGET
    const survivor = mkOpId()
    const Rs = mkRecord('h2s')
    await inTxn(async (run) => {
      await synthRevision(run, Rs, 1, '9210', survivor)
      await insertEndpoint(run, survivor, '9210', 1)
    })
    expect(await endpointOf(target)).toEqual({ endpoint_seq: '9203', event_count: 3 })
    expect(await endpointOf(survivor)).toEqual({ endpoint_seq: '9210', event_count: 1 })

    // prune ONLY the target operation
    await pruneSealedOperation(q, SHEET, target)

    // the target endpoint AND all its events (both tables) are gone — atomic, no partial residue
    expect(await endpointOf(target)).toBeUndefined()
    expect((await q('SELECT count(*)::int AS n FROM meta_record_revisions WHERE sheet_id=$1 AND operation_id=$2', [SHEET, target])).rows[0]).toMatchObject({ n: 0 })
    expect((await q('SELECT count(*)::int AS n FROM meta_record_version_markers WHERE sheet_id=$1 AND operation_id=$2', [SHEET, target])).rows[0]).toMatchObject({ n: 0 })
    // NO orphan event references a vanished endpoint anywhere on the sheet (the deferred FK held atomically)
    expect(
      (
        await q(
          `SELECT count(*)::int AS n FROM meta_record_revisions r
           WHERE r.sheet_id=$1 AND r.operation_id IS NOT NULL
             AND NOT EXISTS (SELECT 1 FROM meta_record_history_operations o WHERE o.sheet_id=r.sheet_id AND o.operation_id=r.operation_id)`,
          [SHEET],
        )
      ).rows[0],
    ).toMatchObject({ n: 0 })

    // the younger operation SURVIVES — endpoint + its event intact (retention is keyed on the exact op id)
    expect(await endpointOf(survivor)).toEqual({ endpoint_seq: '9210', event_count: 1 })
    expect((await q('SELECT count(*)::int AS n FROM meta_record_revisions WHERE sheet_id=$1 AND operation_id=$2', [SHEET, survivor])).rows[0]).toMatchObject({ n: 1 })

    // cleanup the survivor via the same sanctioned path
    await pruneSealedOperation(q, SHEET, survivor)
    expect(await endpointOf(survivor)).toBeUndefined()
  })

  // ── §H3 (owner hardening): fail-closed schema probe (REVERSES the old silent degrade) ────────────────
  // When the writer fence flag is ON and the L6 schema is incomplete (the operation_id column probe misses),
  // mintOperation now THROWS OperationLedgerSchemaError and rolls the writer txn back — it does NOT return an
  // inert ledger (which would let untrusted, endpoint-less writes flow while the operator believes the trust
  // checkpoint is armed). Mutation: reverting mintOperation to `return new OperationLedger(sheetId, null)` on
  // a missed probe makes both fail-closed legs green (no throw) — this test then reds. Positive control:
  // flag ON + FULL schema mints + seals a real operation.
  test('§H3 fail-closed: flag ON + incomplete L6 schema → mintOperation THROWS and rolls the writer txn back (no silent inert success)', async () => {
    process.env[FLAG] = 'true'
    __resetOperationLedgerColumnProbe()
    // simulate the pre-migration deploy window: the operation_id column probe misses; everything else real.
    const missingSchema: LedgerQuery = async (sql, params) => {
      if (/information_schema\.columns/.test(sql) && /operation_id/.test(sql)) return { rows: [] }
      return q(sql, params)
    }
    await expect(mintOperation(missingSchema, SHEET)).rejects.toBeInstanceOf(OperationLedgerSchemaError)

    // fail-closed INSIDE a real writer txn: the whole txn rolls back, leaving NO partial write
    const R = mkRecord('h3fc')
    __resetOperationLedgerColumnProbe()
    await expect(
      inTxn(async (run) => {
        await run('INSERT INTO meta_records (id, sheet_id, data, version, created_by) VALUES ($1,$2,$3::jsonb,1,$4)', [R, SHEET, '{}', ACTOR])
        const runMissing: LedgerQuery = async (sql, params) => {
          if (/information_schema\.columns/.test(sql) && /operation_id/.test(sql)) return { rows: [] }
          return run(sql, params)
        }
        await mintOperation(runMissing, SHEET) // throws → rolls back the INSERT above (fail-closed)
      }),
    ).rejects.toThrow(/schema incomplete|failing closed/i)
    __resetOperationLedgerColumnProbe()
    expect((await q('SELECT count(*)::int AS n FROM meta_records WHERE id=$1', [R])).rows[0]).toMatchObject({ n: 0 }) // rolled back — no silent partial write

    // positive control: flag ON + FULL schema (real probe) → mint is active AND a real single-event op seals
    __resetOperationLedgerColumnProbe()
    const res = await mkRecordService().createRecord({ sheetId: SHEET, data: { [F_STR]: 'h3pc' }, actorId: ACTOR, capabilities })
    const rev = (await q('SELECT operation_id FROM meta_record_revisions WHERE sheet_id=$1 AND record_id=$2', [SHEET, res.recordId])).rows[0] as { operation_id: string | null }
    expect(rev.operation_id).toMatch(/^[0-9a-f-]{36}$/)
    expect(await endpointOf(rev.operation_id!)).toBeDefined()
  })
})
