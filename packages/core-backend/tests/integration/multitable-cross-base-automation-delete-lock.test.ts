...
describeIfDatabase('Phase C2 automation — governed cross-base DELETE + LOCK (real DB)', () => {
  ...
  // The source-side FK cascades, while `meta_links.foreign_record_id` carries NO FK at all (closeout
  // removed it; the containment guard enforces its absence). The executor (mirroring the same-base sinks)
  // explicitly deletes both directions before deleting the record.
