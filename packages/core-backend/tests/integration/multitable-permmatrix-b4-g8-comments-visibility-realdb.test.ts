 ...
 *
 * ══════════════════════════════════════════════════════════════════════════════════════════════════
 * STATUS: 4 of 5 tests below are RED against current `main` — THIS IS A CONFIRMED REAL LEAK, not a test
 * bug. Per the B4 task contract ("if a differential golden goes RED against current main... STOP and
 * report as a candidate runtime finding, do NOT weaken the golden, do NOT fix runtime"), the assertions
 * below encode the CORRECT/intended contract verbatim — they are not weakened to match the leaky
 * behavior. THIS FILE IS DELIBERATELY NOT ADDED to .github/workflows/plugin-tests.yml's real-DB
 * allowlist (adding it would turn the shared "Run multitable real-DB integration" CI step permanently
 * red for every future unrelated PR). See the PR body / report for the full repro and impact writeup.
 * Do not wire this file into CI until the runtime gap is triaged — either fixed (then flip this notice
 * and every test below should already pass as written) or explicitly accepted as intended product
 * behavior by the owner (in which case the CONTRACT — this file AND the spec — must be revised together
 * in the open, not silently).
 * ══════════════════════════════════════════════════════════════════════════════════════════════════
 *
 ...
 *
 * Runs only with DATABASE_URL (sentinel fails-not-skips in CI convention) — but see STATUS above: this
 * file is not part of the CI-wired set regardless of DATABASE_URL.
 */
...
describeIfDatabase('W1-2 B4 G-8 — comments × sheet-visibility (real DB) — CONFIRMED LEAK, see file header', () => {
  beforeAll(async () => {
