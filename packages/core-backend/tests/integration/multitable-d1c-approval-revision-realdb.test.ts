...
async function seedUsers(): Promise<void> {
  ...
  // Lock-11 §10 arm (a) fixture delta: REQUESTER needs exactly one active user_orgs membership
  // or the real start_approval call below 422s (APPROVAL_ORG_UNRESOLVED) before this suite's own
  // D1C4 revision assertions run.
  await q(
    `INSERT INTO user_orgs (user_id, org_id, is_active) VALUES ($1, 'default', TRUE) ON CONFLICT (user_id, org_id) DO UPDATE SET is_active = TRUE`,
    [REQUESTER],
  )
}
