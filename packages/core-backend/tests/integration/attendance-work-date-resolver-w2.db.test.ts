...
describeIfDatabase('W2 AttendanceWorkDateResolver (real DB)', () => {
  ...
  // `attendance.absence.generated` is a run-level operational notification
  // (bare count, no per-record identity) and must NOT be emitted for a
  // W4-postured (shadow/eligible/authoritative) scheduled run — only the
  // legacy side of the posture split keeps the pre-existing synchronous
  // best-effort emit. `runAutoAbsenceForOrgDate`'s own `outcome.kind` branch
  // is exercised directly here against the boundary's published result-type
  // contract (`AttendanceScheduledRunBoundaryResultV1`), matching this
  // suite's existing direct-call convention (see the two tests above that
  // already drive `helpers.runAutoAbsenceForOrgDate` with a stub `emit`).
  it('P1-2: attendance.absence.generated is NOT emitted for a w4-postured scheduled outcome (positive control: legacy outcome DOES emit)', async () => {
    const emittedW4: Array<{ name: string; payload: Record<string, unknown> }> = []
    const w4Result = await helpers.runAutoAbsenceForOrgDate(db, {
      orgId: orgA,
      workDate: '2026-07-18',
      skipDedup: true,
      calendarOverrides: [],
      rule: {
        name: 'P1-2 w4-outcome probe',
        timezone: 'UTC',
        workStartTime: '09:00',
        workEndTime: '17:00',
        workingDays: [0, 1, 2, 3, 4, 5, 6],
      ...
      emit(name: string, payload: Record<string, unknown>) {
        emittedW4.push({ name, payload })
      },
    })
    expect(w4Result.skipped).toBe(false)
    expect(emittedW4.some((event) => event.name === 'attendance.absence.generated')).toBe(false)

    const emittedLegacy: Array<{ name: string; payload: Record<string, unknown> }> = []
    const legacyResult = await helpers.runAutoAbsenceForOrgDate(db, {
      orgId: orgA,
      workDate: '2026-07-18',
      skipDedup: true,
      calendarOverrides: [],
      rule: {
        name: 'P1-2 legacy-outcome probe',
        timezone: 'UTC',
        workStartTime: '09:00',
        workEndTime: '17:00',
        workingDays: [0, 1, 2, 3, 4, 5, 6],
      ...
      w4Boundary: {
        async executeScheduledRun() {
          return { kind: 'legacy', rows: [{ user_id: userId }] }
        },
      },
      emit(name: string, payload: Record<string, unknown>) {
        emittedLegacy.push({ name, payload })
      },
    })
    expect(legacyResult.skipped).toBe(false)
    expect(emittedLegacy.some((event) => event.name === 'attendance.absence.generated')).toBe(true)
  })
})
