...
export const W7_PROVENANCE_WIDENING_LEDGER_V1: readonly W7LedgerEntryV1[] = Object.freeze([
  ...
  // `group_policy_snapshot`. ONE ledger entry covers BOTH emission sites (the
  ...
  // exact line. `write_side_emitter` holds: the arm line TESTS nothing — the
  // selection is made by the ternary CONDITION, which compares the
  // calculation's own `contextSelector`, never the org posture and never a
  // provenance value. The `: 'snapshot'` fallback arms are NOT derived sites
  // (ambiguous member, and the enclosing declaration names no trace-family
  // anchor), matching this function's three pre-existing
  // `frozen_evidence_unavailable` emissions, which have never derived either.
