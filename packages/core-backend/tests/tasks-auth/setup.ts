/**
 * Dedicated setup for the tasks auth/RBAC gate.
 *
 * Must run before any import of auth / rbac. rbac.ts caches RBAC_TOKEN_TRUST
 * at module load; the global integration setup must stay untouched (it sets
 * both flags true).
 */
...
if (!process.env.DATABASE_URL) {
  throw new Error(
    'tasks auth/RBAC gate requires DATABASE_URL; refusing skip-shaped green',
  )
}

if (!process.env.JWT_SECRET) {
  process.env.JWT_SECRET = 'tasks-auth-gate-jwt-secret-min-32-bytes!!'
}
...
if (process.env.PRODUCT_MODE !== 'plm-workbench') {
  throw new Error('tasks auth gate setup must pin PRODUCT_MODE=plm-workbench to keep attendance self-service backfill off')
}
