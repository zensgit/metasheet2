 ...
 * onto course / version / items / exam, all ON DELETE RESTRICT.
 */
...
export async function up(db: Kysely<unknown>): Promise<void> {
  ...
  await sql`
    CREATE TABLE IF NOT EXISTS elearning_course_publish_requests (
      ...
      CONSTRAINT elearning_course_publish_requests_video_item_fk
        FOREIGN KEY (org_id, course_version_id, video_item_id)
      ...
      CONSTRAINT elearning_course_publish_requests_exam_item_fk
        FOREIGN KEY (org_id, course_version_id, exam_item_id)
        REFERENCES elearning_course_version_items (org_id, course_version_id, id)
        ON DELETE RESTRICT,
  ...
  await sql`
    DO $$ BEGIN
      IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
         WHERE conname = 'elearning_course_publish_requests_video_item_fk'
           AND conrelid = 'elearning_course_publish_requests'::regclass
      ) THEN
        ALTER TABLE elearning_course_publish_requests
  ...
  await sql`
    DO $$ BEGIN
      IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
         WHERE conname = 'elearning_course_publish_requests_exam_item_fk'
           AND conrelid = 'elearning_course_publish_requests'::regclass
      ) THEN
        ALTER TABLE elearning_course_publish_requests
