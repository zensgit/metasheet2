...
const MULTITABLE_SHARE_PERMISSIONS = [
  ['multitable:share', 'Multitable Share', 'Manage sheet-level multitable access overrides'],
] as const

const ROLE_PERMISSION_PAIRS = [
  ['admin', 'multitable:share'],
  ['user', 'multitable:share'],
] as const

export async function up(db: Kysely<unknown>): Promise<void> {
  const permissionsExists = await checkTableExists(db, 'permissions')
  if (permissionsExists) {
    for (const [code, name, description] of MULTITABLE_SHARE_PERMISSIONS) {
      await sql`
...
export async function down(db: Kysely<unknown>): Promise<void> {
  ...
  const permissionsExists = await checkTableExists(db, 'permissions')
  if (permissionsExists) {
    for (const [code] of MULTITABLE_SHARE_PERMISSIONS) {
      await sql`
