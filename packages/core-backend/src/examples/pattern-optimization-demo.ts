...
import { PatternTrie } from '../messaging/pattern-trie'
...
const metrics = {
  ...
  histogram: (name: string, value: number, labels?: any) =>
    console.log(`[METRIC] ${name}: ${value}ms`, labels),
...
export class PatternOptimizationDemo {
  ...
  private setupCallbacks(): void {
    ...
    patterns.forEach(pattern => {
      this.callbacks.set(pattern, (topic: string, message: any) => {
   ...
   */
  private generateTestPatterns(count: number): string[] {
   ...
   */
  private generateTestTopics(count: number): string[] {
    const topics: string[] = []
    const categories = ['user', 'admin', 'system', 'notification', 'workflow', 'data', 'event', 'log']
    const actions = ['create', 'update', 'delete', 'read', 'login', 'logout', 'send', 'receive']
   ...
   * Benchmark Trie-based implementation
   */
  private async benchmarkTrieImplementation(patterns: string[], topics: string[]): Promise<any> {
   ...
   */
  async patternTypesDemo(): Promise<void> {
    ...
    patterns.forEach(({ pattern, type }) => {
      this.patternManager.subscribe(pattern, (topic: string, message: any) => {
   ...
   */
  async cachingDemo(): Promise<void> {
   ...
   */
  async memoryOptimizationDemo(): Promise<void> {
   ...
   */
  async realWorldScenariosDemo(): Promise<void> {
