...
async function handleRequest(req, res) {
  ...
  // Approve endpoint (strict state machine: only from pending)
  if (pathname.match(/^\/api\/approvals\/(.+)\/approve$/) && method === 'POST') {
    ...
    // Invalid transition -> 422 (strict contract)
  ...
  // Return endpoint (only allowed from approved)
  ...
  if (pathname.match(/^\/api\/approvals\/[^\/]+$/) && method === 'GET') {
    ...
    if (!approvals.has(id)) {
      ...
      // Reset demo-1 approval to pending for testing
      const approval = approvals.get(id);
      if (approval.status !== 'pending') {
