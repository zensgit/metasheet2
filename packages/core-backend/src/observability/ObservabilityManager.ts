...
export class ObservabilityManager extends EventEmitter {
  ...
  private alerts: Map<string, { rule: AlertRule; triggered: Date; count: number }> = new Map()
  ...
  private async initialize(): Promise<void> {
    try {
      ...
      this.logger.error('Failed to initialize Observability Manager', error as Error)
      throw error
    }
  }
  ...
  private async collectProfilingData(): Promise<void> {
    ...
    if (this.config.profiling.cpuProfile) {
      try {
        ...
        this.logger.error('Failed to collect CPU profile', error as Error)
      }
    }
      ...
      try {
        ...
        if (heapSnapshot) {
          await fs.promises.rename(
            heapSnapshot,
            path.join(profilingDir, `heap-snapshot-${timestamp}.heapsnapshot`)
          )
        }
      } catch (error) {
        this.logger.error('Failed to collect heap snapshot', error as Error)
      }
    }
  }
