/**
 * W6-1 (#4556) — group effective-policy aggregate: response contract validator.
 *
 * Governing document:
 ...
 * aggregate response. It gives W6-R2/R6/R7/R8 a single mechanical judge:
 *  - W6-R2 (values-free): any key outside the closed shape fails validation,
 ...
 *    a client-suppliable override — it only judges shape;
 ...
 * package and there is no shared module across that boundary. The duplication
 * is closed mechanically: `tests/unit/attendance-w6-schedule-route-surface-parity.test.ts`
...
/** Matches the OpenAPI draft's `revision: {type: integer, minimum: 1}` and
 * the column's `integer NOT NULL DEFAULT 1`. */
function isPositiveInt(value: unknown): value is number {
  return typeof value === 'number' && Number.isInteger(value) && value >= 1
}

/** A nullable date-only field: the producer emits either a canonical
 * `YYYY-MM-DD` or `null` for an open-ended managed row. */
...
/** W6-R6 + §4.2: reason codes are closed per position, not merely "an array
 * of strings" and not a single flat union across both positions. */
...
function validateFixedSchedule(value: unknown): AttendanceGroupEffectivePolicyValidationResult {
  ...
  for (const set of drift.managedSets) {
    ...
    // Every field's TYPE is checked, not just key presence.
...
/**
 * Whether `sourceRefs` may appear on a domain summary. Per the fixture pack:
 * `schedule`, `segments` and `rules` always carry it (sometimes `[]`);
 * `membership`, `flex`, `punchMethod` and `requestPosture` never do.
 */
