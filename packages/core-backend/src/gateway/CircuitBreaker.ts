...
export interface CircuitBreakerConfig {
  ...
  windowSize?: number      // Rolling window size for metrics
...
export class CircuitBreaker extends EventEmitter {
  ...
  async execute<T>(fn: () => Promise<T>): Promise<T> {
    ...
    try {
      ...
      const latency = Date.now() - start
      this.recordFailure(latency, error as Error)
...
export function createModerateCircuitBreaker(): CircuitBreaker {
  return new CircuitBreaker({
...
export function createRelaxedCircuitBreaker(): CircuitBreaker {
  return new CircuitBreaker({
