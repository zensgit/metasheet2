...
export function permissionsRouter(): Router {
  ...
  r.post('/api/permissions/grant', rbacGuard('permissions', 'write'), async (req: Request, res: Response) => {
    const { userId, permission } = req.body || {}
    if (!userId || !permission) return res.status(400).json({ ok: false, error: { code: 'VALIDATION_ERROR', message: 'userId and permission required' } })
    if (pool) {
      ...
      await auditLog({ actorId: (req as any).user?.id, actorType: 'user', action: 'grant', resourceType: 'permission', resourceId: `${userId}:${permission}`, meta: { userId, permission } })
      invalidateUserPerms(userId)
      const perms = await listUserPermissions(userId)
      return res.json({ ok: true, data: { userId, permissions: perms } })
    }
    const set = userPerms.get(userId) || new Set<string>()
    set.add(permission)
  ...
  r.post('/api/permissions/revoke', rbacGuard('permissions', 'write'), async (req: Request, res: Response) => {
    const { userId, permission } = req.body || {}
    if (!userId || !permission) return res.status(400).json({ ok: false, error: { code: 'VALIDATION_ERROR', message: 'userId and permission required' } })
    if (pool) {
      await pool.query('DELETE FROM user_permissions WHERE user_id=$1 AND permission_code=$2', [userId, permission])
      await auditLog({ actorId: (req as any).user?.id, actorType: 'user', action: 'revoke', resourceType: 'permission', resourceId: `${userId}:${permission}`, meta: { userId, permission } })
      invalidateUserPerms(userId)
      const perms = await listUserPermissions(userId)
      return res.json({ ok: true, data: { userId, permissions: perms } })
    }
  ...
  r.get('/api/permissions/cache-status', rbacGuard('permissions', 'read'), (req: Request, res: Response) => {
