...
export function auditLogsRouter(): Router {
  ...
  r.get('/api/audit-logs', rbacGuard('audit', 'read'), async (req: Request, res: Response) => {
    if (!pool) return res.status(503).json({ ok: false, error: { code: 'SERVICE_UNAVAILABLE', message: 'DB not configured' } })
    ...
    if (q.resourceId) push('resource_id = $n', q.resourceId)
    if (q.action) push('action = $n', q.action)
    if (q.from) push('occurred_at >= $n', q.from)
    if (q.to) push('occurred_at <= $n', q.to)
