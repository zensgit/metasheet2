...
export function rolesRouter(): Router {
  ...
  r.post('/api/roles', rbacGuard('roles', 'write'), async (req: Request, res: Response) => {
    ...
    if (pool) {
      ...
      await auditLog({ actorId: (req as any).user?.id, actorType: 'user', action: 'create', resourceType: 'role', resourceId: id, meta: { name, permissions: perms } })
  ...
  r.put('/api/roles/:id', rbacGuard('roles', 'write'), async (req: Request, res: Response) => {
    ...
    if (pool) {
      ...
      await auditLog({ actorId: (req as any).user?.id, actorType: 'user', action: 'update', resourceType: 'role', resourceId: id, meta: { before, after: { id, name } } })
    ...
    roles.set(id, next)
    await auditLog({ actorId: (req as any).user?.id, actorType: 'user', action: 'update', resourceType: 'role', resourceId: id, meta: { before, after: next } })
  ...
  r.delete('/api/roles/:id', rbacGuard('roles', 'write'), async (req: Request, res: Response) => {
    ...
    if (pool) {
      ...
      await auditLog({ actorId: (req as any).user?.id, actorType: 'user', action: 'delete', resourceType: 'role', resourceId: id, meta: { before } })
