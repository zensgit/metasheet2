...
import { buildMetaRecordQuery, isFilterOperatorSupported, type MetaFieldType } from '../utils/meta-query-builder'
...
const PUSHDOWN_FIELD_TYPES = new Set<UniverMetaField['type']>(['string', 'number', 'boolean', 'select'])
...
function normalizeLinkProperty(property: unknown): Record<string, unknown> {
  const obj = normalizeJson(property)
  const result: Record<string, unknown> = { ...obj }
  const foreign = obj.foreignSheetId ?? obj.foreignDatasheetId ?? obj.datasheetId
  if (typeof foreign === 'string' && foreign.trim().length > 0) {
    const trimmed = foreign.trim()
    result.foreignSheetId = trimmed
    result.foreignDatasheetId = trimmed
  }
  if (typeof result.displayFieldId === 'string') {
    const trimmed = result.displayFieldId.trim()
    if (trimmed.length > 0) result.displayFieldId = trimmed
    else delete result.displayFieldId
  }
  if (result.limitSingleRecord !== true) {
    delete result.limitSingleRecord
  }
  return result
}

function normalizeLookupProperty(property: unknown): Record<string, unknown> {
  const obj = normalizeJson(property)
  const result: Record<string, unknown> = { ...obj, readonly: true }
  const cfg = parseLookupFieldConfig(obj)
  if (!cfg) return result
  result.linkFieldId = cfg.linkFieldId
  result.targetFieldId = cfg.targetFieldId
  if (cfg.foreignSheetId) result.foreignSheetId = cfg.foreignSheetId
  return result
}

function normalizeRollupProperty(property: unknown): Record<string, unknown> {
  const obj = normalizeJson(property)
  const result: Record<string, unknown> = { ...obj, readonly: true }
  const cfg = parseRollupFieldConfig(obj)
  if (!cfg) return result
  result.linkFieldId = cfg.linkFieldId
  result.targetFieldId = cfg.targetFieldId
  result.aggregation = cfg.aggregation
  if (cfg.foreignSheetId) result.foreignSheetId = cfg.foreignSheetId
  return result
}

function enrichLookupRollupForeignSheetId(fields: UniverMetaField[]): UniverMetaField[] {
  if (fields.length === 0) return fields

  const linkConfigById = new Map<string, string>()
  for (const field of fields) {
    if (field.type !== 'link') continue
    const cfg = parseLinkFieldConfig(field.property)
    if (cfg?.foreignSheetId) {
      linkConfigById.set(field.id, cfg.foreignSheetId)
    }
  }

  if (linkConfigById.size === 0) return fields

  return fields.map((field) => {
    if (field.type !== 'lookup' && field.type !== 'rollup') return field

    const property = normalizeJson(field.property)
    const existing = property.foreignSheetId ?? property.foreignDatasheetId
    if (typeof existing === 'string' && existing.trim().length > 0) return field

    const linkFieldId = typeof property.linkFieldId === 'string' ? property.linkFieldId.trim() : ''
    if (!linkFieldId) return field

    const foreignSheetId = linkConfigById.get(linkFieldId)
    if (!foreignSheetId) return field
    ...
    return {
      ...
      property: {
        ...property,
        foreignSheetId,
        foreignDatasheetId: foreignSheetId,
      },
    }
  })
}
...
function sanitizeFieldProperty(type: UniverMetaField['type'], property: unknown): Record<string, unknown> {
  ...
  if (type === 'link') return normalizeLinkProperty(obj)
  if (type === 'lookup') return normalizeLookupProperty(obj)
  if (type === 'rollup') return normalizeRollupProperty(obj)
...
function toPushdownFieldType(type: UniverMetaField['type']): MetaFieldType | null {
  if (!PUSHDOWN_FIELD_TYPES.has(type)) return null
  switch (type) {
    case 'string':
    case 'number':
    case 'boolean':
...
export function univerMetaRouter(): Router {
  ...
  router.get('/fields', async (req: Request, res: Response) => {
    ...
    try {
      ...
      const fields = enrichLookupRollupForeignSheetId(result.rows.map((r: any) => serializeFieldRow(r)))
  ...
  router.get('/view', async (req: Request, res: Response) => {
    ...
    try {
      ...
      const fields: UniverMetaField[] = enrichLookupRollupForeignSheetId(
        fieldRes.rows.map((f: any) => serializeFieldRow(f)),
      )

      const fieldTypeById = new Map(fields.map((f) => [f.id, f.type] as const))
      const pushdownFieldTypes = new Map<string, MetaFieldType>()
      for (const field of fields) {
        const pushType = toPushdownFieldType(field.type)
        if (pushType) pushdownFieldTypes.set(field.id, pushType)
      }
      ...
      const filterInfo = filteredConditions.length > 0 && rawFilterInfo
        ...
        : null
      const hiddenFieldIds = Array.isArray(viewConfig?.hiddenFieldIds)
        ? viewConfig!.hiddenFieldIds!.filter((v) => typeof v === 'string')
        : []
      const ignoredHiddenFieldIds = hiddenFieldIds
        .map((v) => v.trim())
        .filter((v) => v.length > 0 && !fieldTypeById.has(v))
      const groupFieldId = viewConfig && typeof viewConfig.groupInfo === 'object' && !Array.isArray(viewConfig.groupInfo)
        ? (viewConfig.groupInfo as Record<string, unknown>).fieldId
        : undefined
      const ignoredGroupFieldIds: string[] = []
      ...
      if (ignoredHiddenFieldIds.length > 0) {
        warnings.push(`隐藏字段不存在，已忽略: ${ignoredHiddenFieldIds.join(', ')}`)
      }
      if (typeof groupFieldId === 'string' && groupFieldId.trim().length > 0) {
        const trimmedGroupId = groupFieldId.trim()
        const groupType = fieldTypeById.get(trimmedGroupId)
        if (!groupType) {
          warnings.push(`分组字段不存在，已忽略: ${trimmedGroupId}`)
          ignoredGroupFieldIds.push(trimmedGroupId)
        } else if (groupType !== 'select') {
          warnings.push(`分组字段需为 select，已忽略: ${trimmedGroupId}`)
          ignoredGroupFieldIds.push(trimmedGroupId)
        }
      }
      ...
      const loadInMemory = async (): Promise<{ rows: UniverMetaRecord[]; page?: UniverMetaView['page'] }> => {
        ...
        const rows = paged.map((r) => ({ id: r.id, version: r.version, data: r.data }))
        const page = limit ? { offset, limit, total, hasMore: offset + rows.length < total } : undefined

        return { rows, page }
      }
      const isSortedByRules = (items: UniverMetaRecord[]): boolean => {
        if (sortRules.length === 0 || items.length < 2) return true
        ...
        for (let idx = 1; idx < items.length; idx += 1) {
          ...
          for (const rule of sortRules) {
            const fieldType = fieldTypeById.get(rule.fieldId) ?? 'string'
            cmp = compareMetaSortValue(fieldType, prev.data[rule.fieldId], current.data[rule.fieldId], rule.desc)
            if (cmp !== 0) break
          }
      ...
      if (hasFilterOrSort) {
        ...
        const filterOperatorsSupported = filterInfo
          ? filterInfo.conditions.every((condition) => isFilterOperatorSupported(condition.operator))
          : true
        const filterFieldsPushable = filterInfo
          ? filterInfo.conditions.every((condition) => pushdownFieldTypes.has(condition.fieldId))
          : true
        const sortFieldsPushable = sortRules.every((rule) => pushdownFieldTypes.has(rule.fieldId))
        const forceMemory = process.env.META_VIEW_FORCE_MEMORY === 'true'
        const canPushdown =
          !forceMemory &&
          !needsComputedFilterSort &&
          filterOperatorsSupported &&
          filterFieldsPushable &&
          sortFieldsPushable

        if (canPushdown) {
          const sortInfoForQuery = sortRules.length > 0 ? { rules: sortRules } : null
          const query = buildMetaRecordQuery({
            sheetId,
            filterInfo: filterInfo ?? null,
            sortInfo: sortInfoForQuery,
            fieldTypes: pushdownFieldTypes,
            limit,
            offset,
          })
          if (process.env.DEBUG_META_QUERY === 'true') {
            console.info('[univer-meta] pushdown query', {
              sql: query.sql,
              params: query.params,
              countSql: query.countSql,
              countParams: query.countParams,
            })
          }

          const recordRes = await pool.query(query.sql, query.params)
          if (process.env.DEBUG_META_QUERY === 'true') {
            console.info('[univer-meta] pushdown result', {
              rowCount: recordRes.rowCount,
              rowIds: recordRes.rows.map((row: any) => row.id),
            })
          }
          ...
          if (limit) {
            const countRes = await pool.query(query.countSql, query.countParams)
            const total = Number((countRes.rows[0] as any)?.total ?? 0)
            page = { offset, limit, total, hasMore: offset + rows.length < total }
          }

          const shouldFallbackByFilter = !!filterInfo && rows.length === 0
          const shouldFallbackBySort = sortRules.length > 0 && (rows.length < 2 || !isSortedByRules(rows))
          if (shouldFallbackByFilter || shouldFallbackBySort) {
            const fallback = await loadInMemory()
            if (shouldFallbackByFilter) {
              if (fallback.rows.length > 0 || (fallback.page?.total ?? 0) > 0) {
                warnings.push('筛选下推结果为空，已回退为内存计算以确保结果。')
                rows = fallback.rows
                page = fallback.page
              }
            } else {
              warnings.push('排序下推结果异常，已回退为内存计算以确保结果。')
              rows = fallback.rows
              page = fallback.page
            }
          }
      ...
      const meta = warnings.length > 0 || hasFilterOrSort || ignoredHiddenFieldIds.length > 0 || ignoredGroupFieldIds.length > 0
        ? {
            ...
            ...(ignoredHiddenFieldIds.length > 0 ? { ignoredHiddenFieldIds } : {}),
            ...(ignoredGroupFieldIds.length > 0 ? { ignoredGroupFieldIds } : {}),
          }
        : undefined
