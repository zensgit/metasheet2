...
export class Max extends BaseFunction {
    meta: FunctionMeta = {
        ...
        examples: ['MAX(10, 20, 30)', 'MAX(A1:C5)'],
        params: [
            {
                name: 'value1',
                type: 'number | array',
                required: true,
                description: 'The first number, range, or array for which you want to find the maximum value.'
            },
            {
                name: 'value2',
                type: 'number | array',
    ...
    calculate(...variants: BaseValueObject[]): BaseValueObject {
        ...
        return new BaseValueObject(maxVal);
    }
}
