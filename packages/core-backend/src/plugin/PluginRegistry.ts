...
export interface DependencyGraph {
  nodes: Array<{ id: string; plugin: PluginRegistration }>
  edges: Array<{ from: string; to: string; type: 'dependency' | 'peer' }>
  cycles: string[][] // 循环依赖
}
...
export class PluginRegistry extends EventEmitter {
  ...
  private pluginsByCapability = new Map<PluginCapability, Set<string>>()
  private pluginsByAuthor = new Map<string, Set<string>>()
  private pluginTags = new Map<string, Set<string>>()
   ...
   * 注册插件
   */
  async register(manifest: PluginManifest, capabilities: PluginCapability[] = []): Promise<PluginRegistration> {
    ...
    try {
      ...
      this.logger.error(`Failed to register plugin ${pluginName}`, error as Error)
  ...
  async unregister(pluginName: string): Promise<void> {
    ...
    try {
      ...
      this.logger.error(`Failed to unregister plugin ${pluginName}`, error as Error)
      throw error
    }
  }
  ...
  query(options: PluginQuery = {}): PluginRegistration[] {
    ...
    if (options.tag) {
      ...
      results = results.filter(p =>
        p.manifest.description?.includes(options.tag!) ||
    ...
    if (options.enabled !== undefined) {
      ...
      results = results.filter(p =>
        enabled ? p.status === PluginStatus.ENABLED : p.status !== PluginStatus.ENABLED
      )
    }
   ...
   * 搜索插件
   */
  search(keyword: string, limit: number = 10): PluginSearchResult[] {
   ...
   * 检查版本兼容性
   */
  private async checkVersionCompatibility(manifest: PluginManifest): Promise<void> {
    // 这里可以检查与当前系统版本的兼容性
   ...
   * 检查已启用的依赖
   */
  private async checkEnabledDependencies(pluginName: string): Promise<{
    satisfied: boolean
    missing: string[]
  }> {
    const plugin = this.plugins.get(pluginName)
   ...
   * 解析依赖信息
   */
  private parseDependencies(manifest: PluginManifest): PluginDependency[] {
   ...
   * 查找启用的依赖插件
   */
  private findEnabledDependents(pluginName: string): string[] {
  ...
  private rebuildDependencyGraph(): void {
    const nodes: Array<{ id: string; plugin: PluginRegistration }> = []
    const edges: Array<{ from: string; to: string; type: 'dependency' | 'peer' }> = []
   ...
   * 持久化到数据库
   */
  private async persistToDatabase(registration: PluginRegistration): Promise<void> {
    ...
    try {
      ...
      this.logger.error('Failed to persist plugin to database', error as Error)
    }
  }
  ...
  private async removeFromDatabase(pluginName: string): Promise<void> {
    ...
    try {
      ...
      this.logger.error('Failed to remove plugin from database', error as Error)
    }
  }
   ...
   * 更新插件状态
   */
  private async updatePluginStatus(pluginName: string, status: PluginStatus): Promise<void> {
    if (!this.database) return
    ...
    try {
      ...
      this.logger.error('Failed to update plugin status in database', error as Error)
    }
  }
  ...
  async loadFromDatabase(): Promise<void> {
    ...
    try {
      ...
      this.logger.error('Failed to load plugins from database', error as Error)
    }
  }
}
