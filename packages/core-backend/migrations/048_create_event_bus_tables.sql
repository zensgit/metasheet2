...
CREATE INDEX IF NOT EXISTS idx_subscriptions_priority ON event_subscriptions (priority);
-- Create descending priority index separately (DESC not supported in inline INDEX)
...
CREATE TABLE IF NOT EXISTS event_store (
  ...
  -- event_id should be unique globally
  UNIQUE (event_id, occurred_at)
) PARTITION BY RANGE (occurred_at);

-- CI guard: if a legacy non-partitioned event_store slipped in earlier, drop and recreate as partitioned
DO $fn$
DECLARE
...
BEGIN
  ...
  SELECT EXISTS (
    SELECT 1 FROM pg_class WHERE relname = 'event_store'
  ) INTO exists_event_store;

  IF exists_event_store AND NOT is_partitioned THEN
    -- Fresh CI DBs have no data; safe to drop and recreate to ensure partitioning
    ...
    EXECUTE $ddl$
      CREATE TABLE IF NOT EXISTS event_store (
        ...
        occurred_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        received_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        status TEXT NOT NULL DEFAULT 'pending' CHECK (status IN ('pending','processing','processed','failed','expired','archived')),
        processed_at TIMESTAMPTZ,
        expires_at TIMESTAMPTZ,
        PRIMARY KEY (id, occurred_at),
        UNIQUE (event_id, occurred_at)
...
-- If a non-partitioned table named event_store already exists (e.g., legacy CI),
-- this CREATE will be skipped; ensure partitioning by swapping in an empty partitioned table.
-- Note: Legacy non-partitioned swap logic removed for CI stability.
-- In CI (fresh DB), the partitioned definition above is sufficient.

-- Indexes for event_store
CREATE INDEX IF NOT EXISTS idx_event_store_name ON event_store (event_name, occurred_at);
...
-- Create descending occurred_at index separately (DESC not supported inline)
CREATE INDEX IF NOT EXISTS idx_event_store_occurred_desc ON event_store (event_name, occurred_at DESC);
-- Create partial index for expires_at (WHERE clause not supported inline)
CREATE INDEX IF NOT EXISTS idx_event_store_expires ON event_store (expires_at) WHERE expires_at IS NOT NULL;

-- Create partitions for event store
...
-- Create partial indexes for event_queue (WHERE clauses not supported inline)
CREATE INDEX IF NOT EXISTS idx_queue_status_pending ON event_queue (status, scheduled_at) WHERE status = 'pending';
CREATE INDEX IF NOT EXISTS idx_queue_retry_failed ON event_queue (next_retry_at) WHERE status = 'failed' AND attempt_count < max_attempts;
...
-- Indexes for event_dead_letters
...
CREATE INDEX IF NOT EXISTS idx_dead_letters_retry ON event_dead_letters (retry_after);
-- Create partial index for event_dead_letters (WHERE clause not supported inline)
CREATE INDEX IF NOT EXISTS idx_dead_letters_retry_ready ON event_dead_letters (retry_after) WHERE can_retry = true;
...
-- Indexes for event_replays
CREATE INDEX IF NOT EXISTS idx_replays_status ON event_replays (status);
CREATE INDEX IF NOT EXISTS idx_replays_created ON event_replays (created_at);
-- Create descending created_at index separately
CREATE INDEX IF NOT EXISTS idx_replays_created_desc ON event_replays (created_at DESC);
...
-- Indexes for plugin_event_permissions
...
DO $tg$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'process_subscriptions_on_event') THEN
...
DO $tg$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_aggregates_on_event') THEN
...
DO $tg$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'handle_failed_events_trigger') THEN
