...
exec npx vitest run \
  ...
  只有在名字相邻时才可能碰同一处 \
  字节的单物理行。它是全仓并发冲突最密集 \
  插入到它该在的位置。字母序不是洁癖：它让「新 \
  往末尾追加，多行化就白做了——所以有守卫强制序（见下）。 \
  仍然要做双向子串碰撞检查（本文件通篇的老规矩），以及另一登记点 \
  -- \
  --reporter=dot。 \
  --reporter=dot`（无续行）。 \
  --reporter=verbose` \
  ...
  (dead) \
  (list-load \
  (list/create/rename/archive/unarchive/link/unlink), \
  (measured \
  (mounted \
  (never \
  (no \
  ...
  **大小写不敏感字母序** \
  ...
  `approvalTemplateGroupsClient`, \
  `ApprovalTemplateGroupsPanel.vue` \
  `ApprovalTemplateGroupsPanel` \
  `ApprovalTemplateGroupsPanel`, \
  `apps/web/src/components/SessionOrgSwitcher.vue` \
  `attendance-web-guard.yml:297-301,:397-400`'s \
  `Attendance` \
  `bash \
  `bc66e283e` \
  `bc66e283e`. \
  ...
  `SessionOrgSwitcher` \
  `tail \
  `TemplateCenterView.vue` \
  `tests/AttendanceSessionOrgSwitcher.spec.ts` \
  `useSessionOrg`/`useAuth` \
  `views/attendance/AttendanceSessionOrgSwitcher.vue`, \
  `web-tests` \
  + \
  <sha>:<this \
  <this \
  ...
  P3 \
  P3-b, \
  p3-hygiene-gate2-A2-20260919.md \
  地互撞。插件侧同类问题（package.json \
  ...
  script \
  scripts.test \
  scripts/dev/atg-exec-line-post-rebase-check.sh's \
