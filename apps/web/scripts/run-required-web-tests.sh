...
# (apps/web/tests/multitable-field-config-panel.spec.ts). Run as its OWN standalone
# `npx vitest run` line rather than appended into the giant final `exec` line below —
# that line is a single ~10KB physical line, so a second concurrent PR appending its
# own token there (the parallel #5602 hint/palette/retype branch does exactly this,
# adding `multitable-formula-editor`) always collides on that one line under any
# 3-way merge, regardless of where in the line either side inserts. A separate line
# has no such collision risk and still runs under `set -euo pipefail` (line 436) —
# a failure here aborts the script exactly like a failure in the final exec would.
