...
# Attendance import token/org binding: these specs are also listed in attendance-web-guard.yml;
# keep their execution explicit in the always-on required lane.
npx vitest run attendance-import-preview-regression.spec.ts useAttendanceAdminImportWorkflow.spec.ts useAttendanceAdminImportBatches.spec.ts --reporter=dot || exit $?
