...
# Posture plain-language fence (2026-09-03): `StockPreparationPosturePlainLanguage` — a NEW spec
# (apps/web/tests/StockPreparationPosturePlainLanguage.spec.ts) asserting that
# STOCK_PREP_POSTURE_PLAIN carries a line for every `posture.entries[].id` declared in the plugin's
# app.manifest.json, read from the repo rather than copied into the test.
#
# WHY IT HAD TO BE ADDED HERE, and not merely written: the spec exists because a posture fence
# shipped to the install page with no plain-language line and every check stayed green. Writing a
# guardrail and leaving it outside this list would have reproduced that exact failure one level up —
# vitest's default glob does NOT decide what this required gate runs; this token list does.
# Substring-collision checked per the convention above: `npx vitest run
# StockPreparationPosturePlainLanguage --reporter=verbose` matches exactly one file ("Test Files 1
# passed (1)"), and the token is not a substring of StockPreparationInstallView /
# StockPreparationInstallRun / StockPreparationSourcePreflight / StockPreparationWorkspace /
# StockPreparationProjectWorkspaceView, nor they of it.
