...
// Extracted verbatim from views/DataSourcesView.vue (which is now a thin shell around this
// component) so the 外接数据源 surface can ALSO render inside 数据工厂's 连接管理 section
// (IntegrationConnectionSection.vue). Every data-testid, every store call and every branch is
// unchanged — the only additions are the `embedded` presentation prop and the `changed` emit.
//
// `embedded` is PRESENTATION ONLY: it demotes the page heading to an in-section one and drops
// the page-level padding/max-width chrome. It grants nothing and hides no control — the same
// store, the same API client and therefore the same server-side permission gate apply in both
// mountings.
