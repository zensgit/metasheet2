 ...
 * - the backend may use an id as a missing-directory fallback; ordinary authoring surfaces replace
 *   that internal value with a values-free label rather than exposing a raw identity key.
 */
export function routePreviewAssigneeSummary(node: ApprovalRoutePreviewNode): string {
  ...
  return node.assignees.map((a) => {
    if (!a.name.trim() || a.name === a.id) {
      return a.assignmentType === 'role' ? '角色信息待确认' : '成员信息待确认'
    }
    return a.assignmentType === 'role' ? `角色:${a.name}` : a.name
  }).join('、')
}
