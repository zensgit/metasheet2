...
const t = computed(() => {
  if (isZh.value) {
    return {
      ...
      handoffIntro: '当前设计器收到来自考勤审批构建器的接力上下文，并会自动套用推荐起步模板。它不会替换当前线上考勤审批执行逻辑。',
  ...
  return {
    ...
    handoffIntro: 'This designer received handoff context from the attendance approval builder and auto-applies the recommended starter template. It does not replace the live attendance approval engine yet.',
