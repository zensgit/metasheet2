...
const LABELS: Record<MetaManagerLabelKey, { en: string; zh: string }> = {
  ...
  'formatting.noFieldsHint': { en: 'Add fields to the sheet to create formatting rules.', zh: '请先向数据表添加字段，再创建格式规则。' },
  ...
  // W1 G-10 terminology dictionary (ratified 2026-07-13, display layer only): Sheet = 数据表,
  // Base = 工作区 in every zh string below (was inconsistently '表'/'多维表' — the latter is the
  // PRODUCT name and must not stand in for the Base entity). English values are unchanged.
  'field.targetSheet': { en: 'Target sheet', zh: '目标数据表' },
  'field.selectSheet': { en: 'Select sheet', zh: '选择数据表' },
  ...
  'field.formulaDryRun.recordHint': { en: "Samples this record's saved values across the whole sheet, with your field-read permissions applied — not a mirror of the columns shown in the current view.", zh: '按整张数据表取该记录已保存的值，并按你的字段读取权限脱敏——并非镜像当前视图所显示的列。' },
  ...
  'field.formulaDryRun.forbidden': { en: 'You cannot manage fields on this sheet', zh: '你没有管理该数据表字段的权限' },
  ...
  'field.error.rollupNeedsValidTargetSheet': { en: 'Rollup fields need a valid target sheet', zh: '汇总字段需要有效的目标数据表' },
  ...
  'view.latestMetadataLoaded': { en: 'Latest view metadata loaded from the sheet context.', zh: '已从数据表上下文加载最新视图元数据。' },
  'view.latestFieldMetadataLoaded': { en: 'Latest field metadata loaded from the sheet context.', zh: '已从数据表上下文加载最新字段元数据。' },
