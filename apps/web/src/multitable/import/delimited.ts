...
export async function buildImportedRecords(params: {
  ...
  for (const [rowIndex, row] of parsedRows.entries()) {
    ...
    for (const [colIdx, fieldId] of Object.entries(fieldMapping)) {
      ...
      if (field?.type === 'number' && val !== '') data[fieldId] = Number(val)
      ...
      } else if (field && isPersonField(field)) {
        ...
        try {
          ...
          rowFailure = error?.message ?? `Unable to resolve people value for ${field.name}: ${rawValue}`
          failingField = field
          break
        }
        if (resolved === null || resolved === undefined) {
          rowFailure = `Unable to resolve people value for ${field.name}: ${rawValue}`
          failingField = field
          break
        }
