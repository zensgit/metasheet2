...
// Legacy flat-path field mask. Sectioned rendering also applies B1 visibility and hide-empty rules.
...
// Use the template's groups and controlled disclosure state, not a second field-mask approximation.
function hasRenderedField(fieldId: string): boolean {
  return fieldGroups.value.some(group =>
    (!group.heading || (sectionExpanded[group.key] ?? group.defaultExpanded))
    && group.fields.some(field => field.id === fieldId),
  )
}
defineExpose({ hasRenderedField })
...
function isHideEmptyExempt(field: MetaField): boolean {
  ...
  //     or on record change). PATCH rejection alerts must remain visible too.
  if (attachmentErrors.value[field.id] || fieldError(field.id)) return true
  return false
}
