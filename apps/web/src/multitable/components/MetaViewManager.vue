...
function hydrateExistingViewConfig(view: MetaView) {
...
function saveConfig() {
  const target = configTarget.value
  if (!target) return
  if (viewConfigBlockingReason.value) return
...
watch(
  ...
  () => {
    if (configTargetId.value && !configTarget.value) closeConfig()
      ...
      if (viewConfigDirty.value) viewConfigOutdated.value = true
      else hydrateExistingViewConfig(configTarget.value)
    }
