...
function hydrateExistingFieldConfig(field: MetaField) {
  ...
  if (fieldType === 'select') {
    const options = resolveSelectFieldOptions(field.property)
    selectDraft.options = options.length > 0
      ? options.map((option) => ({ value: option.value, color: option.color ?? '' }))
      : [{ value: '', color: '' }]
...
watch(
  ...
  () => {
    if (configTargetId.value && !configTarget.value) closeConfig()
      ...
      if (fieldConfigDirty.value) fieldConfigOutdated.value = true
      else hydrateExistingFieldConfig(configTarget.value)
    }
