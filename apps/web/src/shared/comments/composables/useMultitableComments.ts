...
// multitable/composables/useMultitableComments.ts, with ONE real behavior change confined to the
// function signature — the composable now depends on the CommentsApiClient INTERFACE (the 7
// methods it actually calls, extracted from this file's own usage; see api-client.ts) instead of
// the concrete MultitableApiClient class, and takes that client as a REQUIRED parameter instead
// of defaulting to the multitableClient singleton. That default-wiring moved to
// multitable/composables/useMultitableComments.ts, which is now a thin wrapper supplying
// `multitableClient` as the default — so multitable's own external call sites
// (`useMultitableComments()` with zero args) are byte-identical in behavior. Every method body
// below is unchanged.
import { ref } from 'vue'
import { useLocale } from '../../../composables/useLocale'
import type { MultitableComment } from '../types'
import type { CommentsApiClient, CommentsTarget } from '../api-client'
import { commentLabel } from '../utils/meta-comment-labels'

export function useMultitableComments(api: CommentsApiClient) {
