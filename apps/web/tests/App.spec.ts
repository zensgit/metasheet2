...
describe('App sheet chrome (full-bleed workbench)', () => {
  ...
  it('declares a narrow-viewport rule for the 40px sheet strip', async () => {
    ...
    expect(source).toMatch(/\.app-sheet-chrome \.brand-text[\s\S]*font-size:\s*13px/)
    expect(source).toMatch(/\.app-sheet-chrome \.brand-text[\s\S]*font-weight:\s*400/)
    expect(source).toMatch(/\.sheet-chrome__back \{[\s\S]*background:\s*transparent/)
    expect(source).toMatch(/\.sheet-chrome__back \{[\s\S]*border:\s*none/)
  })
