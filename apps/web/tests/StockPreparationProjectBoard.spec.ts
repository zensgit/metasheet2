...
describe('项目备料页 — the operator project board', () => {
  ...
  // 上次同步 — three states, and the middle one exists only because the row scan behind it can be
  // TRUNCATED (see stock-preparation-project-board.cjs `lastPulledAtBounded`). The message for that
  // state must say "we could not count", not "never synced" — those are different facts.
  it('B-04: 上次同步 shows the last PLM refresh, a truncation notice, or a dash — never the same text for all three', async () => {
    routeApi({ board: () => ok(boardPayload({ lastPulledAt: null, lastPulledAtBounded: false })) })
    const noneYet = await mountBoard()
    const dash = (noneYet.querySelector('[data-testid="stock-prep-project-board-last-pulled"]') as HTMLElement).textContent
    expect(dash).toContain('—')
    remount()

    routeApi({ board: () => ok(boardPayload({ lastPulledAt: null, lastPulledAtBounded: true })) })
    const truncated = await mountBoard()
    const truncatedText = (truncated.querySelector('[data-testid="stock-prep-project-board-last-pulled"]') as HTMLElement).textContent ?? ''
    expect(truncatedText).toContain('未统计')
    expect(truncatedText).not.toContain('—')
    remount()

    routeApi({ board: () => ok(boardPayload({ lastPulledAt: '2026-09-04T08:00:00.000Z', lastPulledAtBounded: false })) })
    const synced = await mountBoard()
    const syncedText = (synced.querySelector('[data-testid="stock-prep-project-board-last-pulled"]') as HTMLElement).textContent ?? ''
