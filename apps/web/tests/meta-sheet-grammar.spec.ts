...
describe('sheet workbench grammar tokens', () => {
  ...
  it('keeps grid body at 13px with tabular nums, white header, hairline, no card shadow, quiet selection', () => {
    ...
    expect(grid).toMatch(/\.meta-grid__row-num \{[\s\S]*padding:\s*8px 2px/)
    expect(grid).toMatch(/const ROW_NUM_W = 56/)
    expect(grid).toMatch(/\.meta-grid__row-num \{[\s\S]*grid-template-columns:\s*12px minmax\(0,\s*1fr\) 12px/)
    expect(grid).toMatch(/\.meta-grid__expand-btn \{[\s\S]*grid-column:\s*1/)
    expect(grid).toMatch(/\.meta-grid__row-num-lock-slot \{[\s\S]*grid-column:\s*3/)
    expect(grid).toMatch(/class="meta-grid__row-num-lock-slot"/)
    expect(grid).not.toMatch(/row-num-cluster/)
