...
describe('ApprovalDetailView — UI-6 detail tab anchors + audit-derived record table', () => {
  ...
  describe('审批记录 table view', () => {
    ...
    it('table rows equal timeline entries 1:1 (2 history rows) plus a synthetic 提交 bookend (pending — no 结束 yet)', async () => {
      ...
      // Hardcoded, not derived from `mockHistory.value.length`: 1 synthetic 提交 + 2 real history
      // rows (1:1 with the fixture above) + 0 结束 (still pending).
      expect(rows).toHaveLength(3)

      // Synthetic 提交 row: derived from the instance's OWN requester/createdAt, not history.
      ...
      // Real row 1 (hist_1 'created') — 1:1 with the fixture's first history entry.
      expect(rows[1].querySelector('[data-el-cell="节点名称"]')?.textContent?.trim()).toBe('start')
      expect(rows[1].querySelector('[data-el-cell="审批人"]')?.textContent?.trim()).toBe('张三')

      // Real row 2 (hist_2 'approve') — a specific row's actor/node, matching the fixture.
      expect(rows[2].querySelector('[data-el-cell="节点名称"]')?.textContent?.trim()).toBe('approval_1')
      expect(rows[2].querySelector('[data-el-cell="审批人"]')?.textContent?.trim()).toBe('李四')
      expect(rows[2].querySelector('[data-el-cell="审批结果/时间"]')?.textContent).toContain('通过')
      ...
      expect(rows[1].querySelector('.approval-detail__timeline-meta')).toBeNull()
    })
    ...
    it('appends a synthetic 结束 row only once the instance is terminal', async () => {
      ...
      const rows = recordTableRows(container!)
      // Hardcoded: 1 提交 + 2 history + 1 结束 (terminal status).
    ...
    it('reuses the existing badge helpers — auto-approval badge appears in the table exactly as the timeline shows it', async () => {
      ...
      const rows = recordTableRows(container!)
      expect(rows).toHaveLength(4) // 1 提交 + 3 history rows, still pending (no 结束).
  ...
  describe('no second fetch, no persistence of synthetic rows', () => {
    ...
    it('the synthetic 结束 bookend (terminal fixture) is presentation-only — store.history stays byte-identical', async () => {
      ...
      q(container!, 'approval-detail-record-view-table')!.click()
      await flushUi()
      // Confirm the table actually grew the synthetic bookends (sanity: this isn't vacuous) —
      // hardcoded count, so a mutation that leaks a synthetic row into `store.history` (which
      // would inflate BOTH this count and `originalHistory.length` together) cannot hide behind
      // a self-referential `originalLength + 2` comparison.
      expect(recordTableRows(container!)).toHaveLength(4)
    ...
    it('the synthetic 提交 bookend is absent from a real outgoing action payload (positive-control outcome assertion, not just a bare negative)', async () => {
      // Pending fixture (not terminal) so the real 通过 action bar renders — this is the SAME
      // pending baseInstance used by the 1:1 row-count test above (2 history rows, 提交-only
      // bookend, no 结束).
      mockCanAct.value = true
      await mountView()
      ...
      q(container!, 'approval-detail-record-view-table')!.click()
      await flushUi()
      expect(recordTableRows(container!)).toHaveLength(3) // 1 提交 + 2 history, still pending
