...
describe('MultitableApiClient recovery archive routes', () => {
  ...
  it.each([
    ...
    { entries: [{ ...plannedJob, resumeDeadline: Date.parse(plannedJob.resumeDeadline) }], nextCursor: null },
    { entries: [{ ...plannedJob, terminalAt: 'invalid' }], nextCursor: null },
    { entries: [{ ...plannedJob, terminalAt: Date.parse('2026-08-29T01:00:00.000Z') }], nextCursor: null },
