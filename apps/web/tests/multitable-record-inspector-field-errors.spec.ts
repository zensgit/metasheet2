...
describe('useMultitableGrid.patchCell — returns its own GridPatchFailure (PR-B2 §1.3, additive)', () => {
  ...
  it('same-field requests are not serialized: both reach fetch before either response settles', async () => {
    const pending: Array<(response: Response) => void> = []
    const fetchFn = vi.fn(() => new Promise<Response>(resolve => { pending.push(resolve) }))
    const grid = gridWith(fetchFn)
    const first = grid.patchCell('r1', 'f1', 'first', 1)
    const second = grid.patchCell('r1', 'f1', 'second', 1)
    expect(fetchFn).toHaveBeenCalledTimes(2)
    expect(pending).toHaveLength(2)
    for (const resolve of pending) {
      resolve(new Response(JSON.stringify({ ok: false, error: { code: 'VALIDATION_ERROR', message: 'Rejected' } }), { status: 400 }))
    }
    const failures = await Promise.all([first, second])
    expect(failures.map(failure => [failure?.fieldId, failure?.attemptedValue])).toEqual([['f1', 'first'], ['f1', 'second']])
  })
...
describe('MetaRecordInspector.canAnchorFieldError (PR-B2 round 2: the alert must be able to render, else the workbench toasts)', () => {
  ...
  function mountInspector(opts: {
    fieldPermissions?: Record<string, { visible?: boolean }>
    fields?: MetaField[]
    inspectorFieldLayout?: { ordered: MetaField[]; hiddenInView: MetaField[] }
    record?: MetaRecord
  } = {}) {
    const fieldErrors = ref<Record<string, string> | null>(null)
    ...
    const app = createApp({
      render() {
        return h(MetaRecordInspector, {
          ...
          record: opts.record ?? REC_1,
          fields: opts.fields ?? FIELDS,
          inspectorFieldLayout: opts.inspectorFieldLayout,
  ...
  it('B1/B2: a collapsed field cannot anchor; expanding the same section makes it anchorable', async () => {
    const { container, insRef } = mountInspector({ inspectorFieldLayout: { ordered: [TITLE_FIELD], hiddenInView: [NOTES_FIELD] } })
    await flushUi()
    expect(container.querySelector('#drawer_field_fld_notes')).toBeNull()
    expect(insRef.value!.canAnchorFieldError('rec_1', 'fld_notes')).toBe(false)
    container.querySelector<HTMLButtonElement>('[data-testid="record-field-section-toggle-hidden-in-view"]')!.click()
    await flushUi()
    expect(container.querySelector('#drawer_field_fld_notes')).toBeTruthy()
    expect(insRef.value!.canAnchorFieldError('rec_1', 'fld_notes')).toBe(true)
  })

  it('B1/B2: property-hidden fields cannot anchor while ordinary fields still can', async () => {
    const hidden = { ...NOTES_FIELD, property: { hidden: true } } as MetaField
    const fields = [TITLE_FIELD, hidden]
    const { container, insRef } = mountInspector({ fields, inspectorFieldLayout: { ordered: fields, hiddenInView: [] } })
    await flushUi()
    expect(container.querySelector('#drawer_field_fld_notes')).toBeNull()
    expect(insRef.value!.canAnchorFieldError('rec_1', 'fld_notes')).toBe(false)
    expect(insRef.value!.canAnchorFieldError('rec_1', 'fld_title')).toBe(true)
  ...
  it('B1/B2: hide-empty removes the anchor, but an existing rejection stays visible with its draft', async () => {
    const { container, insRef, fieldErrors } = mountInspector({ record: { ...REC_1, data: { ...REC_1.data, fld_notes: '' } } })
    await flushUi()
    expect(insRef.value!.canAnchorFieldError('rec_1', 'fld_notes')).toBe(true)
    ...
    await flushUi()
    const notes = container.querySelector<HTMLTextAreaElement>('#drawer_field_fld_notes')!
    notes.value = 'Rejected draft'
    notes.dispatchEvent(new Event('change', { bubbles: true }))
    fieldErrors.value = { fld_notes: 'Rejected' }
    await flushUi()
    toggle.click()
    await flushUi()
    expect(container.querySelector<HTMLTextAreaElement>('#drawer_field_fld_notes')?.value).toBe('Rejected draft')
    expect(alertsIn(container).map(node => node.textContent)).toEqual(['Rejected'])
    expect(insRef.value!.canAnchorFieldError('rec_1', 'fld_notes')).toBe(true)
  })
})
