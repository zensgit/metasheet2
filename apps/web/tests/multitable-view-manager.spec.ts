...
describe('MetaViewManager', () => {
  ...
  // the header close-× (`.meta-view-mgr__close`) was migrated from a bespoke <button>&times;</button>
  ...
  // This file also has TWO other `×` glyph buttons that are NOT close buttons — remove-item actions
  // (removeFilterRule / removeSortRule, both `.meta-view-mgr__action--danger`) — neither was touched by
  // this migration. The POSITIVE CONTROL test below proves removeFilterRule still fires unchanged.
  describe('UI-P2-1c T1 batch-7 — header close-× → MtIconButton', () => {
    ...
    it('renders the header close-× as a native <button> (MtIconButton) keeping the class + glyph', async () => {
      ...
      const btn = container.querySelector('.meta-view-mgr__close') as HTMLButtonElement
    ...
    it('clicking the header close-× still calls requestClose -> emits SAME `close` event (no unsaved drafts, no confirm gate)', async () => {
      ...
      ;(container.querySelector('.meta-view-mgr__close') as HTMLButtonElement).click()
      await nextTick()

      expect(closeSpy).toHaveBeenCalledTimes(1)
    })

    it('POSITIVE CONTROL: the unmigrated remove-filter-rule × (removeFilterRule, same glyph, different button) still fires — proves this migration left it alone AND that this test file can observe these buttons', async () => {
      const container = mountManager({})
      await nextTick()
      ...
      // removeFilterRule(0) spliced the row out — unchanged behavior, untouched by this migration
