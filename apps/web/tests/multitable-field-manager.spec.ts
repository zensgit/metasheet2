...
describe('MetaFieldManager — layer-2 visibility keys survive a config Save', () => {
  ...
  // UI-P2-1c T1 batch-7 (multitable-ui-p2-1c-tail-resolution-designlock-20260707.md §2-T1, RATIFIED):
  // the header close-× (`.meta-field-mgr__close`) was migrated from a bespoke <button>&times;</button>
  // to the shared MtIconButton primitive — the &times; glyph passes through MtIconButton's default-slot
  // icon fallback (glyph char preserved, size token-normalized to the icon control: 20px -> 14px glyph,
  // 32x32 control box, consistent with the existing glyph-MtIconButton controls already on main).
  // This file also has THREE other `×` glyph buttons that are NOT close buttons — remove-item actions
  // (removeSelectOption @ .meta-field-mgr__action--danger, removeRollupFilter @ .meta-field-mgr__icon-btn,
  // removeAiOption @ .meta-field-mgr__action--danger) — none of them were touched by this migration.
  // The POSITIVE CONTROL test below proves removeSelectOption still fires unchanged.
  describe('UI-P2-1c T1 batch-7 — header close-× → MtIconButton', () => {
    ...
    afterEach(() => {
      ...
      expect(document.body.children.length).toBe(0) // residue guard
    })
    ...
    function mountManager(props: Record<string, unknown>) {
      ...
      const app = createApp({
        render() {
          return h(MetaFieldManager, { visible: true, sheetId: 'sheet_1', sheets: [], fields: [], ...props })
        },
      })
      app.mount(container)
    ...
    it('renders the header close-× as a native <button> (MtIconButton) keeping the class + glyph', async () => {
      ...
      const btn = container.querySelector('.meta-field-mgr__close') as HTMLButtonElement
    ...
    it('clicking the header close-× still calls requestClose -> emits SAME `close` event (no unsaved drafts, no confirm gate)', async () => {
      const closeSpy = vi.fn()
      const container = mountManager({ onClose: closeSpy })
      await nextTick()

      ;(container.querySelector('.meta-field-mgr__close') as HTMLButtonElement).click()
      await nextTick()

      expect(closeSpy).toHaveBeenCalledTimes(1)
    })

    it('POSITIVE CONTROL: the unmigrated remove-option × (removeSelectOption, same glyph, different button) still fires — proves this migration left it alone AND that this test file can observe these buttons', async () => {
      const container = mountManager({})
      await nextTick()
      ...
      const removeButtons = Array.from(
        container.querySelectorAll('.meta-field-mgr__option-row .meta-field-mgr__action--danger'),
      ) as HTMLButtonElement[]
