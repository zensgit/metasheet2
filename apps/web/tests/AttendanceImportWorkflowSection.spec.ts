...
describe('AttendanceImportWorkflowSection', () => {
  ...
  it('summarizes the current import plan with profile, user map, and group sync details', async () => {
    ...
    expect(importGroupTimezone!.selectedOptions[0]?.textContent).toContain('Asia/Shanghai (UTC+08:00)')
  ...
  it('shows manual-plan fallbacks when mapping, user map, and group sync are unset', async () => {
    ...
    const importGroupTimezone = container!.querySelector<HTMLSelectElement>('#attendance-import-group-timezone')
    expect(importGroupTimezone).toBeTruthy()
    expect(importGroupTimezone!.selectedOptions[0]?.textContent).toContain('Use import timezone (UTC+08:00 · Asia/Shanghai)')
  })
})
