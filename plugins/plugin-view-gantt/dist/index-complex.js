...
class GanttPlugin {
    ...
    async activate(context) {
        ...
        // Initialize handlers
        this.apiHandler = new api_1.GanttAPIHandler(context, this);
        this.eventHandlers = new handlers_1.GanttEventHandlers(context, this);
        // Register API routes
        this.registerRoutes();
    ...
    registerWebSocketEvents() {
        const { api } = this.context;
        api.websocket.onConnection((socket) => {
            // Handle subscribe requests
            socket.on('gantt:subscribe', async (data) => {
                const { viewId } = data;
            ...
            // Handle unsubscribe
            socket.on('gantt:unsubscribe', (data) => {
                const { viewId } = data;
                socket.leave(`gantt:${viewId}`);
            });
            // Handle task updates via WebSocket
            socket.on('gantt:task:update', async (data) => {
     ...
     * Create a dependency
     */
    async createDependency(viewId, dependencyData) {
        const { api } = this.context;
        const query = `
     ...
     * Calculate critical path
     */
    async calculateCriticalPath(viewId) {
        ...
        const startTasks = tasks.filter(task => !dependencies.some(dep => dep.targetTaskId === task.id));
        // Find tasks with no dependents (end tasks)
        const endTasks = tasks.filter(task => !dependencies.some(dep => dep.sourceTaskId === task.id));
     ...
     */
    findLongestPath(taskId, tasks, dependencies) {
     ...
     */
    calculatePathDuration(path, tasks) {
        let totalDuration = 0;
