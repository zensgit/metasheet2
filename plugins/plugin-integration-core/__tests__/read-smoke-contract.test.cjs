...
// --- C3 LIST-only preset: explicit intent shape, optional key only maps to internal FNumber filter ---
...
assert.deepEqual(normalizeReadSmokeContract({
  presetId: 'k3wise.material-list.v1',
  intent: { object: 'material', mode: 'list', key: '  MAT-  ' },
}), {
  presetId: 'k3wise.material-list.v1',
  object: 'material',
  mode: 'list',
...
assert.throws(() => normalizeReadSmokeContract({ presetId: 'k3wise.material-list.v1', intent: { object: 'material', mode: 'list', key: '   ' } }), isErr('key_required'))
...
for (const bad of ['limit', 'filters', 'Filter', 'OrderBy', 'Fields', 'cursor', 'watermark', 'pagination', 'readPath', 'readMethod']) {
  assert.throws(
