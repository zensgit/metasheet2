...
| C3 | 「Teable `packages/*`=MIT、有 `packages/formula` ANTLR 解析器（可采用/改写）」（v2 据并行 session 的不同 Teable checkout 转述，**未对本仓 corpus 验证**） | **错（用户核出，已对盘穷尽验证）**。§9 锁定的 `references/teable`（tarball@main）中：root LICENSE = AGPL v3，`packages/*`+`apps/*` 共 **11/11 个 `license` 字段全部 `AGPL-3.0`**，无 MIT、无双授权；**无 `packages/formula`**，公式解析/visitor 在 `references/teable/packages/core/src/formula/`（属 `packages/core` = AGPL）。→ **整个 Teable 仅借鉴模式，无任何可直接采用的部分**。见 §6.2 |
...
| Formula | 自研引擎 `{fld_xxx}` + 跨表 lookup/rollup（`packages/core-backend/src/multitable/formula-engine.ts`） | **Node 应用层逐行算后写回列**（`references/teable/apps/nestjs-backend/src/features/calculation/field-calculation.service.ts`）；**非**下推 PG 表达式、**非** HyperFormula（`grep -ri hyperformula references/teable` = 0 命中）；公式解析/visitor 在 `references/teable/packages/core/src/formula/`（如 `field-reference.visitor.ts`；属 `packages/core` = **AGPL-3.0**；本 corpus **无** `packages/formula`，见 §0 C3） |
...
### 6.2 法律/许可（落地前必读）
`references/` 已在 `.git/info/exclude` 本地忽略，**永不提交**外部源码。

- **Teable**：**整仓 AGPL-3.0** —— 对盘穷尽核实（root `LICENSE` = AGPL v3；`packages/*`+`apps/*` 共 **11/11 个 `package.json` 的 `license` 字段全部 `AGPL-3.0`**，无 MIT、无双授权）。→ **全部仅借鉴模式，无任何可直接采用/改写的部分**。
  - ⚠️ 更正（§0 C3）：本文 v2 曾据**并行 session 的不同 Teable checkout**写「`packages/*`=MIT、有 `packages/formula`」——在 §9 锁定的 `references/teable` tarball@main corpus 中**不成立**（无 `packages/formula`；公式代码在 `packages/core/src/formula/`，AGPL）。差异可能源于 Teable 历史上的 relicense 或对方 checkout 较旧；**以本仓 §9-pinned canonical corpus 为准**。
- **NocoBase**：`plugin-workflow*` = **AGPL-3.0**，仅借鉴模式。
- **APITable**：AGPL-3.0。

→ 三方均 **AGPL，全部仅借鉴模式**。读 AGPL 源码研习架构始终合法；**严禁把 AGPL 源码拷进我们闭源代码库**。本对标只描述架构/模式 + file:line 锚点，不复制实现。
