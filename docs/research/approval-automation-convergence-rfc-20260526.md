...
**automation（线性、无挂起）**：
- 执行：`packages/core-backend/src/multitable/automation-executor.ts:578`（`executeActions`）逐个动作跑，失败即 `execution.status='skipped'` 余下跳过（`:549` 一带），**无分支/循环/挂起**。
