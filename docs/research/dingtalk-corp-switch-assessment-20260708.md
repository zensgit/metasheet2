...
- Baseline: `origin/main` @ `0e088d3b1` (re-pinned in the **Rev 5** post-#4181-landing errata; Rev 4 used `c46be34fb`). **Anchor note (Rev 5):** #4181 merged into `directory-sync.ts` and shifted every anchor in that file by ~+44 lines, so ALL `directory-sync.ts` anchors were re-verified against post-#4181 `main` and refreshed (`corp_id` UPDATE `:2170`→`:2214`; identity `DELETE` `:5370`→`:5411`; bind `:5150`→`:5191`; unbind `:5323`→`:5364`; sweeps `:3100`/`:3088`→`:3141`/`:3129`; etc.). Anchors in the other 7 files (`dingtalk-oauth`, `admin-directory`, `ApprovalDirectoryOrg`, …) were untouched by #4181 and remain valid.
...
- **Revision 3 (2026-07-09), after owner review — migration composition corrected; #3944 §9.3 is authoritative.** §5/§6/§13 described the migration as a naive **unbind-A-then-bind-B** using the public `unbindDirectoryAccount()` + `bindDirectoryAccount()`. That is wrong: the public `unbindDirectoryAccount` **deletes** the login identity (`:5370`), so composing it with a later bind opens a window where the user has no identity. The authoritative migration is a **dedicated single-transaction rebind** — clear the source link *without deleting the identity*, upsert the one `user_external_identities` row in place source→target, enable the grant, then link the target — specified in `docs/development/provider-org-transfer-development-plan-20260709.md` §9.3 (Rev 2). **That development plan supersedes the migration prescription in §5/§6/§13 below**; those sections are corrected to point to it. The description of what the *primitives themselves* do is unchanged and correct; only the "how to compose them for a migration" prescription was wrong.
- **Revision 4 (2026-07-12) — as-built refresh.** Three corrections against the moved `origin/main`: **(1)** §3's "`corp_id` is editable in place" is still true on `main`, but the trap is being closed **by construction** — PR #4181 (`claude/dingtalk-org-transfer-phase1-guardrails`, CI-green, not yet merged) makes `corp_id` unconditionally **immutable once set**, with delete+recreate before first sync as the only correction path and deliberately no production escape hatch. **(2)** §10's "DT-OPS-01 … default-off and not yet merged" was **stale**: it merged via #3905 (commit `8bc8d5aad`) and is on `main`, env-gated default-off — and it guards only the downstream local-user deprovision step, never the §4 `is_active` sweep itself, which remains unguarded (§4 restated accordingly). **(3)** §12/§13 gain a pointer to the local-canonical-org substrate: local `org_id` is the true cross-corp / cross-provider anchor, above even a `directory_integrations` row; #3944 Rev 3 formalizes it as the umbrella anchor. A baseline anchor-drift warning is also added above (~268 commits behind; re-verify file:line anchors before use).
- **Revision 5 (2026-07-13) — post-#4181-landing errata.** The Rev-4 text above was written while **#4181 was still open**; it has since **merged** (`0e088d3b1`), which makes two Rev-4 statements behaviorally false, corrected here: (1) §3/§7 no longer say `corp_id` "is editable in place / no guard has merged" — the immutable-once-set guard is **live on `main`** (`directory-sync.ts:2186`, `409 DirectoryTenantChangeBlockedError`); (2) the merge shifted every `directory-sync.ts` anchor by ~+44 lines, so all of them were re-anchored to post-#4181 `main` (see the baseline Anchor note). §4's sweep-is-unguarded fact is unchanged, but now scoped: #4181 blocked the *corp-swap edit-path* trigger, not the sweep itself. Baseline re-pinned to `0e088d3b1`.
...
The login identity is genuinely corp-scoped: it is written on OAuth login with `ON CONFLICT (provider, external_key)` (`packages/core-backend/src/auth/dingtalk-oauth.ts:486`), it stores `corp_id` (`:477`, `:490`), and the login lookup **requires the corp to match** — `WHERE identity.external_key = $2 ... AND identity.corp_id = $5` (`dingtalk-oauth.ts:577,587`). If it does not match, an unprovisioned login returns `403 unlinked_local_user` (`dingtalk-oauth.ts:744`).
...
- **`corp_id` is now IMMUTABLE once set (was editable in place; corrected in Rev 5).** `updateDirectoryIntegration` still runs `UPDATE directory_integrations SET ... corp_id = $4` (`packages/core-backend/src/directory/directory-sync.ts:2214`), but a guard now sits **before** it: changing an already-set `corp_id` throws `DirectoryTenantChangeBlockedError` → `409` (`directory-sync.ts:2186`). An operator can still overwrite AppKey / AppSecret, but **not** re-tag a set `corp_id`. This landed in **PR #4181 (merged to `main` `0e088d3b1`)**; the "editable in place" statement was true only through the pre-#4181 baseline.
  **Rev 5 update: this trap is now closed by construction — the guard is MERGED.** **PR #4181 (merged to `main` `0e088d3b1`)** makes `corp_id` **unconditionally IMMUTABLE once set** in `updateDirectoryIntegration` — not merely blocked while synced. A mis-entered `corp_id` is correctable only by **delete+recreate before the first sync**, never by editing, and there is deliberately **no production escape hatch**. So the in-place `corp_id` re-tag — and the §4 trap it would trigger — is **no longer reachable on `main`** through an ordinary edit; only the org-transfer workflow (unbuilt) can change a set tenant.
- **`corp_id` is not unique.** The only unique key on `directory_integrations` is `(org_id, provider, name)` (`packages/core-backend/src/db/migrations/zzzz20260324150000_create_directory_sync_tables.ts:36`). Multiple integrations may share a corp, and a second integration for corp B is allowed as long as its `name` differs.
...
```
(`directory-sync.ts:3141–3143`, and the departments equivalent at `:3129–3131`.)

There is **no empty-fetch or mass-departure guard on this sweep — it still runs unconditionally on current `main` (Rev 5)**. Do not conflate this with DT-OPS-01 (§10): the downstream deprovision *executor* is circuit-breaker-guarded, but the `directory_accounts` / `directory_departments` `is_active` sweep itself has **no guard of any kind** — an empty or wrong-corp fetch deactivates the entire directory in one run, before any deprovision logic is even consulted. **Rev 5 nuance:** the specific *corp-swap* trigger for this — re-tagging a set `corp_id` in place — is now **blocked by #4181** (merged), so an ordinary integration edit can no longer arm it; but the sweep's own lack of an empty-fetch guard is unchanged, so a wrong/empty *fetch* (bad token, API blip) can still mass-deactivate. #4181 closed the edit-path trigger, not the sweep.

What it does *not* do by default: it does not deactivate the local `users` rows and does not revoke logins by itself. The offboarding executor (DT-OPS-01) **is merged on `main`** (#3905, corrected in Rev 4 — see §10) but is env-gated default-off (`DIRECTORY_DEPROVISION_ENABLED`); with the env unset, the stored `default_deprovision_policy` is not acted on, and even when enabled, the `DIRECTORY_DEPROVISION_MAX_BATCH` circuit breaker would abort exactly this mass-departure before it cascades into local-user deprovision. None of that protects the sweep above.
...
`unbindDirectoryAccount` (`directory-sync.ts:5364`) clears **both** layers in a single transaction:
- it deletes the corp-scoped login identity — `DELETE FROM user_external_identities WHERE ...` keyed by `buildDingTalkIdentityExternalKey(corp_id, open_id, union_id)` (`directory-sync.ts:5411`);
...
`bindDirectoryAccount` (`directory-sync.ts:5191`) re-establishes **both** layers for a chosen local user. It delegates to `applyDirectoryAccountBindInTransaction` (`directory-sync.ts:4435`), which:
- **upserts the corp-scoped login identity**: it looks up an existing `user_external_identities` row for `(provider, local_user_id)` and `UPDATE`s it — else `INSERT`s a new one — setting `corp_id` and `external_key` to *this account's* (i.e. corp B's) values (`directory-sync.ts:4507` UPDATE / `:4529` INSERT). So bind pre-seeds the corp-B login identity; it does **not** wait for the user's next login.
- can **enable the login grant** when asked (`enableDingTalkGrant`, gated by `assertDirectoryAccountCanEnableDingTalkGrant`, `directory-sync.ts:4450`);
- enforces conflict guards: it throws if the DingTalk identity is already bound to another local user, and if the local user is already linked to another *linked* directory account (`directory-sync.ts:4479,:4494`).
...
A migration **cannot be safely run by hand today** — treat this as the target design, not a runbook. The safe primitive it requires, the **single-transaction rebind** (clear the source link *without* deleting the identity, upsert the one `user_external_identities` row in place source→target, enable the grant, then link the target), **does not yet exist**: it is specified in `provider-org-transfer-development-plan-20260709.md` §9.3 but unbuilt, and composing the *public* `unbindDirectoryAccount()` + `bindDirectoryAccount()` instead is unsafe (§8 below — the public unbind deletes the identity, opening a no-identity window). Once that primitive lands, the order below is what the transfer engine executes; it avoids the §4 trap by never re-pointing corp A's integration:

1. **Stand up corp B alongside corp A.** Create a *new* integration for corp B (different `name`) and run a **preview** first, then a sync. B's accounts land as `pending` links, not `linked` (`directory-sync.ts:3228`) — nothing is auto-attached to a local user yet.
...
- **A decision on the login-identity layer**: **bind already re-establishes the corp-scoped identity immediately, in the same transaction** — it *upserts* `user_external_identities` and does **not** wait for the user's next login (§5, confirmed; the "re-establishes only on next login" framing was the Rev-1 error corrected there). So a migration routed through the rebind carries the identity over at bind time by construction (rewriting `corp_id` + `external_key` to corp B); users are not locked out of 免登 waiting to re-log in. What remains a decision is only whether to also proactively **invalidate** corp-A sessions at cutover.
...
- The seam is the existing unbind route, today a single admin POST: `POST /accounts/:accountId/unbind` (`packages/core-backend/src/routes/admin-directory.ts:1076`). A confirmation gate splits it into *request* → *confirm*.
...
- **DT-OPS-01 (offboarding executor + circuit breaker) — status corrected in Rev 4.** Rev 1–3 said "default-off and not yet merged; on `origin/main` today the offboarding policy is stored but not enforced". **That is stale: DT-OPS-01 merged via #3905 (commit `8bc8d5aad`) and is on `main`** — `evaluateDirectoryDeprovisionCircuitBreaker` / `applyDirectoryDeprovisionPolicies`, env-gated default-off behind `DIRECTORY_DEPROVISION_ENABLED`, with `DIRECTORY_DEPROVISION_MAX_BATCH` as the breaker threshold. The mass-departure of corp A's accounts is exactly the shape the breaker guards: a batch above the max aborts rather than acting, which is what stops a corp switch from cascading into a mass login revocation once the policy is enabled. With the env unset (the default), the stored policy is not acted on, so a bare corp swap still does not auto-lock local users — it only strands them. **Scope caveat (do not over-read): DT-OPS-01 guards ONLY the downstream local-user deprovision step. It does NOT guard the `directory_accounts` / `directory_departments` `is_active` sweep itself — that sweep remains unconditional and unguarded (§4).**
...
**Rev 4 pointer — the anchor above all of this is the local canonical org.** This section frames provider-genericity at the identity / binding-uniqueness level; there is one level above it. §2 established the local *user* as the stable anchor for a person; the same principle scales up to the tenant: the local **`org_id` is the true cross-corp, cross-provider anchor** — it sits above even a `directory_integrations` row, and every corp-scoped binding in the §11.1 catalog ultimately hangs off it. Integrations, corps, and providers are all subordinate, replaceable bindings under one `org_id`; that is what a transfer preserves. #3944 (`docs/development/provider-org-transfer-development-plan-20260709.md`) **Rev 3 formalizes `org_id` as this umbrella anchor.**
...
The implementation is sequenced in `docs/development/provider-org-transfer-development-plan-20260709.md` (design-lock + guardrails + user/group minimal closed loops, no WeCom/Feishu and no plugin SPI up front). That plan's **Rev 3 also formalizes the local canonical `org_id` as the umbrella anchor** (§12, Rev 4 pointer): the transfer engine operates *under* one `org_id`, and integrations / corps / providers are the subordinate bindings it rewires. Two constraints from this assessment bind that plan:
