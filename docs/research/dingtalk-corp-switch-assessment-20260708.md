...
- Baseline: `origin/main` @ `c46be34fb` (re-pinned in the Rev-4 best-of-both merge; was `d1180d8a2`). **Anchor note (Rev 4):** every `file:line` anchor below has been **re-verified against this baseline** by grepping its symbol, and the drifted ones refreshed in place (the §3 `corp_id` UPDATE is now `directory-sync.ts:2170`; the identity `DELETE` is `:5370`; bind/unbind `:5150`/`:5323`; etc.). The 8 referenced source files are unchanged from `f676069716` through `c46be34fb`, so the numbers are current. (An earlier Rev-4 draft left anchors pinned to the old baseline with a "re-verify before use" caveat — that is now resolved by the actual re-anchoring.)
...
- **`corp_id` is editable in place.** `updateDirectoryIntegration` runs `UPDATE directory_integrations SET ... corp_id = $4` (`packages/core-backend/src/directory/directory-sync.ts:2170`). An operator can overwrite Corp ID / AppKey / AppSecret on an existing integration.
  **Forward-note (Rev 4): this trap is being closed by construction.** Still true on current `origin/main` — no guard has merged yet — but a parallel branch (`claude/dingtalk-org-transfer-phase1-guardrails`, PR #4181, CI-green, not yet merged) makes `corp_id` **unconditionally IMMUTABLE once set** in `updateDirectoryIntegration` — not merely blocked while synced. Once it lands, a mis-entered `corp_id` is correctable only by **delete+recreate before the first sync**, never by editing, and there is deliberately **no production escape hatch**. Until it merges, the in-place edit — and the §4 trap behind it — remains live on `main`.
...
```
(`directory-sync.ts:3100–3102`, and the departments equivalent at `:3088–3090`.)

There is **no empty-fetch or mass-departure guard on this sweep — it runs unconditionally, and that is still true on current `main` (Rev 4)**. Do not conflate this with DT-OPS-01 (§10): the downstream deprovision *executor* is circuit-breaker-guarded, but the `directory_accounts` / `directory_departments` `is_active` sweep itself has **no guard of any kind** — an empty or wrong-corp fetch deactivates the entire directory in one run, before any deprovision logic is even consulted. So a bare corp swap silently marks the entire old organization inactive in one sync, turning every previously-linked person into a manual review item.
...
`unbindDirectoryAccount` (`directory-sync.ts:5323`) clears **both** layers in a single transaction:
- it deletes the corp-scoped login identity — `DELETE FROM user_external_identities WHERE ...` keyed by `buildDingTalkIdentityExternalKey(corp_id, open_id, union_id)` (`directory-sync.ts:5370`);
...
`bindDirectoryAccount` (`directory-sync.ts:5150`) re-establishes **both** layers for a chosen local user. It delegates to `applyDirectoryAccountBindInTransaction` (`directory-sync.ts:4391`), which:
- **upserts the corp-scoped login identity**: it looks up an existing `user_external_identities` row for `(provider, local_user_id)` and `UPDATE`s it — else `INSERT`s a new one — setting `corp_id` and `external_key` to *this account's* (i.e. corp B's) values (`directory-sync.ts:4463` UPDATE / `:4485` INSERT). So bind pre-seeds the corp-B login identity; it does **not** wait for the user's next login.
- can **enable the login grant** when asked (`enableDingTalkGrant`, gated by `assertDirectoryAccountCanEnableDingTalkGrant`, `directory-sync.ts:4406`);
- enforces conflict guards: it throws if the DingTalk identity is already bound to another local user, and if the local user is already linked to another *linked* directory account (`directory-sync.ts:4435,:4450`).
...
Both have batch wrappers already: `batchUnbindDirectoryAccounts` (`directory-sync.ts:4249`) and `batchBindDirectoryAccounts` (`:4270`).
...
1. **Stand up corp B alongside corp A.** Create a *new* integration for corp B (different `name`) and run a **preview** first, then a sync. B's accounts land as `pending` links, not `linked` (`directory-sync.ts:3184`) — nothing is auto-attached to a local user yet.
