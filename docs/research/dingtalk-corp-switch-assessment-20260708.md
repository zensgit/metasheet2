...
- Baseline: `origin/main` @ `d1180d8a2`. **⚠️ Anchor-drift note (Rev 4):** `origin/main` has since moved ~268 commits past this baseline. Every file:line anchor below is pinned to the old baseline and **must be re-verified against current `main` before being relied on** (example: the §3 `corp_id` UPDATE has drifted from `directory-sync.ts:1596` to `:2173`). The anchors are deliberately left as-written rather than hand-refreshed; treat them as "where it was at `d1180d8a2`", not "where it is".
