...
| **C3** | Build live PoC packet + GATE contract validation | After C2 PASS | `node scripts/ops/integration-k3wise-live-poc-preflight.mjs --input <gate.json> --out-dir <packet-dir>` and `node scripts/ops/integration-k3wise-gate-contract-check.mjs --init-template <safe-dir>` once if needed, then `node scripts/ops/integration-k3wise-gate-contract-check.mjs --input <contract.json> --out-dir <gate-contract-dir>` | `integration-k3wise-live-poc-packet.{json,md}` written; `safety.saveOnly=true / autoSubmit=false / autoAudit=false`; checklist contains `GATE-01 / CONN-01 / CONN-02 / DRY-01 / SAVE-01 / FAIL-01 / ROLLBACK-01` (plus `BOM-01` when BOM enabled); GATE contract check returns `decision=PASS` for O1-O6/R1-R7 evidence | `normalizeGate` throws (error includes `field` path of the offending key), or GATE contract check returns `GATE_BLOCKED` / `FAIL` |
