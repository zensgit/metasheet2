...
## 放宽打卡必填后的验证（2026-02-04）
- 预览导入：`rows=11966`，首行 `warnings=[]` ✅
- 提交导入批次：`f9356925-f1d1-4942-91c8-ece813c61623` ✅
  - 批次行数：`11966`
  - Import items 总数：`11777`
  - 跳过：`189`（显著下降）
- 回滚批次：状态 `rolled_back` ✅
  - 回滚响应 `deleted=0`（import items 仍保留作审计，attendance_records 已无该 batch 记录）
