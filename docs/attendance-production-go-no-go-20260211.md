...
## Post-Go Verification (2026-03-07): Post-Merge Verifier Re-Run (`verify:attendance-post-merge`)

Goal:

- run one-command post-merge verification on `main` and confirm latest branch-policy, strict, and dashboard gates are consistently green with artifacted evidence.
...
| Gate | Run | Status | Evidence |
|---|---|---|---|
| Attendance Branch Policy Drift (Prod) | #22798252725 | PASS | `output/playwright/attendance-post-merge-verify/20260307-193140/ga/22798252725/attendance-branch-policy-drift-prod-22798252725-1/policy.json`, `output/playwright/attendance-post-merge-verify/20260307-193140/ga/22798252725/attendance-branch-policy-drift-prod-22798252725-1/step-summary.md` |
| Attendance Strict Gates (Prod) | #22798270418 | PASS | `output/playwright/attendance-post-merge-verify/20260307-193140/ga/22798270418/attendance-strict-gates-prod-22798270418-1/20260307-113351-1/gate-summary.json`, `output/playwright/attendance-post-merge-verify/20260307-193140/ga/22798270418/attendance-strict-gates-prod-22798270418-1/20260307-113351-2/gate-summary.json`, `output/playwright/attendance-post-merge-verify/20260307-193140/ga/22798270418/attendance-strict-gates-prod-22798270418-1/20260307-113351-2/gate-api-smoke.log` |
| Attendance Daily Gate Dashboard | #22798350381 | PASS | `output/playwright/attendance-post-merge-verify/20260307-193140/ga/22798350381/attendance-daily-gate-dashboard-22798350381-1/attendance-daily-gate-dashboard.json`, `output/playwright/attendance-post-merge-verify/20260307-193140/ga/22798350381/attendance-daily-gate-dashboard-22798350381-1/attendance-daily-gate-dashboard.md` |

Observed highlights:

- strict smoke log includes:
  - `import upload ok`
  - `idempotency ok`
  - `export csv ok`
  - `group + membership ok`
  - `SMOKE PASS`
- dashboard contract snapshot:
  - `overallStatus=pass`
  - `p0Status=pass`
...
## Post-Go Verification (2026-03-08): Post-Merge Perf Contract Assertion Gate

Scope:

- strengthen post-merge verifier by adding a local perf contract assertion step after perf baseline artifacts download.

Code changes:

- `scripts/ops/attendance-post-merge-verify.sh`
  - new gate: `perf-baseline-contract` (local assert)
  - validates downloaded `perf-summary.json` against expected merge-close profile:
    - `uploadCsv == true` (default)
    - `commitAsync == false` (default)
    - `rows >= 10000` (default)
    - `mode == commit` (default)
  - new env controls:
    - `PERF_EXPECT_UPLOAD_CSV`
    - `PERF_EXPECT_COMMIT_ASYNC`
    - `PERF_EXPECT_ROWS_MIN`
    - `PERF_EXPECT_MODE`
...
| Attendance Daily Gate Dashboard | #22802536897 | PASS | `output/playwright/attendance-post-merge-verify/20260308-001057/ga/22802536897/attendance-daily-gate-dashboard-22802536897-1/attendance-daily-gate-dashboard.json`, `output/playwright/attendance-post-merge-verify/20260308-001057/ga/22802536897/attendance-daily-gate-dashboard-22802536897-1/attendance-daily-gate-dashboard.md` |

Observed highlights:

- post-merge verification now covers policy + strict + perf + perf contract + dashboard in a single command.
- `gh issue list --search "[Attendance" --state open` returned no open attendance issues after the run.
...
## Post-Go Verification (2026-03-08): Final Mainline Sweep After #389 + #390
...
| Locale zh Smoke | #22817746846 | FAIL (non-blocking policy) | `output/playwright/attendance-post-merge-verify/20260308-round13-final-main/ga/22817746846/attendance-locale-zh-smoke-prod-22817746846-1/auth-error.txt` |
