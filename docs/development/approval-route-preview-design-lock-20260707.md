...
- ✅ **RP-3 FE 试运行面板** —— as-built：TemplateAuthoringView 内 `canManageTemplates` 才可见的「试运行」卡；样例发起人 picker（复用 `ApprovalUserPicker`）+ 按 formSchema 渲染样例表单（`detail`/`attachment` 诚实跳过并就地标注，不伪造）；compute-at-click，**复用 RP-2 的 `routePreviewController`**（该模块改为泛型 `<Req>`、默认类型参数保持 RP-2 调用点不变 —— 不另造竞态守卫）；样例数据/样例发起人/草稿 dirty 任一变化即 `invalidate()`。错误映射抽为纯模块 `routePreviewErrors.ts`：按 **`error.code` 而非 HTTP status** 分派，覆盖底座实际会抛的 **5 个** wedge code（2×422 `*_REQUIRED` = 该发起人真实提交会被拒；3×503 `*_UNRESOLVED` = 目录读取瞬时失败、可重试）。未策展角色提示 + 「保存后试运行」（dirty 时禁用并给 tooltip）。测试：错误矩阵 + 路径编码矩阵；RP-2 controller/summary 回归 30/30；authoring 既有 6 套 87/87；vue-tsc 0。
- 🟩 **本线完成** —— 收尾设计与验证 MD：`approval-route-preview-closeout-verification-20260708.md`
