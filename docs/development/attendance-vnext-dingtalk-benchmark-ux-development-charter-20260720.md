...
| Wave 4 onboarding | 设计锁 `attendance-vnext-wave4-onboarding-design-lock-20260721.md`（**RATIFIED——重新 ratify 2026-07-21**，§10 重启序步③：errata #4513=`57d89bc1d` 吸收④=(c) `manual_review_required`/③补信号/①前置票后，前置票 **W4-PRE-1 已落地** PR #4521=`e20371b1a`（Opus 门 APPROVE 0 P1/P2，canonical surface=`POST /api/admin/users` 已回填 §3① 修复动作），owner 终裁以 re-ratify PR comment 为凭据） | 未开始（W4-0 解冻可开工——从 re-ratify 合入后 main 起片；历史 WIP `claude/w4-0-setup-readiness-20260721`=`b2789cce7`（曾开 #4514 已 CLOSED 未合并）**仅作材料库逐项 re-port，禁整分支复活**——owner 裁决③） | 未开始 | RATIFIED / landing——W4-0 → W4-1 → W4-2（锁 §9 切片序，严格串行）；验证 MD 每片必出 |
