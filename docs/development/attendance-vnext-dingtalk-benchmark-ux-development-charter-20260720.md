...
| Wave 5 explainability | 数据合同设计锁 `attendance-vnext-wave5-explainability-data-contract-lock-20260722.md`（**PROPOSED 重呈——owner RATIFY 为唯一生效凭据**;2026-07-23 owner 终审 CHANGES REQUESTED 0P1/3P2 + **OD-W5-1..11 一次性全裁**,三 P2 修订与裁决已入锁,待 owner 对新 exact head 留 RATIFY;曾有受托代行已被 owner 收回,见锁 header 历史注记） | 未开始（runtime 未授权:owner 明示「呈审 → owner RATIFY → 再切 runtime」） | 未开始 | DATA-CONTRACT-GATED / 锁 PROPOSED 重呈中（终审吸收版） |
