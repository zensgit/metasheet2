...
| Wave 5 explainability | 数据合同设计锁 `attendance-vnext-wave5-explainability-data-contract-lock-20260722.md`（**PROPOSED 呈审——owner RATIFY 为唯一生效凭据**;曾有受托代行已被 owner 收回,见锁 header 历史注记） | 未开始（runtime 未授权:owner 明示「呈审 → owner RATIFY → 再切 runtime」） | 未开始 | DATA-CONTRACT-GATED / 锁 PROPOSED 呈审中 |
