...
- 锚点语义 = **「恢复到该批次完成后的状态」**(`asOf = batch.createdAt`,含该批次;与 `created_at <= T` 完全一致,零语义偏移)。注:同一毫秒存在**兄弟批次**时,锚定其一会把并列批次一并含入(两者都 `<= T`)——该措辞在此情形下是近似,精确到批次的排除属 V2。此边界同样适用于已上线的 `ResetToPointPicker`(`asOf = batch.createdAt`,毫秒粒度),非仅未来假设路径。
