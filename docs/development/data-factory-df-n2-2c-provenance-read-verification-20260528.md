...
tenant/workspace + `row_id` (+ optional `pipeline_id`), pushes the optional run-time window into a
structured DB range predicate on `run_created_at`, then applies limit/offset to that bounded result and
sorts by `(run_created_at, event_index)`.
...
- **Range-before-limit regression** — the read test seeds older rows plus a newer row beyond `limit=1`
  and asserts `from=...` still returns the newer row. This locks the important ordering:
  `run_created_at` window first, then limit/offset.
...
- `lib/db.cjs` — structured `select({ range })` support (`gte`/`lte` only, table/column identifiers still
  whitelist-validated; no raw SQL escape hatch).
