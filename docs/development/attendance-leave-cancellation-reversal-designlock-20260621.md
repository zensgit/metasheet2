...
|---|---|---|---|
| 3a | **Expired lot** — a lot that gave minutes has since `expires_at`-expired. Restore to it? | **No** — do not un-expire; restore the expired portion to **no lot** and record a `reverse` event with a `note` that N minutes were unrecoverable (expired). Surface, don't silently drop. | Un-expiring a lot would resurrect already-forfeited balance; silently dropping would mis-account. Honest middle: restore what's still valid, record what expired. |
