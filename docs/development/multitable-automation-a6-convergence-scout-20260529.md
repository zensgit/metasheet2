...
A6-4a then landed in #2568 as a pure compiler module + unit tests only, and the
read-only route later landed in #2577 as A6-4b. Those documents remain the
current contract for any A6-4c UI follow-up; this section is kept as the
original A6-0 positioning.
