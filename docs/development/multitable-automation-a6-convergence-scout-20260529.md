...
| A6-4 | BPMN adapter | Scope gate recorded in `multitable-automation-a6-4-bpmn-compile-preview-scope-gate-20260612.md`; A6-4a pure compiler landed #2568; A6-4b read-only route and any live-runtime path remain not started | named demand |
...
A6-4a then landed in #2568 as a pure compiler module + unit tests only, with the
route deferred to A6-4b. Those documents remain the current contract for any
follow-up route implementation; this section is kept as the original A6-0
positioning.
...
After A6-3 `condition_branch` + parallel `join_all`, A6-4a has now landed by
explicit opt-in: a pure `bpmnCompilePreview` compiler module plus unit tests,
no route, no persistence, no `BPMNWorkflowEngine`, and no live deploy/start
path (#2568). The next A6-4 work still requires a named demand: A6-4b may add
only the read-only compile-preview route after the compiler output is stable,
and must keep the same no-live-runtime / no-persistence boundary.
