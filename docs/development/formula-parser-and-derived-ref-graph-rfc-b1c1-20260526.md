...
本 RFC 要你拍两个板：**B1**（公式求值/解析怎么走）、**C1**（要不要多跳派生字段）。每条给出建议 + 翻盘条件。
...
### 建议：**现在做 A2b，B1a 不做，B1b 推迟**（基于代码核实，非泛泛而论）
...
> 直接回应你对 A2b 的顾虑（"若很快上解析器 A2b 会变短寿命补丁"）：**只有在 B1b 世界里 A2b 才会被丢弃。本 RFC 建议不走 B1b，所以 A2b 是耐久修复，不是短寿命补丁。**
...
**关键：C1 本身（只建表）几乎不产生价值。** 它是契约层，只有配 **C2a（递归 CTE 求传递闭包 + 拓扑级联重算）** 和 **C3（建字段时环检测）** 才兑现。所以你要拍的不是"建不建表",而是 **"要不要多跳派生字段"**：
