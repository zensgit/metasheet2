...
`buildTaskScopeCondition` 输出形:`(tasks.org_id = :org) AND tasks.deleted_at IS NULL AND (<臂>)`;org 子句只在此处出现一次(锁 §4.3「单点发射」)。`buildTaskPendingCondition` 不自行写角色臂(锁 `:243`),只包裹 assigned 臂。参数占位用 `$n` 序号数组形(照 `ApprovalBridgeService.ts:215-229` 的 `{ sql, params }` 形状;计划 :113)。
