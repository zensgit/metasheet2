...
| `src/tasks/task-ids.ts` | `TASK_ID_PREFIXES = {task:'tsk', list:'tlst', comment:'tcmt', event:'tev'}`(`event` 为 owner 2026-09-26 裁决新增);`generateTaskDomainId(kind, random)`(格式 `^(tsk\|tlst\|tcmt)_[A-Za-z0-9]+$`;`random` 由调用方注入,纯);`isValidTaskDomainId(s)`(= DDL CHECK 四合取 `^[!-~]+$ ∧ !~'__' ∧ !~'^_' ∧ !~'_$'`);`isValidPrintableAsciiId(s)`(`org_id`/`created_by` 只一合取);`parseTaskProjectionRecordId('rec_tsk_<listId>__<taskId>')`(先剥 `rec_tsk_` 前缀再取**首个** `__`;失败返回 `null`,不抛);`normalizeUserText(s)`(NFC、去 Unicode White_Space 与 U+200B/U+200C/U+200D/U+FEFF;返回归一串或 `null` 表示空) | 锁 §4.1/§4.2 `:92-93`、§7 `parseTaskProjectionRecordId`、门 10 |
