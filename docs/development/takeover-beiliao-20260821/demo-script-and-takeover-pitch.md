...
- **Who can click 拉取, today (correction to an earlier draft of this pitch)**: the MetaSheet route that actually reads PLM is `POST /api/integration/stock-preparation/mvp/source-runs/plm-bom` (`plugins/plugin-integration-core/lib/http-routes.cjs:129`), and its handler calls `requireAccess(req, 'admin')` (`http-routes.cjs:5786`) — `hasPermission`'s `admin` branch is satisfied **only** by `role:admin` / `integration:admin` (`http-routes.cjs:746-761`). A 生产/PMC-only login **cannot** trigger this today — unlike the legacy system, where (per Risk 1 below) essentially anyone on the network could hit `/stock/refreshProduct`. Script this beat logged in as admin/IT, and say so out loud: MetaSheet's default here is *stricter*, not equivalent. A scoped `stock-prep:pull`-style operator permission (parallel to the existing `stock-prep:read`/`stock-prep:operate` codes in `plugins/plugin-integration-core/lib/stock-preparation-workbench-access.cjs`) is possible future work — **【设计,未实现】**, not built as of this doc.
- **Where the pulled rows actually land**: `apply` refuses the canonical `plm_stock_preparation_main` objectId outright by default — `STOCK_PREP_APPLY_SANDBOX_ONLY` / reason `prod_canonical` (`plugins/plugin-integration-core/lib/stock-preparation-table-actions.cjs:1513-1520`) — and instead writes to a schema-identical **sandbox twin** object under the `plm_stock_preparation_sandbox*` namespace (`plugins/plugin-integration-core/lib/stock-preparation-target-provisioning.cjs:78-128`). The only way to reach the real canonical table is an owner-set, server-config-only, time-boxed production policy (≤7 days, `MAX_PRODUCTION_POLICY_WINDOW_MS`, `stock-preparation-production-policy.cjs:22-25,84-96`, wired at `stock-preparation-table-actions.cjs:1561-1589`) — never an env var, never something a demo session sets itself. For a room demo this sandbox default is the safer thing to show; just don't say or imply the clicks are landing in the customer's live production table unless that policy was actually configured for the session.
- **Demo beat**: type 项目号 → click *拉取* → tree of 备料明细 appears in < a few seconds. Same click, same result (run by admin/IT — see above).
...
### How MetaSheet reproduces it — corrected against origin/main (an earlier draft of this pitch overstated this section)
- **One native grid, one role VIEW per department — column HIDING, not a write permission.** The shipped customer pack defines three `roleViews` (`production` / `procurement` / `warehouse`), each hiding a fixed column set by name (`hideFieldIds`) — `plugins/plugin-integration-core/lib/customer-packs/factory-a.rehearsal.cjs:217-279`; the file's own comment there is explicit: *"Column hiding ONLY — a view never filters rows out of anyone's reach and never changes what anyone is allowed to write."* The pack schema is fail-closed to exactly four keys — `viewId, label, hideOwnerships, hideFieldIds` (`ROLE_VIEW_KEYS`, `plugins/plugin-integration-core/lib/stock-preparation-customer-pack.cjs:89`, enforced at `:434`) — so a role view **cannot structurally express** a row filter or a write permission. The installer turns each view into a grid descriptor whose only payload is `hiddenFieldIds` (`plugins/plugin-integration-core/lib/stock-preparation-customer-pack-installer.cjs:439,704-709`). **This is not server-enforced today**; it is the same cosmetic idea as the old system's `ColumnVo.editable` flag, just done as a native view instead of a front-end read.
- **【设计,未实现 for this pack】** MetaSheet's platform *does* have a real, server-enforced field-write-permission primitive elsewhere — the `field_permissions` table plus the `isFieldWriteForbidden` gate wired into every patch/write code path (e.g. `packages/core-backend/src/routes/univer-meta.ts:10164,10325,10472,10988,16454,18497`) — but the stock-prep customer pack does not install it, and nothing wires the 生产/采购/仓库 role bands to it automatically. Turning it on is a separate, manual, per-tenant configuration pass — enumerate every field × role via `PUT /sheets/:sheetId/field-permissions/:fieldId/:subjectType/:subjectId` — documented as a runbook, not a code change, in `docs/development/general-prep-dept-collaboration-config-pack-20260722.md` §3. It is real and it works once applied; it is just not what this pack ships, and not an owner decision this pitch can promise on its own.
- **The departmental write surface is exactly two plain columns — not a mirror-table collapse.** `procurementReply` (采购回复) and `warehouseConfirmation` (仓库确认) are ordinary `string` fields with `human_preserved` ownership — `plugins/plugin-integration-core/lib/stock-preparation-templates.cjs:707-708`. **No `ext_purchase_*` / `ext_warehouse_*` columns exist anywhere in this codebase** (repo-wide search: zero hits) — an earlier draft of this pitch invented them; correct that on sight if it comes up. 生产's own six fields (材料类型/毛胚类型/备料情况/需求日期/提前周期/备注) are `human_preserved` too — all eight ids together are `HUMAN_PRESERVED_FIELD_IDS`, same file `:104-112`. There was never a `purchase_info`/`warehouse_info`-style mirror table in MetaSheet to "collapse": the two department columns live on the row from day one.
...
- **Demo beat, said honestly**: log in as 生产 → fill 材料类型/毛胚; switch to 采购's view → 采购 sees only 采购's columns (hidden elsewhere, not locked elsewhere); switch to 仓库's view → same. If IT asks "can 采购 write into a 生产 column through the API even though the UI hides it," today's honest answer is **yes, unless the separate field-permission config pass above has been applied** — say so, and offer to show that config pass live as proof the enforcement layer is real once turned on.
...
### How MetaSheet reproduces it
*(Same admin-gating and canonical/sandbox caveat as flow A — 点击拉取 here is the same route, run again; see flow A above for who can click it and where the rows land.)*
- **Refresh = idempotent read-plan re-sync keyed on `ext_pliObjId`** (plus 图号 for hand-added rows). The plan re-materializes BOM structure/quantities; a **merge/upsert step in the plugin's own sync/apply logic** — not the platform's rule-based Automation Engine — matches incoming rows to existing rows by `ext_pliObjId` and:
  ...
  - **existing pliObjId** → **preserve every human-owned column, including the two 采购/仓库 columns** (`procurementReply`/`warehouseConfirmation` — plain `human_preserved` fields, not `ext_` columns; structure/quantity refreshed from PLM, human fills untouched),
  - **hand-added rows** (`ext_pliObjId` empty) → carried and re-parented, mirroring `SIC:577-592`.
- Because 采购/仓库 are **plain `human_preserved` columns on the same row** in MetaSheet (`procurementReply`/`warehouseConfirmation`, `stock-preparation-templates.cjs:707-708` — not `ext_` columns; see flow B's correction above), `reuseForPurchaseAndWarehouse` disappears — preserving them is automatic (they never leave the row). This removes the biggest correctness risk in the old code: the **no-transaction, insert-then-delete** batch swap (prior report §6.2) that can leave half-new/half-old data if it fails mid-way. MetaSheet's sync is transactional.
- **Note on notifications**: a refresh here does **not** emit any `record.*` event, so no rule-based automation (the platform's `field.value_changed`/`record.updated` triggers) can fire off it — confirmed on current `origin/main`: nothing in the plugin's write path (`plugins/plugin-integration-core`) references the record-event emit functions the grid/UI route layer uses (`enqueueRecordEventIfDurable`/`emitRecordEventIfLegacy`, only called from `packages/core-backend/src/routes/univer-meta.ts`'s grid handlers, e.g. `:11297-11332`). If the room asks "does 采购 filling a row automatically notify 生产 the next time PMC refreshes," the honest answer is **no, not today** — see `docs/development/general-prep-dept-collaboration-config-pack-20260722.md` §1 point 3 for the same boundary spelled out for the general-prep line.
- **Demo beat (the money shot)**: 生产 fills 5 rows, 采购 fills 采购备注, 仓库 fills 实际到货日. Admin/IT clicks *拉取* again (simulating a new PLM 批次 — see flow A: this is not a PMC self-serve click today). New rows appear; **every typed value stays put**; a hand-added row survives under its parent. Say out loud: *"same reuse rule as today (`reuseStockInfoExistsInDb`), but atomic — no half-written batches."*
...
- **Fix on day one**: MetaSheet enforces authenticated sessions — every route in this codebase gates on a permission check (`requireAccess`/`rbacGuard`/`hasPermission`), unlike `permitAll`; no endpoint is reachable anonymously. **Correction to an earlier draft of this pitch**: the 生产/采购/仓库 per-column *write* enforcement described in flow B is **not** what the stock-prep customer pack ships by default — that pack only hides columns per role view (cosmetic, same idea as the old system's `editable` flag). The platform's server-side field-write-permission primitive is real and can enforce it once applied, but wiring it to this pack is a separate, manual, owner-approved configuration pass (flow B above; `general-prep-dept-collaboration-config-pack-20260722.md` §3) — not a day-one default to promise in this room.
...
2. **B** — Log in as 生产 (fill 材料类型/毛胚), 采购 (fill 采购备注), 仓库 (fill 到货日) on the same grid; show columns **hide** per role view (say plainly: visibility, not a write lock, unless the separate field-permission config pass from flow B has been applied).
...
| Three-role column ownership | `role_column`, `/purchase`,`/warehouse` update | One grid + per-role VIEW (column hiding, not server-enforced by default — see flow B; server-side write enforcement exists but is a separate, manual config pass) |
| Purchase/Warehouse mirror tables | `purchase_info`/`warehouse_info` | Two plain `human_preserved` columns on the same row (`procurementReply`, `warehouseConfirmation`) — no mirror table, no `ext_purchase_*`/`ext_warehouse_*` (those never existed) |
