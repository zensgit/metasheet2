...
> **✓ 现场可以如实说已发货（见第 7 节 caveat #1）**:"按小时分批"的推导**已进发货
> 代码**（`lib/stock-preparation-batch-identity.cjs` 的
> `mintStockPreparationBatchIdentity`），按部署声明 opt-in:
> `readPlan.batchIdentity = { mode: 'material_create_hour' }`。缺省仍是今天的
> 内容修订版本号做法（`source_revision`），逐字节不变——别演成默认已开。
...
### 5.1 人工填列
在落地表上，由人填 **21 个人列**——规范人列带（材料类型 / 毛胚类型 / 备料情况 /
需求日期 / 提前周期 / 备注 / 采购回复 / 仓库确认 / 自制外购 / 采购完成 /
采购回复日期 / 仓库完成 / 实际到货日期，含 #5447 的五个部门完成列）**加** pack
人列带（备料日期 `ext_stockPrepDate` / 领料节点 `ext_pickingNode` / 交接工段 /
毛胚尺寸 `ext_blank*`）。
...
3. 「改动的列**只有** `rawQuantity / totalQuantity`——也就是 PLM 的量。**任何一条决策都
   不携带人列。**」
4. 「应用刷新后:**7 行 × 21 个人列，逐字节不变。** 你填的一个字都没动。」
5. **负对照（这句最有杀伤力）**:「同样这次刷新，如果**不过**权属过滤器——**21 个人列
   全被冲掉**。所以这堵墙是**承重的**，不是我嘴上说说。」

### 5.3 导出仓库/采购拿走的 Excel
1. Web-UI:多维表工具栏 → **导出 XLSX**，走
   `GET /api/multitable/sheets/:sheetId/export-xlsx`。
2. runner 打印导出投影:**活跃物料行 6 × 列 28**——与真实导出器 `EXPORT_COLUMNS`
   逐字节一致，含 #5447 的五个部门完成列（自制/外购、采购完成、采购回复日期、
   仓库完成、实际到货日期）与 F1c 为老系统补齐的 23 列表头。**停用的那一行掉出
   拣料单。**
...
这三点逐条对照代码现状说，**不要**摆成不实:

1. **按创建小时分批的推导（物料创建日期精确到小时）**——**已发货**
   （`lib/stock-preparation-batch-identity.cjs` 的
   `mintStockPreparationBatchIdentity`），按部署声明 opt-in：
   `readPlan.batchIdentity = { mode: 'material_create_hour' }`；缺省仍是今天的
   内容修订版本号做法（`source_revision`），逐字节不变;声明后源缺 `Createtime`
   会显式降级，不会静默换算法。
2. **多人审批 hand-off 链到备料**——平台**有**审批运行时，但 owner 裁决先上轻量版
   （应用内游标 + 群通知，#5442），不绑定完整审批图——**未接线**到备料流。属净新。
3. **钉钉个人待办推送**——owner 2026-09-02 裁决本轮**不做**（A 工作通知冒充待办 /
   B 单向待办镜像两案均推后）;备料接力（#5442）现状仍只有钉钉**群** webhook，不是
   个人待办;平台侧审批待办单向镜像（#5772）已合入但**默认关闭**、只覆盖审批席位、
   owner 前置（其设计文档 §8）未满足，不适用于备料。

> 追问时的答法:「第 1 条已发货，但按部署配置 opt-in，默认仍是今天的做法;第 2 条
> 未接线，是范围内的下一步;第 3 条本轮不做，不要摆成在跑。今天演示的步骤 1-2-3
> 是**已发货、已验证**的部分。」
...
步骤 2 — 拉取 → BOM 落多维表,列映射;同项目两批次按创建小时区分
  ...
  同项目两批次按创建小时区分 (物料创建日期精确到小时):
    ...
    批 #2 = SYN-XM-0001|2026-08-30T10  (一小时后重拉,创建于 10 点)
    → 两批次快照行 id 0 重叠;同一小时重算 id 逐字节一致(幂等)
  ✓ 按小时分批的推导已发货(lib/stock-preparation-batch-identity.cjs,opt-in:readPlan.batchIdentity = { mode: 'material_create_hour' }) —— 缺省仍是今天的内容修订版本号做法,逐字节不变;见文末与 runbook。
  步骤 2 通过

步骤 3 — 人工填列 + 人列墙(杀手锏)+ 仓库导出
  台词:「你们最怕的:重拉会不会把我填的都冲掉?看好了。」

  人填 21 个人列(材料类型/毛胚类型/备料情况/需求日期/提前周期/备注/自制外购/采购完成/仓库完成/备料日期/领料节点/毛胚尺寸…)于 7 行

  重拉一次(批 #2) → 计划 = add 0 / update 3 / skip 3 / inactive 1
    改动的列只有 rawQuantity / totalQuantity(PLM 的量);任何决策都不携带人列
    应用刷新后:7 行 × 21 个人列取值 逐字节不变 ✅ 人列墙成立
  负对照:同样的刷新若 不过权属过滤器 → 21 个人列 全被冲掉 —— 证明这堵墙是承重的,不是摆设

  导出(仓库/采购拿走的 XLSX 投影,与真实导出器 28 列逐字节一致) —— 活跃物料行 6 × 列 28(停用的那一行掉出拣料单):
    #5447 的部门完成列(自制/外购、采购完成、采购回复日期、仓库完成、实际到货日期)在第 13/14/15/16/17 列,两个完成标记渲染为 是/否,未填时留空;
    其后的列是 F1c 为老系统 exportExcel 那 23 个中文表头补齐的(名称及规格/材料类型/领料节点/毛胚四项…)—— 「后 N 列是某某」这种按位置数列的说法一律不可靠,用 id 认列。
  父组件图号  父组件名称  图号       名称       规格  材料    总数量  备料情况     需求日期    领料节点         备料日期    毛胚长度  自制/外购  采购完成  采购回复日期  仓库完成  实际到货日期  生产编号     名称及规格  材料类型    毛胚类型   备注              交接工段         提前周期(天)  毛胚宽度  毛胚厚度  毛胚数量  毛胚质量
  ──────────  ──────────  ─────────  ─────────  ────  ──────  ──────  ───────────  ──────────  ───────────────  ──────────  ────────  ─────────  ────────  ────────────  ────────  ────────────  ───────────  ──────────  ──────────  ─────────  ────────────────  ───────────────  ────────────  ────────  ────────  ────────  ────────
                          TZ-A-1000  总装配体A        Q345R   2       20 - 已下单  2026-09-20  10 - 示例节点一  2026-09-02  1250      20 - 外购  是        2026-09-05    否        2026-09-18    SYN-XM-0001              30 - Q345R  20 - 管材  按图纸复核后下单  10 - 示例工段一  14            800       12        4         94.2
  TZ-A-1000   总装配体A   TZ-B-2000  筒体组件B        Q345R   6       20 - 已下单  2026-09-20  10 - 示例节点一  2026-09-02  1250      20 - 外购  是        2026-09-05    否        2026-09-18    SYN-XM-0001              30 - Q345R  20 - 管材  按图纸复核后下单  10 - 示例工段一  14            800       12        4         94.2
  TZ-B-2000   筒体组件B   TZ-D-3000  标准封头D        S30408  12      20 - 已下单  2026-09-20  10 - 示例节点一  2026-09-02  1250      20 - 外购  是        2026-09-05    否        2026-09-18    SYN-XM-0001              30 - Q345R  20 - 管材  按图纸复核后下单  10 - 示例工段一  14            800       12        4         94.2
  TZ-A-1000   总装配体A   TZ-C-2100  支腿组件C        Q235B   4       20 - 已下单  2026-09-20  10 - 示例节点一  2026-09-02  1250      20 - 外购  是        2026-09-05    否        2026-09-18    SYN-XM-0001              30 - Q345R  20 - 管材  按图纸复核后下单  10 - 示例工段一  14            800       12        4         94.2
  TZ-C-2100   支腿组件C   TZ-D-3000  标准封头D        S30408  4       20 - 已下单  2026-09-20  10 - 示例节点一  2026-09-02  1250      20 - 外购  是        2026-09-05    否        2026-09-18    SYN-XM-0001              30 - Q345R  20 - 管材  按图纸复核后下单  10 - 示例工段一  14            800       12        4         94.2
  TZ-C-2100   支腿组件C   TZ-F-3200  法兰F            S31603  24      20 - 已下单  2026-09-20  10 - 示例节点一  2026-09-02  1250      20 - 外购  是        2026-09-05    否        2026-09-18    SYN-XM-0001              30 - Q345R  20 - 管材  按图纸复核后下单  10 - 示例工段一  14            800       12        4         94.2
  二进制打包由 packages/core-backend/src/multitable/xlsx-service.ts buildXlsxBuffer + 现有 vitest 覆盖;此处证明的是投影(与真实导出器同一份 EXPORT_COLUMNS/格式化函数)。
  步骤 3 通过

演示要如实说明的边界(逐条对照代码现状 —— 别演成不实)
  1. 按创建小时分批的推导:已发货(lib/stock-preparation-batch-identity.cjs,
     mintStockPreparationBatchIdentity)。按部署声明 opt-in:readPlan.batchIdentity = { mode: 'material_create_hour' },
     缺省仍是今天的内容修订版本号做法(source_revision),逐字节不变;声明后源缺 Createtime 会显式降级,不会静默换算法。
  2. 多人审批 hand-off 链到备料:平台有审批运行时,但 owner 裁决先上轻量版(应用内游标 + 群通知,#5442),不绑定完整审批图 —— 未接线到备料流。属净新,未接线。
  3. 钉钉个人待办推送:owner 2026-09-02 裁决本轮不做(A 工作通知冒充待办 / B 单向待办镜像两案均推后);备料接力(#5442)现状仍只有钉钉群webhook,不是个人待办;平台侧审批待办单向镜像(#5772)已合入但默认关闭、只覆盖审批席位、owner 前置(其设计文档 §8)未满足,不适用于备料。
  若观众追问:第 1 条已发货但默认关闭,需部署方显式开启;第 2 条未接线,是范围内的下一步;第 3 条本轮不做,不要摆成在跑。

DEMO 全绿
  步骤 1-2-3 在结构一致合成源上,驱动发货管线,全部通过。
  现场唯一的变量是客户的数据 —— 见 onsite-connection-test-runbook-20260901.md 的 30 秒体检。
