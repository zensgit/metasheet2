...
登录后 `/stock-prep` 页左侧变成一条分组的竖排导航;窄屏下自动折成横排。底层结构没变:容器仍然是同一个 tablist,每一项仍然是一个 tab,只是换了方向和分组。三组:

- **【工作】**:今天要处理(一线的落地页)/ 项目备料 / 确认队列。
- **【部署与接入】**:开始使用(接入向导,独立成一页,不再挤在安装页顶部)/ 数据来源与体检(装完之后回来复查的地方,不再重复渲染向导内容;2026-09-08 hardening 波改名,原「安装 / 体检」,key 仍是 `install`,一处未改)/ 记录与排查(新面板,见下)/ 深度工具 ▾(七个旧版 tab 折进这个可展开分组,**默认收起**)。
...
向导独立成「开始使用」页之后,「数据来源与体检」页(2026-09-08 hardening 波前标签暂时还是「安装 / 体检」四个字,现已改名;key 仍是 `install`,导航与深链未变)重新分成三块:
