...
- **【部署与接入】**:开始使用(接入向导,独立成一页,不再挤在安装页顶部)/ 安装 / 体检(装完之后回来复查的地方,不再重复渲染向导内容)/ 记录与排查(新面板,见下)/ 深度工具 ▾(七个旧版 tab 折进这个可展开分组,**默认收起**)。
...
| `stock-prep:admin` | 上面四项 + 开始使用、安装/体检、记录与排查 | 装完 → 记录与排查;未装完或读不到 → 开始使用 |
...
向导独立成「开始使用」页之后,「安装 / 体检」页(标签暂时还是这四个字,没有改成「数据来源与体检」,见下方待裁决项)重新分成三块:
...
- **安装页 tab 名**:仍显示"安装 / 体检"这个旧标签,没有改成设计稿里的"数据来源与体检"——纯文案改动,留给下一波统一处理。
- **无项目号时的落地目的地**:「今天要处理」与「项目备料」在没有指定项目号时渲染的是同一个组件、同一屏画面,目前是两个导航项各自指向同一个目的地,没有合并成一个入口;是否要合并,待 owner 拍板(合并涉及导航项集合与既有深链的兼容性)。
