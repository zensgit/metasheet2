...
**覆盖面比"备料"大,这一点必须说清楚**:门装在 integration 插件的三个共享入口上(`requireTableActionAccess` 的 legacy 分支、`resolveTenantId`、`resolveAuthUserTenantId`),所以它覆盖的是**整个 integration 插件面**——备料的四十多条路由,以及同样走这三个入口的外接系统、数据源、模板、管道等路由,都一并要求租户声明,**以下两条路由是例外**:`GET .../stock-preparation/projects`(`stockPreparationProjectList`)与 `GET .../stock-preparation/snapshot-batches`(`stockPreparationSnapshotBatchList`)各自直接走 `requireAccess(req, 'read')` / `requireAccess(req, 'admin')` 这一条更早的 legacy 权限门(`http-routes.cjs:7176` / `:7197`),不经过上述三个共享入口的任何一个,所以这个 flag 开关不影响它们的准入判定——无声明令牌一样能通过这两条路由。222 上只跑备料线,这个范围可接受;**跑了其它 integration 能力的通用部署应当保持 flag 关闭**,否则那些路由也会对无声明令牌 403。
...
| ⑤ | **源预检对本客户 PLM 曾必然报 `bom_store_signals_conflict`(no-go),2026-09-08 客户补齐订单数据后已实测转 `go`;两种结果都不阻断拉取** | 客户 PLM 里有**两套** BOM 存储:`DN_PDM_BomDetailsInfo`(约 1319 行)与 `DN_PDM_DesignBom`(约 2570 行)。预检对每套只抽样 200 行,两边都抽满,因此"哪套行数更多"这一信号**无法判定**;虽然"权威"和"列形状"两个信号都指向 `BomDetailsInfo`,预检仍按设计拒绝下结论,以免用抽样上限制造出虚假的一致。**2026-09-04 在 222 上对客户测试库实测**(当时客户订单数据不全):不声明、声明 `declaredBridge=order-module` 两种情况都是 `no-go` 且拦截码不变;声明 `design-bom` 反而多一条 `declared_bridge_contradicts_measurement`,当时也没有"声明 BOM 存储"的入口可以消掉这条拦截。**2026-09-08 更新(见 §3 步骤 5)**:客户补订单数据之后,222 上实测抽样信号不再冲突,`verdict: go`、blockers 空——"必然 no-go"这句已不再成立;`go` 与 `no-go+bom_store_signals_conflict` 都是正常结果,只有其它拦截码才是真问题。处置:①**照常继续**——两种 verdict 下试算(dry-run)都能正常工作,预检是建议性报告而非拉取的闸门;②我方选用 `BomDetailsInfo` 是有依据的:客户旧备料系统的 mapper 只用 `BomHeadInfo`/`BomDetailsInfo`、从不引用 `DesignBom`,客户自己给出的 SQL 走法也走这一套;③若某次实测又回到 `no-go+bom_store_signals_conflict`,且客户希望预检转 `go`,需要产品侧增加"声明权威 BOM 存储"的入口或提高抽样上限,已记录为待办。 |
...
| ⑪ | **503 `SOURCE_UNAVAILABLE`(#5586)= 源库本身连不上,不是直接给 MetaSheet 账号加权限就能解决** | `DataSourceManager.connectDataSource`(所有按需连接的唯一咽喉,覆盖 `/schema`、`/tables/:table`、`/connect`、`/query`、`/select` 等 `/api/data-sources/*` 路由——即数据工厂连接管理里的结构 / 预览 / 测连接)在源库(PLM、K3 用的 SQL Server 等)连不上时把驱动原始报错统一翻译成固定文案,答 **503 `SOURCE_UNAVAILABLE`**,不回显 host/库名/账号等驱动细节。**这条不覆盖备料源预检**:预检自己的 `adapterRegistry.createAdapter` 直接建 adapter 读,不经这个咽喉;审阅人把该错误注入真实预检模块验证过,拿到的是 `verdict: no-go`、blocker 原因 `unknown_error`,不是这条 503 文案本身,预检页面对应的 no-go/unknown_error 改口留作后续。一线在数据工厂界面看到这个码,先查源库连通性/账号/权限,短时重试无效再升级给实施,不是直接给 MetaSheet 账号加权限;`POST /:id/test`(测试连接)仍会给出脱敏后的具体原因,供实施排障用。人话目录已收录该码(`plainLanguage.ts` 的 `STOCK_PREP_ERROR_PLAIN.SOURCE_UNAVAILABLE`,错误码对照抽屉可搜到)。 |
