...
**性能事实与开关(2026-09-07 补充,三轮追加实测)**:打开上面这个开关之后又做了几轮追加测试,发现一个和直觉相反的现象——**每写一行的服务端耗时不是固定的,会随目标表里已经写入的行数增长而变慢**:同一台机器上,目标表从约 1.3 万行涨到约 4 万行的过程中,每行耗时从约 47ms 一路涨到 80ms 以上;这个现象**开着这个加速开关、不开都会发生**,而且经"同一台机器、只切这一个开关"的对照测试确认,开关本身不是原因(打开后重复读表元数据的次数确实按预期降了下来,但整体耗时反而更高一些)——真正随表变大而变慢的原因出在写入前"这一行是否已经存在"的查重步骤上,目标表越大,这一步要过一遍的既有行就越多。**这不影响正确性,只影响耗时**:不管这个开关开着还是关着,写入结果(哪些行建成功、字段值、审计证据)完全一样,变化的只是每行花多少毫秒。修复正在推进中(思路是把"一行一次查重"改成"一批一次性查重"),**尚未上线**;在此之前,目标表越大、写入越慢是当前的真实状态,建议交付/演示前评估一下目标表当时的规模,不要只按 47ms/行去估算总耗时。

基于以上事实,这个开关**建议保持默认关闭**:它能省下来的那部分表元数据重复读取,相对于随表增长的耗时来说占比有限,实测甚至出现打开后总耗时反而更高的情况(已确认与开关本身无关,只是同一时间目标表恰好更大)——现阶段打开它拿不到确定的收益。**关闭时的正确步骤(重要更正)**:实测发现"只删掉 `app.env` 里那一行再重启"**不可靠**——`pm2 restart --update-env` 只会把重启时当前环境里存在的变量合并进去,并**不会**因为某个变量从 `app.env` 里被删掉就把它从已经在跑的进程里清掉,进程可能仍带着旧值继续运行。正确做法分两步:①先把这一行**显式**改成 `MULTITABLE_ENABLE_REQUEST_METADATA_CACHE=false` 并重启一次;②确认已经生效后,再把这一行从 `app.env` 里删掉(删不删这一行不影响关闭状态,只是让配置文件保持干净)。**怎么确认现在到底开没开**:不要只看 `app.env` 里有没有这一行,要看后端进程实际读到的值——查一下 pm2 里该进程当前的环境变量,核对 `MULTITABLE_ENABLE_REQUEST_METADATA_CACHE` 这一项的值,那才是真正生效的状态。
