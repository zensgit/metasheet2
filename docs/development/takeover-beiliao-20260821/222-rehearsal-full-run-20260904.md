...
**修法**:后台展开作业改用自己的一套上限——不写配置时 = 交互上限 × 倍数(行 ×20、翻页 ×10、读次数 ×20、耗时 ×6),再钳到代码硬顶;需要另行指定时用动作配置的可选 `largeBom` 块。交互试算行为字节级不变。222 现行配置(`maxRows:10000`/`maxPages:100`/`maxReadCount:30000`/`maxElapsedMs:600000`)推导出的后台上限是 **200000 行 / 1000 页 / 600000 次读 / 3600000 ms**,足够覆盖这个 13151 行项目。

**状态**:修复已实现(#5501),**222 上的复测(13151 行合成项目走通后台作业 → plan → 分批 apply)待 r11 部署后进行,结论届时补记**。复测时的核对方法:run 之后看作业记录的 `budgets`,四个键应等于上面推导出的值(`maxReadCount`/`maxElapsedMs` 若交互侧未配则显示 `null`=无上限,不是 0)。交付说明 §7.1 已按同一口径更新。
## 11. 源绑定作用域回退(方向 B;2026-09-10 起方向 A 也已落地,见文末订正)
...
- **调用方传了非 null 的 `workspaceId` 但精确未命中**——2026-09-10 起这条不再是"仍然只返回 `null`,不回退"(见文末订正):#5581(`b677d6ccb`)在 `get()`(`stock-preparation-source-binding-store.cjs:217-235`)里加了**方向 A**:精确未命中后,再点查**同一租户**下 `(tenant_id, workspace_id IS NULL, action_id)` 这一行——命中就返回,并标 `matchedWorkspaceId: null, scopeFallback: 'tenant_null_row'`;不命中才真的返回 `null`。三条边界仍然守住:**不跨 `tenant_id`**(方向 A 的点查带的是调用方自己的 tenant);**不回退到别的非 null workspace 的行**(方向 A 只点查 null 行,从不触发下面这条"零/一/多"的 sibling scan);**写路径 `set` 不加宽**(带 hint 的写仍然落在自己的 workspace 行,null 行不受影响)。这就是**方向 A**,和下面这条"null hint 缺行 → 找具体 workspace 行"的**方向 B** 是两条独立、不叠加的回退。测试:`stock-preparation-source-binding.test.cjs` 的 R-08 用例里 `get({ ..., workspaceId: 'ws_2', ... })` 这条断言已经从"返回 `null`"翻转成断言 `scopeFallback === 'tenant_null_row'` 且 `matchedWorkspaceId === null`;新增的 `stock-preparation-source-binding-scope-fallback.test.cjs` 里 F-07 钉住"不会猜到别的非 null workspace"这条边界(F-04 翻转,F-11..F-16 覆盖 tenant 隔离/null-row 精确性/写路径不加宽);端到端见 `stock-preparation-source-binding-routes.test.cjs` R-24。
...
- **受益名单要拆成两层,不是一份**:上面"绑定解析"受益的六个调用点(reconcile、mvp-persist、carry、export、handoff、project board)只是拿到了正确的 `externalSystemId`;真正会调 `loadTableActionSourceAdapter`、因此吃到 F3"取源"这个修法的,只有它的五个调用点——dry-run、reconcile、mvp-persist、apply、大 BOM 后台任务(large-BOM run)。carry/export/handoff/project board 从不调 `loadTableActionSourceAdapter`,停在"知道绑的是哪个 id"这一步,F3 那部分修法轮不到它们。
- **"请求本身没带 workspace hint"这个前置条件本身也要守住**:`loadTableActionSourceAdapter` 里 `!resolveWorkspaceId(req, {})` 这个判断不是装饰——如果去掉它,一个带着自己 workspace hint(比如 `ws_b`)的请求,只要"回退查询"(内部固定查 null hint)恰好解出跟这次请求已经拿到的 `externalSystemId` 相同的 id(哪怕是因为 ws_b 自己没绑、退到了部署默认值这种巧合),就会被回退的 `matchedWorkspaceId` 悄悄覆盖成别的 workspace——请求方明明指定了 ws_b,却读到了 ws_a 的东西。测试见 R-23:外接源只建在 `ws_a`、把它的 id 设成部署默认值、请求带 `?workspaceId=ws_b`,断言取源必须仍按 `ws_b` 走并 404(与这行代码加之前的行为一致),不能被静默改道。
...
> **2026-09-10 订正**:本节写于 2026-09-04,当时只有"方向 B"(null hint 缺行 → 找具体 workspace 行)。#5581(`b677d6ccb`,2026-09-09 合入,"源绑定读取对非 null workspace hint 回退同租户 null 行(作用域洞第三象限)")又加了**方向 A**(非 null hint 精确未命中 → 点查同租户 `workspace_id IS NULL` 行,标 `matchedWorkspaceId: null` / `scopeFallback: 'tenant_null_row'`;不跨租户、不回退到别的非 null workspace、写路径不加宽)。上面 §11「规则」里"调用方传了非 null 的 `workspaceId` 但未命中"那条已按此改写,`R-08` 用例的断言已从"返回 `null`"翻转为断言 `tenant_null_row`。这两条回退方向不叠加、互不覆盖,读这一节时以本条订正之后的文字为准。
