...
**这句话当时只对"带矩形(reconcile)的安装"成立,对追加式调用曾经是假的(#5455 后续 C0/P2,已修)。**宿主端口的 `INSERT ... ON CONFLICT DO UPDATE` 里那道归属守卫,曾把**裸标记无条件**绑进 `IN ($4, $5)`——与 `legacyAdoptable` 无关。于是一次**不带矩形**的 `applyRoleWriteScopes`(entries-only,插件能力面对所有插件开放的那条路径)会把裸标记行的 `visible`/`read_only` 一并改回 `true` 并重盖成调用方的 pack 标记:如果操作员在"盖章路由"上线前**藏过列**(`visible=false`)或**放宽过禁写**(`read_only=false`),那次人工决定会被静默改回,且计入 `applied`、不出现在 `removed`、`operatorHeld` 或任何报告里(追加式调用根本没有这三样)。现在改写与退役共用**同一个条件数组**(本包标记恒在;裸标记仅在 `legacyAdoptable` 为真时加入),未声明收养的裸标记行在 DO UPDATE 的三列上一律走 ELSE,原样返回。注意两点:①**带矩形的安装路径一直是安全的**——矩形内未证明的裸标记行先 422,压根到不了 upsert;②**不传 packId 的调用者**写的就是裸标记本身,它的"自己的行"与"裸标记行"字面同形,这个条件对它天然无效(见端口文件头)。对已在 222 上按 Step 3-3a 跑过回填、且只经安装器(始终带矩形)装包的部署,行为无变化。
