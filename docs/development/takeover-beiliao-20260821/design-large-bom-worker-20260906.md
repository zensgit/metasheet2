...
>
> **再另(2026-09-07)**:§1.8 促成的 #5524(常量元数据 memo,即 L1 最小形状)已合入,但"常量元数据重读是 47.07ms/行主因"这条推论**已被否证**——2026-09-07 的 memo 重测(§1.9)与根因诊断(§1.10)证实:打开 memo 后常量扫描确实按预期降到 1/chunk,但服务端每 chunk 墙钟反而更慢;经同状态对照实验排除"目标表增长"这个混杂因素后,判定真正随目标表行数线性增长的主因是**幂等键查重**(`findExistingRecord` 对 `meta_records.data ->> key` 的无索引整表顺扫,§1.10),不是常量元数据重读。**L1 的 memo 开关保持默认关**;不删除以上原有段落,仅在此更正,详见 §1.9/§1.10。
...
**2026-09-07 更新,已被 §1.9/§1.10 否证,标注见此**:上面"据此本 PR 做什么"这一节把 §1.3 的常量元数据重读当作解释 47.07ms/行(本节表格)的主要机制,并据此把 L1(`MULTITABLE_ENABLE_REQUEST_METADATA_CACHE`,已随 #5524 合并)当作直接优化项。**这个因果推论已被否证**:2026-09-07 打开 memo 重测(§1.9),常量元数据扫描确实按预期降到 1/chunk,但服务端每 chunk 墙钟不降反升(+30.6%);经同状态对照实验(§1.9 的 0004 轮,memo 关但表更大)排除"目标表增长"这个混杂因素后,判定表增长(而非 memo 开关)才是主导因素;根因诊断(§1.10)进一步定位到随表增长线性上涨的真正来源是**幂等键查重**(`findExistingRecord`)对 `meta_records.data ->> key` 的无索引整表顺扫。**不删除以上原有结论**——常量元数据重读确实存在、也确实被 memo 按设计消除,这两条本身没被推翻;被更正的只是"它是 47ms/行主因"这条因果判断。**L1 的 memo 开关保持默认关。** 详见 §1.9(memo 重测与同状态对照)、§1.10(根因诊断)。

### 1.9 memo 重测与同状态对照(2026-09-07)

**范围**:PR #5524(`MULTITABLE_ENABLE_REQUEST_METADATA_CACHE`,即上面 §1.8 描述的 L1 最小形状,基线 `origin/main` @ `e349efade`)合入后,在 222 上做了两轮追加实测:①打开 memo,用从未写入过的合成项目 `SYN-PROJ-LARGE-0003` 重跑 CREATE 路径(与 0002 轮同形);②协调方指出"打开 memo 更慢"可能是"目标表比 0002 轮又大了一倍"这个混杂因素造成的,要求在同一台机器、memo 关的前提下再跑一轮 `SYN-PROJ-LARGE-0004` 作同状态对照。0003 轮因驱动脚本单次铸造的管理员令牌 15 分钟(900s)到期,在第 132(最后一)个 chunk 收到 401,服务端只留下 131/132 chunk(13100/13151 行,99.6%)——如实标注分母,job 停在可续跑的 `paused`,未做任何续跑/取消操作;0004 轮把测试驱动(`large-bom-drive.cjs`,仅改 tmp 本地副本与 222 上 `tools-r15` 的测试工具副本,**未改仓库**)的令牌 TTL 临时放宽到 1800s,132/132 完整跑完,跑完后已将 222 上的副本还原回 900s。

**四列对比**(目标表状态一行取开跑前 `meta_records` 累计 `n_tup_ins`,作为"表已经多大"的代理指标):

| 指标 | skip 轮(0001) | CREATE·无 memo(0002,第 2 次) | CREATE·memo 开(0003,第 3 次,**131/132 chunk**) | CREATE·memo 关对照(0004,第 4 次,同状态对照) |
|---|---|---|---|---|
| 目标表开跑前状态 | 不适用 | ≈13338 | ≈26489 | **≈39589** |
| 服务端每 chunk 墙钟(均值) | 490.8 ms | 5179.8 ms | 6763.2 ms(+30.6% vs 0002) | **8759.4 ms**(+29.5% vs 0003,memo 关但更慢) |
| chunk 时长分布(histogram) | ≤0.5s 命中 85/132(64%) | 0% ≤2s;59.8% >5s | 0% ≤5s;100%(131/131)>5s | **0% ≤5s;100%(132/132)>5s**(比 0003 更极端) |
| 每行服务端墙钟 | 不适用 | 47.065 ms/行 | 62.724 ms/行(裸口径 67.632) | **82.994 ms/行**(裸口径 87.921)——没有回到 47 附近,比 0003 更高 |
| meta_fields / registry `seq_scan`(每 chunk) | 背景噪声量级 | ≈2/行(≈199.3/chunk) | ≈1.02/chunk(降幅≈99.5%) | **≈199.3/chunk**(回到"关"状态水平,与表大小无关) |
| meta_sheets `idx_scan`(每 chunk) | 0(机制不同) | ≈3/行(≈298.9/chunk) | ≈101.02/chunk(降幅≈66%) | **≈298.9/chunk**(与 0002 逐位相同) |
| meta_records n_tup_ins / idx_scan | 0 / 0 | 13151 / 13151(精确) | 13100 / 13100(精确,131×100) | **13151 / 13151**(精确,132×100+51) |
| plugin_kv 四计数器(折算每 chunk) | 不适用 | 2.053/2.038/2.023 | 2.053/2.038/2.023(逐位同前) | **2.053/2.038/2.023**(三轮全部一致) |
| 作业是否跑完 | 是(132/132) | 是(132/132) | 否——401 于第 132 个 chunk,job 停在 `paused` | **是(132/132)**——测试驱动令牌 TTL 临时放宽后跑完,跑完即还原 |
| 总驱动耗时 | 93.9s | 708.3s | 905.5s(超出驱动令牌原 900s TTL) | **1175.8s** |

**协调方三条判据的结论**:

- **(a) 对照轮每行 ms 与 47.07ms 相比是否也涨了?** 涨了,而且比 0003 轮(memo 开)涨得更多:0004 轮(memo 关)每行 82.994ms(裸口径 87.921ms),既没有回到 47 附近,也没有停在 0003 轮的 62.724ms,而是继续涨了约 20.3ms/行(+32.4% vs 0003)。落在"仍然更高,而非回落到 47"这一支——说明减速的主导因素是目标表增长/机器状态,不是 memo 开关。
- **(b) 对照轮 seq_scan/idx_scan 每 chunk 是否回到 200/200/300?** 是,精确回到:`meta_fields`/`registry` seq_scan≈199.28/chunk,`meta_sheets` idx_scan≈298.90/chunk,与 0002 轮(同为 memo 关)逐位相同,不随目标表从 ~1.3 万行长到 ~4 万行而改变——扫描次数只由 memo 开关决定,与目标表已经多大无关,这一点在对照实验后被加强确认。
- **(c) 两轮总墙钟**:0003 轮(memo 开,131/132 chunk,未跑完)905.5s;0004 轮(memo 关,对照,132/132 chunk,TTL 补丁后完整跑完)1175.8s——比 0003 轮长 270.3s(+29.9%),尽管 0004 轮是"关掉了 memo"这个理论上应该更快的状态。

**growth_explains 裁决**:判定为 **growth_explains——表增长(而非 memo 开关)是服务端墙钟这条曲线的主导因素**。三轮 `(13338, 5179.8)`、`(26489, 6763.2,memo 开)`、`(39589, 8759.4,memo 关)` 近似落在同一条直线上;且 0003→0004 这一步(memo 从开变关、表继续变大)墙钟**没有因为关掉 memo 而回落,反而继续涨**(+29.5%)——如果 memo 是主因,这一步应该变快,实测方向相反。

**线性拟合(0002→0003→0004 三点,两段斜率)**:0002→0003 段斜率约 0.120 ms/chunk/行,0003→0004 段(memo 未变,纯表变大)斜率约 0.152 ms/chunk/行,量级接近、方向一致。**推断**(用 0003→0004 这段"纯增长"斜率反推"0003 轮如果不开 memo、只受增长影响"应落在哪):`5179.8 + 0.152×(26489-13338) ≈ 7183.8 ms/chunk`;实测 0003 轮(memo 开)是 6763.2ms/chunk,比这个"纯增长"预测值还低约 420.6ms/chunk——扣掉表增长这个主导因素后,memo 本身的边际效应看起来是**轻微正向(更快),不是原判的"更慢"**。这只是基于 3 个点、2 段斜率的线性外推,样本量小,未排除超线性增长(如 B 树索引变深)的可能,**标注为推断**,不作为确证结论。

**运维事实(0003/0004 轮实测中发现,与本节数字无关但一并记录)**:① 驱动脚本 `large-bom-drive.cjs` 铸造的管理员令牌 15 分钟(900s)不续期,超过该时长的作业会在末尾收到 401(作业停在可续跑的 `paused`,不是 `failed`)——这是测试工具本身的缺陷,不是产品缺陷;② `pm2 restart --update-env` 只合并/新增调用方环境里存在的键,**不会**因为某个键从 `app.env` 里被删除就把它从目标进程里删掉——关闭 `MULTITABLE_ENABLE_REQUEST_METADATA_CACHE` 必须先显式写 `=false` 重启一次、再删行,否则"关闭"后 `pm2 env` 仍会读到旧值;③ 确认当前是否开着的可靠办法是 `pm2 env 0`,不是看 `app.env` 是否有该行。

### 1.10 根因:幂等键查重的整表扫描(2026-09-07)

**方法**:222 上只读 `EXPLAIN ANALYZE` 实证(未改代码、未改 PG 配置),定位 §1.9 那条随表增长线性上涨曲线的真正来源。

**代码路径**:`findExistingRecord`(`plugins/plugin-integration-core/lib/stock-preparation-apply-writer.cjs:250-267`,ADD/UPDATE 两条决策分支各调一次,调用点 `:376` 与 `:399`;**SKIP/MANUAL_CONFIRM 分支不调用它**,这也是 skip 轮(0001)完全测不到这条成本的原因)→ 宿主 `query-service.ts:273-283` / `:294` / `:306-322` 生成:

```sql
SELECT … FROM meta_records WHERE sheet_id = $1 AND data ->> $2 = $3 ORDER BY id ASC LIMIT 2
```

`data ->> key` 是对 jsonb 列取文本再比较,**没有索引覆盖这个谓词**。规划器改用 `idx_meta_records_sheet_id_id`(仅覆盖 `sheet_id`)做前缀扫描,再逐行取出约 1.3KB 的 JSONB 做过滤——等价于对目标 sheet 的**全部现有行**做一次顺序扫描。`UPDATE` 路径走的是同一个 `findExistingRecord`,同样的 `LIMIT 2` 语义(要确认"至多两行同 key"这个不变量)**必须扫到表尾才能确认没有第二行匹配**,所以同样是 O(n)。

**222 上的实测证据**:`pg_stat_user_tables` 的 `idx_tup_read`/`idx_scan` 比值约 26,992(即命中一次 `idx_scan` 平均要读近 2.7 万个索引项才能把整个 sheet 的行过一遍);对一个已有 52,560 行的 sheet 做 `EXPLAIN ANALYZE`,`Rows Removed by Filter: 52560`,单次执行 114ms。

**三点最小二乘拟合**(用 §1.9 三轮 CREATE 数据的目标表行数与每行服务端墙钟做回归,**标注为推断**——三个数据点的线性回归,不是逐点实测):斜率 **1.382 ms/千行**,截距 **28.34 ms/行**,残差 ≤1.54ms。斜率项对应"表越大、`findExistingRecord` 顺扫得越久";截距项(**推断**:每行独立事务 + 修订历史写入的固定开销,对应 §1.2 的 E…L 段)与目标表大小无关。

**与 #5524 的关系**:#5524(即 §1.8 的 L1 memo)砍掉的是常量元数据(`meta_sheets`/`meta_fields`/registry)的重复读取,**与这条 `findExistingRecord` 的整表顺扫是两件独立的事**——memo 命中的 §1.3 机制不覆盖 `meta_records` 本身的存在性查询,这就是为什么 §1.9 里打开 memo 后常量扫描降到 1/chunk,但服务端墙钟仍然随表增长线性上涨。

**否决过的替代方案**(均未实施,只读评估):

- **GIN `@>` 索引**:带 `sheet_id` 前缀 + `LIMIT` 时规划器实测改走 Seq Scan,不生效;且 `@>` 的包含语义与 `data ->> key = value` 的字符串相等语义不等价,需要额外的值编码改造。
- **逐 sheet 的表达式索引**:属于 DDL 变更,须先过已证的租户门,本次未做。
- **唯一索引 + upsert**:长期看是正解(把"查重后二选一"的两次往返收成一次 `ON CONFLICT`),但改动面大(涉及 §1.2 的整段写入语义、`applyStockPreparationPlan` 主循环的 decision 分类),本次不做。

**最小修法(已完成,PR #5526,已合入 main `8b9aed5f0`,r16 已上线)**:把逐行查重改成每 chunk 一次批量查重(`WHERE … = ANY(...)`),原在 `perf/stock-prep-batch-key-lookup` 分支实现,已合入。验收数字见下方新增小节——不再是预测,是 222 上 r16 的实测结果。

**下一个杠杆(推断,未实测)**:批量 `INSERT`——截距 28.34ms/行本身是"每行独立事务 + 独立修订历史写入"的固定开销,批量查重解决的是斜率项、不解决截距项;若要继续压缩,下一步要看能不能把多行的 `INSERT`/修订写入合并成更少的往返,这与 §1.2 讨论过的"L3:一个 chunk 一个事务"是同一个方向,同样需要先解决 G3(见 §4.3/§6)。

### 验收(2026-09-07,222,r16,合成项目 0005)

**范围**:PR #5526 合入 main(`8b9aed5f0`)并上线 r16 后,用一个从未写入过的合成项目 `SYN-PROJ-LARGE-0005` 重跑与 0004 轮(§1.9,memo 关,对照轮)完全相同的大 BOM CREATE 路径实测,只改合成项目号与驱动脚本 TTL 补丁打在 `tools-r16` 副本上;memo(`MULTITABLE_ENABLE_REQUEST_METADATA_CACHE`)全程保持默认关闭、未触碰。`MULTITABLE_STOCK_PREP_BATCH_KEY_LOOKUP` 全程未在 `app.env`/`pm2 env` 中设置,按代码默认值(开)运行。

**A1/A2 断言**(不看耗时,直接看 `pg_stat_user_indexes`/`pg_stat_user_tables` 原始计数器差分):

| 断言 | 读数 | 判定 |
|---|---|---|
| A1:`idx_meta_records_sheet_id_id` 的 `idx_scan` 增量 | 51124 → 51256,Δ=**132** | 精确等于本轮 chunk 数(132),不是"≈132"。对照:批量查重上线前三轮(0002/0003/0004)`meta_records` 表级 `idx_scan` 增量分别是 13151/13100/13151——精确等于当轮创建的行数(每行一次)。 |
| A1 附加:同一索引的 `idx_tup_read` 增量 | Δ=**7,802,520**,平均每次扫描 59,110 行 | 表在本轮跑动过程中从约 52,740 行长到约 65,891 行,均值约 59.3k,与实测平均扫描深度几乎重合——批量化省的是"查几次",不是"每次查多深":`data->>key` 无索引覆盖,规划器仍要顺着 `idx_meta_records_sheet_id_id` 扫完整个 sheet、逐行取 JSONB 用 `= ANY(...)` 过滤。 |
| A2:`meta_records` 表级 `n_tup_ins` 增量 | 52740 → 65891,Δ=**13151** | 精确等于本轮创建行数,写入路径本身未被 #5526 改动。 |
| A2 附加:`meta_records` 表级 `idx_scan` 增量 | Δ=**132** | 与 A1 逐位相同——证明本轮 `meta_records` 上只有 `idx_meta_records_sheet_id_id` 这一枚索引被扫描过,批量存在性探针是本轮唯一的索引访问来源。 |

**结论**:批量查重确定生效,索引扫描次数从"每行一次"(13151)精确降到"每 chunk 一次"(132),降幅 99.0%。

**五轮对比**(0001 skip / 0002 create / 0003 memo 开 / 0004 对照·memo 关 / 0005 批量查重开):

| 指标 | 0001 skip | 0002 create | 0003 memo 开(131/132) | 0004 对照·memo 关 | 0005 批量查重开 |
|---|---|---|---|---|---|
| 目标表开跑前 `n_tup_ins` | 不适用 | ≈13338 | ≈26489 | ≈39589 | **≈52740** |
| 服务端每 chunk 墙钟(均值) | 490.8 ms | 5179.8 ms | 6763.2 ms | 8759.4 ms | **2669.06 ms**(−69.5% vs 0004) |
| 每行服务端墙钟 | 不适用 | 47.065 ms/行 | 62.724 ms/行 | 82.994 ms/行 | **21.864 ms/行**(裸口径 26.79) |
| `meta_records` idx_scan(表级) | 0 | 13151(=行数) | 13100(=行数) | 13151(=行数) | **132(=chunk 数)** |
| 总驱动耗时 | 93.9s | 708.3s | 905.5s(131/132) | 1175.8s | **378.3s** |

`idx_meta_records_sheet_id_id` 的细粒度 `idx_scan` 此前四轮均未采集,0005 轮首次采集,数字见上方 A1。

**每行与每 chunk 数字**:服务端每 chunk 墙钟 2669.06ms(0004 对照 8759.4ms,−69.5%);每行服务端墙钟 21.864ms(扣 490.8ms/chunk 地板口径,裸口径 26.79ms)。相对 0003→0004 纯增长斜率外推出的"若无批量修复、同等表状态"拟合基线约 103.1ms/行,降幅约 78.8%。132/132 chunk 全部跑完,0 skip、0 failed;总驱动耗时 378.3s(0004:1175.8s)。

**idx_tup_read 说明(省次数不省深度)**:批量化把索引扫描的次数从 13151 降到 132,但每次扫描本身仍然要顺着 `idx_meta_records_sheet_id_id` 扫过目标 sheet 的全部现有行——`idx_tup_read` 增量 7,802,520,折算平均每次扫描 59,110 行,与本轮表大小均值(约 59.3k)几乎重合。换句话说,#5526 省下来的是"查几次",查询本身的扫描深度仍然与表大小成正比,不是把 O(n) 顺扫本身消灭掉。

**附带发现(推断,整数分解支持,机制未经源码逐行确认)**:`meta_fields`/`plugin_multitable_object_registry` 的 `seq_scan` 从 0002/0004 型态的约 199.3/chunk 降到约 100.65/chunk(降约一半),`meta_sheets` 的 `idx_scan` 从约 298.9/chunk 降到约 200.27/chunk(降约三分之一)。整数分解:`meta_fields`/registry = 13151(写入路径,仍按行) + 132(批量存在性探针,按 chunk) = 13283,实测 13286(多出 3,与历轮"dry-run/expansion/plan 阶段常量读"噪声同一量级);`meta_sheets` = 2×13151(写入路径,仍按行) + 132(按 chunk) = 26434,实测 26436(多出 2,同一量级)。**推断的解释**:批量化之前,存在性探针每行都会独立调用一次宿主 `queryRecords`,而 `queryRecords` 内部会先加载一次 `meta_fields`/registry/`meta_sheets`;批量化后这条路径从"每行一次"变成"每 chunk 一次",连带把它捎带的那部分元数据加载频率也降了下来——写入路径自身(`createRecord`/`patchRecord`)对元数据的加载未被 #5526 触碰,仍按行。若这个推断成立,意味着 #5526 顺带把 #5524(memo,仍默认关)本该解决的"常量元数据重复读取"问题的一部分(仅限存在性探针触发的那部分)也一并降了下来,是提交说明未主动声称的正向副作用。

**剩余成本结构(推断,非本轮直接测得)**:实测 21.86~26.79 ms/行与本节上文线性拟合给出的截距"28.34ms/行(残差≤1.54ms)"量级相近,该截距被解释为"与表大小无关的固定项"。**推断**:剩余成本很可能主要是"每行一次独立事务 + 一次 `meta_record_revisions` 插入"这类结构性固定开销(本轮 A2 已确认每行精确对应一次 `meta_records` 插入和一次 `meta_record_revisions` 插入,1:1),而不是残留的 O(n) 扫描——但这一点未用 `pg_stat_statements`(仍未装)或逐行 `EXPLAIN` 做独立验证,标注为推断。下一个杠杆仍是批量 `INSERT`(见上方"下一个杠杆"段,同样标注为推断)。

**并发窗口口径**:批量化把重复插入检测的窗口从一行拉宽到一个 chunk(≤1000,本轮实际 100)——查重与写入之间,若另一写者在这个窗口内删掉了本应匹配到的行,本行的结果会由本该判定的 `created` 变成 `failed`,而不是像逐行版本那样窗口只有一行那么窄。语义上仍与逐行一致(两者都不保证"查完到写完"之间没有别的写者插队),只是命中这个窗口的概率随窗口变宽而变大。真正关死这个窗口需要唯一索引 + upsert(上文"否决过的替代方案"已记录为长期正解,本次未做,另开)。

**回落条件**(不需要人工干预,系统对当次那一批自动退回逐行查重,语义与逐行一致):宿主环境不支持批量查询能力;单批要查的键数超过 1000;批量查询结果撞到上限;键值不是字符串;写入落库后发生异常。

**方法论缺口(不影响以上结论)**:本轮 M5(`pg_stat_activity` 采样)因发起采样进程的 SSH 会话在采样窗口结束前关闭、连带杀死子进程而缺失数据(定位,推断,未读源码确认:Windows 上的 OpenSSH Server 疑似把会话下的进程放进带 `JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE` 语义的 Job Object,会话一关连带杀掉所有子孙进程)。M5/A3 是任务书标注的可选项,A1/A2 已经从原始计数器拿到更强的确定性证据,此项缺失不影响本节任何结论。
