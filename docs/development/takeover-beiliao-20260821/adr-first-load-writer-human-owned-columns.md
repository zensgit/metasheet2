...
**核对结果：状态账本 §4 没有这一条**（该节现有七项:V3 抽取债、~~`ext_` 守卫接线缺口~~(**已闭合**, 2026-09-10 初核;2026-09-11 全集复核)、`ext_` mapper 未接线、提案文档 stale、大 BOM 路径未接、迁移必须先做身份归一、本机
pin 重算不可信,`beiliao-takeover-status-ledger.md:96-102`。**2026-09-11 订正**:原文"六项"与行号
`:65-76` 均已 stale——`ext_` mapper 未接线是之后新增的第七项,`:65-76` 实际落在 §3 owner 决策队列而
非 §4;`ext_` 守卫接线缺口一项已按逐路径复核降级为已闭合,详见该节本身）。
而代码里的顺序是**刻意如此、且有文档**：
