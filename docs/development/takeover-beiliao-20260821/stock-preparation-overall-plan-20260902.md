...
- ~~插件服务账号的记录写入是否绕过 `field_permissions`~~ —— **2026-09-11 已核实:绕过。** 插件写路径上没有列级权限门(SDK 通道自始至终不带 actor,`multitable/records.ts`、`multitable/plugin-scope.ts`、`stock-preparation-persist-unit-of-work.ts` 三个文件里没有任何一处读 `field_permissions`)。所以 W1-5 只保护网格,不保护插件/SDK 写入。
  - 可执行断言(结论从此有测试托底,不再只是散文):
    ...
    - **M1(腿 2 的行为级变异断言)已经在腿 2 里面了,不再是文本代理**:插件路径上一旦出现列权限门,**已知的三种表现**都会让腿 2 变红并打印人话 —— 门抛错(`try/catch` 点名 M1)、门只过滤不抛(返回值里这一列没有新值)、写被下游吞掉(回库读这一列没变)。**这是"已知三种",不是穷举**:一个把库里也改写成同一探针值的"门"三条都骗得过去(腿 2 自己的注释 `:1099-1101` 已写明这条逃逸,并说明那已经不是"门");所以 **加门 ⇒ 腿 2 红,是对这三种表现而言**的直接推论,不需要再手工插一次门来演示,超出这三种的形态要另行论证。
    - **仍未执行(风险,保留在此)**:腿 1 / 腿 2 **到目前为止一次都没有真正跑过** —— 开发机没有 PostgreSQL(5432 无监听),整支套件在本机是 `1 passed | 22 skipped`(那 1 条是 DATABASE_URL 哨兵)。它的执行者是 `.github/workflows/plugin-tests.yml` 的 `multitable-real-db-integration` 步骤(该文件在 `packages/core-backend/vitest.config.ts` 的 `exclude:` 里,所以无库的默认 job 收集不到它、也就不可能"跳过即绿")。**第一次真实结论要到带库的 CI 跑完才有**;在那之前,关于这两条腿的任何绿都不是证据。
    ...
    - **这支守卫在 CI 里跑不跑?跑。** 它由 `pnpm --filter @metasheet/core-backend test` 收集执行 —— 即 `plugin-tests.yml` 的 `Run core-backend tests` 步骤(`pull_request` 触发没有 `paths:` 过滤,每个对 main 的 PR 都跑)。该 script 是裸 `vitest`,`vitest.config.ts` 没有 `include:` 键,本文件也不在 `exclude:` 里,所以默认 include 收得到它。"没有被 workflow 逐文件点名"与"CI 上不会跑"是两件事,只有后者才决定一条守卫是否在把门 —— 曾有评审按前者误判成后者,并据此建议加一个多余的独立 step。这条结论现在由守卫自己的 `wiring:` 断言托底(同时钉住上面那条真库 golden 的两点接线),接线被拆会直接红在一条每个 PR 都跑的 unit 测试上。
  - **这两条绿是"特征化当前行为",不是认可插件可以绕过列权限。** 要不要给插件路径加门是另一次裁决(牵动备料写入链);一旦加了或撤了,上面两处会立刻变红,请到那两个文件改断言并同步本条。
  - **守卫的覆盖边界(别把绿读大了)**:`multitable-w13-write-path-layer3-gate.guard.test.ts` 的"全集"只在**三个写面模块**(`RecordWriteService` / `RecordService` / `multitable/records.ts`)之内成立 —— 在这三个模块内它是穷举的(成员按 **TypeScript AST** 从源码枚举 —— 属性式箭头函数成员、`static`、访问器、计算属性名、以及模块的值 re-export 都看得见;调用站点全 `src` 扫描),但它**没有**断言"只有这三个模块能往 `meta_records.data` 写值"。直写 raw SQL 的写脊在覆盖面之外,**另案,未评估列级门**:`multitable/automation-executor.ts`、`multitable/automation-service.ts`、`multitable/derived-write-fence.ts`、`multitable/exact-anchor-recovery-execute.ts`、`multitable/auto-number-service.ts`、`multitable/approval-record-projection-service.ts`、`services/elearning-stats-multitable-projection.ts`、以及 `routes/univer-meta.ts` 里的若干条 raw 语句。这些文件由守卫末尾的 `chokepoint: the set of files running raw INSERT/UPDATE on meta_records is frozen` 冻结成一份清单 —— 新增写脊会变红,但"它们有没有列级门"这个问题仍然没人回答过。该 chokepoint 的表名匹配已归一化(可选 schema 限定、可选 `ONLY`、可选双引号、大小写不敏感),在此之前 `UPDATE public.meta_records` / `UPDATE Public."META_RECORDS"` / `UPDATE ONLY meta_records` 三种写法都能无声绕过它。它的两条排除面也写在那条测试的注释里,别把绿读大:**① `src/db/migrations/**` 不在扫描范围内**,其中 `zzzz20260430163000_add_meta_record_modified_by.ts` 与 `zzzz20260516113000_repair_onprem_multitable_record_create.ts` 两支确实在裸写 `meta_records`(一次性修复,不是写脊);**② 只扫 core-backend 的 `src` 树**,其它包/插件发出的 SQL 它看不见。
  - 补全枚举时还发现一条**裸名**调用 `records.ts` 的 `patchRecord` 的腿:`src/attendance/attendance-multitable-cleaning-authority.ts:319`(`cleanupAttendanceCleaningProposal`)。它**不是**第二条绕过腿 —— 它必经的锁链 `lockAttendanceCleaningSource → lockAttendanceCleaningProjectionAccess` 在 :123-125 做了 `loadFieldPermissionScopeMap` + `deriveFieldPermissions` + `isFieldWriteForbidden`,而且门的正是它随后写的那两个字段。守卫里有一条同名 smoke 断言把这条"门在调用方"钉住。
