...
- ~~插件服务账号的记录写入是否绕过 `field_permissions`~~ —— **2026-09-11 已核实:绕过。** 插件写路径上没有列级权限门(SDK 通道自始至终不带 actor,`multitable/records.ts`、`multitable/plugin-scope.ts`、`stock-preparation-persist-unit-of-work.ts` 三个文件里没有任何一处读 `field_permissions`)。所以 W1-5 只保护网格,不保护插件/SDK 写入。
  - 可执行断言(结论从此有测试托底,不再只是散文):
    ...
    - **待办(合并前必须做掉)**:上面那条真库 golden 的腿 1 / 腿 2 **在本波从未执行过一次** —— 开发机没有 PostgreSQL,整支套件 `1 passed | 22 skipped`。它已在 `.github/workflows/plugin-tests.yml` 的 `multitable-real-db-integration` 清单里,请在带库的环境上跑一次再合;同时在那里做真正的变异自证(临时给 `src/multitable/records.ts` 的 `patchRecord` 前加列权限门 ⇒ 腿 2 必红、腿 1 仍 403,然后还原)。本波只做到了文本级代理断言(守卫里的 characterization)。
