...
4. **Bash 工具的 heredoc 会折叠反斜杠**，带 Windows 路径的脚本要先用文件写盘再跑。
...
**PR #5625 —— 草稿，三选项等裁决。** 大 BOM 计划与分片 apply 补传 `installedFieldProperties`（让大 BOM 写入口径与小 BOM 对齐）。问题在于：接上之后，**默认部署（包括 222）每轮刷新都会真写一遍每一行携带 `ext_` 值的记录**，而且记录下来的刷新理由指向的列名和这个补丁实际携带的列名对不上。三个选项写在 PR 正文里。在你裁决前不合并。
...
- 第 5 波（本文写作时在跑）：#5455 遗留的字段权限归属缺陷 C0/C6/C8；外接源列表不看 workspace 导致选源面板误报「源不可用」（单条读 #5471 已修，列表没跟）。
- `plans-15` 对抗覆盖、`plans-38` `ext_` 守卫回归测试与账本、`today-followups-01` 预检 `scopedInput`（后者阻塞在并行会话的 #5590）。
- **r28 应包含 #5626**（§2.2 末尾）。
