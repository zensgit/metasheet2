...
13. **备料落地表的长期落点(参见 `stock-preparation-overall-plan-20260902.md` §8 决策 D1)**:该文档把落地表分成三个选项——A 正式主表(+ P4 文件加载器)、B sandbox 命名空间表(零代码,当前在用)、C 授权表(Wave 2,明确不做)。当时的裁决是"B 过窗口,A 在 Wave 1 末补上",但截至本文核对时尚未开始向 A 迁移。是否/何时把 B 升级为 A,待拍板。
...
17. **`audit_logs` 分区的创建时机**:数据库函数 `create_audit_partition()`(建下个月分区)存在但没有任何调用者——不在应用启动流程,也没有接每日调度,分区目前只能靠手工 SQL 或"撞上缺分区错误后自愈"。#5506 只修复了自愈的识别逻辑(改按 SQLSTATE 而不是英文报错文案),没有改变"只能事后自愈、不能提前预建"这个姿态。是否要把 `create_audit_partition()` 接到启动检查或每日调度,待拍板。
