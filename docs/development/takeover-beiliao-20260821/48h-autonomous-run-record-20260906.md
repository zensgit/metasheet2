...
13. **W3a 缺件清单设计的三条既有留白(PR #5500 正文"未做 / 留给 owner"一节)**:
    - **13a(D-A)缺件 CSV 是否加"每次探测一行"模式**——**2026-09-10 拍板:默认不做,理由与重新排期触发条件如下**。原问题(引):"当前 CSV/复制按零件号去重成一行(用 `parentCount` 角标标出该零件挂在几个父件下);要不要额外提供'探测到几次就展开几行'的明细模式,待拍板。" 裁决:
      - **理由一**:expander 在采集点按零件号聚合是**有意设计**,不是遗漏——聚合行本身就是"这个零件缺,挂在几个父件下"这条判断需要的形状,一线要处理的是"缺什么"而不是"在哪几处被引用"。
      - **理由二**:明细模式会把导出体量放大到"父件引用次数"量级(同一个缺件零件号在大 BOM 下可能有两位数父件引用),且**触及 values-free 边界**——展开明细必然携带更多结构上下文(父件路径/层级),偏离"缺件清单只给零件号 + 计数"的最小披露设计。
