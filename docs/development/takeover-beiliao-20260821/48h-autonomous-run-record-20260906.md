...
24. **合成目标表已 52,560 行 / 122MB,要复现 47ms 基线需清理合成行(写操作,待裁决)**:三轮 CREATE 实测(0002/0003/0004)都往同一批合成源(`SYN-PROJ-LARGE-*`)累计写入,目标表(`meta_records` 等)已长到 52,560 行 / 122MB,后续再实测测到的都是"表已经很大"这条增长曲线上更靠右的一点,测不回 47.07ms/行这个起点(0002 轮开跑前目标表仅约 1.3 万行)。清理合成行是写操作,不在本轮只读护栏授权范围内,待 owner 裁决是否清理、何时清理。
