...
13. **W3a 缺件清单设计的三条既有留白(PR #5500 正文"未做 / 留给 owner"一节)**:
    - **13a(D-A)缺件 CSV 是否加"每次探测一行"模式**——**2026-09-10 拍板:默认不做,理由与重新排期触发条件如下**。原问题(引):"当前 CSV/复制按零件号去重成一行(用 `parentCount` 角标标出该零件挂在几个父件下);要不要额外提供'探测到几次就展开几行'的明细模式,待拍板。" 裁决:
      ...
      - **理由二(2026-09-10 改写,原文"触及 values-free 边界"不成立,已删)**:逐次明细的数据本就没有完整保留下来——采集器对细节带上限,超限的探测点被"dropped detail"(同文件 `:1707` 注释);要做明细模式得先改采集器,让它保留每一次探测的上下文,而不是在汇总层(`summarizeMissingComponents`)加个开关就能出。改了采集器之后,明细模式还会把导出体量放大到"父件引用次数"量级(同一个缺件零件号在大 BOM 下可能有两位数父件引用)。**不是** values-free 问题:`parentSourceId`/`bomId`/`path`/`depth` 这几个字段今天已经在每条汇总记录里(`stock-preparation-bom-expansion.cjs:1146` 采集、`:1750-1755` 原样透出到响应),缺件清单本身是请求-响应件、不落库(`apps/web/src/services/integration/stockPreparation/projectSync.ts:328` `missingComponentsOf` 客户端解析),明细模式只是把已有字段多打几行,不引入新的披露维度。
