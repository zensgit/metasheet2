...
| 2026-09-08 12:17–14:20 | U4 真浏览器验收 lane(PR #5560)对抗核验 + 修复 | 11 条 blocker 反驳全部修完:分类器手打清单漏记 lane 自己读的 spec 与四个运行时依赖 → 改为从 harness+fixtures 的 import 闭包与两支 spec 的落盘读路径推导守卫,再喂给真跑的分类器;P0-10 阈值 ≥5 对着 12 处、删 D2 两条断言仍绿 → 改按 KEY 计数、下界=当日实际条数;R-01 原判"jsdom 盲区",反驳指出是"jsdom 没人写"而非"看不见",describe 拆两组并改名;P1-03 即时断言计数改 `expect.poll`;P0-08 点完立刻读剪贴板改成先等按钮翻「已复制」再读;P0-02 后半只证明了输入框改成断言发出去的查询串(换号前后各带各的号);`reader`/`readerOnly` 是死码、D2 第四态没跑,补三条落地用例。已合入(`be93b630f`) |
| 2026-09-08 15:31 | #5561 已合(`17173f639`) | InstallView 三形态(`full`/`wizard`/`review`)补 jsdom 用例,坐实上面 R-01 判定属实(报错条只在 `mode='wizard'` 时渲染,jsdom 完全写得出、此前只是没人写);`run-required-web-tests.sh` 加一段注释钉住备料 vitest token 全 CamelCase 大小写约定,防止未来新增小写 `stock-prep-` 前缀 vitest token 被这条必需检查的路径子串过滤误收进来 |
| 2026-09-08 14:21–16:59 | P2-1 项目查询面板(PR #5562)对抗核验 + 修复 | 7 条 blocker 全部修完:`?tab=project-query` 离开面板后残留(两次点击可复现)→ `hasStaleProjectQueryState()` 把它也算作残留,判据从 `activeKey` 换成 `effectiveKey`;右栏「表里有多少行」漏 `pullTargetReady===false` 分支、把「读不到」印成自信的 0 → 逐字沿用项目备料页的三分支;姿态 chip 计数是全量、不含来源与搜索 → 改成调用 `filterStockPrepProjectQueryRows(...).length`,计数与列表不再是两条代码路径;URL 里 `source=` 在字段缺席时仍照样过滤、把人锁进没有出口的 `filter_empty` → 过滤用 `effectiveSource`(不可用时夹回 `all`),URL 保留原值并加提示说明未生效;新 spec 漏登 `run-required-web-tests.sh` → 本机先复现真红(CI 覆盖枚举点名该文件),补 token 后复跑 4/4 绿;空态下左栏自相矛盾(零项目租户被告知"后端未提供来源"、目录读失败时本机记忆的行被空态顶掉)→ 筛选区与清单改成与首页同形。已合入(`2794494f0`) |
| 2026-09-08 17:03 | r19 上 222 验证全过 | main `2794494f0`,包 12.55 MB;就地升级 8 步全过(pg_dump 备份、443 文件哈希 OK、迁移 0、health 200);经 nginx 前端 smoke PASS(`index-COZgEh9X.js`);U2/P0/P1/P2 代码标记全 True(P2=项目查询面板);§5-5 租户声明硬门 flag 有效;操作员链(角色 `stock-prep-operator`):目录 200、确认队列 200,看板/拉取/导出/对账 404(外接源尚未在界面建立,预期),建账本/切源 403 正确拒绝;P0 接口验收 PASS |
| 2026-09-08 17:15–19:36 | 收尾小修波(PR #5565)对抗核验 + 修复 + 合入(`db13f7b6a`) | 全部来自 rail / P2-1 反驳留下的 minors:导航项改名「数据来源与体检」(只改 label 不改 key)、rail 键盘漫游 + 「深度工具」disclosure 移出 tablist、无项目号时两入口高亮收敛、`FILTER_LABELS`/`STATUS_LABELS` 与 pullBanner 三句抽成共享纯函数、`useMobileViewport` 监听 matchMedia change。三路核验 8 条 blocker(去重 4 类):「深度工具」按钮上按方向键被 rail 劫持并切走面板、自动激活路过「今天要处理」会清掉已打开项目号且回不去、tablist 拆分后丢 aria-label、组标题 `role=presentation` 只摘元素不摘文字而守卫只枚举带 role 的元素看不见 → 修法:handler 加事件源门、改成手动激活(方向键只移焦点,回车/空格激活)、aria-label 搬回 tablist、标题 aria-hidden + 文本断言守卫;12 条 minor 顺带修;两条不修有据(组标题降级为"不作为可访问内容暴露";窄屏 aria-orientation 先于本波)。真浏览器 lane 从 28 增到 29 条。 |
| 2026-09-08 19:42 | r20 上 222 验证全过 | main `db13f7b6a`,包 12.55 MB;就地升级 8 步全过(pg_dump 备份、443 文件哈希 OK、迁移 0、health 200);经 nginx 前端 smoke PASS(`index-CpDeNWOD.js`);U2/P0/P1/P2 标记全 True;本波标记 `data-rail-key`、`sp-rail__tablist`、新导航项名(按 UTF-8 读 dist:新名 1 文件、旧名 0)全 True;租户声明硬门 flag 有效;操作员链与 r19 一致;P0 接口验收 PASS。交付目录 r20 已生成。 |
| 2026-09-08 20:30–20:50 | 绑定波:owner 已录回 PLM 连接 → 外部系统 / 源绑定 / 服务账号 / 计划任务 | 巡检发现 `data_sources` 里有 owner 11:39 在界面录的 SQL Server 连接(测试 PLM、connected、workspace 空、租户 default)。按交付说明 §3:以绑定者本人(admin,令牌本机签发不落盘)建 `data-source:sql-readonly` 外部系统 201(归属戳服务端自动写入)、连通测试 200、切绑定 200、读回 persisted(带 / 不带 workspace 都命中同一条,W3b);一线操作员试算 200(委派生效)、管理员试算 200;服务账号按 §3.2 重建、两条计划任务重新启用、手动触发 `LastTaskResult=0`。两条未通:①源预检 500 `SOURCE_PREFLIGHT_VALUES_FREE_SELF_CHECK_FAILED`(见 §7 ⑩,已开 PR 记 warn 细节);②试写入 409 `TABLE_ACTION_MANUAL_CONFIRM_REQUIRED`——该项目 211 行全部缺件,按设计整批不写(§6.2),看板 / 导出 404 同因,属客户数据侧。 |
| 2026-09-08 20:45–22:21 | 源预检 500 诊断链:#5569 拒绝日志 → r21 上 222 → 定位根因 | #5569(sonnet 实现 → opus 一路核验:1 blocker——掩码首尾两字符 + 精确长度对长度 3–4 的短值实质等于原值 → opus 修:<5 字符地板、四键必须齐全、值形状白名单、secret 类只发 path/kind)首跑 4 红——http-routes.cjs 被 sealed-export S6-A 溯源清单钉了 sha256,重算 `s6a-package-provenance-pins.json` 后 26/26 绿,合入 `fd7cd2b28`。r21 上 222 全过;重跑源预检 500 但 pm2 warn 给出 `probes[10].columns[27]` / observed-row-value / length 4——列名被一个 4 字符行值"包含"而被拒。读代码:(2) CONTAINMENT 的标识符豁免按行值而非叶子值,与三处注释矛盾。 |
| 2026-09-08 22:23–23:38 | 源预检自检修复(PR #5570)对抗核验 + 修复 + 合入(`97dc5cefa`)→ r22 上 222 | opus 实现:标识符叶子按叶子值豁免((a) 新增)+ 原有按行值豁免((b) 保留),S-19 复现修前 stash 真红(`checks.projectData.matchField` / observed-row-value / length 4)、反向四条;两路 opus 核验:values-free 视角 hold(逐字段推演 IDENTIFIER_LEAF_FIELDS 的来源,唯一来自行内容的 JSON 槽键被 preset 锚定正则限死;变异体"所有 identifier 免检"被 S-12 与反向 1/2 抓住),regression 视角 1 blocker = 489 行注释 "Exempt ONLY when" 仍与代码矛盾 → opus 修(注释改两条理由、secrets 反向补真交叉、e2e 补"植入值真被采样"武装断言、三个变异体如预期红)。核验者点出的设计内残留(改后标识符类在包含检查上实际只剩 dictionarySlot 在守;是否收紧 (b))记 owner 知情项。20/20 绿合入。r22 上 222:升级 / verify(P5 标记)/ smoke / P0 验收全过;**源预检 200、verdict go、blockers 空**,pm2 无新 warn。 |
...
13. **W3a 缺件清单设计的三条既有留白(PR #5500 正文"未做 / 留给 owner"一节)**:
    - **13a(D-A)缺件 CSV 是否加"每次探测一行"模式**——**2026-09-10 拍板:默认不做,理由与重新排期触发条件如下**。原问题(引):"当前 CSV/复制按零件号去重成一行(用 `parentCount` 角标标出该零件挂在几个父件下);要不要额外提供'探测到几次就展开几行'的明细模式,待拍板。" 裁决:
      - **理由一**:expander 在采集点按零件号聚合是**有意设计**(`stock-preparation-bom-expansion.cjs:1701-1707`,注释原话"ONE ROW PER PART NUMBER, not per probe"、"Creating a part in PLM is a per-part job"),不是遗漏——聚合行本身就是"这个零件缺,挂在几个父件下"这条判断需要的形状,一线要处理的是"缺什么"而不是"在哪几处被引用"。
      - **理由二(2026-09-10 二次改写,"不引入新的披露维度"这句绝对话已删,收窄为下面两条)**:逐次明细的数据本就没有完整保留下来——采集器对细节带上限,超限的探测点被"dropped detail"(同文件 `:1707` 注释);要做明细模式得先改采集器,让它保留每一次探测的上下文,而不是在汇总层(`summarizeMissingComponents`)加个开关就能出。改了采集器之后,明细模式还会把导出体量放大到"父件引用次数"量级(同一个缺件零件号在大 BOM 下可能有两位数父件引用)。**与 13b 的落库口径无关**:缺件清单确实不落库(`stock-preparation-table-actions.cjs:26-28` 四条负向守卫钉住"不进 buildRevision 的展开投影、不进 evidenceForDryRun、不进 dry-run token、不进账本/审计行";`apps/web/src/services/integration/stockPreparation/projectSync.ts:328` `missingComponentsOf` 只是客户端解析响应,不是落库)。**但披露量本身会被明细模式放大**:今天 `addMissingComponent`(`stock-preparation-bom-expansion.cjs:1145-1156`)每个零件号只保留**第一次**探测到的 `parentSourceId`/`bomId`/`path`/`depth`("parent / bom / path / depth are the FIRST place this part was wanted, and stay so",`:1145`),之后只累加 `occurrenceCount`/`parentCount`(`missingComponentParents` 是模块内部 Map,`:1094`,本身从不出模块);明细模式要展开的正是"全部父件/全部 path/depth",把这个模块**唯一带值的键**的披露量从"每件一个父件"扩到"每件全部父件"。这仍然是 `missingComponents` 这唯一带值键的披露范围变化,按 `stock-preparation-table-actions.cjs:23`"GATED at the route: operate ∧ a PROVEN tenant"的既有门槛,明细模式一旦要做,同样要重新过一遍这道值面门评估,不能因为"不落库"就默认放行。
      - **重新排期触发条件**:客户明确提出"需要看到每次探测的具体上下文"这一诉求(例如需要按父件定位缺件而非只按零件号去重)时,再单独设计并排期,不作为本次交付的默认路径。
...
8. **分类器的 paths 手打清单必然漂移,守卫要从依赖闭包推导并真跑分类器**:PR #5560 反驳阶段坐实——`stock-prep-browser-ci-wiring.test.mjs` 最初用 `classifier.run.includes(anchor)` 只能证明"已列的没被删",证不出"新长出来的没进清单";改成从 harness+fixtures 的 import 闭包(递归走相对 import,今天 67 个文件)与两支 spec 里的落盘读路径(`join(REPO_ROOT|WEB_ROOT, …)`)反推出真正应该命中的文件集合,再逐个喂给**真跑的**分类器,才补出七条此前漏记的路径。以后凡是"改这些文件会/不会触发某条 CI 检查"这类守卫,判据都应当是依赖闭包而不是维护者手记的清单——手打清单只能证明没删漏,证不出没新漏。

9. **新增 vitest spec 必须同时登记两处,漏登任一处的后果不同但都严重**:一处是 `run-required-web-tests.sh`(必需检查 `test (18.x)/(20.x)` 的活扫入册)——漏登会让这条必需检查悄悄跳过新文件,P2-1 本轮在本机用 CI 覆盖枚举复现了真红,补 token 后复跑绿;另一处是 `integration-guard` 的 guarded-paths token——它与工作流 `on.push.paths` 逐字节钉死,漏登不会立刻红,但会让该守卫失去"这个改动应该触发它"的自证能力,是一个更隐蔽的漂移。以后凡是新增会被某条 required check 覆盖的 spec 文件,合并前要把它涉及的每一处登记点过一遍,而不是只跑一次本地测试看绿。

10. **Bash heredoc 里的反斜杠续行/转义在某些写入路径下会被折叠成真实的 NUL 字节,提交 blob 会从文本静默变成二进制**:这类问题不会在 diff 里显形——文本编辑器看着内容正常,`git show HEAD:<path> | tr -cd '\r' | wc -c` 之类只查 CR 的检查也查不出 NUL。以后往仓库文件里写多行、含反斜杠的内容,优先用不需要转义的写法(单引号 heredoc、或分多次结构化编辑),写完必须额外扫一遍 NUL 字节(而不仅是 CRLF),再确认 `git status`/diff 显示为文本而非二进制变更。

11. **远端默认 shell 是 PowerShell 5.1,`&&` 是语法错;探针读 dist 校验中文标记必须显式 UTF-8**:r20 暂存脚本里 `cmd /c copy … && dir …` 被远端 PS 5.1 当成一条 PS 命令解析直接报错,`.cjs` 探针没复制过去,verify 第一跑 MODULE_NOT_FOUND;同一轮里 `Get-Content -Raw` 不带 `-Encoding UTF8` 读 dist,中文标记「数据来源与体检」判成 False 而 ASCII 标记全 True——用 `[IO.File]::ReadAllText(path, UTF8)` 复核为 1 个文件命中、旧名 0。结论:远端命令用 `;` 串接;读文件找中文标记必须显式 UTF-8,否则"标记缺失"是探针在撒谎。
