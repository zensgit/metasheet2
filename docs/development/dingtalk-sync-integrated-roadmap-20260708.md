...
- Validate cron at save time. (Landed: `routes/admin-directory.ts` POST `/integrations` and PUT `/integrations/:integrationId` now reject an invalid or unschedulable `schedule_cron` with `400 DIRECTORY_SCHEDULE_CRON_INVALID` before it reaches the DB, instead of the DB accepting it and `directory-sync-scheduler.ts`'s `applySchedule` silently `logger.warn`-ing and leaving the job unscheduled. Validates with `SimpleCronExpression` from `packages/core-backend/src/services/SchedulerService.ts` — the SAME parser the directory sync scheduler executes with — rather than any other cron grammar. (An earlier draft reused the multitable automation scheduler's `parseCronExpression`; that parser has OR day-of-month/day-of-week semantics and diverges from the runtime — e.g. `0 0 30 2 1` validates there but never fires here — so validator-runtime identity is the load-bearing property; do NOT swap parsers in follow-ups.) Empty/null `schedule_cron` stays allowed (= no schedule). Per-integration timezone is now supported — see the next bullet, which supersedes the "always UTC" note this bullet originally carried.)
- Add timezone support. (Landed: `directory_integrations.schedule_timezone` — a nullable IANA-zone column added alongside `schedule_cron` by a zzzz migration, NULL for every pre-existing row. `directory-sync-timezone.ts` is the single pure module deciding "what timezone does this integration's cron run in" (`isValidDirectoryScheduleTimezone` at the save boundary, `resolveDirectoryScheduleTimezone` at read/runtime — both default absent/`''`/`'UTC'`/`'Etc/UTC'` to `'UTC'`, byte-identical to pre-§7.8). `admin-directory.ts` rejects an invalid zone at save time with `400 DIRECTORY_SCHEDULE_TIMEZONE_INVALID`, mirroring the cron gate above. The fix that actually makes the zone matter: `SimpleCronExpression` (`SchedulerService.ts`) previously stored its `timezone` constructor argument but never consulted it in `matches()` (it read local `Date` getters unconditionally) — it now branches to an `Intl`-based zoned match (reusing `getZonedParts`/`isValidIanaTimeZone` from the multitable automation trigger's T2-5 module, per that module's own reuse invitation — NOT the multitable automation scheduler's separate OR-semantics cron parser, which stays out of scope per the note above) for a resolved non-default zone, and keeps the untouched local-getter path for every absent/UTC caller. `directory-sync-scheduler.ts` reads `schedule_timezone` alongside `schedule_cron` and passes the resolved zone into `scheduler.schedule()`; because `SchedulerServiceImpl.reschedule()` cannot change a job's `options.timezone` in place, a timezone change on an already-scheduled integration unschedules and re-creates the job rather than silently keeping the stale zone. `updateDirectoryIntegration` distinguishes an ABSENT `scheduleTimezone` key on a PUT (preserves whatever is currently saved) from an explicitly PRESENT one including `''` (overwrites) — there is no admin UI field for this yet, so the existing form always omits the key, and without that distinction the very next unrelated edit through the UI would silently reset a directly-API-configured zone back to UTC. **Known limitations — corrected after adversarial review; two earlier claims here were FALSE:** (1) DST fall-back double-fire is REAL for a DST-observing zone (executed: `30 1 * * *` @ `America/New_York` fires at BOTH `2026-11-01T05:30Z` and `06:30Z`), and the sync lease does **NOT** mitigate it — the two fires are an HOUR APART, while the lease only blocks a *concurrent* run, so it cannot dedupe them. The earlier "the sync lease makes a double-fire low-harm" claim was wrong. `Asia/Shanghai` (the primary target) observes no DST, so this is bounded in practice — but do NOT configure a DST-observing zone until `isZonedFallbackRepeat`-style dedup is ported into the stateful `next()`/`prev()` iteration model. (2) The `'UTC'` default keeps the ORIGINAL local-getter path, so on a non-UTC HOST directory sync still fires at host-local time while reporting UTC — i.e. the root bug is **not** fixed for the default path, only for an explicitly-configured non-UTC zone. Harmless in production today (`node:20-slim`, no `TZ` set ⇒ the container clock IS UTC) and not a regression, but the suite cannot catch it either: `scheduler-service.test.ts` pins `process.env.TZ='UTC'`, which is exactly why every test passes under `TZ=Asia/Shanghai`. No admin UI field for `scheduleTimezone` yet — direct API/DB configuration only. No admin UI field for `scheduleTimezone` yet — direct API/DB configuration only.)
