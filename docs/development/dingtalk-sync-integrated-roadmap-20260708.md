...
- Validate cron at save time. (Landed: `routes/admin-directory.ts` POST `/integrations` and PUT `/integrations/:integrationId` now reject an invalid or unschedulable `schedule_cron` with `400 DIRECTORY_SCHEDULE_CRON_INVALID` before it reaches the DB, instead of the DB accepting it and `directory-sync-scheduler.ts`'s `applySchedule` silently `logger.warn`-ing and leaving the job unscheduled. Reuses the multitable automation scheduler's cron grammar — `parseCronExpression` + `cronHasNoMatchingDay` in `packages/core-backend/src/multitable/automation-scheduler.ts` — rather than adding a new cron-parsing dependency. Empty/null `schedule_cron` stays allowed (= no schedule). Per-integration **timezone is still NOT supported**: the directory sync scheduler hardcodes `timezone: 'UTC'` at `applySchedule`, so a validated `schedule_cron` runs on UTC wall-clock time — see the next bullet.)
