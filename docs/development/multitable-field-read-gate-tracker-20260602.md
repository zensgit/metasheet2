...
The masks in 2a/2b gate a record's OWN fields. A **formula/lookup/rollup** field, however, is itself source-sheet-visible while its *persisted value* can encode a **foreign** field that is DENIED to the requesting actor (a layer-2∧3-readable formula whose materialized value was computed over a cross-base lookup the actor cannot see). The own-field mask passes it through, so the foreign data leaks via the stored value. The fix centralizes one rule — drop, per (sheet, actor), any formula field whose value is tainted by a denied foreign field — into a single read **chokepoint** `maskStoredRecordFieldIds` (subtracts `resolveTaintedFormulaFieldIds(...)` from the existing layer-2∧3 allowed set; **subtract-only — never widens**) plus a symmetric single-display-projection chokepoint `resolveDisplayFieldTaint`. Every stored-`data` read/echo/summary sink derives its allowed set through the chokepoint; the unchanged `filterRecordDataByFieldIds(...)` sinks receive the now-taint-dropped set. A second **always-on structural guard** (`tests/unit/multitable-stored-data-taint-chokepoint.guard.test.ts`, no DB) freezes the sink allowlist (each disposition CHOKEPOINT / SAFE+reason) and forbids ad-hoc re-bolting of the resolver outside the two chokepoints + the two write-path recompute-skips — so a new sink can't drift back to the source-only mask.

| ✅ | Channel (sink) | Sev | PR | Locking test (real-DB canary: denied foreign-derived value absent) |
|---|---|---|---|---|
| ✅🔒 | **GA-T2a** `GET /view` read/JSON sink (materialized formula over denied lookup) | **High** | #2549 | `multitable-lookup-foreign-field-mask-view` |
| ✅🔒 | **GA-T2b** xlsx export sink (formula column) | Med-High | #2549 | `multitable-lookup-foreign-field-mask-export` |
| ✅🔒 | **B1** denied **write** must not corrupt the shared materialized formula value (link/non-link edit) | **High** (shared-state corruption) | #2549 | `multitable-lookup-foreign-field-mask-write` |
| ✅🔒 | **B1-CREATE** denied authenticated create + **public form-submit** must not persist the masked formula value (`recalcNewRecordFormulas`) | **High** | #2549 | `multitable-lookup-foreign-field-mask-create` |
| ✅🔒 | **M1** view-aggregate omits the tainted formula-column aggregate | Med | #2549 | `multitable-lookup-foreign-field-mask-aggregate` |
| ✅🔒 | **M1-READ** sibling raw-read/emit sinks — single-record `GET /records/:id` + cursor list `GET /records` (exhaustive sweep) | High | #2549 | `multitable-lookup-foreign-field-mask-sibling-reads` |
| ✅🔒 | **C1** form-submit EDIT echo (`POST /views/:id/submit` with `recordId`) + **C2** `/records-summary` attacker-chosen `displayFieldId` (display-projection chokepoint) | Med-High | #2549 | `multitable-lookup-foreign-field-mask-convergence` |

All seven are wired into `.github/workflows/plugin-tests.yml` (multitable real-DB integration step) and `describeIfDatabase`-gated. The taint-chokepoint structural guard + the §3 egress-coverage change gate both run in the always-on (no-DB) core-backend test job.
