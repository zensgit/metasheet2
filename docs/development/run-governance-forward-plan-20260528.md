...
| **收敛引擎能力半**（A6：persistent jobs / suspend/resume / branch / DAG / approval-as-job） | automation+approval 统一到 WorkflowJob 引擎 | **🟡 A6-1/A6-2/A6-3 已落地**（persistent jobs、suspend/resume、exclusive branch + parallel `join_all`）；A6-4a pure compiler #2568 + A6-4b read-only route #2577 + A6-4c read-only UI #2604 已落地；A6-3-3 scope-gate 已记录，runtime/frontend 未启动；join-any / live BPMN runtime / W7 follow-ups 未启动 | 否 —— **剩余 A6 rungs 仍需具名 opt-in；A6-4 live runtime / A6-5 follow-ups 受需求闸** |

> **grounded 2026-06-15**：`/retry` 路由实现 A5 whole-execution retry；A6-1 persist-job runtime 已生产可达（#2130/#2191/#2193）；A6-2 suspend/resume 已生产可达（#2237/#2245，admin-gated v1，operator UAT #2257）；A6-3 has `condition_branch` and parallel `join_all` runtime/editor/runs-readability (#2321/#2339/#2348/#2496/#2500/#2501)。C1 契约现被 A2 读边界、A6-1 job 持久化、A6-2 suspended job/resume、A6-3 branch/parallel job lineage 使用。A6-4/W8 BPMN compile-preview has a docs-only scope gate, an A6-4a pure compiler landed in #2568, an A6-4b read-only route landed in #2577, and an A6-4c read-only UI landed in #2604. A6-3-3 has a docs-only branch-local wait scope gate; **仍无** A6-3-3 runtime/frontend、join-any、live BPMN runtime、W7 result backwrite runtime, or public webhook/token emitter.
...
- **剩余依赖序（未建，各需具名 opt-in）**：**A6-3-3a backend branch-local wait runtime**（让 `wait_for_callback` live inside selected `condition_branch` paths; scope-gate 已记录）→ **A6-3-3b frontend/readability** → **A6-3-5 join-any/cancellation** → **W7 result backwrite / A6-5 follow-ups**（live BPMN runtime / BPMN create-from-preview 各自单独 scope）。触发须点名具体用例（"分支内需要等待外部回调" / "需要先完成任一并行分支即继续" / "审批结果需要写回记录字段"），"继续自动化"不够。
...
- **锁姿态**：A6-3 已落地的 exclusive branch + join-all slices closed；A6-4a pure compiler + A6-4b route + A6-4c read-only UI landed; A6-3-3 runtime/frontend、join-any、live BPMN runtime、W7 result backwrite、public webhook/token emitter 仍 frozen / demand-gated。
