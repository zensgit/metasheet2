...
> **2026-06-15 update**：A6-1 已 end-to-end 完成（runtime #2130 + enable-writer #2191 + admin UI toggle #2193）；A6-2 suspend/resume 已 end-to-end 完成（design #2236 + runtime #2237 + frontend #2245 + UAT #2257）。A6-3 branch/parallel DAG 已分段落地：exclusive `condition_branch` #2321/#2339/#2348，parallel `join_all` #2496/#2500/#2501。A6-4 BPMN compile/preview scope-gate 已记录在 `multitable-automation-a6-4-bpmn-compile-preview-scope-gate-20260612.md`，A6-4a pure compiler 已落地 #2568 `0f214a222`，A6-4b read-only route 已落地 #2577 `b5646fa71`，A6-4c Workflow Designer read-only UI 已落地 #2604（无 persistence / deploy/start / live BPMN runtime）。A6-3-3 branch-local wait scope-gate 已记录在 `multitable-automation-a6-3-3-branch-local-wait-scope-gate-20260615.md`；其 runtime/frontend、A6-3-5 join-any、W7 follow-ups 仍 demand-gated。
...
| **收敛引擎能力半**（A6：persistent jobs / suspend/resume / branch / DAG / approval-as-job） | automation+approval 统一到 WorkflowJob 引擎 | **🟡 A6-1/A6-2/A6-3 已落地**（persistent jobs、suspend/resume、exclusive branch + parallel `join_all`）；A6-4a pure compiler #2568 + A6-4b read-only route #2577 + A6-4c read-only UI #2604 已落地；A6-3-3 branch-local wait runtime+frontend 已落地（#2626/#2702）；join-any / nested branches / live BPMN runtime / W7 follow-ups 未启动 | 否 —— **剩余 A6 rungs 仍需具名 opt-in；A6-4 live runtime / A6-5 follow-ups 受需求闸** |

> **grounded 2026-06-15**：`/retry` 路由实现 A5 whole-execution retry；A6-1 persist-job runtime 已生产可达（#2130/#2191/#2193）；A6-2 suspend/resume 已生产可达（#2237/#2245，admin-gated v1，operator UAT #2257）；A6-3 has `condition_branch` and parallel `join_all` runtime/editor/runs-readability (#2321/#2339/#2348/#2496/#2500/#2501)。C1 契约现被 A2 读边界、A6-1 job 持久化、A6-2 suspended job/resume、A6-3 branch/parallel job lineage 使用。A6-4/W8 BPMN compile-preview has a docs-only scope gate, an A6-4a pure compiler landed in #2568, an A6-4b read-only route landed in #2577, and an A6-4c read-only UI landed in #2604. A6-3-3 branch-local wait runtime+frontend 已落地（#2626 backend + #2702 frontend）；**仍无** join-any、nested branches、live BPMN runtime、W7 result backwrite runtime, or public webhook/token emitter.
...
- **A6-4 scope-gate + A6-4a compiler + A6-4b route + A6-4c UI**：`multitable-automation-a6-4-bpmn-compile-preview-scope-gate-20260612.md` pins BPMN as side-effect-free compile-preview + deterministic gap report only; no deploy/start/live BPMN runtime. #2568 landed the pure compiler, #2577 landed the read-only route, and #2604 landed the read-only Workflow Designer compile-preview UI. Live BPMN runtime stays out of scope.
- **剩余依赖序（各需具名 opt-in）**：A6-3-3a backend + A6-3-3b frontend branch-local wait 已落地（#2626/#2702）；剩 **A6-3-5 join-any/cancellation** → **nested branches** → **W7 result backwrite / A6-5 follow-ups**（live BPMN runtime / BPMN create-from-preview 各自单独 scope）。触发须点名具体用例（"分支内需要等待外部回调" / "需要先完成任一并行分支即继续" / "审批结果需要写回记录字段"），"继续自动化"不够。
...
- **锁姿态**：A6-3 已落地的 exclusive branch + join-all + A6-3-3 branch-local wait slices closed（#2626/#2702）；A6-4a pure compiler + A6-4b route + A6-4c read-only UI landed; join-any、nested branches、live BPMN runtime、W7 result backwrite、public webhook/token emitter 仍 frozen / demand-gated。
...
**治理半已完成、A5 whole-execution retry v1 已完成、A6-1/A6-2 已 end-to-end 落地，A6-3 已覆盖 exclusive branch + parallel join-all，A6-4a pure compiler + A6-4b read-only route + A6-4c read-only UI 已落地。** 近期"开发安排"实质是：**本 session 待命做点名 review；A6-3-3 branch-local wait 已有具名用例与 scope-gate，但 runtime/frontend 仍需按 §7 独立 build；join-any / result backwrite / live BPMN runtime 仍按具名需求推进。** 触发出现 → 走 §7 流程。无触发 → 不动，这就是正确的安排。
