...
| **收敛引擎能力半**（A6：persistent jobs / suspend/resume / branch / DAG / approval-as-job） | automation+approval 统一到 WorkflowJob 引擎 | **🟡 A6-1/A6-2/A6-3 已落地**（persistent jobs、suspend/resume、exclusive branch + parallel `join_all`）；A6-4a pure compiler #2568 + A6-4b read-only route #2577 已落地；A6-4c UI、A6-3-3 / join-any / W7 follow-ups 未启动 | 否 —— **剩余 A6 rungs 仍需具名 opt-in；A6-4/A6-5 follow-ups 受需求闸** |
...
- **A6-4 scope-gate + A6-4a compiler + A6-4b route**：`multitable-automation-a6-4-bpmn-compile-preview-scope-gate-20260612.md` pins BPMN as side-effect-free compile-preview + deterministic gap report only; no deploy/start/live BPMN runtime. `multitable-automation-a6-4a-bpmn-compile-preview-implementation-plan-20260612.md` narrowed the first build slice to a pure compiler module + unit tests; #2568 landed that compiler, and #2577 landed the read-only route. The Workflow Designer UI remains deferred to A6-4c.
- **剩余依赖序（未建，各需具名 opt-in）**：**A6-4c Workflow Designer compile-preview UI** → **A6-3-3 branch-local wait/nesting**（让 `wait_for_callback`/nested branches live inside graph branches）→ **A6-3-5 join-any/cancellation** → **W7 result backwrite / A6-5 follow-ups**。触发须点名具体用例（"BPMN 只做 compile-preview UI" / "分支内需要等待外部回调" / "需要先完成任一并行分支即继续" / "审批结果需要写回记录字段"），"继续自动化"不够。
...
- **锁姿态**：A6-3 已落地的 exclusive branch + join-all slices closed；A6-4a pure compiler + A6-4b route landed; A6-4c UI、A6-3-3、join-any、W7 result backwrite、public webhook/token emitter 仍 frozen / demand-gated。
