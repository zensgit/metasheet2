...
are re-examined **in this slice**, per Lock-3 §3 — asserted by gate G-17, not reviewed.
...
|---|---|---|---|
| G-1a | The derivation is DERIVED, not three copies | mutating ONLY `resolveFieldAccessAtNodes` reds a named test for **each** of the three consumers — snapshot echo, attachment byte gate, write refusal. The mutation MUST be one that changes the answer for a **single-node** set, because the write consumer is always given exactly `[nodeKey]`: flip the absent-key default from `editable`, or make the function ignore `access === 'hidden'`. **A precedence flip is inadmissible here** — precedence is unobservable on a one-element set, so it would leave the write test green against a correct implementation and fail the gate for the wrong reason (the ineffective-mutation trap) | if fewer than three red, a consumer holds its own copy and the gate FAILS. Plus: a fixture matrix asserts the derived hidden set equals `collectHiddenFieldIds`' output by exact set equality |
| G-1b | Precedence, scoped to the multi-node read path only | with two active nodes disagreeing (`hidden` vs `readonly`, and `readonly` vs `editable`), the instance-scoped consumers take the most restrictive; flipping `hidden ≻ readonly` reds its OWN named test on that path | the shipped parallel-union case (`approval-form-redaction.test.ts:66-75`) stays green throughout; a single-active-node fixture is asserted UNAFFECTED by the same flip — which is what makes G-1a's exclusion of this mutation load-bearing rather than a caveat |
...
they are not re-proposed):
  ...
  OD-L7-6  What a write mutates — (a)[R] UPDATE `form_snapshot` in place inside Lock-3 §3's single
           ...
           re-examined in the SAME slice per Lock-3 §3, gated by G-17 · (b) a delta table composed at read time [rejected
