...
- A6-2 suspend/resume runtime (backend, admin-gated v1; `wait_for_callback` + admin resume route,
  webhook/external resume) — LANDED #2237 c363a78db (2026-06-03); A6-2b frontend deferred.

Deferred (capability half — A6, frozen/demand-gated):
- A6-2b suspend/resume FRONTEND (backend LANDED #2237 — see above); delay/timer resume later
  (forces durable scheduler/worker/leader)
...
- [x] A6-2 suspend/resume runtime (backend, admin-gated v1; webhook/external resume) — LANDED #2237 c363a78db (2026-06-03). A6-2b frontend + delay/timer resume deferred (still demand-gated).
