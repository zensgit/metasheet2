...
| Synthetic checkpoint pool cleanup | A later main push at `4189aa096a8a8054315bb97db79dfbac1823954e` failed its first Node 20 attempt on immediate post-test `pg_stat_activity` residue (1 instead of 0); the second attempt passed. [#6059](https://github.com/zensgit/metasheet2/pull/6059) is a one-file test-harness drain fix, exact-head green but still Draft/HOLD. Recovery product code is unchanged. | Merge authorization for #6059, then exact merged-main acceptance with zero persistent owned-DB residue. A successful retry alone is not closure. |
