...
竞品("用第三方模型,数据是安全的")今天实际怎么做:云端租户隔离(Azure OpenAI / Bedrock 模式)、或合同/零留存承诺、或数据脱敏、或传输加密。这些方式都做不到"数据物理不出客户网络"——但有一个例外必须诚实承认,不能回避:**科大讯飞星火、百度文心一言、华为盘古**这类中国闭源模型厂商本身也在卖**私有化一体机**(客户机房整机部署,推理服务与模型权重一起交付到客户网络内),物理隔离这件事本身它们也做得到;任何竞品只要愿意打包同类一体机方案,同样能满足"数据不出客户网络"。旧稿这里曾写"**没有一种能满足**"——这类"唯一/没有一种"式的绝对表述会被客户的技术尽调直接戳破,本稿不采用,改为下面有范围限定、可核验的差异化陈述。

我们能够诚实说死、且有明确范围限定的一句话是:**GPT / Claude 这两家没有私有化一体机产品,只有云端 API——选它们做后端就意味着数据出网,这是产品形态决定的物理事实,不是合同承诺能改变的。** 这句话只对这两家闭源模型成立,不能推广成"所有第三方模型都做不到内网部署"。

真正站得住的差异化不是"只有我们能做到物理隔离"(私有化一体机厂商同样做得到,这句话是假的),而是三件具体、可核验、不依赖"相信我们"的事:

- **成本**——开放权重模型 + 自己采购的 GPU,没有闭源厂商私有化一体机项目常见的许可证费、定制集成费、按年续费的模型访问费。
- **可审计性**——开放权重本身可以被独立第三方审查(不是一个黑盒一体机,内部是什么谁也不知道);加上我们自己这层 AI 边界代码对签约客户开放只读审计(§6.4),客户能自己核实"业务数据只路由到本地模型"这句话,而不是被要求相信供应商一体机"没有后门、没有隐藏遥测"这类无法验证的承诺。
- **无厂商锁定**——开放权重模型可以替换、可以迁移到另一家云/另一台硬件,不依赖单一厂商私有化产品线的持续维护节奏与定价策略;闭源一体机一旦部署,后续模型升级、定价、支持全部由供应商单方面决定。

### 4.1 中国生成式 AI 合规现状(备案)——客户法务会问,这里先给一段诚实的话

本稿默认路径描述的场景——客户内网自托管、供企业内部业务人员使用、不对外提供生成式 AI 服务、不用于生成对外发布的内容——按我们目前的理解,大概率落在《生成式人工智能服务管理暂行办法》的备案要求之外:该办法规制的核心是"向境内公众提供生成式人工智能服务",企业内部自用、不对外提供服务的部署不在其规制的核心场景里。**但这是一句工程判断,不是法律意见书**——我们不是律师,不能替客户的法务部门下最终结论。诚实的立场是:今天的部署形态(内网自托管、内部使用、不对外)按我们的理解不触发备案;但如果未来产品形态变化——例如把摘要/建议能力包装成客户对其下游客户开放的对外服务、或者输出内容被用于对外发布——需要重新评估是否落入备案范围,不能假设"我们做的是 to B 内部工具,所以永远不用管这件事"。客户法务问起时,给出这段判断 + 建议客户自行核实,而不是含糊带过。

### 4.2 谁运维客户现场的那台 GPU 机器——"护城河"的真实运维成本

内网自托管不是把机器和模型交付给客户就结束了,它是一台需要持续运维的生产设备:模型/推理服务(vLLM 一类)的安全补丁、模型版本升级(何时评估换用更新更好的开放权重模型)、服务健康监控与告警、故障发生时的响应(客户凌晨报警,谁接、多久响应)。这是"内网自托管是我们的护城河"这句话背后一项没有定价的持续成本,本稿必须承认它的存在,不能只讲差异化不讲代价:默认由我们远程运维(不要求常驻客户现场,除非客户网络策略或合同另有要求),这是一项持续的服务成本,应当计入交付定价的一部分,而不是当作硬件采购之外的免费附赠。具体的人力配置、SLA、定价模型留给商务/交付侧裁决,但战略层面必须明确"谁负责"这一点,不能让这项工作落在没人认领的地方。

### 4.3 AI 建议出错时的责任边界——采购语境下人工确认门不只是技术安全阀

§1.2 的人工确认门(copilot not captain)在技术上阻止了 AI 直接造成损失——权威写入只能来自人工确认之后的确定性路径。但采购是一个真实花钱的业务语境,还有一层技术之外的问题:一个人确认了一条错误的 AI 建议,导致采购了错误的物料或数量,责任算在谁身上?技术事实清楚地指向答案——AI 只产出建议,人工确认后才生效并写入正式主表,所以采购决策的责任在做出确认的人和确认所代表的公司,不在 AI 建议本身。这一点不能只是内部认知,应当在合同条款/客户沟通材料里显式写清楚,而不是含糊带过、指望日后不会被追问:人工确认门既是一道技术安全阀,也是一条责任归属的分界线——这句话现在不写清楚,客户出问题时的第一反应会是"是你们的 AI 让我们买错的",而不是"是我们自己确认放行的"。具体条款措辞留给合同侧裁决,战略层的要求是:这条责任边界必须被显式陈述,不能靠"我们有人工确认门"这句技术事实自动带过法律/商务层面的责任归属。
