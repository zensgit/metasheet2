...
- `apps/web/src/views/AttendanceView.vue`:
  ...
  - **Honesty (design §5.1):** when `attendanceSettings` is `null` (not loaded / failed / forbidden) the card shows a single neutral status line ("Loading…" or "Unavailable"), **never** a confident unrestricted policy. Only a successfully-loaded settings object (including an empty `{}` workspace default) renders the value lines. `loadSettings()` 403/catch now clear `attendanceSettings` to `null` so a failed reload never keeps showing a stale policy as current.
  - **Liveness (F2):** `saveSettings()` success now sets `attendanceSettings.value = (data.data || payload)` (then maps the form), so saving via the only edit path (Open Settings) refreshes the punch card immediately — no stale policy until an admin reload.
...
| F2 | Saving settings refreshes the punch card immediately (no stale policy). | "refreshes the punch card after saving workspace settings without a reload" (old 100 m/1-range/2-min → save → 300 m/2-ranges/7-min; old value gone). |
