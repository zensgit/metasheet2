...
The three actions share one operator mental model — *preview, read the result, then commit* — and one substrate: `attendance:admin`, `getOrgId`, the same `apiFetch` / `apiGet` client, the same active-membership target population (`user_orgs uo JOIN users u WHERE uo.org_id=$org AND uo.is_active=true AND u.is_active=true`), and the same auditable summary shape (a `{ code → count }` reasons map). Folding them into one section keeps that model in one place and lets the cards reuse one confirm/preview/summary-table scaffold rather than three near-duplicates.
...
- Success → `{ runId, periodKey, asOf, dryRun, granted, skipped, grantedMinutes, lotsCreated, alreadyGranted, skipReasons }`, where `skipReasons` is an **object/map** (`reasonCode → count`), not an array — render as a code → count table. Known reason codes are exactly those emitted by `computeAnnualLeaveAccrualForUser` on `main`: `NOT_YET_HIRED`, `HIRED_AFTER_PERIOD`, `MISSING_HIRE_DATE`, `MISSING_SERVICE_START_DATE`, `NOT_ELIGIBLE_UNDER_ONE_YEAR`, `NO_MATCHING_TIER`, `PRORATION_BELOW_ONE_DAY`.
...
- **Expiry backfill — idempotent by nature.** Each lot's `expires_at` is set once `WHERE expires_at IS NULL`; a concurrent/prior set is counted under `ALREADY_SET` in `reasons`. Re-running is always safe; the card frames `ALREADY_SET` as "already stamped," not skipped-in-error.
