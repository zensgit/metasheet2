...
| U11-b | Expired/terminal approval cards are inert. Inertness itself is enforced by `ApprovalProductService.dispatchAction`'s in-transaction row lock + `card_state='sent'` gate — **not** by the retention sweep. What produces the `expired` state is `DINGTALK_DELIVERY_RETENTION_DAYS` (the approval-card/person delivery retention family, `dingtalk-card-person-delivery-retention.ts`, #4142) — a **separate** control from `DINGTALK_GROUP_DELIVERY_RETENTION_DAYS` (that one sweeps the *group webhook* ledger only; see DT-CLOSE-02B). `DINGTALK_DELIVERY_RETENTION_DAYS` is **disabled by default** — unset means cards never auto-expire via this path (they remain `sent` indefinitely, though still gated by the same dispatch-time lock) | Clicking an expired/terminal card: no engine write, stale terminal-state message | pass/fail (n/a if not exercised) | ⬜ |
