...
| U11-b | Expired/terminal approval cards are inert (card expiry is a card-level state, **not** governed by `DINGTALK_GROUP_DELIVERY_RETENTION_DAYS` — that sweeps the *group webhook* ledger, not approval cards; see DT-CLOSE-02B). NOTE: there is currently **no** retention sweep for `dingtalk_approval_card_deliveries` at all (gap — see ledger) | Clicking an expired/terminal card: no engine write, stale terminal-state message | pass/fail (n/a if not exercised) | ⬜ |
