...
| 3 | **临时班次** | 🟡 schema + replace runtime + admin UI + smoke harness 已落，pending staging smoke | backend runtime + admin UI + 反向测试 + staging smoke；不伪装成完整调度/换班系统 |
...
| 临时班次 | SHOULD | 🟡 | design-lock `attendance-temporary-shift-design-lock-20260609.md` 已锁；**schema + replace runtime + admin UI + smoke harness 已落 main**：T0 temp metadata schema #2437；replace-only backend runtime #2439（draft create only、direct immediate-active 拒绝、publish revalidate、effective-calendar/planned-minutes overlay 不双算、默认单班 temp day 暴露 `effective.slots` metadata）；T5 admin UI #2441（`5edaf08b`，Assignments 面板临时替班草稿卡，替换目标仅已发布常规分配，payload 精确锁定，越界日期前端拒绝且不 POST）；T6 smoke harness 本 PR（`staging-attendance-temporary-shift-t6-smoke.mjs` + runbook）。rotation replacement / add mode / immediate-active interim 仍稳定拒绝；**仍 pending staging smoke，未翻 ✅** |
