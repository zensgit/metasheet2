...
| 自动对班 | SHOULD | 🟡 | **design-lock 已落待实现**：`attendance-auto-shift-matching-preview-design-lock-20260609.md`。owner 口径：feature-flag 默认关；A0 只读 preview/建议态，不写 `attendance_shift_assignments`；A1 才允许 admin 选择后应用且复用排班写入 guard；A2 自动写入另起 design-lock。只匹配 scheduled_shift 组内“未排班但有打卡”的 user/day，不覆盖人工/导入/固定/轮班排班 |
| 一天多班次 | SHOULD | ⬜ | 0 |
| 排班发布/草稿 | SHOULD | ⬜ | 0 |
| 临时班次 | SHOULD | ⬜ | 0 |
