...
| 自动对班 | SHOULD | 🟡 | **A0/A1 built, pending staging smoke before ✅**：design-lock `attendance-auto-shift-matching-preview-design-lock-20260609.md` → A0 backend preview #2403 + A0 admin review UI #2405 → A1 admin selected apply（本 PR：`POST /auto-shift-matching/apply` + UI 勾选应用）。owner 口径不变：feature-flag 默认关；A0 只读 preview/建议态，不写 `attendance_shift_assignments`；A1 仅 admin 选择后应用且复用排班写入锁、排班修改窗、排班合规引擎与 provenance；A2 自动写入另起 design-lock。只匹配 scheduled_shift 组内“未排班但有打卡”的 user/day，不覆盖人工/导入/固定/轮班排班 |
| 一天多班次 | SHOULD | ⬜ | 0 |
| 排班发布/草稿 | SHOULD | ⬜ | 0 |
| 临时班次 | SHOULD | ⬜ | 0 |
...
⑥ **自动对班**（🟡 design-lock 2026-06-09；灰度门，feature-flag 默认关 → A0 只读 preview → A1 admin apply → A2 自动写另起锁）
