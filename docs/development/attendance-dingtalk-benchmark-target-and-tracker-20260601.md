...
| 未排班提醒（复用 attendance scheduler 基座） | MUST | 🟡 | **引擎 code-green，未 ✅**（design-lock `attendance-unscheduled-reminder-design-lock-20260604.md`）：⑤ 作为 `AttendanceScheduler` **第二个 job**（非新调度器）落地——`UnscheduledReminderService` set-based scan（**镜像 + 真值 parity-locked 对 `isUserScheduledForDate`**，fixed/free 不误报由 applicability guard 继承）→ `attendance_unscheduled_reminder_dispatch` claim（`ON CONFLICT DO NOTHING` = at-most-once 去重）→ `AttendanceNotifier` seam（默认 0 渠道 = 不外发）。默认 OFF（`ATTENDANCE_UNSCHEDULED_REMINDER_ENABLED`）。real-DB integration（a–f 含 parity）入 CI attendance step + unit；本地 90/90 真库通过。**v1 无外发渠道/无消费者（C5）· 负责人 fan-out deferred（v1=本人）· ⑤ 自己的 staging smoke deferred → 未 ✅**。所复用的 ④ C4 scheduler 基座**已 staging-proven**（#2274 + C4 staging smoke PASS 2026-06-04，见行 57/58 与 §3 closeout）。处理策略已移入 ③ |
