...
| 3 | **临时班次** | 🟡 design-lock 已落（`attendance-temporary-shift-design-lock-20260609.md`） | backend runtime + admin UI + 反向测试 + staging smoke；不伪装成完整调度/换班系统 |
...
| 临时班次 | SHOULD | 🟡 | design-lock `attendance-temporary-shift-design-lock-20260609.md` 已锁：v1 = replace-only one-off assignment overlay，引用现有 shift，不为每天新建 shift catalog row；rotation replacement / add mode / immediate-active interim 均不进 v1；完成口径仍需 backend runtime + admin UI + 反向测试 + staging smoke 后才 ✅ |
