...
| 外勤审批 | MUST | 🟡 | **S3 design-lock #2304（`attendance-outdoor-approval-s3-design-lock-20260605.md`）→ backend runtime code-green**（PR S3 runtime）：新增 `outdoor_punch` request type（migration `zzzz20260605120000` + `REQUEST_TYPES`/`types.ts`/OpenAPI source enum）；PUT settings 暴露 `punchPolicy.outdoor.{requireApproval,requireNote,approvalFlowId}`（`requirePhoto` 仍 latent）；`requireApproval=true` + 围栏外/`meta.outdoor` → pending `outdoor_punch` request（**不写 `attendance_events`/`attendance_records`** → 不计入 summary）；final approve 写 **exactly one** `source='outdoor_approval'` 真打卡 event + record（无 generic adjustment）；reject 不写；dup key 含 `eventType`（同日 check_in/check_out 并存，同 eventType 第二条 409）；flow 缺失 422、note 缺失 422。**默认关闭无回归**（围栏外仍 `LOCATION_RESTRICTED`，`meta.outdoor` 被忽略）。real-DB route-level integration **7/7** + 全 attendance 套件 **97/97 无回归**（本地真库）。**仍 🟡 未 ✅**：S3-2 frontend config card + S3-3 staging smoke 才可 ✅（同 ④/⑤ staging gate）。OpenAPI dist/SDK 重生成 deferred（无 parity gate 强制）。**不含 S2 内外勤合并 / C5 外发 / 移动端外勤体验** |
