...
| 外勤审批 | MUST | 🟡 | **S3 design-lock #2304（`attendance-outdoor-approval-s3-design-lock-20260605.md`）→ backend runtime code-green**（PR S3 runtime）：新增 `outdoor_punch` request type（migration `zzzz20260605120000` + `REQUEST_TYPES`/`types.ts`）；**仅 `/punch` 路由创建，generic `/requests` create/update 显式 422 `OUTDOOR_PUNCH_VIA_PUNCH_ONLY`**（防绕过 geoFence/flow/note 策略；故 OpenAPI `/requests` input enum **不含** outdoor_punch，无 dist 变更）；PUT settings 暴露 `punchPolicy.outdoor.{requireApproval,requireNote,approvalFlowId}`（`requirePhoto` 仍 latent）；`requireApproval=true` + 围栏外/`meta.outdoor` → pending `outdoor_punch` request（**不写 `attendance_events`/`attendance_records`** → 不计入 summary）；final approve 写 **exactly one** `source='outdoor_approval'` 真打卡 event + record（无 generic adjustment；metadata 缺失则 422 不伪造）；reject 不写；dup key 含 `eventType`（同日 check_in/check_out 并存，同 eventType 第二条 409）；空 `approvalFlowId` 要求**唯一** active outdoor flow（0 或 ≥2 → 422）；note 缺失 422。**默认关闭无回归**（围栏外仍 `LOCATION_RESTRICTED`，`meta.outdoor` 被忽略）。real-DB route-level integration + 全 attendance 套件无回归（本地真库）。**仍 🟡 未 ✅**：S3-2 frontend config card + S3-3 staging smoke 才可 ✅（同 ④/⑤ staging gate）。dist-sdk SDK 类型未含 outdoor（不在该 gate；deferred）。**不含 S2 内外勤合并 / C5 外发 / 移动端外勤体验** |
...
② **打卡策略组**（design-lock #2203）：S0 基座 ✅#2204 · S1 未排班打卡 ✅#2209 · **子序修正 → S1→S3→S2**（S2 内外勤合并依赖 S3 先建"外勤打卡"事实类型）· **S3 外勤 2026-06-05 恢复为下一主线并补 delta design-lock**（仍仅设计，implementation 🔒）· S2 继续等 S3
