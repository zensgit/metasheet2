...
| 3 | **临时班次** | 🟡 schema + replace runtime 已落，pending admin UI + staging smoke | backend runtime + admin UI + 反向测试 + staging smoke；不伪装成完整调度/换班系统 |
...
| 临时班次 | SHOULD | 🟡 | design-lock `attendance-temporary-shift-design-lock-20260609.md` 已锁；**schema + replace runtime 已落 main**：T0 temp metadata schema #2437；replace-only backend runtime #2439（draft create only、direct immediate-active 拒绝、publish revalidate、effective-calendar/planned-minutes overlay 不双算、默认单班 temp day 暴露 `effective.slots` metadata）。rotation replacement / add mode / immediate-active interim 仍稳定拒绝；**仍 pending admin UI + staging smoke，未翻 ✅** |
