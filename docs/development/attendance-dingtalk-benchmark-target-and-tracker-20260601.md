...
| 外勤审批 | MUST | 🟡 | **S3 design-lock delta 已起草（2026-06-05，见 `attendance-outdoor-approval-s3-design-lock-20260605.md`）**：复用 `punchPolicy.outdoor` + `attendance_approval_flows`，锁定 `outdoor_punch` request type、pending 不计入考勤、final approve 才写有效 punch event/record、reject 不写、同日 check_in/check_out 不能互相去重。实现仍 🔒：DDL/types/OpenAPI + backend runtime + frontend config + real-DB/web tests + staging smoke 后才可 ✅ |
