...
- **C5 外发通知渠道 / 负责人 fan-out**：仍是通知可靠性增强，另起目标。
...
| 加班三段引擎 | SHOULD | ✅ | design-lock `attendance-overtime-segmentation-engine-design-lock-20260609.md` 已锁；O1 pure helper + O2 request metadata snapshot + O3 records/summary loaders + O4 report fields/sync + O5 comp-time consume + O6 staging smoke 已闭环：overtime request create/update 与 final approval 在 `overtimeSegmentation.enabled=true` 时写/刷新 versioned segmentation snapshot，records/summary/report fields/report sync/comp-time grant 从 snapshot 聚合三段 bucket，并稳定拒绝 cross-midnight v1 窗口；历史无 snapshot 记录仍走 documented fallback。staging smoke PASS 2026-06-10（deploy `a55576fa66683719a643d4e7c480da068a214e20`，stamp `overtime-o6-mq89b8gp`，log `/tmp/staging-overtime-o6-smoke-20260610T160158Z.log`，47/47，residue=0）：effective-calendar parity workday/restday/holiday；request metadata versioned snapshot；records/meta/report_values 三段 bucket；summary total + workday/restday/holiday + comp-time buckets；comp-time lots/events consume `compTimeGrantMinutes` as 45/75/105 |
| C5 外发通知 / 负责人 fan-out | 通知增强 | 🟡 | **§0.3 新目标**：design-lock `attendance-c5-notification-delivery-design-lock-20260611.md` 锁定 outbox、delivery status/retry、未排班 + `comp_time` 到期两 source、本人 + owner/sub_owner fan-out、DingTalk work-notification channel gating、staging smoke。未改变已完成能力口径，后续 C5-0…C5-5 各自 opt-in；fake-channel smoke 不翻 ✅ |
| 调度 / 换班 / 小组织 | OPTIONAL | ⬜ | 0 |

**余下量（2026-06-11 复核）**：MUST 已闭环；§0.1 的 **H2 SHOULD 收口**四项已全部按完成口径闭环；§0.2 的 **H2+ A2 自动写入**也已 staging-proven。当前后续目标切到 §0.3 **C5 外发通知与负责人 fan-out**；调度/换班/小组织/多门店仍需另起设计锁，不得混入已关闭能力尾声。
...
⑩ **C5 外发通知与负责人 fan-out**（🟡 2026-06-11 design-lock；见 §0.3）：outbox → 未排班/调休到期 producers → delivery worker/retry → DingTalk work-notification channel → observability → staging；不纳入调度/换班/OUT
