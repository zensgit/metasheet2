...
- **显式谓词**:单一权威出处 `packages/core-backend/src/services/approval-template-group-name-rule.ts`(模块头注释 + `isVisibleCodePoint`,两者逐行相同);其逐字副本只允许存在于该文件与 PR body 两处(PR body 副本标注「逐字抽取自 …@<sha>,以文件为准」并保持 `cmp` 一致),**本文不转抄谓词表达式**。名字被接受 ⟺ 边缘裁剪后至少含一个 `visible` 为真的码点(长度闸最前,读提交值)。`BLANK_GLYPH_CODE_POINTS` **显式列 56 个成员**(与 DI 重叠的也列,防属性表版本差异):U+2800、Hangul filler 家族 U+115F/U+1160/U+3164/U+FFA0、U+3000、U+180E、U+200B–U+200F、U+2028–U+202F、U+2060–U+206F、U+FEFF、U+FE00–U+FE0F、U+034F、U+00AD、U+061C。
...
| `name text NOT NULL CONSTRAINT atg_name_nonblank CHECK (btrim(name, ' ' \|\| chr(9) \|\| chr(13) \|\| chr(10) \|\| chr(12288) \|\| chr(8203) \|\| chr(8204) \|\| chr(8205) \|\| chr(8288) \|\| chr(65279)) <> '')`(**勘误 3 候选 REDRAFT v2**;ratified 原文是 `CHECK (name ~ '[!-~]')`,第一版候选 `CHECK (btrim(name) <> '')` 已被第 8 轮门审 P2-1 证伪) | 候选分支现状,符号定位:迁移文件内 `CONSTRAINT atg_name_nonblank`(原 ratified 行号 `:41-42`) | §2 约束清单「`atg_name_nonblank`」——**候选改写此行的谓词**,ratify 对象本身未变 | **是**——候选把该 CHECK 从「可打印 ASCII」改为「去掉显式不可见字符集后非空」;单列请示等 owner 亲自确认。**第 2 轮修复后**:应用层 `requireName` 不再只是镜像 —— 它是**主规则**(名字必须含至少一个可见字符;谓词以 `packages/core-backend/src/services/approval-template-group-name-rule.ts` 的 `visible(cp)` 为准,本文不再转抄谓词表达式),并新增 255 码点上限(400 `GROUP_NAME_TOO_LONG`;**第 3 轮修复后该上限读的是提交值,不是裁剪后的值**,见验证 MD §30);裁剪集**成员未变**,仍是 DB 集的严格超集(第 3 轮把它的**实现**从带量词的字符类正则改成显式码点集合 + 线性扫描,集合等价由全码点区间扫描用例机械断言),所以 `GROUP_NAME_UNSUPPORTED` 的 `name` 分支经生产路由不可达(见服务层 `mapGroupConstraintError` 文档注释)。**已披露残留(两族,直连 SQL 仍可入库)**:(a) 空白族 U+00A0 / U+000B / U+000C / U+1680 / U+2000–U+200A / U+202F / U+205F / U+2028/9;(b) 不可见族 U+00AD / U+180E / U+2800 / U+3164 / U+034F / U+FE0F / U+115F(门审 P2-1)。两族都由应用层 400,真库用例以 `RESIDUE` 命名正向断言 DB 侧确实放行 |
