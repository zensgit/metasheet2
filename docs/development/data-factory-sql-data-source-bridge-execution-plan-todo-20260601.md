...
### 🟡 C3 — Incremental / watermark reads — DESIGN DONE 2026-06-03 (impl 🔒)
Gated on: C1/C2 (done) + a real volume/perf signal + opt-in. **Design landed:** `data-factory-sql-data-source-bridge-c3-watermark-incremental-read-design-20260603.md`.
- [x] **Design** — lift the C0 Seam-3 deferral: the runner already speaks watermark (`mode`/`watermarkStore`/`deriveNextWatermark`, `watermark.cjs` types `updated_at`/`monotonic_id`) and `BaseAdapter.QueryOptions` already has `where`/`orderBy`; the only gap is the bridge `read()` ignoring `watermark`+`filters` and the read-only facade narrowing `select` to `{limit,offset}`. Five seams locked: **(A)** widen facade `select` `+where+orderBy` (parameterized-only, writable still rejected); **(B)** type-conditional keyset — `monotonic_id`→strict `>`, `updated_at`→**composite `(field,tiebreaker)`** (strict-`>` misses ties / `>=` stalls — composite is the only no-miss+no-stall shape); **(C)** in-run opaque mode-tagged cursor + across-run resume on the existing single-`{type,value}` store (`>=`+first-page dedup for `updated_at`); **(D)** pass the resolved `watermarkConfig` (`{type,field,tiebreaker?}`) into `read()` — single source of truth in `pipeline.options.watermark`; **(E)** honor `filters` in the same `where` (close the current silent drop). K3 sqlserver executor cited as a precedent we **generalize-not-copy** (red line, not a dependency).
- 🔒 **C3-1** facade widen (host) + test · 🔒 **C3-2** adapter watermark mode (type-conditional keyset + cursor) + no-miss/no-stall negative controls · 🔒 **C3-3** watermark-config plumbing (`type`/`tiebreaker`) + bind-time fail-closed validation (column orderable/indexed; `updated_at` requires tiebreaker) · 🔒 **C3-4** honor `filters` (parameterized) · 🔒 **C3-5** **real-DB locking test** (the keystone — fake-facade units don't prove the facade→manager→adapter→DB keyset path; cf. #2205). Build order C3-1→C3-3→C3-2→C3-4→C3-5. **All 🔒 until a real volume/perf signal + separate opt-in; until then this source kind stays full/manual.**
