...
**"还有多少开发"的答案:新功能为零,代码侧亦已落尽。** 原先列在这里的两个 PR
(#4784 修真 bug、#4768 让 runbook 对着真实部署包跑一遍)**均已完成并合并**;
其后的 #4790(D2 窄修复)也已合。

**剩余 = 两份文档收尾 + 实体机窗口。** 文档收尾指 #4788 / #4794 的放行;
之后的关键路径不在代码,在客户窗口排期。
...
> 本节写于 #4784 / #4768 合并之前。**两项均已完成**,保留作过程记录;
> 当前剩余见 §0 与 §6。
...
| #4785 剩余交付计划与模型分配 | — | — | **已合**(裁决更正走 #4794 follow-up) |
| 彩排 dispatch | #4768 合并 | 我方触发 | **已跑两次,均 17/17**;最终跑 `31103021849` @ `aa48c3f18`(12:48:48Z→12:51:44Z) |
| 验收 MD 实测值回填 | — | 我方 | **已完成**(#4788):最终跑 `31103021849` 的 17 步实测值已回填 |
...
**我方侧的全部工作沿这条主干收敛到「彩排跑绿」为止 —— 该点已越过(2026-08-06)。**
彩排已跑两次均 17/17(最终跑 `31103021849` @ `aa48c3f18`),包已出并冻结,
验收 MD 实测值回填已完成(#4788),B4 D2 已裁(`ACCEPT_SEPARATION_NO_DISSOLVE`)。

**因此关键路径现已整体在 owner 侧**,这不是进度陈述而是位置陈述:表里剩下的每一项 ——
#4788/#4794 的放行、实体机窗口的排期、GIP HOLD 名单 —— 都以 **owner 的裁决或 owner 排定的
窗口**为前置,没有一项存在我方可单方面推进的路径。

我方唯一仍在推进的是**按 owner 复审意见修文档**,而那本身就是被 HOLD 的对象。
