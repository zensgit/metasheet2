...
> `origin/main@d9a3feba2615a9c3660907451f27c3a3bff1d6c6`.
...
carries that wiring; re-verified against `origin/main@d9a3feba` (§1 spine
...
| CI wiring contract | Green with the guard converted to a **derived-completeness assertion** (a suite missing from run-list or exclude is red by construction — the hardcoded-allowlist form is the mechanism that let L8 stay invisible), extended to every W6/W7 suite added since; run-list + path-filter wiring is itself asserted, not assumed. **Landed**: `scripts/ops/attendance-w4c2-ci-wiring.test.mjs`'s own header, re-read at `origin/main@d9a3feba`, records the OBS-1 conversion from the hardcoded `FILES` allowlist to a disk-walk-derived corpus; this leg is executed green on the W8 head, not merely asserted landed here. |
...
5. the database-backed predicate set — **all of W4C-5 §3, verbatim** —
   ...
   against `origin/main@d9a3feba`, §1 spine row). Its executed-green run on
...
- L8 wiring evidence — **the gap this evidence chain tracked is now closed**
  ...
  `origin/main@d9a3feba2615a9c3660907451f27c3a3bff1d6c6`, after `PR 4805`
  ...
  `d9a3feba`: 41 files on disk match `attendance-w4c*.db.test.ts`, the step now
  ...
  `d9a3feba`): the `FILES` hardcoded allowlist is gone, replaced by the OBS-1
...
`origin/main@d9a3feba2615a9c3660907451f27c3a3bff1d6c6`** (this branch has
