...
> requires the ratified W6 semantics (met) and separately gated W6 runtime
> completion (unmet; see the W7 draft lock §8). Every W6/W7-derived row below
> is marked conditional on the implementation it needs; the owner may also choose to
...
> **L11 (Gate C implementation landed — PR 4839 MERGED)**, L12, L13, and §10.
> Re-verified 2026-08-11 against
> `origin/main@325917c0a484522ef9ce87b286d5d986d4e205b3`.
...
| W4C-5 transition tooling and named predicates landed — **not every soak prerequisite** | The W4C-5 transition-safety arc is on main: hardened transition boundary and evidence-manifest discipline (PR #4773); recovery-sweep fairness and values-free counters (PRs #4774/#4779); the complete 8-cell request-snapshot precondition (PR #4780); and the operator `plan`/`apply` CLI plus preparation runbook (PR #4839, merge `60afbffe…`). The merge authorizes no execution, org selection, staging, flag, deployment, soak, or production/customer data. It also does not close issue #4844: that issue explicitly records transaction-ownership hardening as a W4C-5 staging-soak prerequisite while stating that the fresh-connection PR #4839 tool itself is unaffected. | `packages/core-backend/src/attendance/w4c3a-rollout-control.ts`; `scripts/ops/attendance-w4c5-rollout-transition.ts`; `docs/development/attendance-issue-4556-w4c5-operator-runbook-20260809.md`; `w4c2-scheduled-run-ops-worker.ts`; `zzzz20260805120000_w4c2_scheduled_run_sweep_fairness.ts`; issue #4844 |
...
- Repository and GitHub artifacts inspected at this baseline contain no
  authorization or recorded evidence of a seven-day soak. The transition
  boundary and its preconditions are landed code, but that absence is **not
  proof of runtime non-execution**. W8 entry must establish the runtime fact
  from the named environment's transition ledger, audit records, and soak
  evidence; this docs-only plan neither queries nor pre-clears those sources.
- Issue #4844 remains OPEN and labels its transaction-ownership work a
  W4C-5 staging-soak prerequisite. Its two named production boundaries issue
  `BEGIN`/`COMMIT` on caller-supplied connections without first establishing
  ownership. PR 4839 uses a fresh pooled connection and is not regressed by
  this debt; soak entry is a separate, broader claim and cannot omit it.
...
| Group workspace (conditional on landed W6 runtime; semantics ratified) | aggregate panel labels/conflicts vs. seeded configuration; gate-OFF byte-identical | W6 lock §5 |
...
carries that wiring; re-verified against `origin/main@325917c0` (§1 spine
...
| CI wiring contract | Green with the guard converted to a **derived-completeness assertion** (a suite missing from run-list or exclude is red by construction — the hardcoded-allowlist form is the mechanism that let L8 stay invisible), extended to every W6/W7 suite added since; run-list + path-filter wiring is itself asserted, not assumed. **Landed**: `scripts/ops/attendance-w4c2-ci-wiring.test.mjs`'s own header, re-read at `origin/main@325917c0`, records the OBS-1 conversion from the hardcoded `FILES` allowlist to a disk-walk-derived corpus; this leg is executed green on the W8 head, not merely asserted landed here. |
...
5. the database-backed predicate set — **all of W4C-5 §3, verbatim** —
   ...
   against `origin/main@325917c0`, §1 spine row). Its executed-green run on
...
(`W4C2_AUTHORITATIVE_MODE_NOT_DELIVERED`, §5 ledger L12, Gate D). A third,
independent pre-entry blocker is issue #4844 (Gate E): transaction-owning
boundaries still issue `BEGIN`/`COMMIT` on caller-supplied connections without
the required idle/ownership proof. The former missing-command blocker is
closed: PR 4839 landed a shipped `plan`/`apply` caller and runbook (§5 ledger
L11), and that tool takes a fresh pooled connection, so Gate E does not reopen
Gate C or invalidate PR 4839. No command execution is evidenced or authorized
merely because that code is on `main`.
...
| Additional pre-entry prerequisite (transaction ownership, issue #4844) | **Blocked** — the issue explicitly makes its two-site hardening a W4C-5 staging-soak prerequisite; PR 4839 itself is unaffected because its tool takes a fresh pooled connection | Gate E |
| Exit 1 (≥ 7 days in target posture) | **Blocked** — the authoritative target is still unusable for live/scheduled entrypoints, multi-segment scope remains disabled, and Gate E prevents starting the window | Gate A + Gate D + Gate E |
| Exit 2 (zero critical diffs / zero unresolved reviews over the window) | **Blocked** — no valid window exists without exit 1 | Gate A + Gate D + Gate E |
| Exit 3 (reversal + suspend/resume drills) | **Blocked** — drills eventually exercise the still-undelivered authoritative entrypoints, and soak entry is independently blocked | Gate D + Gate E |
| Exit 4 (valid pointers, unchanged historical hashes across the window) | **Blocked** — no valid window exists | Gate A + Gate D + Gate E |
| Exit 5 (PASS marker + residue zero) | **Blocked** — there is no valid completed run to mark PASS | Gate A + Gate D + Gate E |
| Exit 6 (off-roster diffs dispositioned) | **Blocked** — no valid run produces the required diff set | Gate A + Gate D + Gate E |

**Net**: of the 11 inherited entry items, 10 have no remaining product-code
blocker in the category their rows describe, and 1 is blocked — row 4, on
Gate A + Gate D. That count does **not** mean soak can start: the additional
Gate E prerequisite is independently blocked. This is not an authorization
and records no live evidence: entries 5, 9, and 10 still require a separately
owner-authorized operator run before W8 may cite them as live observations.
Of 6 exit items, **all 6** remain unreachable while the Gate A/Gate D
dependency chain or Gate E remains open. PR 4839 closed Gate C's
missing-caller implementation gap only; it did not clear the other product
blockers or start a soak clock.
...
practice: gates A, C, D, and E (L10-L13) existed and were mechanically verifiable
...
| L5 | Soak itself: no authorized or repository-recorded rollout execution/soak evidence is present at this baseline; runtime non-execution is unverified here. | W4 lock §12.8; §4 above | **OPEN — not evidenced or authorized in the inspected repository/GitHub record; runtime/DB state not audited by this docs-only PR.** | Before W8 may claim completion, query the named synthetic environment's transition ledger and audit records, reconcile them with owner authorization, then execute §4 in full and record the resulting evidence in the verification MD. |
| L6 | W6/W7 conditional debts: the exclusions fixed by ratified `OD-W6-*` choices plus whatever OPEN `OD-W7-*` choices later leave deliberately out (e.g. employee self-projection of the aggregate is OUT by OD-W6-5(a); per-group punch enforcement remains governed by OD-4556-9). | respective locks | Re-verified 2026-08-11 at `origin/main@325917c0`: PR 4821 has landed the RATIFIED OD-W6-0..9 record, prospectively authorizing W6-1 only. PR 4849 remains Draft/HOLD and unmerged at `c2ac8284ab38756133c74ce54fde50d07d5bf13a`; W6-2/3/4 and every W7 decision remain unlanded. The route literal is still confined to the design lock and out-of-build draft, and the backend contains only the types-only W6 contract, so no W6 runtime is on main. The earlier whole-attendance-subtree empty-diff proof is retired because PR 4839 legitimately changed unrelated W4C files in that subtree. | Listed with the owning lock, not silently absorbed into issue 4556. The W6 decision record is settled; W6/W7 implementation remains open, and W8-R7 re-derives it on the W8 head. |
...
| W8-R7 | Ledger completeness is re-derived, not remembered: the §5 ledger is regenerated against the then-current main before execution. | The regeneration commands (issue queries + doc/grep sweeps) are recorded in the MD with their outputs, and the spot-check runs in **both** directions, because either alone is passable by a broken regenerator. **Positive**: named items verified OPEN at regeneration time — issue 4792 (L3), issue 4629 (L4), and the soak-prerequisite issue 4844 (L13) — must appear in the regenerated ledger. **Negative**: a named item verified CLOSED must appear as *closed*, not vanish and not stay open — issue 4791 (L2) is that worked example, CLOSED COMPLETED 2026-08-07T15:44:55Z, and this document's own L2/OD-W8-3 rewrite is the recorded instance of the negative half firing. An earlier draft of this cell named only "#4791 if still open", which the closure has since made unrunnable; a spot-check whose only example can no longer fire is not a check. |
...
- `packages/core-backend/src/attendance/w4c0-operation-registry.ts:917, 925, 933`
  and `packages/core-backend/src/attendance/w4c2-outbox-dispatcher.ts:84,
  104, 155` (issue #4844 / Gate E transaction-ownership sites)
- L8 wiring evidence — **the gap this evidence chain tracked is now closed**
  ...
  `origin/main@325917c0a484522ef9ce87b286d5d986d4e205b3`, after `PR 4805`
  ...
  `325917c0`: 41 files on disk match `attendance-w4c*.db.test.ts`, the step now
  ...
  `325917c0`): the `FILES` hardcoded allowlist is gone, replaced by the OBS-1
...
`origin/main@325917c0a484522ef9ce87b286d5d986d4e205b3`** (this branch has
...
- Issues OPEN: 4556; 4629 (PQA-01..10 all still unchecked, PQA-10 still marked
  ...
  (timeCycle residue, zero comments); **4844** (transaction ownership / W4C-5
  staging-soak prerequisite, OPEN, created 2026-08-10T06:19:36Z).
