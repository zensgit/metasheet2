...
> re-verified against current `main` at each round (this branch is not
> rebased — see §10 for why re-verification against a moving tip is used
> instead). Items that later changed state are corrected in place with their
...
> and §10. Re-verified 2026-08-10 against
> `origin/main@d78b27d37c96b66cd8d898dc6b8b17e2a5f294a5`.
...
carries that wiring; re-verified against `origin/main@d78b27d3` (§1 spine
...
5. the database-backed predicate set — **all of W4C-5 §3, verbatim** —
   ...
   against `origin/main@d78b27d3`, §1 spine row). Its executed-green run on
...
| L6 | W6/W7 conditional debts: whatever OD-W6-*/OD-W7-* choices leave deliberately out (e.g. employee self-projection of the aggregate, OD-W6-5; per-group punch enforcement, OD-4556-9). | respective locks | OPEN by construction at the pinned baseline. **Movement, re-verified 2026-08-10 against `origin/main@d78b27d3`**: the OD-W6-0..9 resolutions — adopt the W6 lock, option (a) for each — are **MERGED**, PR 4821 `ecf77d2433596bbdd8b67c312a37178dbc97f715` (2026-08-08T10:41:37Z). The W6 lock's own header on `origin/main` now reads RATIFIED (W6-1 backend aggregate only). The authorization those resolutions carry remains narrow by their own text: the **W6-1 backend aggregate slice only**, Draft/HOLD, stopping after a fresh exact-head gate (PR 4814, OPEN/Draft, unmerged, head `4cc0122883846900a1325cdacd5eda0355d77215`) — no W6-2 contract wiring, no W6-3 UI, no W6-4 verification, no merge. Consistent with that, no W6 runtime exists on main: `git diff 4e6a35d9 origin/main` over `packages/core-backend/src/attendance`, `packages/core-backend/src/services`, `plugins/plugin-attendance`, `packages/openapi`, and `apps/web/src/views/attendance` is empty (re-verified 2026-08-10; PR 4821 touched only the one lock MD). OD-W7-0..8 carry no ruling of any kind. | Listed in the ledger with their owning lock — not silently absorbed into issue 4556. The decision-record half of this row is now settled by a merged PR; the implementation half (W6 runtime, W7 anything) is not, and W8-R7 continues to re-derive this row on the W8 head |
...
- L8 wiring evidence — **the gap this evidence chain tracked is now closed**
  ...
  while `PR 4805` was open; re-verified 2026-08-10 against
  `origin/main@d78b27d37c96b66cd8d898dc6b8b17e2a5f294a5`, after `PR 4805`
  MERGED `4c28467c54f376ad5a68718d3dbe6ad50c76a917`):
  `.github/workflows/plugin-tests.yml` no-DB core-backend test step ("Run
  core-backend tests", no `DATABASE_URL`, `~L621` at the current tip — this
  ...
  `~L1219` at the current tip); the **facts**, re-counted at `d78b27d3`: 40
  files on disk match `attendance-w4c*.db.test.ts`, the step now carries 101
  whole-file args of which 40 are `attendance-w4c*` — **both** former L8 files
  (`attendance-w4c3b-request-snapshots.db.test.ts`,
  ...
  `d78b27d3`): the `FILES` hardcoded allowlist is gone, replaced by the OBS-1
...
- PRs OPEN / unmerged: **4814** (W6-1 backend slice itself, Draft, head
  `4cc0122883846900a1325cdacd5eda0355d77215` — the one PR whose merge would
  put W6 runtime on `main`); **4839** (Draft operator CLI for the
...
runbook`), OPEN, Draft, head `2d35fdeb09a36d3b9ecbadfc4c645d12231e9be5` — see
ledger row L11. Naming it authorizes nothing; it is not merged.

Commands behind this block (re-run 2026-08-10; recorded so W8-R7's
