...
>
> **Erratum（follow-up，2026-07-26）**：本文原始版本随 #4615 合入 main（`9fdf68fa5c34d2224fbe6bd0d71b14ca78263502`）。外部复核在已合版本上发现 2 处 P2：①§0 标题「闭集票据表」及其「机器验证」结论对「考勤线整体」过度声明——本次改为 query-scoped 表述并如实列出盲区；②§0.1/§4 的 #4615/#4582/#4613 状态在合入时已漂（写 OPEN/待决，实为已合）——本次刷新为当前事实。**本 erratum 本身范围克制**：只动上述两类问题触及的行 + 顺带按 API 现势刷新 #4612/#4617 的 head（§0.1），未重新审阅或改动其余内容。
...
> 本表由 **API 查询**生成（`gh pr list --state all --limit 200`，标题正则 `attendance|考勤|4556|W4C|W5-|Wave 5`，**最新 200 张 PR 窗口内**命中 **55** 张——该窗口已跑满，不是「全量」，详见下方盲区 2），**再逐张分类**——不是「列出我正在推进的票」。
...
>   ```
>   本表最近一次刷新时该查询在**最新 200 张 PR 窗口内**命中 **55** 张，且「（标题正则、窗口内）命中集 − 本文提及集 = 空」（机器验证；**这只证明「该查询在该窗口内」闭合，不证明「考勤线全部改动」闭合**——见上方盲区 1、2）。
>
...
> ```
> 命中 **63** 张（vs 标题正则 55 张，**同样是最新 200 张 PR 窗口内**——两个查询用的是同一个窗口，差集比较在窗口一致的前提下有效，但都不覆盖窗口外的历史票，如 `#3487`）。逐一比对两个命中集：
> - 仅路径命中、标题未命中：9 张（`#4427`/`#4442`/`#4444`/`#4521`/`#4524`/`#4526`/`#4530`/`#4534`/`#4602`）——均已在上方盲区 1 逐张列出改动路径。
...
> **本次交叉校验只做到「报告差集」，未做到「逐张裁定新增的 9 张是否该补入 §0.2 并注明理由」**——那需要对每张票重新做归属判断，超出本次「修正过度声明」的范围，如实留作残留（见 PR body「薄弱环节」一节）。
