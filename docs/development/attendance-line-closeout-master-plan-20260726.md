...
> **枚举方法（写成规则，因为方法本身可以按构造够不着某些票）**：本表**列出本线全部开票**——包括**不在关键路径上**的——并**逐条标注在线/不在线及理由**。
> **静默缺席不是本文档的做法**：任何本线开票若未出现在此表，即为**缺陷**，不是「不相关」。
> 初版本表曾用「列出我正在推进的票」这一方法，**结构性地漏掉了 #4584 / #4585 / #4586 / #4608 四张已合入的本线票**——它们不在我当时的工作焦点内，于是既不被列出、也不被标为「不在线」。此段即该缺陷的更正与规则化。
> 表中 SHA 与状态经 API/读码核实，非记忆转述；**基线新鲜度**见表末注。
...
| **（补列）W1-W3 审计 P1 修复** | #4584 已合 | `78b4133ba` | **不在关键路径**——W1-W3 合并后审计的 P1（flag-OFF 遏制击穿）修复，先于 W4 线；已由二轮复核销账 CLOSED |
| **（补列）guard 检查名稳定化** | #4585 已合 | `e0defbe26` | **不在关键路径**——但它是 `attendance-web-guard` 升 required 的前置，该治理动作已完成 |
| **（补列）W1 时间线守卫证明** | #4586 已合 | `c81b3bc39` | **不在关键路径**——W1-W3 审计 P2-2（守卫可 no-op 而全绿）的修复 |
| **（补列）E1 teardown 竞态根治** | #4608 已合 | `d75d3b828` | **不在关键路径**——test-infra；但它解掉了连续两次打断 W4C-1 的「753 全绿却 exit 1」flake |

> **基线新鲜度**：本表以 API 实读为准，但 `main` 在本线推进期间**持续前进**（本文成稿期间至少经历 #4613 合入）。表中「已合」条目的 SHA 为其 merge commit（不随 main 前进而变）；「待裁/在飞」条目的 head 会因 rebase 变化，**以 PR 现势为准**。**明知陈旧却不记，声明即强于证据**——此注即为该纪律的落实。
