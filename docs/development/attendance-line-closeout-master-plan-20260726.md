...
> 2. **`--limit 200` 是分页硬界**：若命中数逼近或超过 200，更早的票会被截断、不出现在结果里。本次两个查询（标题正则 55 张、路径查询 63 张）均远低于 200，**未触发**该界，但界本身仍在，不因这次没触发而消失。
...
>   ```
>   本表最近一次刷新时该查询命中 **55** 张，且「（标题正则）命中集 − 本文提及集 = 空」（机器验证；**这只证明该查询范围内闭合，不证明「考勤线全部改动」闭合**——见上方盲区 1）。
>
...
> ```
> 命中 **63** 张（vs 标题正则 55 张）。逐一比对两个命中集：
...
4. **#4615** W4 开发及验证记录（3 文件）是否合入 —— **已裁：MERGED** `9fdf68fa5c34d2224fbe6bd0d71b14ca78263502`
