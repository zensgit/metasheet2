...
命中集中在本 MD 一个文件内(计数见上),不是上一版压成一句括号概括时暗示的单条命中。**第五次修正(`p3-hygiene-gate4-A2-20260919.md` §P3-1)**:本条及下方三条的输出一律从 `cut -d: -f1,2` 的行号清单改为 `cut -d: -f1 | sort | uniq -c` 的"文件 + 计数",判读改用符号锚点——硬编码行号在文档自身持续编辑时必然漂移,上一轮点名修了一个,同段落的兄弟一个没扫,本轮整批换掉。正文命中分别落在:§12.9 开头的复核修正段(用"本节最初的这条更正说……——这句全称式判断是错的"的回顾句式转述并撤销)、§12.1 处置表里转述报告原文指出错误的那一行、§12.9 一处小标题式的否定句("这条重复不是……——逐 head 核对……")、§12.9「影响声明更正」段(以"……这句全称式判断是一次错误撤回"收尾);此外本条命令行自身构成一条自指命中。**逐条判读:回顾式/自指,零处以"成立"口吻残留。**
...
上表每一行的命令,都在本次第五次修正的树上重跑过一次(父提交 `ca5d50b71`;提交后 `git status --porcelain` 为空,故工作树与提交树一致),输出即各行"现场输出"列所载,未做转抄;重跑的原始记录见 §12.15。§1–§11 与本节之外的 §12.0–§12.13 历史段落不在本轮点名范围内,本轮未重新逐句复核,不在此表列出。
...
| §7 / §12 其余数字(客户端 8 用例、面板 3 用例、`SessionOrgSwitcher` 3 用例) | —— | **全部仍然成立且在合流分支重跑绿**(phase-3 验证 MD §15.4 闸 4:8 文件 / 65 用例全绿)。 |
| 全文各处「验收 J 由本面板承载」的表述 | 组件级 403→选择器→重试→201 | **组件级仍然成立**(3/3 绿);**页面级入口被收窄**为「分组视图 + 展开管理器」两步,且第一跳先撞分节视图的通用错误 —— 升 owner 裁,记为 phase-3 设计 MD §8.4 的 **D3-1**。 |

#### 第 2 轮(2026-09-20)对上表的再求值 —— 两处数字前进,一处表述改写

- 上表末行「客户端 8 用例 …(phase-3 验证 MD §15.4 闸 4:8 文件 / **65** 用例全绿)」:
  合流分支第 2 轮后为 **8 文件 / 69 用例**。本文件的同族 spec
  `apps/web/tests/approvalTemplateGroupsClient.spec.ts` 从 **8 条增至 10 条** ——
  新增的 2 条覆盖 A-4 独有的 `listTemplatesBySection` / `reorderApprovalTemplateGroups`
  (门审 P2-1:此前整条 required 前端闸在这两个函数被掏空时仍全绿)。
  这 2 条**不走本文件既有的裸 `vi.stubGlobal('fetch')` 夹具**:那两个函数带 `USE_MOCK` 短路,
  裸夹具下实测 `fetch` 调用数为 0 ⇒ 用例会是空转绿。理由与实现形状见
  `approval-template-groups-phase3-sections-design-20260918.md` §8.8 (a)。
  **本文件的 8 条原用例一条未改、仍全绿**,`ApprovalApiError` 类契约仍由它们承载。
- 上表末行「**页面级入口被收窄**为『分组视图 + 展开管理器』两步,且第一跳先撞分节视图的通用错误」:
  后半句**已被第 2 轮推翻** —— 分节视图的 `loadAll()` 现在识别 `SESSION_ORG_REQUIRED` 并直接弹共享选择器。
  「两步才到管理面板」仍然成立(那是面板的入口,不是选择器的入口)。
  **owner 对该收窄的裁决仍未记录**,不得把本条读成裁决已发生。
