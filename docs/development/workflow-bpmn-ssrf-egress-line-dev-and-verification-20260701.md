...
| A3-c runtime policy injection | this PR | Injects the normalized backend-only `BPMN_HTTP_TASK_EGRESS_POLICY` source into both BPMN route engine construction sites. Missing/malformed/invalid config still injects the default empty allowlist. No concrete production/staging destination is committed. |
