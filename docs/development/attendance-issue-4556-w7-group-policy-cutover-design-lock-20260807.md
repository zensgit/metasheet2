...
> deployment, no production/customer data use, and no closure of issue #4556.
> The runtime order is now **ratified W6 semantics (MET) → separately gated W6
> runtime completion (UNMET) → W7 → W8**. W7 consumes the settled
> `OD-W6-0..9=(a)` contract; references to W6 are conditional on the named
> runtime slice being implemented and gated, not on those decisions reopening.
>
...
subsequently landed. The semantic decisions `OD-W6-0..9` are therefore settled
as their recorded option (a) outcomes. The narrower phrase in the status line
is an **implementation-authorization boundary**, not a reopening of those
semantics: only review and repair of W6-1 was authorized, while W6-2 contract
wiring, W6-3 UI, W6-4 verification, and every merge/runtime action remain
unimplemented or separately gated. Section 6 lists each W7 dependence on the
settled W6 contract and separately states the implementation precondition.

**Re-verified 2026-08-11 against `origin/main@325917c0a484522ef9ce87b286d5d986d4e205b3`
...
2. **No W6 runtime has landed.** Re-verified at `origin/main@325917c0`: the
...
3. **W7 remains blocked by implementation, not by an undecided W6 contract.**
   §8 item 1 makes ratified W6 semantics and W6 runtime completion a hard
   precondition. The first half is met; the second is unmet. Every §6
   implementation row therefore remains conditional on landed W6 runtime,
   while no `OD-W7-*n*` has any ruling at all.
...
| The W3 canonical shift-service surface does not expose its multi-segment writer as implemented: `SEGMENT_CALCULATION_IMPLEMENTED = false` gates that service's capability/reference path. This is deliberately narrower than a repository-wide or runtime claim; the W4 posture layer has its own capability state, and repository evidence alone does not prove that every environment is non-authoritative. | `plugins/plugin-attendance/lib/attendance-shift-service.cjs:60`, guard `:495`, export `:1208`; distinct W4 capability `packages/core-backend/src/attendance/w4c0-identity.ts:361,481` |
...
| W6 preparation (types only, **no runtime**) landed closed unions for source labels, domains, conflict codes (incl. `CALCULATION_GROUP_MEMBERSHIP_OVERLAP`), a read-only calculation-posture mirror, and the editorRef union. Those semantic shapes were subsequently owner-ratified as `OD-W6-0..9=(a)` by PR 4821; that ratification does not itself land or authorize W6-2/3/4 runtime. | `packages/core-backend/src/attendance/w6-group-effective-policy-contract.ts:25-96`; PR #4771; W6 lock §9 |
...
- There is **no** W6 aggregate service or route at this baseline — only the
  ...
  Any W7 sentence that needs the aggregate is conditional on its separately
  gated runtime landing; its `OD-W6-*` semantics are already ratified.
...
### 2.1 W7 delivers (all conditional on W6 runtime completion; semantics RATIFIED)
...
| --- | --- | --- |
| W7-R1 | The **new group-policy** frozen-context issuance boundary stays singular: no second production group-policy builder and no selector spelling outside the closed union. The pre-existing V1 legacy builder is a separate byte-frozen legacy arm, not a second group-policy builder. Parsing a previously frozen snapshot is validation, never another policy-selection path. | Origin and content are separate proofs. For the **new group-derived schema only**, one core-owned canonical module owns a private `WeakSet` and exactly two issuance operations: (i) build a new context from normalized facts returned by the in-transaction resolver, and (ii) rehydrate an already-frozen serialized context by exact-key/schema/fingerprint validation **without any resolver, policy-table, or live-config read**. Plugin adapters pass normalized facts; they never inject a construction callback. Both operations return deep-frozen objects registered in the private set. Every W7 group-derived context that crosses a request-snapshot, import, or `frozen_prior` database serialization boundary must pass through operation (ii) before the canonical writer; direct parsed objects are not accepted. Legacy V1 objects keep their existing validators and bytes unchanged (W7-R3) and are not retrospectively required to carry this process-local identity. The writer checks private-set membership for the new schema before serialization, then independently recomputes the domain-separated fingerprint from the exact-key payload. The fingerprint is **content integrity only and MUST NOT be described as origin proof**. A production-root AST/data-flow inventory (roots derived by directory walk, with `unclaimed=0`) proves that group-policy resolution reaches operation (i) only and that writer-bound group contexts originate from one of the two canonical operations; direct object literals, spreads, `Object.assign`, computed-key construction, casts, alternate factories, an unclassified file, or an unsupported syntax fail closed. Independent mutations: a cloned group context with every field plus a copied valid fingerprint fails the private-identity leg; sending that clone through rehydration after changing payload bytes fails the content leg; an unchanged serialized group snapshot rehydrates as the positive control; a second policy-resolving builder reds the inventory. A novel selector fails exact-key/closed-union validation. Text grep is only a secondary tripwire. |
...
| W7-R6 | A `frozen_prior` recomputation consumes only the previously frozen snapshot: it must not re-read live group/membership/shift config. Stored calculation evidence is immutable. The distinct post-rollback behavior of `current_policy` remains owned by OPEN decision `OD-W7-10` and is not silently decided here. | Real-DB legs, one per §4.1 fact family: freeze a calculation, UPDATE (i) W1 membership rows, (ii) group policy rows, or (iii) FSER-composed schedule facts, then invoke the **actual recompute entrypoint with `policy=frozen_prior`** for the same calculation. Each appended calculation must preserve the prior `attribution_snapshot`, `context_snapshot`, `evidence_snapshot`, `approved_facts_snapshot`, and `manual_override_snapshot` byte-for-byte and reproduce the same source-definition and semantic-input fingerprints; new calculation identity, operation provenance, version, and timestamps are explicitly outside that equality domain. A query-trace assertion proves no membership/group-policy/shift/FSER live table is read during `frozen_prior`. A targeted seam mutation that introduces one such live read must red the corresponding family alone. Detail/trace byte-exact re-read remains a separate stored-evidence immutability leg, not the proof of recomputation behavior. `current_policy` gets its own positive/negative matrix only after `OD-W7-10` is resolved; until then no criterion may infer its live-versus-frozen source. |
...
### 4.1 Resolution input (under ratified OD-W6-2(a)/OD-W6-6(a))
...
(OD-W6-3(a)): W7 adopts the ratified spellings and mints none of its
...
4. **No restatement on rollback** (parent R3) — **corrected this round; the
   ...
   **arbitrary (including past) work date** — recompute is not restricted to
   ...
   with the adapter building that fresh context at `~L35338`
...
Every W7 dependence on the settled W6 contract, plus the still-unmet W6
implementation precondition, is stated once here:

| W7 element | Depends on | Conditionality |
| --- | --- | --- |
| Group-policy domain grouping used in resolution completeness checks | OD-W6-4(a) (domain/conflict closed inventory) | W7 adopts the ratified eight-domain/seven-conflict inventory. Any narrowing now requires a W6 amendment; W7 must not reinterpret it locally. |
...
| Whether W7's resolver may share code with the W6 aggregate's readers | OD-W6-2(a) (FSER composition) and W6-R4 (single FSER derivation) | W7 composes the same FSER service rather than re-deriving it. The aggregate's ratified embed-verbatim shape is settled; changing it requires a W6 amendment. |
