...
| Post-baseline synthetic checkpoint cleanup | Main [run 36161617263](https://github.com/zensgit/metasheet2/actions/runs/36161617263) attempt 1 failed Node 20 after TM product tests passed: immediate owned-DB activity was 1, expected 0; attempt 2 passed. Main [run 36232013310](https://github.com/zensgit/metasheet2/actions/runs/36232013310) at `8048743cf23f5e9e60bdb85a12b3583b0bf980b0` repeated the final `1 != 0` failure. [#6059](https://github.com/zensgit/metasheet2/pull/6059) changes only `verify-recovery-manual-checkpoint.mts`: allow a bounded two-second drain, still reject persistent residue, and omit caller-controlled `application_name` from diagnostics. Its prior head `1f9cffa3` passed both TM steps but had a later, separate Cloud Classroom catalog timeout in [run 36193091228](https://github.com/zensgit/metasheet2/actions/runs/36193091228); no root cause is inferred from that timeout. Current head `32ddb1b819ea7d31cf3a67ad9e7c16d9989e63b3` is byte-equivalent for the target script. On intermediate replay `d4db5cf4`, the isolated PostgreSQL 15.17 runner passed fresh/replay migration, 47 + 59 + 127 real-DB tests, HTTP restore/download and final DB/connection/cluster residue zero. On current head, core type-check, wiring 43/43 and diff-check passed. Exact-head [Plugin run 36238499605](https://github.com/zensgit/metasheet2/actions/runs/36238499605) succeeded on Node 18 and Node 20; all PR checks total 25 SUCCESS and 1 intentional SKIP. | PASS for this Draft test-only exact head. It is not merged; main has advanced beyond the replay parent, so merged-main acceptance is NOT RUN. No recovery product behavior change or production evidence is claimed. |
