...
| Post-baseline synthetic checkpoint cleanup | On main `4189aa096a8a8054315bb97db79dfbac1823954e`, [Plugin System Tests run 36161617263](https://github.com/zensgit/metasheet2/actions/runs/36161617263) attempt 1 failed Node 20 after the product tests passed: immediate owned-DB activity was 1, expected 0. Attempt 2 succeeded, which does not erase the first failure. [#6059](https://github.com/zensgit/metasheet2/pull/6059) changes only `verify-recovery-manual-checkpoint.mts` to allow a bounded two-second drain while still rejecting persistent residue; follow-up removes caller-controlled `application_name` from failure diagnostics. Prior head `974b945210cf1856b7bc1bba8335401f182abc7e` had 25 SUCCESS and 1 intentional SKIP. Current-main replay head `1f9cffa3a7f1ca59a672461f197088a36d917e1d` kept the script blob unchanged; local isolated PostgreSQL, core type-check, wiring 41/41 and a values-free query mutation passed. Its [exact-head run 36193091228](https://github.com/zensgit/metasheet2/actions/runs/36193091228) had both Node 18/20 TM checkpoint steps PASS, but Node 20 later timed out on the separate Cloud Classroom 10,000-rule catalog test (23 SUCCESS, 2 SKIPPED, 1 FAILURE overall). | Test-only fix remains Draft/HOLD; overall exact-head CI is NOT GREEN and merged-main gate NOT RUN. No recovery product behavior change or production evidence is claimed. |
