...
3. **NS-2 reverse/adversarial matrix** — the proof, at the PURE-PRIMITIVE level (the route still rejects, so this can't run cross-midnight *through* the snapshot yet): same-day unchanged; 23:00→01:00 splits 1h/1h by date; restday-/holiday-after-midnight buckets correctly (each sub-span by its own date); reversed / multi-midnight still rejected; re-run idempotent (byte-identical, total == window minutes). *(reconciled per implementation: the **real-DB integration over the actual segmentation snapshot** moves to **NS-3**, because the snapshot rejects cross-midnight until NS-3 lifts it — until then the only thing real-DB could prove is the preserved guard. "Idempotent" here is the pure-function notion; the replay-through-persistence no-double-count is also an NS-3 concern.)*
