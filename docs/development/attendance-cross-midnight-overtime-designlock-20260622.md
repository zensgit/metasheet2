...
- **`validateOvertimeSegmentationWindow`** gains a "cross-midnight allowed" path: a window whose start/end differ by exactly one local calendar day (and `end > start` in absolute time) is **valid**, not rejected. Reversed windows, or windows spanning more than one midnight, keep returning the stable reject code.
...
2. **NS-1 boundary-aware compute (behind the guard)** — `validateOvertimeSegmentationWindow` + the per-date split, computed but **still rejected at the route** (feature-flagged or internal); unit tests for the split + idempotency.
