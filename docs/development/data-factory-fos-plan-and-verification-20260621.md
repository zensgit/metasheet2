...
| 项 | gate | 备注 |
|---|---|---|
| **FOS-4b-3(action apply)** | opt-in + own review + negative controls | action dry-run 已合并(#3072);真实执行动作引用仍关闭,需 owner 明确 ratify apply。 |
...
→ **分刀**:FOS-4b-1(contract,lock-safe)**✅ done(#3066 `7a602a013`)**;FOS-4b-2(generic route dry-run runtime)**✅ done(#3072 `6f6ff921d`,owner ratified dry-run-only)**;FOS-4b-3(apply,若选)/ UI 后续。**真实执行层本轮未开**(安全敏感;dry-run 已就位,ratify apply 后再单独做)。
