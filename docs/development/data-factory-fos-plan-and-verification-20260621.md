...
### 7.1 FOS-4b action-binding 泛化 — ✅ 已 design-lock(committed 安全模型,待 owner ratify)
**设计锁已落地**:`data-factory-fos-4b-action-binding-generalization-design-lock-20260622.md` —— 不再只列开放问题,而是**提出可 ratify 的 committed 安全模型**:
...
→ **ratify §2/§3 后 impl**:FOS-4b-1(contract:注册表 + preset.permittedActionIds + normalizer,lock-safe)→ -2(generic route dry-run runtime)→ -3(apply,若选)→ UI。**impl 本轮未开**(安全敏感,待 ratify)。
