...
- **FOS 全可自主构建段已落地**:FOS-0(设计锁)→ FOS-1(合同)→ FOS-2(runtime 泛化,stock-prep 零漂移)→ FOS-3(UI,dual-route 不破坏既有能力)。Data Factory 的选项同步从「同步备料选项」(business-specific)泛化为「同步字段选项」(generic + preset),**stock-prep 降为第一个 preset / 兼容锚**。
...
| FOS-4 | 额外 preset 编写 + action-binding 泛化(首个引入 stock-prep 以外 source/target 配置) | 🔒 gated(opt-in;own review + negative controls) | 设计锁 §11 |
...
preset 存储 = 冻结目录常量(无 migration);syncMode 默认 = `replace`(零漂移);`disable_missing` = 只禁不物理删(FOS-2 runtime 语义,尚未实现);conflictPolicy 默认 = `update_from_source`;triggerMode = `manual`;route = 新增 generic route + 保留 stock-prep route 兼容。enums 已暴露 FOS-2+ 的 append/disable_missing、keep_existing/manual_confirm、scheduled/after_source_refresh(运行时待后续刀)。
...
| **append / disable_missing / conflictPolicy 新运行时语义** | opt-in(FOS-2 已留 enum,runtime 待补) | 当前 runtime 恒 replace + update_from_source;新模式各带测试 + negative controls |

---

_校验脚注(2026-06-21):FOS-0..3 合并态——#3021/#3024/#3030/#3032 已 MERGED;#3037 见 §1 SHA。重复 #3022 已 closed(价值折回 #3024)。每刀红线:metadata-only / own-sheet / admin / 无外部写 / 无 K3 / values-free 均守;stock-prep 零漂移经验非空洞;FE gate = 真实 `vue-tsc -b`。_
