...
| FOS-4b-2(action runtime) | generic route 接受 actionId 引用 → dry-run 门控执行;FOS-3 dual-route 收窄 | 🔒 gated(**待 owner ratify 安全模型**;own review + negative controls) | 设计锁 §4/§7 |
...
| 项 | gate | 备注 |
|---|---|---|
| **FOS-4b(action-binding 泛化 + 真实业务域 preset)** | opt-in + own review + negative controls | action-binding 泛化 **已 design-lock**(committed 安全模型,待 owner ratify;见 §7.1 + `…fos-4b-action-binding-generalization-design-lock-20260622.md`);真实业务域 preset 仍需 named target。FOS-4 prove-the-path 已 closed P2 + 解封 stock-prep 表新模式 |
...
### 7.1 FOS-4b action-binding 泛化 — design-lock + ✅ FOS-4b-1 contract built;FOS-4b-2 runtime 待 ratify
**FOS-4b-1(contract,#3066 `7a602a013`)已合并**:动作注册表 + `permittedActionIds` + normalizer(完备 negative controls;经复审 APPROVE)。**LOCK-SAFE 零执行**——generic route 仍 fail-closed,注册表未接执行路径,stock-prep 零漂移;**output-allowlist 为承重控制**(denylist 仅 fail-loud 纵深)。**FOS-4b-2(让 generic route 真正执行动作引用的 runtime)= 安全门,待 owner ratify,本轮未开。**
...
→ **分刀**:FOS-4b-1(contract,lock-safe)**✅ done(#3066 `7a602a013`)**;FOS-4b-2(generic route dry-run runtime — **安全门**)**待 ratify §2/§3**;FOS-4b-3(apply,若选)/ UI 后续。**执行层本轮未开**(安全敏感;contract 层已就位,ratify 后 -2 即机械化)。
...
结论:**contract → runtime → UI → sync-mode → prove-the-path 整线在 merged main 上集成绿 + 不变式经验非空洞**。余下全部 decision-gated(§7;FOS-4b 安全模型见其设计锁 §8 终态)。

---

_校验脚注(更新 2026-06-22):FOS-0..3 + FOS-2b-pre + FOS-2b + FOS-4 全合并——#3021/#3024/#3030/#3032/#3037/#3040(`a47114c02`)/#3046(`4af6bb6fa`)/#3050(`1d6e05299`)。重复 #3022 已 closed(价值折回 #3024)。每刀红线:metadata-only / own-sheet / admin / 无外部写 / 无 K3 / 无生产写 / values-free 均守;stock-prep replace 零漂移经验非空洞(断 kernel patch → stock-prep fail);FOS-2b 三锁(不删除 / 不静默写 / replace 零漂移)经验证;FOS-2b-pre getObjectField read-only;**FOS-4 P2 closed**(HTTP wire-test 走真实 getObjectField,revert-proof;disable_missing 真实 route 只禁不删);**FOS-4b-1 action contract 已合并 #3066(`7a602a013`)= LOCK-SAFE 零执行**(注册表+permittedActionIds+normalizer 完备 negative controls;generic route 仍 fail-closed;未接执行;stock-prep 零漂移);FE gate = 真实 `vue-tsc -b`。剩余 gated:**FOS-4b-2 action runtime(待 ratify 安全模型)**、FOS-4b 真实域 preset(需 named target)、scheduled 触发(需 named demand)。可自主构建段(含 FOS-4b-1 lock-safe contract)已尽;余下三类各缺仅 owner 能给的决策。_
