...
- **剩余 gated(各带 gate,非自动构建)**:**FOS-4b-3-prod 首笔生产 apply**(写 prod canonical;sandbox apply gate 已 shipped 且 **sandbox validation 实机 COMPLETE — Path 1 + Step 1 + Path 2 均 PASS,见 §9**,但生产写仍是独立 owner gate)、**真实业务域 preset**、**scheduled / after-source-refresh 触发**(demand-gated)。见 §6/§9;**每项需先签的 scope decision 见 §7**(余下开发 = decision-gated,本轮不在 blanket 指令下盲开)。
...
### 7.1 FOS-4b action-binding 泛化 — ✅ FOS-4b-1 contract + ✅ FOS-4b-2 dry-run + ✅ FOS-4b-3 sandbox gate;prod apply gated
**FOS-4b-1(contract,#3066 `7a602a013`)+ FOS-4b-2(action dry-run path,#3072 `6f6ff921d`,owner ratified dry-run-only)已合并**:注册表 + `permittedActionIds` + normalizer(完备 negative controls)+ generic route `dryRun` 模式校验动作引用并 values-free preview。**先不执行写**:dryRun 在 kernel 前 return → 零 patch / 零执行;非 dryRun + 动作 → fail-closed(apply 未开);pure-option 零漂移;复审 APPROVE(no-write / apply-gated / no-bypass / values-free 经验证)。**output-allowlist 为承重控制**。**FOS-4b-3 sandbox apply gate 已 shipped 并完成实机 sandbox validation(见 §9);FOS-4b-3-prod(真实生产 canonical 写)仍是独立 owner gate,本轮未开。**
...
## 9. FOS-4b-3 sandbox 实机验证 — COMPLETE(entity machine,#3093,2026-06-24)

> **实机(entity machine)real-deploy 验证**,补在 §8 单元/集成绿之上。此前本 MD 只记录 Path 1 PASS;本节补记后续 Step 1 Windows first-hop deploy PASS + Path 2 large-BOM checkpoint sandbox PASS,让 main 记录与 #3093 values-free evidence 对齐。
...
部署硬化补充链:**#3137 `199c01258`**(Windows default staging root + ZipFile)→ **#3144 `e9f3a3ed8`**(first-hop bootstrap sidecar)→ **#3148 `aaa52884a`**(launcher/apply-helper logging off success stream + regression guards)。这些是 deployment tooling fixes,不是 FOS apply/runtime 改动。
...
**Step 1(Windows first-hop deploy)= PASS**(实机,values-free evidence):

```text
release/package         : multitable-onprem-first-hop-stdout-fix-20260624-aaa52884
bootstrap sidecar       : bootstrapSidecarUsed=true
staging root            : observedStagingRoot=C:\ms-tmp
zip extraction          : zipObservedExpandArchiveAbsent=true
deploy exits            : zip/tgz bootstrap deploy exit=0
```

**已验证(Step 1):** Windows first-hop stale launcher + stdout-pollution 两个 deploy blocker 均闭合;entity-machine deploy path 可承载后续 sandbox validation package/retest。该 PASS 不改变 apply/prod gate。

**Path 2(large-BOM checkpoint sandbox)= PASS**(实机,values-free evidence):

```text
sandbox plan            : sandboxBoundAuthoritativePath2PlanFound=true,rowsExpanded=707,manualConfirmRows=85,conflictTypeCount=2
target binding          : canonicalTargetCount=0,allowlistedSandboxTargetCount=1,sandboxModeEnabled=true
route gate              : routeGateObserved=true,checkpointWriterObserved=true,sandboxOnly=true
checkpoint apply        : path2ApplyJobStarted=true,path2ApplyExecuted=true,applyRunCalls=7,applyTerminalStatus=partial
held rows               : manualConfirmRowsHeld=85,manualConfirmRowsWritten=0
clean decisions         : cleanDecisionCountsWritten=537,cleanDecisionCountsFailed=0
re-pull 幂等            : add=0 / skip=537 / update=0
write boundary          : canonicalWriteExecuted=false,productionApplyExecuted=false,externalWriteExecuted=false
evidence                : values-free(无 workspace/object/sheet id / projectNo / row values / payload JSON / raw PLM rows / creds)
```

**已验证(Path 2):** large-BOM checkpoint route gate(`tableActionLargeBomApplyJobRun`)在 checkpoint writer 前实机触发;历史 bypass 路径闭合;`acceptManualConfirmHold=true` 仅作为 held-row acknowledgement,85 条 `manual_confirm` / `duplicate_expanded_key` 仍 held/unwritten;537 条 clean decision 只写入 sandbox,并在 re-pull 上保持幂等。
...
**仍 gated / 仍关闭:**
- **FOS-4b-3-prod(首笔生产 apply / 写 prod canonical)**= **仍 gated**(独立 owner gate)。sandbox validation COMPLETE **不**自动解锁生产;production gate 需要单独授权、rollback/evidence plan、目标范围和 stop rules。
- **FOS-4b 真实业务域 preset**= 仍需 named target。
- **scheduled / after-source-refresh**= 仍需 named demand。

**边界:** production apply 仍关闭;canonical/external/K3 Save/Submit/Audit/BOM 均未开;本节只记录 sandbox validation closure。

**Operational note(非 blocker,需在未来 production gate 前保留):** Path 2 source lookup 需要 `sourceRegistryScopeAdjustedByOmittingWorkspaceQuery=true`;`sourceLookupSucceeded=true`,且未打印 workspace-scoped source id / source credentials。该 nuance 不影响 sandbox apply boundary,但属于 source-registry/operator lookup 行为,production gate 讨论前要保留。

状态行:
```text
sandbox validation runbook shipped; sandbox option seeding deployed; Windows first-hop deploy PASS; Path 1 sandbox apply/idempotency/human-field PASS; Path 2 sandbox-bound large-BOM checkpoint route-gate/idempotency/held-row PASS; sandbox validation COMPLETE; production apply still closed.
```

---

_校验脚注(更新 2026-06-24):FOS-0..3 + FOS-2b-pre + FOS-2b + FOS-4 全合并——#3021/#3024/#3030/#3032/#3037/#3040(`a47114c02`)/#3046(`4af6bb6fa`)/#3050(`1d6e05299`)。重复 #3022 已 closed(价值折回 #3024)。每刀红线:metadata-only / own-sheet / admin / 无外部写 / 无 K3 / 无生产写 / values-free 均守;stock-prep replace 零漂移经验非空洞(断 kernel patch → stock-prep fail);FOS-2b 三锁(不删除 / 不静默写 / replace 零漂移)经验证;FOS-2b-pre getObjectField read-only;**FOS-4 P2 closed**(HTTP wire-test 走真实 getObjectField,revert-proof;disable_missing 真实 route 只禁不删);**FOS-4b-1 action contract #3066(`7a602a013`)+ FOS-4b-2 action dry-run path #3072(`6f6ff921d`,owner ratified dry-run-only)已合并**(注册表+permittedActionIds+normalizer 完备 negative controls;dryRun 校验动作引用 + values-free preview,**先不执行写**:preview 在 kernel 前 return → 零 patch / 零执行;非 dryRun + 动作 fail-closed;pure-option 零漂移;复审 APPROVE);FE gate = 真实 `vue-tsc -b`。**FOS-4b-3 sandbox apply gate 已 shipped(#3089)守住 small+large-BOM 两条真实写入口(large-BOM bypass 在 route 写入前已闭,复审 APPROVE);#3127(`04e1fd884`)补齐 sandbox 目标 select options;#3137/#3144/#3148 关闭 Windows first-hop deploy blocker;Path 1 small-BOM 与 Path 2 large-BOM checkpoint 均已实机 sandbox PASS(#3093,见 §9)。** 剩余 gated:**FOS-4b-3-prod 首笔生产 canonical 写(独立 owner gate;sandbox validation COMPLETE 不自动解锁)**、FOS-4b 真实域 preset(需 named target)、scheduled 触发(需 named demand)——各缺仅 owner 能给的决策/输入。**production apply 仍关闭。**_
