...
- **剩余 gated(各带 gate,非自动构建)**:**FOS-4b-3 action apply**(真实执行动作引用)、**真实业务域 preset**、**scheduled / after-source-refresh 触发**(demand-gated)。见 §6;**每项需先签的 scope decision 见 §7**(余下开发 = decision-gated,本轮不在 blanket 指令下盲开)。
...
| FOS-4b-3(action apply) | 真实执行动作引用(dry-run→apply token);**已 design-lock**(committed apply 安全模型)| 🔒 gated(**待 owner ratify apply**;impl 未开)| `…fos-4b-3-action-apply-design-lock-20260622.md` |
...
| 项 | gate | 备注 |
|---|---|---|
| **FOS-4b-3(action apply)** | opt-in + own review + negative controls + **ratify apply** + sandbox-first | action dry-run 已合并(#3072);**apply 已 design-lock**(`…fos-4b-3-action-apply-design-lock-20260622.md`:token-bound / allowlist 闸 / 复用 C6 per-row+dead-letter / 凭据走 store / sandbox-first / K3 红线不开 / values-free);真实执行仍**关闭、未授权**,待 owner ratify apply;首笔生产 apply = 独立 owner gate。 |
...
→ **分刀**:FOS-4b-1(contract,lock-safe)**✅ done(#3066 `7a602a013`)**;FOS-4b-2(generic route dry-run runtime)**✅ done(#3072 `6f6ff921d`,owner ratified dry-run-only)**;FOS-4b-3(apply)**已 design-lock**(committed apply 安全模型,见 `…fos-4b-3-action-apply-design-lock-20260622.md`;token-bound + allowlist 闸 + 复用 C6 per-row/dead-letter + 凭据走 store + sandbox-first + K3 红线不开)→ impl 待 **ratify apply**(sandbox-only 首版,首笔生产 apply 独立 gate);UI 后续。**真实执行层本轮未开**(安全敏感;设计锁不开任何执行)。
