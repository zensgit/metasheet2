...
| PIT **reset-execute** link sync (univer-meta.ts:9676 INSERT) | `…/reset-execute` (flag-gated) | ✅ **gated at the pre-existing preview PREFLIGHT** (univer-meta.ts:9322 — `guard.readOnly !== true` via `isFieldAlwaysReadOnly`): a mirror-diff reset → `409 RESET_BLOCKED`, nothing written. (Initial enumeration read the replay loop only and missed the preflight; the replay loop also got an explicit defense-in-depth mirror skip, :9652.) |
