...
本轮把这句话从注释变成断言:`ApprovalProductService.ts:13406-13413` 的 try/catch 一旦删掉(M-PC1),
...
`dispatchAction` 的提交后区段 `:12667 COMMIT` → `:12668` → `:12681` → `:12688 投递` **共用** `:12699` 一个 catch。
今天上游两步都自吞异常(`:13301-13302` / `:13359-13361`),所以**不可达**;但「宣告恰一次」因此**依赖两个不相干方法的性质**,
而不是依赖本地结构。

- **建议值**:把 `:12688` 上移到紧接 `:12667` 的 `COMMIT` 之后。
...
`plugins/plugin-attendance/index.cjs:19398` 的幂等门本轮**经生产调用路径被执行**,且**对其所见账本状态承重**
(M-INNER 删门即重复冲销 +60)。

**可以写**:内层门已被执行并承重;外层 W4 preflight 是今天挡在生产与这一行之间的东西。
**不得写**:无限定的「余额冲销幂等已验证」。

原因写在设计侧而不只写在验证侧,因为它是**结构性**的:全仓 `reverse` 行只有 `:19439` 一个写入方,而它与
`:35288-35296` 的状态写同事务,所以「source_id X 已有 reverse 行、请求 X 仍 approved」这个状态,
