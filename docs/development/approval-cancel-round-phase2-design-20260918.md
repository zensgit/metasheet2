...
## post-commit 异常隔离 + 内层幂等(2026-09-19,owner 对聚焦闸两条 P3 的裁决)

**性质:补证据,生产代码零改动。** 完整台账、逐字输出、四组 mutation 见验证 MD
`approval-cancel-round-phase2-verification-20260918.md` **§7**。本节只登记**设计侧**的三件事。

### D1. 投递的隔离是**有意的非对称**,本轮由用例钉住而非由注释声称

执行端口 fail-CLOSED、宣告投递 fail-OPEN,理由写在 `attendance-cancellation-execution-port.ts:328-335`:
到读投递的时候,业务取消**已经提交**,抛错既撤不回它,又只会把一次成功的取消变成 500。
本轮把这句话从注释变成断言:`ApprovalProductService.ts:13479-13486` 的 try/catch 一旦删掉(M-PC1),
approve 立刻由 **200 变 500**(`APPROVAL_ACTION_DISPATCH_FAILED`)——**这正是 owner 说「会诱发重试」的那个形状**。

⚠️ 支撑这条的是**同步性**:合同是 `(…) => void`(`:306-309`),唯一绑定方与整条 emit 链逐跳同步(验证 MD §7.2 有普查表)。
**若将来有人把投递合同改成返回 `Promise`,这道 try/catch 会静默失效**——那时必须同时改成 `.catch()`,否则隔离只剩形状。
登记为设计约束,不是实现细节。

### D2. ⚠️ OWNER 待裁(新增一条):提交后区段的外逃

`dispatchAction` 的提交后区段 `:12740 COMMIT` → `:12741` → `:12754` → `:12761 投递` **共用** `:12772` 一个 catch。
今天上游两步都自吞异常(`:13374-13375` / `:13432-13434`),所以**不可达**;但「宣告恰一次」因此**依赖两个不相干方法的性质**,
而不是依赖本地结构。

- **建议值**:把 `:12761` 上移到紧接 `:12740` 的 `COMMIT` 之后。
- **本轮不做**:(i) 会改动聚焦闸在当前顺序下放行的提交后次序;(ii) C-2 仍要 `rebase --onto` 新 C-1,该文件是冲突面;
  (iii) 本轮定位是补证据。
- **已测**:验证 MD §7.3 的 `P3-1 residual` 用例注入上游外逃,并用两条 ⚠️ TRIPWIRE 钉住今天的后果
  (approve `500`、宣告 `0`)。采纳建议之日这两行会红,**应改写成更好的值,不得删除**。

### D3. 内层 `alreadyReversed` 的**声明范围**(owner:「按声明范围处理」)

`plugins/plugin-attendance/index.cjs:19425` 的幂等门本轮**经生产调用路径被执行**,且**对其所见账本状态承重**
(M-INNER 删门即重复冲销 +60)。

**可以写**:内层门已被执行并承重;外层 W4 preflight 是今天挡在生产与这一行之间的东西。
**不得写**:无限定的「余额冲销幂等已验证」。

原因写在设计侧而不只写在验证侧,因为它是**结构性**的:全仓 `reverse` 行只有 `:19466` 一个写入方,而它与
`:35319-35327` 的状态写同事务,所以「source_id X 已有 reverse 行、请求 X 仍 approved」这个状态,
**今天的单一调用点产生不出来**。两道门是串联冗余;外层挡住就够——这是设计意图,不是覆盖缺口。

### D4. 回滚路径必须三分(owner 指令,写给合并裁决稿的引用者)

**代码回退 / 迁移回退 / 已兑现业务的补偿是三件事。** 本切片:代码回退**能**(生产零 diff);迁移回退**不适用**(零 DDL);
**已兑现业务的补偿不能**——撤回 PR 不会恢复已取消的请假,也不会撤回已返还的余额。逐格见验证 MD §7.8。
C-2 整体一旦上线并真的兑现过,第三类同样成立。
