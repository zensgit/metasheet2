...
**口径订正（本次，不改逻辑）**：上面这套约定的净效果是**有损转换并告警**，不是保真/无损修复——不可表示的条目仍然被跳过，只是跳过时会报 `loadWarnings` 而不是静默改写。正向解析器 `parseDictionaryMap`（`integrationMappingTransform.ts:114-123`）本就支持 JSON 对象文本（`trimmed.startsWith('{')` 分支直接 `JSON.parse`）；逆函数 `dictionaryMapToText` 仍序列化成逐行 `key=value`，这正是条目 5、6 会丢的原因。把逆函数改成输出 JSON（同一个解析器就能原样读回）即可让 dictMap 往返无损，本次未做。**G08 接线前必须先解决这一点**，否则「编辑器里看不见的丢失」会变成「读一次存一次就真丢了」。
...
- 视图侧仍无「打开已保存 pipeline → 回填编辑器」的入口（G08）。逆函数已经就位并有测试，但整体仍是**有损转换并告警**，不是保真/无损修复。**接上之前**，§5 那六项有损只是「编辑器里看不见」；接上之后它们会变成「读一次再存一次就真丢了」。**dictMap（条目 5、6）尤其必须先解决**：正向解析器 `parseDictionaryMap` 本就支持 JSON 对象文本，只是逆函数 `dictionaryMapToText` 仍序列化成逐行 `key=value`（见 §5.1）；改成 JSON 输出即可无损往返，本次未做，本 PR 不宣称无损完成。`pattern.flags` / `rule.message` 两项则应在 G08 落地时先给控件，或在回填时对带有这两项的规则加只读提示。
