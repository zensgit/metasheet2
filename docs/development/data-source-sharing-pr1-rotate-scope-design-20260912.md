...
上级设计写的 `:716` 在 main 上仍然成立(改动前实读确认),本刀给该路由加了 22 行注释块,**改动后**它落在 `:738`。
...
| 要件 | 位置 | 说明 |
|---|---|---|
| `DO $$` + `information_schema.tables` 表存在守卫 | `:113-118`、`:132-137`、`:149-154`、`:166-171` | `permissions` 表不存在时整块跳过,不报错 |
| `INSERT INTO permissions (code, name, description)` 三行 | `:119-123` | 三行的 `code` 就是三个新码 |
| `ON CONFLICT (code) DO NOTHING` | `:124` | 幂等的全部依据 |
| **不写 `role_permissions`** | `up()` 全体 `:111-128` | 刻意省略,见 §4 |
| `down()` 子表先于父表 | `role_permissions :131-146` → `user_permissions :148-163` → `permissions :165-180` | FK 是 `ON DELETE CASCADE`,只删父表也够;显式删子表是为了让爆炸半径在源码里看得见 |
| 导出常量 | `DATA_SOURCE_SHARING_PERMISSION_CODES :86`、`DATA_SOURCE_PREEXISTING_PERMISSION_CODES :99`、`DATA_SOURCE_ALL_PERMISSION_CODES :106` | 供测试对账,沿用 `STOCK_PREP_PERMISSION_CODES` 的既有约定 |
...
| 文件 | 行 | 改动 |
|---|---|---|
| `packages/core-backend/src/routes/data-sources.ts` | **`:738`** | `rbacGuard('data_sources', 'write')` → `rbacGuard('data_sources', 'rotate')` |
| 同上 | `:712-737` | 新增 26 行注释块(其中 22 行是新增),写明「独占、不是 `rbacGuardAny`」、fail-closed 的理由与上机前置的指向 |
...
| `PUT /api/data-sources/:id/credentials` | **`:738`** | **`rotate`** ← 本刀 |
| `DELETE /api/data-sources/:id` | `:840` | `write` |
| `POST /api/data-sources/:id/connect` | `:920` | `write` |
| `POST /api/data-sources/:id/disconnect` | `:972` | `write` |
| `GET /api/data-sources/:id/test` | `:1011` | `read` |
| `POST /api/data-sources/:id/query` | `:1059` | `execute` |
| `POST /api/data-sources/:id/select` | `:1165` | `read` |
| `GET /api/data-sources/:id/schema` | `:1225` | `read` |
| `GET /api/data-sources/:id/tables/:table` | `:1279` | `read` |
...
`manager.assertAccess(id, actor)`(改动后 `routes/data-sources.ts:764`)与 `resolveActor` 都没动。
...
非管理员持有者则会**立刻失去轮换能力**,直到管理员显式授权。

**因此上机前必须先跑这条只读查询**,它写在迁移头注释 `:60-65` 里,也抄在这里:
...
- **返回 0 行** → 直接上,零回归面。
- **返回任意行** → 先把 `data_sources:rotate` **经角色**授给这些主体,再上门改动。
  「经角色」不是风格偏好:直接写 `user_permissions` 会被命名空间准入过滤掉,表现为 403
  (2026-09-08 备料那条教训,`stock-prep-operator` 角色就是为此建的)。
  授完还要**确认该用户的 `data_sources` 命名空间准入是开的**,并且**让他重登**(RBAC 权限缓存 60s)。

**刻意不在迁移里写 `role_permissions` 自动补权。** 迁移一旦发权,这次授权就是「没有人做出的授权」,
正是备料 R-11「映射零自动」与 #5611 定的口径要排除的东西。这条由测试 `:382`(正面)+ `:387`(变异探针)钉住;
变异 M-D 的实测见验证文档。

**本刀 fail-closed 的准确表述**:切独占后,未获 `rotate` 的 `write` 持有者**暂时不能轮换凭据**,
但**仍然能改连接串、能删、能连断**——因为那些路由的门没动。这不是「把 write 收窄了」,是「把 rotate 摘出去了」。
...
- 这**不是** bug,是两支 PR 的真实耦合:#5611 的对账刻意做成「网关集合 == 种子集合」的等式,
  而本刀合法地扩大了网关集合。
- **修法(给后合的那一支)**:把 `data_sources:rotate` 加进 #5611 那条断言的期望集合,
  或把它的 `assertSeedMatchesEnforcement` 的"已知可授予"集合并上本刀的
  `DATA_SOURCE_SHARING_PERMISSION_CODES`。后者更稳,因为 PR-3/PR-4 还会再加 `share`/`use` 两个网关。
- 本支**没有**替它改——#5611 的文件不在本分支上,盲改会制造冲突。
...
位于 `@@ -712,8 +712,30 @@`(credentials 路由的注释块 + 门那一行),与 #5593 的 `:336`/`:421` 不重叠,
预期 git 自动合并。副作用是该行以下的行号整体 **+22**,#5593 若在描述里写了绝对行号需要重算。
