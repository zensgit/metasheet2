...
(另一份更早的 `closeout-approach-review-20260822.md` 针对上一版计划)。裁定 **NEEDS-AMENDMENTS**(0822 那份对上一版计划的报告用的是 `PLAN-NEEDS-AMENDMENTS`,两者不可混引),九条修正皆为加法。其中三条改变了计划形状:
...
| S4b | required-lane 提权:`approval-org-writer-w4-s1`(G-L11-5/G-L11-8)+ `s1-consumers` 按 #5095 三点程序进 required `test (20.x)`(**`p1c` 不在此列——它早已在 required lane 内**:`plugin-tests.yml:1228`,run `32675738976` 的 `test (20.x)` 步骤 #71 实证 success;裁定书 F3 的证据把它一并列入,系继承性错误,已剔除);碰 `plugin-tests.yml` ⇒ s6a pin 重算,需安静窗口 | bounded | sonnet / opus 门审 | **A-5 owner 决策后、S5 之前**(新增 2026-08-24,外部审阅裁定 F3) |
...
(t) **只读分类 + 定点修复人口,再重跑链**——先只读分类那 6 个零成员活跃用户(目录绑定/管理员建/JIT(`dingtalk_%@placeholder.local`)/自注册/测试孤儿;其中 1 个是仅停用行)与 31 张单据的 class-6 类比普查(谓词去掉 `org_id IS NULL` 合取)及 class-2 附件冲突普查;可归属者经产品级准入写入器(admin-users.ts:3874-3878 / directory-sync.ts:5648 / user-activate.ts:210)定点入 org,孤儿停用;之后 `u1b=0` ⇒ provisioning 迁移在 :153 早返、单 org 前提**不被评估**,class-3 按 requester 逐一盖章横跨 4 个 org,class-6 若为 0 则 Migration B 亦不触前提,gap-closer 窗口为空。**(t) 不假设无条件成功——三处已知裂口须在分类作业里一并量出**:①Migration B 还有**第二处 abort**(`zzzz20260823100000:486`,class-2 附件跨 org 冲突),它独立于单 org 前提、(t) 不加约束,故分类普查必须包含它;②停用孤儿账号会**改变 class-6 人口**(其名下单据可能因此落入 class 6),故 class-6 类比普查须在修复**前后各测一次**,不能只测前;③目录桶的判别源当前为空——run 7 实测 `u1c_directory_integration_distinct_orgs=1`、`non_default_integration_rows=0`,即目录集成无法区分那 4 个 org,「可归属者定点入 org」需要另找证据源(邀请记录/创建者/邮箱域/单据归属),这是分类作业要回答的问题之一而非已解决的前提。**仅当分类后仍有不可归属残留、或上述任一裂口非零时,才回落到 (a)/(b)**。代价=一次只读普查作业 + 若干管理员操作;保留多 org 测试资产,且多 org staging 是 pin 唯一能做出判别性演练的环境(单 org prod 上 pin 对全员放行)。
