...
日期:2026-08-25 · **v2(2026-08-26 修订)** · 状态:**PROPOSED**(本文不 ratify 任何内容;§7 为四个独立裁决包)
...
**行数边界合同(v2 补,审阅第 5 条)**——导入前置,缺此合同导入即无界:
- 现状:`maxRows` 可不填,服务端仅校验非负整数,**无系统硬上限**;`minRows≥1` 时发起页初始
  仍播种空数组(不自动出首行)。
- 合同:引入**系统导入上限**(具体数值待裁,建议 200);生效上限=`min(字段 maxRows, 系统上限)`;
  `minRows>0` 时发起页初始播种 `minRows` 行空行;导入超限**整批拒绝**并报明细(不静默截断)。
...
**幂等(v2 更正,审阅 P1-1)**:不另立键。既有账本 `meta_fwb_action_applied` 的业务键为
`(instance_id, rule_id, action_key, node_key, entry_epoch, application_mode)`(唯一索引
`uq_fwb_action_applied_business_claim`;哨兵约定覆盖 FWB-1/2/3)。v2 在其上**延展一列
`row_index`**(取自冻结快照内行序,快照不可变故行序稳定)。v1 曾提议 `(instanceId, rowIndex)`
——同实例配两条规则/两个 FWB 动作时第 0 行即碰撞,**作废**。
...
text/textarea→text;date→date;datetime→date(截断,格内注明)或拒绝;select→select(闭集选项);
...
**D. 共享编辑层与行公式包**:①是否投入抽域中立 codec/editor contract(vs 维持各自实现);
②通用行公式是否立锁(§6b 五要素),及与 lineDerivation 的收敛方向。
