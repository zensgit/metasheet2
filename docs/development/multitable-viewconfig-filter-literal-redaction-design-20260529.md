...
**`allowedFieldIds` binding — bind to the sheet whose views are RETURNED, not the request's `sheetId`.** Authenticated callers build `allowedFieldIds` from the requester's `fieldScopeMap` + the fields of the sheet **whose views are in the response**:
- `/view` (#2015) + `GET /records` already load `fieldScopeMap` for their resolved sheet.
- **`GET /context` is the subtle trap (verified):** it builds access from `resolveRequestAccess` (`:3402`) — NOT `resolveSheetCapabilities` — and on a **base-only request (`?baseId=...`, no `sheetId`/`viewId`)** `resolvedSheetId` is **null** while the returned views bind to an inferred **`effectiveSheetId`** (`= resolvedSheetId ?? readableSheetRows[0].id`, `:3471`; views queried on `effectiveSheetId` `:3501`; `activeFields` on `effectiveSheetId` `:3511`). Its existing `fieldScopeMap` load is gated on `resolvedSheetId` — `(access.userId && resolvedSheetId) ? load(query, resolvedSheetId, userId) : new Map()` (`:3530`) → **empty on base-only even for an authenticated user** → no layer-3 denials → **fail open**. **(b) MUST compute `allowedFieldIds` against `effectiveSheetId` — BOTH inputs keyed to it, at the serializer site, or the fail-open returns in a different seam:** (1) `fieldScopeMap` loaded off `effectiveSheetId` (not `resolvedSheetId`), **AND** (2) the `fields`/`activeFields` fed to `deriveFieldPermissions` also off `effectiveSheetId` (the code already builds `activeFields` on `effectiveSheetId`, `:3511` — **reuse that source**; do **not** re-derive fields from `resolvedSheetId`). Threading the right scope map but the wrong `fields[]` source recreates the leak. Concretely: `allowedFieldIds = deriveFieldPermissions(activeFields@effectiveSheetId, caps, { hiddenFieldIds: [], fieldScopeMap@effectiveSheetId }).visible !== false`. Same two-input rule for any future base-only readback. *(Related pre-existing quirk, NOT (b)'s literal scope: the returned `fieldPermissions` **metadata** at `:3531` is also computed off the `resolvedSheetId`-bound empty map on base-only — a #2015-metadata gap; flagged, not fixed here.)*
- **`GET /views`** must likewise load `fieldScopeMap` for its sheet (it currently loads none).
- **`GET /form-context`** (anonymous) → empty `allowedFieldIds`, fail closed (above).

(Cost: one `loadFieldPermissionScopeMap` per authenticated view-config read on the list/context paths — acceptable; same primitive already on the hot `/view` path.)
...
|---|---|---|---|
| **R1** (req) | **`GET /context?baseId=...`** (authed, **base-only — no `sheetId`/`viewId`**) | the layer-3-denied literal is **absent** from the body; `fieldId`+`operator` retained (chip still renderable). MUST use the **base-only** form — that is the `effectiveSheetId`-not-`resolvedSheetId` fail-open vector (`:3530`); a generic `?sheetId=`/`?viewId=` context call binds `fieldScopeMap` correctly and would **miss** the bug. | RED |
...
- 🔒 **Impl**: shared `redactViewConfigFilterLiterals(view, allowedFieldIds)` helper (or a centralized `serializeViewConfig` — §3 tradeoff) + compute `allowedFieldIds` via the #2015 composite for authed callers / **empty set for anonymous `GET /form-context`** + plumb `fieldScopeMap` into `GET /context` (off **`effectiveSheetId`**, not `resolvedSheetId` — the base-only fail-open) / `GET /views` + apply at all **10** points + R1–R9 fail-first (incl. **R1 base-only `?baseId=` /context**, R5 anonymous fail-closed, R8 layer-2) + **complete the compat scan** (`MetaViewManager`, `MetaAutomationRuleEditor`) to finalize the wire shape + golden-doc note — separate explicit opt-in.
