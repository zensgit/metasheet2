...
2. **No new migration for `attendance_*`.** Comprehensive-hours facts already live in `attendance_*`; PR6 reads them, it does not add fact tables.
...
| T6 | No new `attendance_*` migration; no raw `meta_*` write | **review checklist** (no CI grep wired in this track) | Reviewer confirms the impl PR's `migrations/` diff is empty and all writes go via `context.api.multitable.*`. Honest note: this is review-enforced, not machine-enforced, unless a CI guard is added as a separate decision. |
