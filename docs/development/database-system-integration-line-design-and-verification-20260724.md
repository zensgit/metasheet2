...
> - a **server-bound source executor**, **plugin-layer and HTTP-only in v1** — resolution-derived handle and field set, first-party statement builders admitted by module-private identity, restricted statement seam **within the plugin layer** (B-1, B-4). **⟲OD3 AMENDED (owner, 2026-07-26) — the bounded core-backend statement seam is NO LONGER in this gate; it defers with the SQL path to (b′) + B1c.** This bullet previously included it "of §3.2". Under decision (δ)'s **v1 = (c)** that became self-defeating on three counts: §3.2's *entire* justification for the seam is that **probe SQL** uses `GROUP BY`/`HAVING` shapes the structured facade cannot express — and (c) makes SQL **unreachable** in v1, so the requirement has no v1 source; §3.2's named consumer is **B1b's SQL builders**, which (c) also makes unreachable, so the seam would ship with **zero v1 consumers**; and it lives in a **second package**, so building it now would land unreachable production code in core-backend under an "already authorized" reading — which is exactly what owner finding P1-1 was raised about. **v1 step 1.4 is therefore plugin-layer and HTTP-only.** The seam re-enters with (b′) + B1c, behind that gate. ⟲B2-self **Credential and connection scope, stated rather than left silent:** this executor unavoidably needs connection material. ⟲OD **TWO MATERIALS, TWO RULES — an earlier draft applied the identity rule to both, which no implementer could satisfy** (you cannot simultaneously "discard the secret" and "complete an authenticated connection"):
...
the three: the **live approved-config validator** change and — until ⟲OD3 — the **bounded core-backend
statement seam**. ⟲OD3 With the seam deferred (Gate bullet 4), the widening's live scope **NARROWS to the
validator change alone**; the seam's unlock travels with (b′) + B1c and must be re-approved there. Narrowing
an approved scope needs no new authorization, but it is recorded because a reader comparing the approved
three-item list against this document must not conclude an item went missing silently. The
...
- Probe-execution seam note: probe SQL uses `GROUP BY`/`HAVING` shapes the structured facade cannot express, so a **restricted internal probe-executor seam** is needed — accepts ONLY strategy-minted statements (object-identity trust), read-only, statement-bounded, never reachable from HTTP input. Per the owner's dependency direction, core-backend contributes this bounded seam and nothing else; qualification definitions stay in the GIP layer. ⟲OD3 **Which slice owns it: NOT B1a step 1.4 — it defers to (b′) + B1c** (owner, 2026-07-26). Two earlier revisions got the ownership wrong in opposite directions: the first said "B1b needs…", putting the artefact behind a different gate than the one authorizing it; the second moved it into B1a step 1.4's gate. Both were written **before** decision (δ) fixed v1 = (c). Under (c) the seam's justification (SQL `GROUP BY`/`HAVING`) and its consumer (B1b's SQL builders) are **both unreachable in v1**, so assigning it to 1.4 would build a second-package production surface nothing can call. It travels with the SQL path.
...
1. **B1a (REDO)** — the authority substrate. Substeps 1.1-1.5 are the **owner-set order verbatim**;
   ...
   4. **server-bound source executor** — handle and field set derived from the resolution, first-party
      statement builders admitted by **module-private identity** (**B-4**), and a restricted statement seam
      **within the plugin layer**. ⟲OD3 **The bounded core-backend seam of §3.2 is REMOVED from this step**
      (owner, 2026-07-26) — see Gate bullet 4: under (c) both its justification and its consumer are
      unreachable in v1. **Certified source-column translation for `orderingKeySpec` is likewise not v1 work**
      — (c) already establishes that v1 does not reach a SQL source, so the non-existent per-system
      column mapping is not a precondition. **v1 scope of 1.4: a plugin-layer, HTTP-only executor** whose
      handle and field set derive from the resolution (**B-1**), admitting only connector-owned, named,
      certified HTTP probe actions (**B-4**). **B-6**'s translation requirement applies within that certified
      HTTP surface and re-enters in full with (b′) + B1c. **⟲OD RULED (owner, 2026-07-25) — B-6 v1 = option (c);** ⟲B2-self this
