...
- **部署台账（2026-08-12，实际顺序 = 生产先行，非原计划的 staging→prod）**：#4654 合并**自动触发** `Build and Push Docker Images` run **`31615811214`** → 部署 `12f1f8c466` 到生产（`metasheet-backend`），3 条 recovery 迁移成功执行、`/health` build==`12f1f8c466`。其后依次：
  ...
  - **补注（2026-08-21，指纹已变——上两条 run 的 `14c180aa…` 为 pre-fix 历史观测，保留不改写）**：其后新增 migration `zzzz20260821120000_recovery_authority_functions_fix_search_path`（CVE-2018-1058 型 shadow 根治：helper 调用 schema-qualified + 固定 `SET search_path = pg_catalog, public`，并把 proconfig 纳入指纹）改了三个 trigger 函数体，**functions 指纹自该迁移起为 `e4a78f6cc9c993ed5ed7d2c81dfc44b94d844c7fb046160d8d13077208fa2498`（两次全新迁移字节稳定），triggers 指纹 `8c1be0b0…` 不变**（不发 trigger DDL，9/9 仍 DISABLED，行为不变——同锁语义/同 40001 raise）。这两条 run 是当日镜像 `12f1f8c466` 的真实证据，不代表当前指纹。
