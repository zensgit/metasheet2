...
**Date:** 2026-06-28 · **Design-lock:** `multitable-t9w-tier4-undelete-designlock-20260628.md` (#3338, **PROPOSED — built on the recommended forks, subject to ratification**). **Flag default-off** (`MULTITABLE_ENABLE_CONFIG_UNDELETE`). Stacked on Tier 3 un-create (#3335).
...
| Real-DB goldens (a–m) | `multitable-undelete-config-realdb.test.ts`, **registered in `plugin-tests.yml`** | ⏳ run in CI (no local postgres) |
| Coverage | (a) flag-off → 403 · (b) reader → 403 · (c) happy field undelete (row recreated at `before.order`; trailing shifted `+1`; `create` `source='restore'` revision) · (d) happy view undelete · (e) **definition-only** (recreated field has no record values / no `meta_links` / no auto-number) · (f) **id-collision / idempotency** (occupied id → 409, no duplicate) · (g) **plan-drift** (changed trailing set → one generic `PLAN_DRIFT`) · (h) **token/response opacity** (JWT has only `undeleteHash`) · (i) no-oracle (no count) · (j) typed-confirm · (k) **single-txn atomicity** (forced mid-recreate failure → field absent, order-shift rolled back) · (l) audit `create` revision · (m) recreate lands on the revision's `sheet_id` | ⏳ in CI |
| **Honest gaps** | (1) goldens CI-proven, not local — **watch (c)/(k)** (the recreate write + atomicity). (2) **Design-lock #3338 not yet ratified** — built on the recommended forks (definition-only · original-order · lazy); if the owner picks differently the impl adjusts. (3) the field-undelete order re-insertion shifts fields created after the delete (`+1`) — faithful to original position but a behavioral choice (vs append-at-end). (4) **flag-on live smoke** before rollout. | ⬜ stated |

## Scope
**field/view `delete`-undelete only, definition-only.** OUT — each its own decision: permission-undelete (= permission-revert, the held last slice) · `sheet_config` · whole delete-batch reversal · value/link/auto-number recovery (impossible) · cross-base · FE. Enabling `MULTITABLE_ENABLE_CONFIG_UNDELETE` stays a runbook step **after #3338 ratification + #3335 merge**.
