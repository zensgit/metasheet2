...
**AE §9 + §3.5a (#3274, ratified 2026-06-27):** editWindowDays 180 (1..366); target vocab incl `adjusted`; evidence = attachmentId/text + HTTPS-only URL allowlist; closed/archived cycle always 409 fail-closed; notify default-on + skipped-reason audited; source-not-editable = 422; **§3.5a metric table = the proposed normalization (work_minutes preserved by default; `absent→normal` no auto-fill), owner-confirmed 2026-06-27.** Changing `absent→normal` to auto-fill a scheduled workday is a future code-only PR (it is already live on main).
