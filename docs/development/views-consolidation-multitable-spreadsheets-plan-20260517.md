...
**下一步**：Phase B — Grid 退役合并 PR（`/grid` → `/multitable` redirect + 删 GridView + 删 Grid 耦合死代码）。约 1 天，零 K3 风险，能减少约 3500 行代码。后续 Phase C（30 天后）删 `/grid` redirect 本身。
...
| C. /grid redirect removal | ✅ 允许 | 30 天观察后删除 redirect 配置本身 |
...
### Phase C — 删除 `/grid` 路由（30 天观察后）

- **目标**：Phase B 上线 30 天 + 无 404 流量告警 → 删除 `/grid` redirect 配置本身
- **PR 数**：1
- **体量**：XS（删除 1 行路由配置）
- **风险**：极低
- **阶段一**：允许
...
- Phase B 已 merged ≥ 30 天
- 监控显示 `/grid` 命中流量 < 1 次/天（或 0）
- 无内部文档 / wiki / 测试链接报告失效
...
| C | `frontend/grid-route-removal-20260620` | frontend | XS | B + 30 天观察 | 待启动（删 `/grid` redirect 本身） |
...
- **Phase C**（30 天后，XS）：删 `/grid` redirect 本身

**B 之后**：
- C 在 B 上线后 30 天 + 无 404 告警时启动
...
| 阶段 ③ 删 route 本身 | 删除 `/grid` redirect 配置（30 天观察 + 无 404 流量后） | Phase C |
