...
- **状态**：
  ...
  - **Phase H1 已完成**（multitable 首页 home recents / favorites / search / template quickstart / default entry 已合到 main，详见 §11 已实现状态）
  - **Phase B 已完成**（merged via #1642 on 2026-05-18 — `feat(grid): retire /grid + purge Grid-coupled dead code`；`/grid` 现 redirect 到 `/multitable`，GridView + Grid 耦合死代码已删）
  - **Phase H2 已完成**（merged via #1651 on 2026-05-18 — 模板中心独立路由 `/multitable/templates` + `MetaTemplateCard` + `useTemplateInstall` + category 翻译层）
  - **Phase H3 已完成**（merged via #1655 on 2026-05-19 — 静态模板库 3→8 个行业模板 + 参数化质量 harness）
  - **多维表 H 系列收官**：首页（最近/收藏/搜索）→ 模板中心（8 模板/分类/搜索）→ install → 新 base 的完整"对标飞书"上手路径已落地
  - **当前阶段：观察 H 系列线上效果**（不立即开 Phase E1；等真实使用反馈再决定针对性增强）
  - **Next executable（非阻塞）**: Phase C 前期准备（access-log 命中量核对 SOP/脚本，为 2026-05-25 触发就绪）。Phase C 删除本体仍受观察窗口硬约束
  - Phase E（ViewManager / legacy views 选择性退役）必须独立 PR，**不可与其它 phase 混合**（ViewManager 仍被多维表 contract test 守护，参 §0.2）；价值密度低于消化 H 系列反馈，暂不排
...
**当前状态**：Phase A（#1639）、Phase B（#1642）、Phase H1、Phase H2（#1651）、Phase H3（#1655）**全部已完成**。多维表"对标飞书"上手路径收官：首页（最近/收藏/搜索）→ 模板中心（8 模板/分类/搜索）→ install → 新 base，模板库有参数化质量 harness 守护。
**下一步**：**先观察 H 系列线上效果**，不立即开 Phase E1。唯一非阻塞推进项 = Phase C **前期准备**（access-log 命中量核对 SOP/脚本）；Phase C 删除本体仍受 ≥ 2026-05-25 + 命中归零观察窗口硬约束。Phase D/E/F 暂缓（详见头部状态块与 §6）。
...
> **2026-05-18 (5) 调整**：原 30 天观察期为保守默认值。本项目无大规模生产用户群（K3 PoC 外无外部用户），且 Phase A (#1639) 已将 `/grid` 从 nav 移除，新用户无暴露面。观察窗口由 30 天缩短为 **≥ 7 天 + 数据/质量信号双轨**。`retireBy: '2026-06-30'` meta 保持不变作为 maintainer review 提醒日期（详见 2026-05-18 (6) 调整）。
...
**Maintainer review 触发点**（C）：

- **C. `meta.retireBy = 2026-06-30`**：到达此日期时**触发 maintainer review**，**不**做无条件自动删除。Review 内容：
  - 检查 access log：仍有有效 `/grid` 命中？
  - 若 **无** 有效命中 → 启动 Phase C 删 redirect
  - 若 **仍有** 有效命中 → 显式决定：(i) 延长观察窗口 + 调整 `retireBy` 到新日期，或 (ii) 仍删除并接受短期 404 风险，并通知命中来源（书签持有者 / 旧脚本作者）

> **2026-05-18 (6) 调整**：原 C 写为"不晚于 retireBy（不管 A/B）"——这与"数据驱动观察"原则冲突。新版 C 是 review trigger，不是 auto-delete trigger。`apps/web/src/router/appRoutes.ts:58` 的 `retireBy: '2026-06-30'` meta 字段保持不变（作为 review 提醒日期），但其语义从"硬性删除日"调整为"maintainer review 日"。
...
| B | `frontend/grid-retirement-20260519` | frontend | M | A | ✅ **已完成** via #1642 on 2026-05-18（合并原 B + C1 = `/grid` redirect + 删 GridView + 删 Grid 耦合死代码） |
| C | `frontend/grid-route-removal-20260527` | frontend | XS | B + ≥ 7 天观察 + 命中归零 | 待启动（删 `/grid` redirect 本身；2026-06-30 触发 maintainer review，非无条件删除） |
| D | `frontend/spreadsheet-detail-*-20260521` | frontend | S | (无强依赖) | ⏸ **暂缓**；启动前 license audit |
| H1 | `frontend/multitable-home-*` | frontend | M | A | ✅ **已完成**（home recents/favorites/search/template quickstart/default entry 已 merged） |
| H2 | `frontend/multitable-template-center-20260518` | frontend | M | H1 | ✅ **已完成** via #1651 on 2026-05-18（独立 `/multitable/templates` + MetaTemplateCard + useTemplateInstall + category 翻译层） |
| H3 | `contracts/multitable-industry-templates-20260518` | contracts | S | H2 | ✅ **已完成** via #1655 on 2026-05-19（模板库 3→8 + 参数化质量 harness） |
...
**执行顺序（2026-05-19 (7) 修订 — H 系列收官）**：

```
[A]  ✅ #1639 完成 (nav 重命名)
[H1] ✅ 完成 (multitable home recents/favorites/search/template quickstart/default entry)
[B]  ✅ #1642 完成 on 2026-05-18 (Grid 退役 + 配套清理 — /grid → /multitable redirect + 删 GridView + 删 Grid 耦合死代码)
[H2] ✅ #1651 完成 on 2026-05-18 (模板中心独立路由 + MetaTemplateCard + useTemplateInstall + category 翻译层)
[H3] ✅ #1655 完成 on 2026-05-19 (静态模板库 3→8 + 参数化质量 harness)
   │
   └─ 当前：观察 H 系列线上效果（不立即开 E1）
      ├─ C  ⏳ 等观察窗口 (≥ 2026-05-25 + 命中归零；2026-06-30 maintainer review)
      │      非阻塞前期准备：access-log 命中量核对 SOP/脚本
      ├─ E1 ⏸ 暂不排 (legacy 视图前端退役；价值密度低于消化 H 反馈；独立 PR，不可与 C 合并)
      │  └─ E2 (legacy 视图后端退役, E1 + 7 天观察)
...
**B 之后**：
- C 在 B 上线后 ≥ 7 天 + redirect 命中归零（或 ≥ 10 天 + 无失效报告）时启动；2026-06-30 触发 maintainer review，仍有命中时显式决定延长或删除
...
- **2026-05-19 (7)** 多维表 H 系列收官锚点：
  - Phase H2 merged via #1651（模板中心独立路由 + MetaTemplateCard + useTemplateInstall + category 翻译层；CI 11/11 绿）
  - Phase H3 merged via #1655（静态模板库 3→8 + 参数化质量 harness；CI 14/14 绿）
  - H1→H2→H3 完整"对标飞书"上手路径落地（首页 → 模板中心 → install → 新 base）
  - 决策：**先观察 H 系列线上效果**，不立即开 Phase E1（清理项价值密度低于消化真实反馈）
  - 下一非阻塞步：Phase C 前期准备（access-log 命中量核对 SOP/脚本）；Phase C 删除本体仍受 ≥ 2026-05-25 + 命中归零观察窗口硬约束
  - §4 PR 表 H1/H2/H3 标记已完成；头部状态块同步
...
- **2026-05-18 (5)** Phase C 观察窗口从 30 天缩短为 "≥ 7 天 + redirect 命中归零（或 ≥ 10 天 + 无失效报告）"。理由：本项目无大规模生产用户群，#1639 已将 `/grid` 从 nav 移除，新用户无暴露面；30 天为保守默认，对本场景过度保守。`meta.retireBy = 2026-06-30` 保持不变（详见 (6)）。
- **2026-05-18 (6)** Phase C 触发条件 C 的语义修正：原写"不晚于 retireBy（不管 A/B 是否满足）"与"数据驱动"原则冲突。新版 C 是 **maintainer review 触发点**，**不**做无条件自动删除。若到 2026-06-30 仍有有效 `/grid` 命中，需显式决定延长观察窗口或接受短期 404 风险后删除。`apps/web/src/router/appRoutes.ts` 的 `retireBy` meta 字段保持不变（语义从"硬性删除日"调整为"review 提醒日"）。同 commit 同步 Phase B 状态 → 已完成 via #1642。
...
| 阶段 ③ 删 route 本身 | 删除 `/grid` redirect 配置（≥ 7 天观察 + redirect 命中归零 / 无失效报告；2026-06-30 触发 maintainer review，非无条件删除） | Phase C |
