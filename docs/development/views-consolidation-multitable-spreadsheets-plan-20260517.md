...
- **状态**：
  ...
  - **Next executable phases**: Phase C（`/grid` redirect 删除，见 §3 Phase C 触发条件）/ Phase H2（模板中心独立路由 + UI）/ Phase E1（legacy 视图前端退役）—— 三者可并行，独立 PR
  - Phase E（ViewManager / legacy views 选择性退役）必须独立 PR，**不可与其它 phase 混合**（ViewManager 仍被多维表 contract test 守护，参 §0.2）
