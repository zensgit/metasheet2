...
- **A6-3 … A6-5 ADD capability** (branch/parallel graph execution, modeling import,
  ...
  A6-3-3 branch-local wait **landed via #2626 (backend) + #2702 (frontend)**
  under scope gate `multitable-automation-a6-3-3-branch-local-wait-scope-gate-20260615.md`;
  nested branches and A6-3-5 join-any stay
  ...
  **landed via #2577**, and the A6-4c read-only UI **landed via #2604**. Live BPMN
...
> shows selected branch `label (key)` plus branch child jobs. **A6-3-3 branch-local wait LANDED** (#2626 backend `8b0468920` + #2702 frontend `20da674c8`)
> under scope gate
> `multitable-automation-a6-3-3-branch-local-wait-scope-gate-20260615.md`. Nested `condition_branch` inside branches and
...
> jobs. A6-3-3 branch-local wait now LANDED end-to-end (A6-3-3a backend #2626 `8b0468920` + A6-3-3b frontend #2702 `20da674c8`). **Still deferred:**
> A6-3-5 join-any/cancellation, nested `condition_branch` inside branches, public webhook/token emitter, live BPMN runtime (note: the legacy
...
- **Gate:** A6-3-4/W3 `join_all` landed after named opt-in (#2496/#2500/#2501).
  A6-3-3 branch-local wait LANDED (#2626 backend + #2702 frontend). `join_any`, nested branch shapes beyond
...
## 4. A6-4 BPMN compile/preview adapter — A6-4a/A6-4b/A6-4c LANDED, live runtime deferred
...
- **A6-4b landed:** #2577 `b5646fa71` adds the read-only
  `POST /api/workflow-designer/workflows/:id/compile-preview` route. It loads
  visual or BPMN XML drafts read-only, preserves `sourceMode`, calls the pure
  compiler, and does not persist, deploy, start, test-run, or invoke live BPMN
  runtime.
- **A6-4c landed:** #2604 adds the read-only Workflow Designer compile-preview
  UI (a dialog consuming the A6-4b route): source mode, supported status,
  backend-redacted automation actions + approval graph as collapsible JSON,
  mapping/gap reports, warnings. No deploy/start/persist; a request-id composable
  drops stale out-of-order responses.
...
- **Gate:** A6-4c read-only UI landed #2604. Any further A6-4 work (live BPMN runtime, BPMN create-from-preview) still requires a named modeling/preview demand. "Continue automation" is not enough.
...
- A6-1 enable-writer **landed** (#2130/#2191/#2193) and A6-2 suspend/resume **landed**
  ...
  A6-3-3 branch-local wait **landed** (#2626 + #2702). A6-3-5 join-any, A6-4 live BPMN
  runtime (A6-4a/b/c compile-preview landed), public webhook/token emitter, and W7 result backwrite
