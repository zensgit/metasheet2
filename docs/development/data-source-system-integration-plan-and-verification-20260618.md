...
- **S3(模版对象)= 设计锁 + S3-1 + S3-1b + S3-2 已落地**:S3-1 合同/存储(#2919:`integration_templates` 表/migration 061 + CRUD)+ S3-1b 版本语义(#2926:编辑 auto-bump + optimistic 409)+ **S3-2 实例化**(#2937:单事务 bind→pipeline+mappings + snapshot provenance,无外部写)。**仅 S3-3(参考模板)仍 gated**。
...
| S3 | first-class `integration_templates` 对象 | 🟢 design-lock + **S3-1 + S3-1b + S3-2 已落地**;仅 S3-3(参考模板)gated | #2919 + #2926 + **#2937(`289778700`)** + 设计锁 |
...
| **S3-3(参考模板)** | opt-in | 设计锁 §3 已 owner 裁定。**S3-1/S3-1b/S3-2 已落地**(#2919/#2926/#2937:表+CRUD+版本语义+单事务 bind 实例化+snapshot provenance,无外部写)。**Q4 实测裁定 = bind**(声明式模板无 sheetId,自动建系统不可行)。S3-3 = 参考模板示例(multitable 先行;K3 模板可 bind 但 runtime 写仍等 S2) |
