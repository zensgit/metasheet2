...
- **S3(模版对象)= 全弧完成**:设计锁 + S3-1 合同/存储(#2919:`integration_templates` 表/migration 061 + CRUD)+ S3-1b 版本语义(#2926:编辑 auto-bump + optimistic 409)+ S3-2 实例化(#2937:单事务 bind→pipeline+mappings + snapshot provenance,无外部写)+ **S3-3 参考模板目录**(#2944:只读 `GET /templates/references` + values-free 常量,operator 经现有 upsert opt-in 复制,无新写路)。S3 已无 gated 子项。
...
| S3 | first-class `integration_templates` 对象(全弧) | ✅ done — 设计锁 + S3-1 + S3-1b + S3-2 + **S3-3**(无 gated 子项) | #2919 + #2926 + #2937 + **#2944(`71628e783`)** + 设计锁 |
...
> S4(自描述元数据,#2903)与 **S3 全弧(S3-1/S3-1b/S3-2/S3-3,#2919/#2926/#2937/#2944)** 均已落地,从本"剩余 gated"表移除——见 §1 阶梯。Q4(实例化系统 create-vs-bind)实测裁定 = **bind**(声明式模板无 sheetId,自动建系统不可行)。
