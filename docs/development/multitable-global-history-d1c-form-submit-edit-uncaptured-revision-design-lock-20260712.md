...
- **Status**: **DECISIONS RECORDED (owner 2026-07-13) — NOT YET RATIFIED; verdict = REQUEST CHANGES.** The owner ruled OD-1…OD-6 (§0.5) but was explicit that the lock **cannot be ratified yet**: 「当前设计锁还不能直接 ratify，需要先把以下裁决和"既有污染历史预检"写进去，再复审一次。」 Two things had to land before ratification: (1) the **`HISTORY_INCOMPLETE` fail-closed integrity precheck** (§0.6 — the owner's headline hard-lock) — **DONE**, and (2) an **independent re-review** — **DONE 2026-07-13** (`/tmp/pr4187-rereview-claude-20260713.md`): verdict NOT-READY with 2 P1 + 1 P2 against §0.6/§5, **all three folded back into this text** (§0.6 item 2 user-authored-field projection + item 3 zero-revision rule + goldens G-HI-3/G-HI-4; §5 OD-4 cell reconciled). The re-review otherwise verified: OD encodings faithful, A1–A8 audit accurate **8/8**, 5-slice grouping consistent across §0.5/§7a/§10, execute-TOCTOU re-check necessary (the `previewIdentity` scopeHash is revision-derived and structurally blind to uncaptured writes). **The lock is now, per the re-review, complete enough to ratify — but implementation stays NOT authorized until the owner ratifies.** *(An earlier revision of this header read "RATIFIED … three parallel lanes (A/B/C)" — that was premature on both counts: the owner's verdict is REQUEST CHANGES, and OD-1 is **five** slices, not three — corrected below. Original PROPOSED framing preserved for provenance; §0.5/§0.6 are authoritative where they differ.)*
...
5. **Real-DB goldens (all four mandatory, before ratify):**
