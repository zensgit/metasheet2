...
- **Status**: **RATIFIED — 2026-07-13 (owner), as REVISED.** Superseding the 2026-07-12 PROPOSED draft. The owner ruled OD-1…OD-6 (see **§0.5**), and an independent revision-disposition audit (2026-07-13, `/tmp/r13-revision-disposition-audit-20260713.md`) **confirmed the 8-site content class, corrected two draft errors** (the OD-4 link-branch "patch empty" claim and the §0 CREATE "non-destructive" claim — both were wrong), and produced the OD-6 guard matrix. Implementation is **authorized** under the ratified terms, as **three parallel lanes (A/B/C) each with per-lane transaction-boundary proof** + the OD-6 guard as a separate rung. This is R13-A of the R13/R14 line. *(Original PROPOSED framing preserved below for provenance; the §0.5 rulings are authoritative where they differ.)*

## §0.5 RATIFIED owner rulings + audit confirmation (2026-07-13)
...
|---|---|---|
| **OD-1** scope | **Full bucket-A A1–A8 sweep, split into 3 cuts (lanes A/B/C).** Not EDIT-only. | Lane **A** = form CREATE (`14470`) + EDIT (`14423`) + attachment-delete (`15693`); Lane **B** = plugin create (`records.ts:546`) + update (`507`); Lane **C** = automation create (`2475`) + update (`2217`) + resultWriteback (`automation-service.ts:2818`). 3+2+3 = 8. |
| **OD-2** source | **By WRITE SURFACE**, not actor auth-level: `'public-form'` (form CREATE/EDIT), `'plugin'`, `'automation'`; **attachment-delete uses `'rest'`**. | resolves the draft's "misleading name" caveat: source describes the surface. |
| **OD-3** actor | **Known actor MUST be recorded; anonymous public-form CREATE → `actorId = null`.** | audit: form CREATE/EDIT is anonymous-reachable (`created_by/modified_by` NULL on public path); plugin/automation actor per token/context. |
| **OD-4** link edges | **IN SCOPE — the draft's "link-only edit ⇒ patch empty ⇒ no UPDATE" is WRONG.** Audit: `patch[fieldId] = ids` at `univer-meta.ts:14306` → enters the UPDATE, bumps version — a real data mutation. Lane A revision snapshot MUST include the link ids; add a **revision-snapshot ↔ `meta_links` restoration-consistency golden**. Do NOT defer. | corrects §5 OD-4 below. |
...
**Per-lane proof obligation (all three lanes):** real entry-point golden · same-txn `recordRecordRevision` with **full snapshot** · PIT-correct after fix · failure rolls back (no half-write) · **source mutation → test red**. Transaction-boundary is **re-verified per lane** (D-1「偏差1」: these lanes were not uniformly transactional; D-2 §0 says they now are — prove it per lane, don't assume).
