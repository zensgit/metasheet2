...
- **Provenance**: independent audit finding, then **verified primary-source against `origin/main` @ `087ffa47a`** and **reproduced end-to-end on real Postgres 14** (§3). Every file:line below was read; every behavioural claim below was executed, not inferred.
...
## §2 Blast radius (measured through the real routes — §3, not inferred)
...
This is **verified**, not projected: §3, leg 6 — a `LIKE '%v2-edited-via-form%'` scan across `snapshot` **and** `patch` of every revision for that record after the revert returns **0 rows**.
...
**Unlock phrasing, for reference** (mirroring D-1 §8): 「ratify D-1c EDIT-only」/「ratify D-1c + CREATE(D-1b)」/「ratify D-1c + 委托 revision-disposition guard」.
