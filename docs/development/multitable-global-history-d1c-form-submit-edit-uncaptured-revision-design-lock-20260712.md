...
- **Status**: **DECISIONS RECORDED (owner 2026-07-13) — NOT YET RATIFIED; verdict = REQUEST CHANGES.** The owner ruled OD-1…OD-6 (§0.5) but was explicit that the lock **cannot be ratified yet**: 「当前设计锁还不能直接 ratify，需要先把以下裁决和"既有污染历史预检"写进去，再复审一次。」 Two things must land before ratification: (1) the **`HISTORY_INCOMPLETE` fail-closed integrity precheck** (§0.6 — the owner's headline hard-lock), and (2) an **independent re-review** of this revised lock. Implementation is **NOT authorized** until the owner ratifies post-re-review. *(An earlier revision of this header read "RATIFIED … three parallel lanes (A/B/C)" — that was premature on both counts: the owner's verdict is REQUEST CHANGES, and OD-1 is **five** slices, not three — corrected below. Original PROPOSED framing preserved for provenance; §0.5/§0.6 are authoritative where they differ.)*

## §0.5 Owner rulings (RECORDED 2026-07-13; lock not yet ratified — see Status) + audit confirmation
...
2. **Compare CONTENT, not version.** The precheck compares the **authoritative data content**, not merely `version`. A **version-only** change that leaves the data identical — e.g. lock / unlock, which bump version via a lock-disposition path without changing `data` — **must NOT false-positive** into `HISTORY_INCOMPLETE`. (The whole point of the uncaptured-write bug is that `version` moved but no revision recorded the `data`; so `version` alone is not the signal — the *data ≠ snapshot* comparison is.)

3. **Execute re-checks.** `execute` **must re-run the precheck**, not trust the preview's result — an uncaptured write can appear in the window **between** preview and execute (TOCTOU). A preview that passed does not license an execute that skips the check.

4. **Real-DB goldens (both mandatory, before ratify):**
   - **Polluted → refused, zero writes.** A record whose live `data` disagrees with its last revision snapshot ⇒ **both** preview **and** execute return `HISTORY_INCOMPLETE`, **and a post-assertion proves zero rows were written / no token minted** (not just that the response was an error — prove the DB is untouched).
   - **Version-only, data-consistent → NOT refused.** A record whose `version` moved by a lock/unlock (data identical to snapshot) ⇒ the precheck **passes** (no false `HISTORY_INCOMPLETE`), and the revert/reset proceeds normally. This is the false-positive guard, and it must be a *positive control* — without it, a precheck that refuses everything would pass the polluted-case golden while being useless.
...
**What §0.6 is NOT:** not a backfill (OD-5 forbids that); not a `version`-equality check (item 2); not preview-only (item 3). It refuses to act on untrustworthy history; it does not try to reconstruct it.
...
| **OD-4** | **`meta_links` side-effects — ⚠️ RULED IN SCOPE (owner 2026-07-13); the draft claim below was WRONG** | ~~A link-only form edit (`patch` empty ⇒ no `UPDATE`, no version bump)~~ — **FALSE.** Audit (2026-07-13, source-verified): the link branch runs `patch[fieldId] = ids` at `univer-meta.ts:14306`, so `patch` is **non-empty** ⇒ control **enters** the UPDATE at `:14423`, writes the ids into `data`, and **bumps version**. A link edit **is** a real `meta_records.data` mutation, and it is UNCAPTURED like every other bucket-A site. | **IN SCOPE for lane A (OD-4 ruling).** The revision snapshot MUST include the link-field ids (they are part of `data`), and lane A MUST carry a **revision-snapshot ↔ `meta_links` restoration-consistency golden** (restore/PIT reproduces both the cell ids and the `meta_links` edges consistently). Do NOT defer. |
