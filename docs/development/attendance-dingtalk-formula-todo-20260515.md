...
- [ ] multi-punch ingest-persist（独立 P2）：import 已解析 `clockIn2/3`/`clockOut2/3` 但仅作 engine.evaluate 临时输入未落库（3+ site 无 chokepoint）；持久化进 `attendance_records.meta` 后上面 slot-2/3 + 全部 result 字段自动点亮，无需再改 catalog 形状。
- [ ] 内联公式编辑器与函数参考面板。
- [ ] 公式字段依赖图与循环检测。
- [ ] 周期汇总级公式。
- [ ] 请假/加班子类型拆分字段。
