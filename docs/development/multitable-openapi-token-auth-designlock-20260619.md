...
1. **OAPI-1** `requireScope` guard + mount `apiTokenAuth` on the **read** routes only (`records:read`, `fields:read`, `comments:read`) — read-only blast radius first. Real-DB golden: valid-scope 200, missing-scope 403, revoked 401, creator-permission-still-applies (denied row/field stays denied via token).
   ...
   - **`comments:read` (narrow set, this slice)**: `GET /api/comments`, `/api/comments/summary`, `/api/multitable/:spreadsheetId/comments/presence`. **DONE** — composes `apiTokenAuth → requireScope('comments:read') → rbacGuard('comments','read') → ensureSheetReadable` = **min(token scope, creator RBAC, creator per-sheet read)** (the existing `rbacGuard` is NOT replaced — a token whose creator lacks the comments RBAC still 403s); presence **forces `includeViewers=false`** on the token path (no viewer-identity egress); the per-user surfaces (`inbox`/`unread-count`/`mention-candidates`/`mention-summary`) stay **deferred**. Verified end-to-end through the full server (incl. the gate): wrong-scope→403, revoked/no-token→401, creator-RBAC-still-applies→403, and an over-match on a deferred route (`/api/comments/inbox`)→401 (no bypass).
     - **G-8 / decision A′ amendment (2026-07-06)**: the original contract was `min(token scope, creator RBAC)` — no sheet-read term — which let a `comments:read`/`comments:write` token read/write comments on a sheet its creator could NOT read (the token-flavored version of the G-8 cross-sheet comment-visibility leak). Decision A (enforce per-sheet read on the comments surface) therefore extends to the token path: the `ensureSheetReadable` gate now applies here too, so the composition gains a **per-sheet read** term. Effect: a comment token whose creator holds the comments RBAC but no per-sheet read (no `multitable:read` / sheet grant) now **403s** (previously 200/201). Goldens updated accordingly (`multitable-oapi1-comments-read-realdb.test.ts` adds the no-sheet-read→403 case; `-oapi2a-` grants the writer sheet-read for its 2xx path).
