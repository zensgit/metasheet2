...
> **范围 = canonical 计划授权的只读 MVP（T0 ratify + T1 + T1b + T4 + T2a/T3），已落 `main`。** T2a = 最小时间线（actor/source/action 筛选 + 可展开明细 + 入口/空/加载/错误态）；**T2b（时间区间/sheet/字段筛选 #2988、可见数据搜索 #2989、cursor 分页 #2990）= 全部 SHIPPED 2026-06-21**（只读、在 MVP 范围内、未 gated；搜索 post-mask leak-free + searchTruncated 透出，cursor `total` 保持 post-filter）。restore/PIT/config（T5/T6/T8/T9）按计划保持 gated（T5 design-lock #2985 已合，runtime 待 owner ratify D1/D2/D5；其余各自独立 opt-in）。
...
| **T2b** | 时间区间/sheet/字段筛选 #2988、可见数据搜索 #2989（post-mask leak-free + searchTruncated）、cursor 分页 #2990（post-filter total） | ✅ SHIPPED 2026-06-21（详见 `…-t2b-filters-dev-verification-20260621.md`） |
