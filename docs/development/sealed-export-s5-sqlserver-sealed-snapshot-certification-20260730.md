...
- `sealed-export-package-provenance.cjs` — candidate-tree module/migration/runtime/evidence files + whole-lockfile pins (068–072)
...
- migration `072_harden_integration_sealed_export_terminal_signer_history.sql` — persists terminal signer-key history across rotation/deletion and refuses UPDATE/DELETE/TRUNCATE under ordinary DML
...
| Durable terminal-key history | `integration_sealed_export_terminal_signer_keys` (072) | terminal key ids and status after rotation/deletion; rows are immutable under ordinary DML |
...
Migration 072 retains that terminal identity after rotation or deletion and
refuses UPDATE, DELETE, and TRUNCATE against the history table.
...
- S5 assumes trusted first-party code and ordinary DML. It does not claim
  integrity against an arbitrary SQL-capable application actor, table owner,
  DDL-capable role, or database superuser. Before S6 runtime wiring, the
  migration-owner/runtime-role split and the only-authorized history append
  path require a separate ratified privilege contract and real-Postgres tests.
...
- The workflow uses real SQL Server snapshot transactions and proves a
  ...
  gate-check also runs migration 057 plus migrations 068–072 against
...
- Repository-frozen provenance pins prove candidate-tree consistency only.
  They bind the runtime module set, migrations, transitive dependency closure
  through the whole `pnpm-lock.yaml` digest, and the workflow/runner/verifier/
  real-DB evidence files. Evidence includes the frozen-manifest digest and
  explicitly requires an independent package/release pin before S6 delivery.
