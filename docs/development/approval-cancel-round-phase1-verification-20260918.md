...
**全量那一行的对象,点名(不含糊)**:它是在 **P3 的两处注释修改之前**的文件上跑的
(那次修改**只动注释**,零可执行语句)。`tsc` 与七件真库件**已在交付 head 上重跑**、读数逐字相同
(`EXIT 0 / 0 行`;`7 files / 98 passed`);全量 1124 件未再跑一遍,所以这里**写明它属于哪个对象**,
而不是让它看起来像交付 head 的读数 —— 这就是门审第 4 轮 NIT 与第 3 轮 NIT-1 的同一条纪律。
...
**这九格是在上面那个 sha 的文件上跑的 —— 实测,不是声称**:网格第一次是在 P3 注释修改**之前**的文件上跑的,
发现之后**用交付文件重新跑了全部九格**,读数**逐格相同**(锚点是字符串匹配,P3 的改动只碰注释),
跑完 `shasum` 回到 `dd8ac540…`。上一轮的 NIT 正是「网格的数属于上一个对象」,本轮不靠「应该没影响」收口。
