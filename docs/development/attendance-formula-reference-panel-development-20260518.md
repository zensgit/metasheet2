...
3. 引用范围可切换的 toggle（Record / Summary），文案随之变化，并且 chips 内容也随之切换
4. 内联编辑器内提示用户去看上方参考面板的辅助行

Chips 策略：
- 静态 chips 通过 `formulaStaticReferenceChips` computed 按 scope 切换：
  - record：`{field_code}` / `{late_duration}` / `{leave_type_annual_duration}` / `{total_minutes}`
  - summary：`{total_minutes}` / `{leave_minutes}` / `{overtime_minutes}` / `{work_duration}` / `{late_duration}` / `{early_leave_duration}`（documented summary aliases，最终由后端 Preview 校验）
- catalog-derived chips（`formulaReferenceableCodes`）仅在 record scope 输出，summary scope 直接返回 `[]`（避免把 per-record 字段 code 引导到 summary 公式里被后端拒）
- 在有 field filter 时静态 chips 仍展示，catalog-derived 被抑制（既有 `hasActiveFieldFilters` 模式，避免污染搜索测试的全文断言）
...
- `formulaStaticReferenceChips` computed：scope-aware 静态 chip 列表（record 4 / summary 6）
- `formulaReferenceableCodes` computed：summary scope 直接返回 `[]`；record scope 从 `reportFields` 派生「enabled 且非 formula 且不在当前 static 集合」的 code 列表

**Template 增量（4 处）：**
1. **Field references 块**（lines 159-190）：在标签下、chips 上方新增 toggle（两个按钮，`aria-pressed` 反映当前 scope）+ 一句 scope hint；静态 chips 改为 `v-for over formulaStaticReferenceChips`（scope-aware 内容）；其后动态追加目录派生 chips，summary scope 与 `hasActiveFieldFilters=true` 都使其为空
...
`apps/web/tests/AttendanceReportFieldsSection.spec.ts` 新增 7 项（既有 13 → 20）：
...
7. `switches static chips to summary aliases and hides catalog-derived chips in summary scope`
   - record 默认下 `[data-report-field-formula-static-chip]` = `[field_code, late_duration, leave_type_annual_duration, total_minutes]`,catalog-derived chips 数量 > 0
   - 切到 summary：`[data-report-field-formula-static-chip]` = `[total_minutes, leave_minutes, overtime_minutes, work_duration, late_duration, early_leave_duration]`,catalog-derived chips 数量 = 0
   - 切回 record：静态 chips 恢复为 record 集合,catalog-derived chips 重新出现
   - summary hint 含 `summary aliases` + `Preview validates`——文案显式把校验权交给后端
