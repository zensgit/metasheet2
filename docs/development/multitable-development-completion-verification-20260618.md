...
Per the current authoritative plan (#2828), updated for the work shipped since: **the multitable mainline has no non-gated residual runtime work, and the gated arcs have now largely shipped.** 2a (live CRDT — full scalar set) is COMPLETE (#2832/#2838/#2849); 2b (conditional permission rules) shipped through S3 (#2836/#2841/#2847) with only **S4 (perf/caching) demand-gated**; 2c #16 person directory shipped its **fail-closed assignment enforcement** (#2833 + route-parity hardening #2854), leaving only the directory-source-of-truth decision + picker-UX as gated. Everything from #2801's P3–P7 list remains a **future roadmap pool** (#2828), not active remainder.

So "complete all development" resolves to: the planned arcs are **built + verified + on main**; what remains is **demand-gated** (2b-S4 perf, only on a real large-rule-set need) and **owner-decision-gated** (2c directory source + picker UX) — neither to be built autonomously. (§2/§6 below are updated to this state; the original snapshot is preserved in git history.)
...
| **2b · #18 phase-2 conditional permission rules — SHIPPED through S3** | Dynamic field-predicate read-deny on top of static #18 | **DONE: S1 evaluator #2836 · S2 enforcement #2841** (wired into the #18 `loadDeniedRecordIds` seam, flag-off inert, fail-closed, admin-bypass, no cardinality leak) **· S3 authoring UI + API #2847.** S0 rule/threat model is settled by those. Only **2b-S4 (perf/caching)** remains — **demand-gated** (do only on a real large-rule-set perf need). |
| **2c · #16 Person field → org-member directory — enforcement SHIPPED** | Deepen the person field into a real org directory | **DONE: fail-closed assignment validator** (#2833) + **route-parity hardening across all four write paths** (#2854, real-PG verified). `restrictToMemberGroupIds` is now a hard rule (sheet members ∩ allowed groups, grandfathered). **Remaining (gated):** the directory source-of-truth decision (internal `users` / directory-synced / external IdP) + picker-UX filtering + inactive/deleted-user policy. |
...
The planned arcs are built, verified, and on main: 2a live CRDT (full scalar set, #2832/#2838/#2849), 2b conditional rules through S3 (#2836/#2841/#2847), and 2c #16 person-directory enforcement (#2833 + #2854). What remains is **not** a fresh gated arc to open but a small set of **demand-gated tails**: **2b-S4** (rule-eval perf/caching — only on a real large-rule-set need), **2c** directory-source-of-truth decision + picker-UX, and **#17** grid-virtualization (verdicted not-needed by the D2 baseline — reopen only on a higher-scale re-baseline). None is autonomously buildable; each needs an explicit owner opt-in + the listed decisions. I will not start any of them without that opt-in.
