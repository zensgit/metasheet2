...
|---|---|---|
| **2a · Live CRDT remaining field types** (`select` / `date` / `dateTime` / `duration`) | Open these field types to live collaborative editing | **Persisted-doc migration gate.** Old Yjs docs hold these keys as `Y.Text`; flipping to plain-value LWW risks corruption. Owner must choose: migration strategy (lazy / offline / dual-reader), rollout compatibility, date/dateTime semantics (no TZ drift), and a real-DB corruption golden. Slices 2a-S1..S4 (+ duration 2a-D1..D3) only after the gate opens. |
