...
### 2.2 发放锚点 = 自然年
1/1 发放；**首年按入职/服务起算日折算**（按当年剩余日历天数折算 entitlement）。入职周年制 v1 不做。

### 2.3 工龄阶梯 + 口径
- **默认 preset（法定，可配）**：累计工作满 1–10 年 = **5 天**；满 10–20 年 = **10 天**；满 20 年以上 = **15 天**；不满 1 年按折算（owner 已对《职工带薪年休假条例》《企业职工带薪年休假实施办法》校准）。org 可改阶梯。
...
- **结转/过期边界是"日历年末"，不是 C4 的 `granted_at + N×24h`** → 用 **org 时区的年末时间戳**（如 `次年 local 12-31 23:59:59.999`）。时区来源 = `annualLeavePolicy.timezone`，默认继承 attendance default rule / org timezone；**缺失 → 拒绝启用，或 fallback UTC 但必须可见（不隐式猜）**。
- expire event `source_type`：`comp_time_expiry` 不变，annual 用 `annual_leave_expiry`。
- 法规允许"确有必要时跨 1 个年度安排"，故默认结转合理，但必须可配（org 可设 use-it-or-lose-it = 当年末）。
...
- **`attendance_leave_accrual_run_items`**（per-user）：`run_id`、`user_id`、`tenure_years`、`tier_days`、`proration_factor`、`entitlement_minutes`、`status`（granted / skipped）、`skip_reason`（如 `MISSING_SERVICE_START_DATE`）。
- lot 的 `source_id` → run-item（`source_type='annual_accrual'`、`source_key='annual_accrual:{user}:{period_key}'`）。
- **manual adjustment 只能纠偏、不能替代原始计算证据**：写独立 grant/deduct event（`source_type='annual_manual_adjust'`）引用 run，但不覆盖 run-item 的原始快照。
...
2. **工龄正确性**：跨 1/10/20 年界阶梯 + 首年折算正确；**缺累计工龄起算字段 → skip + 可见原因**（不顶替 hire_date、不静默发错）。
3. **snapshot limitation 显式**：文档写明 + manual adjustment 纠偏路径可用且留独立证据。
4. **扣减**：`standard_day` 基准、不足 block、**comp_time 路径不受影响**。
5. **结转/过期**：org 时区年末边界、`annual_leave_expiry` event、重复 tick 不重复。
...
- **法规来源（默认 preset 依据）**：《职工带薪年休假条例》《企业职工带薪年休假实施办法》。
