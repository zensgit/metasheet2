...
3. **风险形态最可控（但理由已按 F3 复核收窄）。** C 的主要成本是**一次性环境搭建**
   （SQL Server + 本机 Agent），A 的主要成本是**动冻结的读预设上限**——后者是不可逆的安全决策，
   前者只是工时。
   ⚠️ 原文还写过「C 之外两条的 n8n 跳是动授权边界」并把它算作 C 的优势——**这条已删除**：
   按统一后的四条轴，**「读取已落库结果」三个场景都不需要新端点**，而**「触发源同步」三个场景
   都要过各自的 JWT 边界**（A/B 走 `/api/integration/stock-preparation/mvp/source-runs/*`，
   C 走 integration JWT，见 C.6）。授权边界不是区分 A/B 与 C 的轴。
...
且可用本机 PG 假 BOM 表替掉 mock PLM，环境最轻。
⚠️ 原文此处写「代价是 n8n 那一跳在 B 上几乎全是『无』」——**已删除，该表述不准确**：B 落库的
MVP 表本身就是真实多维表（`stock-preparation-erp-material-sync-persist.cjs:253-277,474-475`、
`stock-preparation-sync-run-persist.cjs:154-182,634-636` 解析 sheetId 并绑 records API），
**n8n 读取结果这一跳 B 与 C 等价**；B 与 C 的实际差异在**源侧连接器档位**与**触发路径**，
不在 n8n 能力有无。
