...
11. **B2 error routing — acknowledgement (2c data flow, additive)**
    「知悉（2026-09-05 PR-B2 第二轮修订，取代第一轮措辞）：`patchCell` 新增按调用返回的结构化拒绝 `GridPatchFailure | null`（回滚与 `error.value` 不变；成功与本地行权限拒绝返回 null；不再有共享的「最近一次失败」引用）；`onDrawerPatch` 仅在 `fieldErrors` 非空、或 HTTP 状态为 422、或错误码为 `VALIDATION_ERROR` 时改为字段内联提示——按错误码而非按状态：HTTP 400 携带其他错误码（如记录锁定的 400 `FORBIDDEN`）沿用 toast——且仅当检查器确认该字段告警此刻可渲染（`canAnchorFieldError`：可见、正显示该记录、details tab 激活、该字段行已渲染）且提示文本非空，否则回落 toast；`VERSION_CONFLICT` 显示既有冲突横幅 + 字段标记并保留其 toast；其余错误码沿用 toast，记为行为变更。」
