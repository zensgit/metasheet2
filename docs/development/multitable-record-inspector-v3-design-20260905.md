...
Tab order after PR-A (P3-3, 2026-09-05 correction: this line previously read "title → prev → next → …", which contradicts both the mock's own §1.6 layout and the shipped DOM — Row A, the toolbar, precedes Row B, the title, in both): prev → next → comment chip → copy-link → expand → kebab → close → title → tabs → hide-empty → fields → splitter.
