...
- **Sections** (new `MetaRecordFieldSection.vue`, button `aria-expanded`/`aria-controls`, ids off `useId()`): §1 = view-ordered fields (`grid.visibleFields ∩ layer-3 ∩ layer-2`, via useMultitableGrid L522–537 order, fail-soft on stale ids); §1 is always headerless and always expanded (PR-B1 round 2 correction, 2026-09-05: the §1.6 mocks show the in-view fields directly under the tabs bar with no §1 heading — an earlier draft of this sentence read "expanded, headerless when §2 is empty", which round 1 implemented as a collapsible "Fields in this view" heading; struck); §2 = "hidden in this view" (`recordHiddenFieldsHeading(count,isZh)`), the only headed section, collapsed by default, component-local session-only state — held by the fields panel keyed by section key, not by the section component, so an expanded §2 comes back expanded after hide-empty empties it and unmounts the section (PR-B1 round 3, 2026-09-05; nothing persisted, reset on panel remount). Passed as new optional prop `inspectorFieldLayout: { ordered, hiddenInView }` WB→INS→FP; absent prop → today's flat `props.fields` path (deprecated `MetaRecordDrawer` consumers, router-less specs). Applying layer-2 here closes P8 — declared behaviour change with negative golden N3.
...
- **Hide empty**: session-only (OD-W2-2). Predicate = the existing `isEmptyValue` in `conditional-formatting.ts:147`, **exported and also called by field-display.ts's empty-glyph branch (L106–115)** so glyph and filter share one definition (no second predicate). Never hides: primary field, focused field, field with AI status/error, field with comment presence, field with a pending server error. Hidden = (empty at snapshot) ∩ (empty now) ∩ (not exempt), the snapshot retaken on record-id / toggle change (PR-B1 round 2 correction, 2026-09-05 — an earlier draft read "hidden set snapshotted on record-id / toggle change", which round 1 implemented as snapshot-only and a just-filled field vanished on blur; PR-B1 round 3 precision, 2026-09-05 — the round-2 sentence "a field that gains a value … stays visible; a field that loses its value stays visible until the next snapshot" was over-general: only the snapshot decides, never the edit history). Precisely: a field NON-EMPTY at the snapshot is never hidden until the next snapshot (record change / toggle), whatever happens to its value meanwhile — so a value being cleared cannot vanish mid-edit; a field EMPTY at the snapshot is shown while it holds a value and re-hides when it is emptied again — immediately if it is not focused, on blur if it is (the focused-field exemption is evaluated live).
...
- **Field-anchored server errors** (graft from P2, PR-B2; **amended 2026-09-05, PR-B2 round 2; precedence stated 2026-09-05, round 3**): `useMultitableGrid.patchCell` RETURNS an additive per-call `GridPatchFailure | null` (status / code / server `fieldErrors` / message; `null` on success and on the local row-action refusal; rollback and `error.value` untouched; no shared "last failure" ref, so two in-flight patches never read each other's outcome). WB `onDrawerPatch` routes in this order (precedence made explicit 2026-09-05, PR-B2 round 3; `patch-failure-routing.ts` and the routing-matrix test already enforced it): `VERSION_CONFLICT` is checked **first** — it shows the existing conflict banner + a field marker **and keeps its toast**, and a conflict response that also carries `fieldErrors` routes there, never inline; **then** inline **iff** `fieldErrors` is non-empty **or** HTTP status is 422 **or** code is `VALIDATION_ERROR` — code-keyed, not status-keyed: a plain HTTP 400 with any other code (e.g. the record-lock refusal, 400 `FORBIDDEN`) keeps the toast; **then** all other codes keep the toast. Inline only when the inspector confirms the alert can render right now (exposed `canAnchorFieldError(recordId, fieldId)`: visible, showing that record, details tab active, field row rendered per the shared `visibleRecordFields` rule) and the anchored message is non-empty — otherwise the toast, so a rejection never fails silently (attachments-tab / header-title-on-another-tab / closed-mid-request / record-switched-mid-request all toast). FP keeps a per-field draft so the rejected value stays in the control, renders `role=alert` under it, sets `aria-invalid` + `aria-describedby`, clears on next successful patch of that field or record change; the conflict-clear watch removes only markers the conflict path wrote (origin-tagged), never a validation alert on the same field.
...
| Field-anchored PATCH errors (per-call `GridPatchFailure` return from `patchCell` + `fieldErrors` prop + `role=alert`/`aria-invalid`/`aria-describedby`, mutation-backed test; round 2: code-keyed routing + render-gated inline) | P2 | Synthesis P2 is S1; `client.ts` already normalises `fieldErrors`, only `patchCell` drops them. None of P1's phases touched it. |
...
- Files (round 2): `composables/useMultitableGrid.ts` (`patchCell` returns `GridPatchFailure | null`), `utils/patch-failure-routing.ts` (`resolvePatchFailureRoute`, `fieldAnchoredPatchMessage`), `utils/recordDisplay.ts` (`visibleRecordFields`, shared by FP and INS), WB (`onDrawerPatch` routing, origin-tagged `inspectorFieldErrors`, `recordInspectorRef`), INS (`fieldErrors` prop, exposed `canAnchorFieldError`), FP (draft map, alert, aria wiring). Pre-check: read `packages/core-backend` `patchRecords` error shape for `fieldErrors` presence per type; state findings in PR body.
- Tests (round 2): composable — rejected `patchRecords` with `fieldErrors` RETURNS the failure AND still sets `error.value` and rolls back; two in-flight rejections settling in one flush each return their own; routing matrix — fieldErrors / 422 / `VALIDATION_ERROR` → field, 400 with any other code → toast; workbench — 422 with `fieldErrors` → no toast + `[data-test=drawer-field-error]` under that field; 400 `FORBIDDEN` and 403 still toast; `VERSION_CONFLICT` → conflict banner + field marker + toast; not-anchorable → toast and no entry; same-record/other-field failure → toast; concurrent rejections → no cross-talk; conflict raised+cleared on a field with a validation alert leaves it; empty message → toast; INS — `canAnchorFieldError` true only while visible, on that record, details tab active, field row rendered; FP — rejected value stays in the control, `aria-invalid="true"`, `aria-describedby` resolves to the alert, cleared after a successful patch of the same field and on record change, other fields unaffected.
- Mutations (round 2): return `null` from `patchCell`'s catch → the composable return-asserting tests red (nothing else); re-add a bare `status === 400` leg → matrix row + WB 400-`FORBIDDEN` test red; drop the anchorability check → the not-anchorable WB tests red; ignore `activeTab` (or the rendered-field set) in `canAnchorFieldError` → the INS predicate tests red; suppress the conflict toast → WB conflict test red; read the message from the shared `grid.error.value` → concurrency test red; drop the `fieldId` half of the guard → same-record/other-field test red; drop the origin check on conflict-clear → origin test red; drop the empty-message guard → empty-message test red.
...
11. **B2 error routing — acknowledgement (2c data flow, additive)**
    「知悉（2026-09-05 PR-B2 第二轮修订，取代第一轮措辞；2026-09-05 第三轮补记判定先后顺序，代码与路由矩阵测试原已如此执行）：`patchCell` 新增按调用返回的结构化拒绝 `GridPatchFailure | null`（回滚与 `error.value` 不变；成功与本地行权限拒绝返回 null；不再有共享的「最近一次失败」引用）；`onDrawerPatch` 按固定先后顺序判定：**先**查 `VERSION_CONFLICT`——显示既有冲突横幅 + 字段标记并保留其 toast，携带 `fieldErrors` 的冲突响应亦走此路、绝不内联；**再**查内联规则——仅在 `fieldErrors` 非空、或 HTTP 状态为 422、或错误码为 `VALIDATION_ERROR` 时改为字段内联提示——按错误码而非按状态：HTTP 400 携带其他错误码（如记录锁定的 400 `FORBIDDEN`）沿用 toast——且仅当检查器确认该字段告警此刻可渲染（`canAnchorFieldError`：可见、正显示该记录、details tab 激活、该字段行已渲染）且提示文本非空，否则回落 toast；**否则**其余错误码沿用 toast，记为行为变更。」
