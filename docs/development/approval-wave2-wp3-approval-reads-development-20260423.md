...
- 行为：单条原子 `INSERT ... SELECT id FROM approval_instances WHERE id=$2 ON CONFLICT (user_id, instance_id) DO UPDATE SET read_at = now()`，幂等，同时避免先查实例再插入读状态之间的 FK race。
- 响应：`{ ok: true }`。
...
- 行为：对当前 actor 的**活跃 pending assignments**（与 pending-count 同口径：`is_active=TRUE` + `status='pending'` + user/role 匹配）里尚未有 `approval_reads` 行的实例批量 insert。
- 实现：`INSERT ... SELECT DISTINCT a.instance_id FROM approval_assignments a INNER JOIN approval_instances i ... LEFT JOIN approval_reads r ... WHERE r.instance_id IS NULL ON CONFLICT DO UPDATE ... RETURNING instance_id`，一次 round-trip，`DISTINCT` 去重角色 join 造成的同 instance_id 重复。
- 响应：`{ markedCount: N }`，N 是本次从未读转换为已读的 distinct instance_id 数量；已读行不重复刷新 `read_at`。
...
- 所以 handler 使用 `INSERT ... SELECT ... FROM approval_instances WHERE id=$1`；如果 `RETURNING` 为空，就返回 `{ ok:true, skipped:true, reason:'instance_not_materialized' }`，HTTP 仍然 200。
...
- `sourceSystem` 过滤变更时同时刷新列表与 pending-count，避免列表切换后角标和“全部标记已读”禁用态停留在上一个过滤器。
...
4. mark-all-read bulk insert unread-only targets → `markedCount: 2`（前一用例已标记 1 条），`unreadCount: 0`
