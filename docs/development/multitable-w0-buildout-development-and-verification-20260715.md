...
| **L4** canonical fence (slice) | fence module + main-writer wiring + honest gap-exposure goldens; coverage map gate-verified ACCURATE | opus | L3 seq ✅ | **◐ #4346 gate BLOCK 1 P2 → fixing** |
| **L4-cov** coverage completion | close partial (plugin-create/backfill/form-submit block-check) + unfenced families (automation/approval/formula/projection/derived-value/config-restore/presets) | opus | L4 | ⏸ (real remaining work) |
...
- **◐ L4 canonical fence — Draft #4346, gate BLOCK (1 P2, 0 P1), coverage map ACCURATE → fixing.** Delivered a legitimate **slice**: the canonical fence module (renamed auto-number key, one per-sheet fence), fence-before-check wiring on the main writer families (REST create/patch/delete/restore, bulk/AI/OAPI, plugin patch/recoverable-delete, attachment strip, HTTP lock/unlock, revert-execute, reset-execute), durable blocking states `{fencing,applying,paused_retryable}`, and **honest gap-exposure goldens** (GX1/GX2) for the writers it does NOT yet fully fence. The gate **independently swept every write site and confirmed the coverage map is accurate** — no writer silently missed. Mutations load-bearing (F2 patch-fence red; R1 fence-before-check race red with A parked = genuine TOCTOU); migrate clean; flag-off parity.
  - **NOT the finished "all-writer" fence.** `seq` is only causal for records written through the fence, so these remain **open holes** (strict/Revert stay gated until closed): **partial** (fence but no active-block check) plugin-create / backfill / form-submit; **unfenced** automation / approval-writeback / formula-recompute / projection / derived-value-recompute / config-restore / presets. → **L4-coverage** follow-up lane.
  - **🔨 Fixing now (gate P2):** reset-execute must `assertNoActiveWriterBlock` after the canonical fence (+ symmetric direction) so two destructive recoveries (reset vs revert) cannot interleave; correct the overclaiming comment; add a constructed reset-vs-revert race golden. Re-gate after.
