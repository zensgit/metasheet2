# 备料对接线 T3 弧收口 — 设计与验证记录（T3a → T3b → corrective-3/4/5/6/7）

日期：2026-07-16（2026-07-17 corrective-6/7 addendum）
...
| corrective-7 | corrective-6 实体 provisioning 失败的 values-free HTTP/错误类诊断扩展 | 本变更，待合并 |
...
- **corrective-6（#4410）**：corrective-5 的 smoke 五项本身全 PASS，但实体后验发现验收 token 被 PM2 继承。修复把选中 carrier 在 orchestration 入口捕获为 `SecureString` 并立即 scrub；PM2 restart 前检查 runner env，restart 后及 smoke 后分别检查 PM2 投影中的 token-presence boolean。corrective-6 实体结果证明旧 failure class 未复现（独立 hygiene PASS），但 smoke 在 `PROVISIONING` 首次失败，早退前 3/3 检查均失败；现有五字段只能给出 phase/count，不能区分 tenant、权限、404、API unavailable 或 5xx。
- **corrective-7（本变更）**：在不保留 response body/code/message 的前提下，把诊断面从五字段扩为七字段：新增 `firstFailedHttpStatus`（仅 `0` 或 `100..599`）和 `firstFailedErrorClass`（固定枚举 `NONE/NOT_HTTP/TENANT_CONTEXT/AUTHENTICATION/AUTHORIZATION/NOT_FOUND/API_UNAVAILABLE/CONFIG_INVALID/SERVER_ERROR/CLIENT_ERROR/RESPONSE_CONTRACT/UNKNOWN`）。smoke 只在 check 记录安全二元组，runner 再做独立 regex + allowlist 钳制；恶意/业务值只能降为 `0` 与 `NOT_RUN/UNKNOWN`。该变更只提高下一次授权执行的可判定性，不把 corrective-6 FAIL 解释成某个未经证明的根因，也不授权自动重跑。
...
- corrective-7 增量：smoke 单测 30/30（含完整 child smoke + 本地 HTTP 403 provisioning 早退链）；PowerShell contract 全 PASS（精确 17 字段）；behavior 57/57；Windows PowerShell 5.1 仍由 required CI arm 验证新增字段捕获与 allowlist 解析。
...
- #4101 corrective-6 结果为 reviewed FAIL：package/migration/PM2/health PASS；smoke `lastCompletedPhase=AUTH / firstFailedCheck=PROVISIONING / failedCheckCount=3`；独立 post-run credential hygiene PASS。该 1/1 结果不满足六条 PASS 判据，不得解释为可关闭或自行 rerun。

## 6. 冻结面与剩余门（如实边界）

- **实体机 reviewed FAIL，代码侧诊断扩展中**：corrective-6 不重跑；先合并 corrective-7、经 required Windows PowerShell 5.1 arm 与完整 CI 验证，再由 owner 另行决定是否切包及授权 1 次 stage-1 执行。本文不将实体机结果计为已达成。
...
T3 弧的主体代码已落 main：T3a（锁+runtime）、T3b-0/1a/1b/1c/2（锁 + persist 硬化 + 纯桥 + 结构守卫 + route 接线 + 真库 smoke）、T4-final（approved-source 前置 harness）、corrective-3/4/5/6。corrective-7 只补足实体失败的安全判定面；它不声称修复尚未分类的 provisioning 根因。剩余门为：corrective-7 合并/发包决策、owner 授权的 RC-0 终判、RC-A 单切（含 T4-final 活体执行）、P4 设计门。
