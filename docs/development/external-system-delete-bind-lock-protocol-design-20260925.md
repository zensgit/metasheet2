...
- 「必须是第一条」在需要它的场景由 PG 强制（第三轮终审订正：本条原写「任何查询之后再 SET，得 `25001` 并中止事务」，是绝对表述，按 PG 实际语义收窄如下）。PG 拒绝的是**改变**级别的晚到 SET：请求的级别不同于事务当前级别、且事务已执行过查询（取过快照）时报 `25001`；请求的级别与当前级别相同时不检查、静默接受。参与方经 `db.cjs` 发出的除这条 SET 外都是 SELECT / INSERT / UPDATE / DELETE（都取快照）。所以：**继承级别不是 READ COMMITTED 时**，任何查询之后再 SET 得 `25001` 并中止事务（后续语句 `25P02`）；**继承级别已是 READ COMMITTED 时**，晚到的 SET 为空操作，事务本来就在 RC 下运行。写错顺序的参与方因此不会在 RC 以外的级别上跑完——fail-closed 恰在需要它的场景（继承 RR / SERIALIZABLE）成立，PR 正文保证 11 与下表结论不变。本机 PG 16.10 实测（三个库，默认分别为 read committed / repeatable read / serializable）：`BEGIN; SELECT …; SET TRANSACTION ISOLATION LEVEL READ COMMITTED` 在后两个库得 `25001`、下一条语句 `25P02`，在第一个库成功、读回 `read committed`；晚到的 `SET … REPEATABLE READ` 只在默认 repeatable read 的库上成功；`LOCK TABLE`（不取快照）之后再 SET 在三个库都成功。
...
| 代价 | 每个参与事务多 1 次往返 | 同 |
| 缺方法 / 顺序写错 | 抛错拒绝 / 继承级别不是 RC 时 PG `25001` 中止（继承 RC 时晚到的 SET 为空操作，本来就在 RC 下运行）；都不会在 RC 以外的级别上跑完 | 抛错拒绝；位置不受数据库约束（检查放到后面照样读得出级别，只是之前的语句已按继承级别执行，随后整个事务被拒） |
...
- **插件内存套件**（`__tests__/external-systems-delete-bind-lock-protocol.test.cjs`，登记于 `test-chain.txt:40`，紧接同族 `:39`）：带行锁冲突表与缓冲事务的内存 db，跑 `integration-guard.yml` 的「Run plugin-integration-core CJS test chain (hermetic, no DB)」步骤（`pnpm --filter plugin-integration-core test`，`integration-guard.yml:535-538`，改动命中该 workflow 的路径时运行）；`plugin-tests.yml` 不跑这条插件链（第三轮终审订正：原文写成「与 `plugin-tests.yml`」，不成立——`plugin-tests.yml` 里没有任何 `plugin-integration-core` 调用）。覆盖 L-01…L-11、F-25P02、FC-01…FC-07、R-062-REUSE-A/B、R-073、M-D1/M-W079/M-W062/M-WPIPE 四个去锁变异 + M-FC-LOCK/M-FC-079/M-FC-062/M-FC-PIPE 四个「守卫降级为无锁读」变异（源文本 → `_compile`，不落盘）。第二轮复审补的：
...
## 11. 第三轮终审订正记录（2026-09-26，文档与注释，运行时零改动）

| # | 终审结论 | 处置 |
|---|---|---|
| blocking 1 | 「任何查询之后再 SET 得 `25001`」是绝对表述：默认 read committed 的库上 `BEGIN; SELECT 1; SET TRANSACTION ISOLATION LEVEL READ COMMITTED` 静默成功、读回 read committed；PG 只在新级别不同于事务当前级别时报错 | 先复现：本机 PG 16.10 三个库（默认 read committed / repeatable read / serializable），同一序列在后两个库 `25001` + 下一条 `25P02`、在第一个库成功；晚到的 `SET … REPEATABLE READ` 只在默认 repeatable read 的库上成功；`LOCK TABLE`（不取快照）之后的 SET 三个库都成功。**成立**。收窄为「继承级别不是 READ COMMITTED 时，任何查询之后再 SET 得 `25001` 并中止事务；继承级别已是 READ COMMITTED 时晚到的 SET 为空操作（本来就在 RC 下运行）——fail-closed 恰在需要它的场景成立」：第 2.7 节正文与取舍表、`lib/db.cjs` `setTransactionIsolationLevel` 注释（`pluginDb` pin 重算）、`lib/external-system-pointer-lock.cjs` 文件头两处与 `pinLockProtocolIsolation` 注释、`lib/external-systems.cjs` 删除方钉定处注释、PR 正文。三个 `.cjs` 的改动只在注释内且行数不变，本文其它 `path:line` 引用不动。内存协议套件的假件比 PG 严（对任何晚到 SET 抛 `25001`），在第 6 节与套件注释里写明；以内存改写把它改成与 PG 同义后整套照样通过、用例输出行逐行相同，关掉这条检查（`if (false)`）则 F-ISO 红——说明改写确实打到了那条检查，且没有用例依赖假件更严的那部分 |
| 顺手 1 | 第 6 节与 PR 正文说内存协议套件也跑 `plugin-tests.yml` | 不成立：`plugin-tests.yml` 里没有任何 `plugin-integration-core` 调用，插件链只在 `integration-guard.yml:535-538` 跑。已改 |
