...
**Applied at every view-config echo (7 real sites):** `GET /context` (3599), `GET /views` (5066), `GET /view` (6514), `GET /records/:recordId` (7498), `POST /views` (5155), `PATCH /views/:viewId` (5260), and `GET /form-context` (6648, **field-permission-aware**: anonymous/no-subject ⇒ empty set ⇒ fail-closed (all redacted); authenticated ⇒ the requester's allowed set — denied redacted, readable kept — via `loadAllowedFieldIds(…, access.userId, …)`). `/view`+`/records` reuse their existing `allowedFieldIds` (#2015/#2028); `/views`/create/update compute it per requester; `/context` computes it **off `effectiveSheetId`** (see below).
