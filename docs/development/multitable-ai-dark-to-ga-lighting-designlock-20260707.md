# 多维表 AI 字段 DARK → GA 点亮阶梯 · 设计锁（RATIFIED）

> 状态：**RATIFIED（owner directive 2026-07-11）**。本锁只定「怎么从 DARK 安全地分档点亮到 GA」的阶梯与闸门，**不翻任何生产 env、不发起任何 live 请求**。真正点亮的节奏、金额上限、canary 环境、指定测试 actor(s) 与 cap 由 owner 在每一档显式拍板。
