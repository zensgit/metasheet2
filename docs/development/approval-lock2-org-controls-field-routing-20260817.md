...
| D-3 | Coverage is derived, not enumerated | the detector's kind set AND the roster order array are each asserted by exact set equality against the registry-derived set | four mutations: drop a kind from the detector set, add an unrelated one, drop a kind from `SHIPPED_ASSIGNEE_SOURCE_KIND_ORDER`, and drop a registry row — each reds a named test; a count assertion is explicitly insufficient |
