...
- **RA-1b `requester.role`** (independent verification, 5 lenses, infra recovered): **0 P1.** One **P2** — the dry-run preview didn't thread `roles`, so `requester.role in [...]` conditions were unpreviewable — **fixed** (+ a role-preview rbac-boundary test). Quality **P3s** addressed: the fresh-`user_roles` resolver's normalization (dedup / trim / blank-strip / order) now has a direct unit test (was mocked everywhere); the `in` array literal rejects blank-string elements + gives a clearer "missing closing `]`" message. Cosmetic P3s **accepted as fail-closed-safe + documented**: a parenthesized role LHS (`(requester.role) in [..]`, semantically identical); a missing `user_roles` table surfaces as 503 on a degraded deployment (edge). Lenses covered: in-grammar (`in` scoped to role; strings-only/≤32/comma-separated; scalar `requester.role ==` type-rejected), trust-boundary (fresh `user_roles` not token; frozen; no spoof), fail-closed (reject-at-create + the keystone wedge-guard detection).

## 5. Verification log
- Unit **169/169** (`approval-condition-formula` + `approval-graph-executor` + `approval-product-service` + `approval-rbac-boundary` + `approval-requester-roles`) — title + RA-1b together.
