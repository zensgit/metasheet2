...
| 5 | `DINGTALK_GROUP_DELIVERY_RETENTION_DAYS` family | Confirm `_DAYS` meets this deployment's **group-delivery diagnostic/audit** retention need (there is **no** approval-SLA coupling — that was the DT-CLOSE-02 error; this sweeps `dingtalk_group_deliveries`, not approval cards). Confirm the sweep is not `_DISABLED=1` (i.e. actually running). A short window deletes GROUP-webhook delivery history (`dingtalk_group_deliveries`) — **not** approval cards (DT-CLOSE-02B correction; approval-card retention is unimplemented, see the gap note). | Raise `_DAYS` back up, or set `_DISABLED=1` to pause the sweep entirely if group-delivery rows are being deleted prematurely. `_MIN_DAYS`=7 is a hard floor regardless of misconfiguration, so a bad value cannot go to zero/negative. |
...
### 1.2 GAP (DT-CLOSE-02B): approval-card / person delivery retention is unimplemented

A repo-wide search found that **`dingtalk_approval_card_deliveries`** (the approval-card / person /
work-notification delivery ledger) has **no retention or cleanup sweep at all** — no `DELETE`, no
scheduler, no env control. `DINGTALK_DELIVERY_RETENTION_*` does not exist anywhere in the codebase.
The only DingTalk delivery retention is `DINGTALK_GROUP_DELIVERY_RETENTION_*`, which sweeps the
**group webhook** ledger (`dingtalk_group_deliveries`), a different table.

Consequence: the approval-card delivery ledger grows **unbounded** over time (the same failure mode
`DINGTALK_GROUP_DELIVERY_RETENTION_*` was built to prevent for the group ledger). This is a real gap,
not a switch — it cannot be "owned" as a default-off toggle because the toggle does not exist.

**Disposition:** file as its own follow-up ticket — implement a card/person delivery retention sweep
mirroring the group-retention pattern (days / disabled / interval / leader-lock env family + a
partial `(created_at)`-predicated batch sweep + a two-point-wired real-DB test), then add its env
family to both deploy templates and the DT-CLOSE-02 contract test. **Not** folded into this closeout
(it is new feature work, not a documentation correction). Until then, monitor `dingtalk_approval_card_deliveries`
row count.
