...
## 附录 F — 剩余工作的规划、排期与模型分配(2026-08-05,加固轮之后)

> §4 的模型分配规则覆盖的是**已完成**的 S1–S7 切片。本附录把同一套规则应用到**剩余**工作,
> 使"按开发难度选模型"对未完成部分同样可查。人天为**净开发**,不含等待授权/运维排期。

### F.1 剩余工作分解(按依赖序)

| # | 工作 | 判据(如何算完成) | 人天 | **模型** | 依据(§4 规则) |
|---|---|---|---|---|---|
| **R1** | #4769 加固轮收敛:通过 exact-head 对抗复审(当前第五轮) | verdict CLEAN **或** 剩余 findings 全为 P3/NIT 且逐条有处置 | 0.5–2.0(不可预估:取决于是否出现第五条轴) | **opus** | 交付物是**判据**(利用能否构造),非实现 |
| **R2** | #4768 预演 lane rebase 消费真门 + 复审 | 驱动器在新门下仍走通七步;exact-head 复审通过 | 0.5 | **opus**(闸)+ sonnet(夹具适配) | 门的正确性=判据;夹具=机械 |
| **R3** | 预演首跑至全绿 | run 全绿,含线外 save-only 正控与 token 单次性 | 0.5–1.5 | **opus** | 失败诊断=判据类;历史上首跑必红 |
| **R4** | 文档同步:窗口 runbook 反映加固后姿态 | runbook 六步与实际路由/错误码逐字一致;PASS 判据的每条不变量注明载体 | 0.25 | **fable** | 形状已固定,值由前序产出 |
| **R5** | B4 mint 在目标环境执行 + 三元组记录 | store 铸出 approvedConfigVersion;三元组入 provenance | 0.25 | sonnet | 判据已冻结(合同模块),此处是机械执行 |
| **R6** | 最终包构建 + 矩阵验证 | lane 全绿 @ 最终 SHA;五字段 provenance 齐全 | 0.25 | sonnet | 跑既有 lane |
| **R7** | 实体机窗口执行 + §8 验收记录 | 六步 PASS;`0 Submit/0 Audit`;回读值校验 | 0.5 | **opus**(现场判据)+ fable(成文) | 窗口不可重试,判据必须最强 |

**净开发合计 2.75–5.25 人天**(R1/R3 的区间即不确定性所在)。**日历时间由授权与运维排期决定,非开发量决定。**
...
R1(加固收敛,关键路径)
 ├─→ R2 → R3 ──┐
 └─────────────┴─→ R4 ─→ [owner: 无需再裁] ─→ R5(运维)→ R6 → R7(窗口)
                                    ↑
                        #4628 三授权位翻正(运维,与 R1–R4 并行)
```

**唯一关键路径是 R1。** R5–R7 的前置是运维授权,与 R1–R4 天然并行 —— 不应串行等待。

### F.3 模型分配的实证依据(本轮新增证据)

§4 的规则「判据类→opus」在本加固轮得到**四次独立验证**:四条 P1 全部由 opus 对抗复审代理
构造出**可执行利用**发现,而它们此前都通过了作者自测 + mutation + CI。相对地,
sonnet 承担的四次套件语义变换(BOM 迁移、直插承接、profile 武装、preview 适配)**零缺陷**,
且其中一次(preview 适配)代理**自己发现**了一个作者未察觉的产品缺陷。

**结论**:规则不变,并补一条 —— **凡"某一类已关闭"的声明,必须由 opus 级独立审计尝试证伪后才可发布**。
本线四轮的记录是:作者每轮都相信自己关闭了该类,四次都错。

### F.4 不确定性(诚实标注)

- **R1 的区间无法收窄**:第五条轴是否存在,只能由复审回答。前四轮每一轮都出现了新轴。
- **R3 首跑**:按本线记录(R9 十三次 dispatch、验证 lane 首跑抓真缺陷),预演首跑大概率红。
- **R7 不可重试**:窗口一次性,故其判据取最强档(opus),且恢复路径已定为「重走 dry-run 重新批准」。

### F.5 勘误与实测回填(2026-08-05,加固轮结束后)

**先撤回一条我自己的断言。** 我在开这条文档 PR 时写道「附录 F 只存在于对话、从未提交,
所以『按难度选模型』没有可查工件」。**该断言为假** —— 本附录早已随 #4769 落在 main,
F.1–F.4 齐备。我断言「不存在」时没有对 `origin/main` 复核,这正是本线反复记录的失误形态
(见 `feedback_empty_read_is_not_absence`、`feedback_verify_against_current_main_not_stale_base`)。
重复的一份已删除;以下只补**事件已经改变的事实**。

**F.1 表格中被事件覆盖的两行**:

| # | F.1 原记 | 2026-08-05 实况 |
|---|---|---|
| R1 | 「当前第五轮」,0.5–2.0 人天 | **共十一轮**;#4769 已 **MERGED**(main `65edb98c6`,9/9 required 绿)。**实耗约 1.5 人天,约为原估上限的 3 倍** |
| R2 | 「驱动器在新门下仍走通七步」0.5 人天 | rebase 干净,但 exact-head 复审 **CHANGES-REQUESTED**:驱动器把 K3 当 pipeline **source**,而任何 K3 配置都不能充当 C6 source ⇒ 步骤 3–9 不可达。**改哪个 source 是 owner 裁决**,见交付 MD §7.3 |

**F.3 的实证样本可以收紧了**(原文写的是加固轮进行中的中间值):

| 观察 | 终值 |
|---|---|
| 同一缺陷类逃逸次数 | **8**(字段→跨文件→跨 profile 条件→检查早于归一化→`?`/`#`→单点段→段内尾点→匹配锚点) |
| 「守卫被测但**接线**未被测」 | **4 次**(healthPath、wire choke point、B4 scope、apply 侧) |
| 独立审发现中「过强声明」占比 | 多数;含三次 commit/注释与代码不一致、一次 retraction 只写在 commit 未写进源码、一次正控判的是逻辑的**副本** |
| sonnet/fable 切片回归 | **0** |

**F.4 应补的一条估算教训**:R1 超估三倍的原因不是实现难,是**同一安全类有八条逃逸轴**。
⇒ 跨安全类的加固,**按「逃逸轴条数」估,不按 diff 行数估**;而轴数在开工时不可知,
所以这类工作的正确表述是**区间加不确定性来源**,不是点估。
