...
A later exact-head Node 20 run for draft #4890 reproduced the same post-suite `57P01` class in the W4C-3a canonical-import real-DB suite, whose two pools were not yet using the shared drain/drop discipline. Issue #4820 was therefore reopened. Draft [#4928](https://github.com/zensgit/metasheet2/pull/4928), exact head `42ab0e64c5a86004f9395726f61ccb88228d8cfc`, closes that additional cleanup path; its true-PostgreSQL battery is 25/25, the cleanup mutation fails as intended, and exact-head CI settled at 17 successes plus one designed skip with no failure or pending check. The issue remains **OPEN** until that draft is reviewed and merged. Future runtime evidence must use lane-owned databases and every scratch suite must drain all pools before dropping its database.
