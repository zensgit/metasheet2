...
2. **router 级也没有角色门。**
   `admin-routes.ts` 全文只有两处 `router.use`（修前 `:2021`、`:2022`；修后（2026-09-21 追平 main 后）`:2386`、`:2387`），都是挂子路由
...
- **它是可被自己关掉的。** 确认层的开关本身若只由确认层把守，就构成自引用：一个 LOW 操作能把整层拆掉。
  授权门必须在确认层**之外、之前**。
- **`allowBypass` 是死配置。** `initAdminRoutes` 在 `:2407` 传
...
（沿用文件已有的 `import { ..., requireAdminRole, ... } from '../guards'`，与 `:172`、`:471` 等
...
|---|---|---|---|---|---|---|
| `:221` | POST | `/safety/enable` | —（无） | — | **零中间件** | 200 直接开 |
| `:244` | POST | `/safety/disable` | `RESET_METRICS` | LOW | 仅确认层 | **200 直接关掉整层** |
| `:1158` | POST | `/cache/clear` | `CLEAR_CACHE` | MEDIUM | 仅确认层 | 403 + 令牌（一次重试即可过） |
| `:1214` | POST | `/metrics/reset` | `RESET_METRICS` | LOW | 仅确认层 | **200 直接执行** |
| `:1387` | DELETE | `/data/bulk` | `DELETE_DATA` | HIGH | 仅确认层 | 403 + 令牌 |
| `:1506` | PUT | `/data/bulk` | `BULK_UPDATE` | HIGH | 仅确认层 | 403 + 令牌 |
| `:1721` | POST | `/dlq/:id/retry` | `BULK_UPDATE` | HIGH | 仅确认层 | 403 + 令牌 |
| `:1753` | DELETE | `/dlq/:id` | `DELETE_DATA` | HIGH | 仅确认层 | 403 + 令牌 |
| `:1942` | POST | `/dlq/retry-all` | `BULK_UPDATE` | HIGH | 仅确认层 | 403 + 令牌 |
| `:1994` | POST | `/dlq/cleanup` | `DELETE_DATA` | HIGH | 仅确认层 | 403 + 令牌 |
| `:2155` | POST | `/ratelimits/:key/reset` | `RESET_METRICS` | LOW | 仅确认层 | **200 直接执行** |
| `:2194` | POST | `/ratelimits/reset-all` | `RESET_METRICS` | LOW | 仅确认层 | **200 直接执行** |
...
- **读端点，一律不动**：`GET /safety/status`(`:159`)、`GET /slo/status`(`:1646`)、`GET /dlq`(`:1693`)、
  `GET /shards`(`:1795`)、`GET /shards/:name`(`:1844`)、`GET /queues`(`:1895`)、`GET /ratelimits`(`:2043`)、
  `GET /ratelimits/:key`(`:2112`)、`GET /health/detailed|summary|subsystem/:name`(`:2241` / `:2279` / `:2316`)。
...
- **`POST /health/check`（`:2347`）**：不带 `requireSafetyCheck`、也不带 admin 门。它只调用
  `healthAggregator.checkHealth()` 取一次健康快照并返回摘要，不写任何状态——按「写/破坏性」判定
  不属于本次范围，**未加门**。（它仍是一个**任意已认证用户**可触发的探测面——全局 JWT 门挡匿名、不鉴角色，归入残余。）
- **`POST /plugins/reload-all-unsafe`(`:834`)、`POST /plugins/:id/reload-unsafe`(`:885`)**：
...
- **同挂载面的子路由 `/safety/rules`（`admin-routes.ts:2387` 挂 `protection-rules.ts`）**：**本段的「零授权门」判断已于 2026-09-20 rebase 时作废，见 §5.3**。
...
- **已经有门的**：`/safety/confirm`(`:172`)、`/plugins/*`(`:471 :484 :554 :597 :631 :665 :679`)、
  `/yjs/status`(`:1663`)，以及走 `protectAdminOperation` 的 `/plugins/:id/reload`、
...
- **「admin」的口径 = `user_roles` 里有 `admin` 行**（§1.2，`rbac/service.ts:19-23`）。`users.role='admin'`、`is_admin`
  ...
  另 6 条修前就被 `/safety/confirm`(`:172`) 的同一口径挡住，无变化。目标部署若存在只有 `users.role`
...
- `POST /safety/confirm` 那条链一个字没动（它在 `:172` 早就有 `requireAdminRole()`）。
...
3. **`data_sources` 仍留在 `validTables` 里**（`:1408` / `:1528` 两份）。是否把它（以及
   `users` / `protection_rules` 等）摘出白名单是 issue **#5655 待用户裁决**的另一个问题，本次不动。
4. **`POST /health/check`（`:2347`）无角色门**，任意已认证用户可触发的健康探测/放大面（读，不写；匿名被全局 JWT 门挡住）。
...
7. ~~**`/safety/rules` 子路由四条写端点零授权门、身份取自请求头**~~ —— **2026-09-20 作废**：#5667 / PR #5677 已合，
   四条首位都是 `requireAdminRole()`（见 §5.3）。同段的「12 条无门 GET」也已**全部**收口：`GET /dlq`（`:1693`）与
   protection-rules 的两条 GET 由 #5710 补门，`GET /slo/status`（`:1646`）由 #5914 补门，`/safety/status`（`:159`）、
   `/yjs/status`（`:1663`）本就有门。2026-09-20 实读：根 GET 无门数 = 0，详见 §5.4。
6. **`allowBypass` 仍是死配置**，留在 `types.ts` / `SafetyGuard.ts` / `initAdminRoutes` 里没人读。
   它今天无害，但是一个会误导读者（以为测试环境会绕过）的悬挂旋钮。

---

## 5. 2026-09-20 / 09-21：追平 main 与复核

本 PR（#5665）自 2026-09-12 起挂在 `main` 上未合，其间 `admin-routes.ts` 被 #5710 / #5884 / #5886 /
#5897 等若干 PR 改动，行号整体下移；09-20 复核当天 main 又并入 #5914 / #5916，09-21 再并入 #5903
（读侧 GET 的 500 分支脱敏，只动 `catch` 体，不动任何路由注册行）。本节记录追平事实、行号订正口径，
以及**自 09-12 以来事实发生变化、导致上文某些判断作废**的地方。上文 §1–§4 的正文行号已就地订正为
**追平 `origin/main` @ `5edf4c3e1` 之后**的行号；显式标着「修前」的行号是 09-12 基线（`1e98d9d3f`）上的
历史值，刻意保留不动。

### 5.1 追平事实

- 起点：`fix/admin-safety-toggle-and-bulk-require-admin` @ `3c79b2059`（09-12 的 PR head）。
- 09-20：`git rebase origin/main`（`36d659c8a` → 期间 main 又并入 #5914 / #5916，再 rebase 到
  `ce9ac29cb`）两次都**无冲突**，三个 commit（代码 / 文档 / 文档收口）原样重放。
- 09-21：main 走到 `5edf4c3e1`（其间只有 #5903 动过 `admin-routes.ts`），这一次用
  `git merge origin/main` 追平（分支已推过，不再 rebase 重写已推历史），**无冲突**。
- 代码 diff 与 `3c79b2059` 逐字相同：`git diff --stat origin/main HEAD` 在三次追平前后都是
  `admin-routes.ts | 83 ++-`，没有任何一行因 rebase / merge 被改写。
- #5903 只改读侧 GET 的 `catch` 体（改调 `sendAdminReadFailure`），并新增一段模块级辅助函数，
  与本 PR 的 12 处门**零重叠**；它让 `:73` 以下的行号整体 +58、又在若干 `catch` 体里各减几行，
  所以本节以下行号全部按 `5edf4c3e1` 重新实读。
...
本 PR 新增的 12 处门在 §2 的表里逐条列出了追平 `5edf4c3e1` 之后的行号。

同一口径对着 `origin/main`（`5edf4c3e1`）自己跑一遍作为**修前对照**：25 条写路由里只有
...
`x-user-id` 请求头。**2026-09-20 / 09-21 两次实读，这个洞已经补上了**（`protection-rules.ts` 自 09-20 起未再变动）：
...
### 5.4 2026-09-21 实读（`5edf4c3e1` 底座）：仍然无门的端点（本 PR 依然不碰）

**写侧 —— 3 条，全部是 §2.1 已登记的永久豁免**（闭世界用例的豁免表就是这三条，逐条带理由）：

| 行 | 方法 | 路径 | 为什么不加中间件门 |
|---|---|---|---|
| `:2347` | POST | `/health/check` | 只读探针，不写任何状态（§2.1）；「任意已认证用户可触发的探测面」记在 §4 第 4 条 |
| `:834` | POST | `/plugins/reload-all-unsafe` | 自带 in-handler 双门（`ALLOW_UNSAFE_ADMIN` + token 上的 `roles`）；口径不一致记在 §4 第 5 条 |
| `:885` | POST | `/plugins/:id/reload-unsafe` | 同上 |

**读侧 —— 归零**。§2.1 当时列的那一族无门 GET（`/dlq`、`/shards*`、`/queues`、`/ratelimits*`、
`/health/*`、`/safety/status`）已由 #5710 / #5897 补门；最后一条 `GET /slo/status` 由 #5914 补上
（`:1646`），同一个 PR 还给子路由 `/snapshots` 补了门。实测 16 条根 GET 全部有门，无门数 = 0。
本 PR 一个读端点都没碰。
...
- **载荷性实测**（验证文档 §8.3.1）：12 条门**逐条**在内存里换成 passthrough 后，
  这条闭世界用例**本身**每次都红（不是只有探针里的复制品会红），且每次只红它与该路由对应的
  点名用例（`/safety/disable`、`PUT /data/bulk` 另外还红它们本来就被覆盖的整链 / fail-closed 用例），
  其余 21–23 条全绿。既有载荷，又不过度耦合。

**范围仍然只到根路由**，不含 `router.use('/snapshots', ...)` / `router.use('/safety/rules', ...)`
（`:2386`、`:2387`）两个子路由挂载点。整棵树（含子路由、含「临时豁免只提示不拦」机制）的结构性
...
| 3 | `data_sources` 仍在 `validTables`（`:1408` / `:1528`） | 仍在，待裁决 |
| 4 | `POST /health/check` 无角色门 | 仍在（`:2347`） |
| 5 | 两条 `*-unsafe` 的 admin 口径与 `requireAdminRole()` 不一致 | 仍在（`:834` / `:885`） |
| 6 | `allowBypass` 是死配置（`:2407` 赋值，无人读） | 仍在 |
