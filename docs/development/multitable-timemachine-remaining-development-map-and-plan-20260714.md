...
**Date:** 2026-07-14 · **Status:** ASSESSMENT (rev 2 — owner 6th-round corrections: #4269 closed only reset-side race not all-writer; revert is per-record non-atomic; prod-flags-off is code+repo not host-proven; L8 Revert-atomicity is must-before-enable not Option-A-only; #4273 measurement re-runs POST-W0) (answers "还有哪些开发量" — grounded on `origin/main`, not PR titles) · **Method:** a 4-agent survey (landed-runtime inventory ‖ open-PR triage ‖ unbuilt-gaps) + Opus synthesis, every capability checked against `origin/main` file:line or a merged PR#.

> **Headline (corrected against `main`):** the Time Machine is **much further along than the open drafts suggest** — the read/PIT/revert/reset/undelete/trash/config-history surface and the full revision-emission substrate are **landed** (behind default-off flags). What is **not** yet built (to safely enable the *existing* destructive capabilities) is the **W0 trustworthiness correction** AND **Revert-execute outer-txn atomicity** on top of the merged-but-defective #4269 precheck: causal `seq`, all-writer fence, trust baseline, deleted-chain enumeration, non-forgeable identity. Until that lands, the destructive-recovery flags must stay off (they are). The remaining line is **~20–32 dev person-weeks** (Option-B granular-async path; **+6–10 pw** if Option-A atomic base-wide), but the **real critical path is owner ratify + benchmark-gated cycles, not raw pw** — Phases 0–1 gate everything and are mostly already built.
...
| #4269 generation-aware contiguity precheck (replaces live-vs-latest §0.6) — closes the **healed-gap read**; the check→write race is closed **only on the reset side** (reset's in-txn re-check), NOT the all-writer race | ⚠️ partial | merged `3356a7ed6`; `:163/258`, generation/marker `:197-236`. **All UPDATE/DELETE/plugin/automation/lock writers remain unfenced (§2 P1-2); the merged precheck is also the defective one (§2)** |
...
| revert preview+execute · reset preview+execute (signed identity, size ceiling, drift guard) — **reset-execute is all-or-nothing (one txn); revert-execute is per-record BEST-EFFORT (each field-revert its own txn), NOT atomic** | ✅ present / ⚠️ revert non-atomic | revert `:10161/10204` (per-record loop `:10140+`), reset `:10383/10425` (single txn) |
...
**Every destructive flag is CODE-default-off and NOT enabled in repo config** (`SHEET_REVERT` / `PIT_RESET` / `PIT_UNDELETE`) — but *code-default-off is not host evidence*; whether the running hosts have them off is unproven until #4283 produces the host-side containment evidence. Read-only previews are ungated (revert-preview) or gated (reset-preview). This is the correct posture until W0 trust closes.
...
| **L8** | **Revert-execute outer-txn atomicity** — wrap the best-effort per-record field-revert loop (`univer-meta.ts:10140+`) in one transaction | opus | **MUST before enabling `SHEET_REVERT` at all** (revert is non-atomic today; NOT Option-A-only) | 2–4 | loop-atomicity, partial-failure |
...
- **Phase 1 — safety gates (MUST precede any W0-1 runtime), parallel:** **L1 (#4283 containment) ‖ L2-tool (#4273 harness fixes)**. L1 is the host-side containment leg #4262 §0 requires. **Correction (owner): the #4273 harness FIX is the gate, but its MEASUREMENT is not a W0 prerequisite** — the current benchmark ran pre-#4269 (old precheck, old order-key), so its numbers must be **re-taken AFTER W0-1 lands** (moved to Phase 3-verify, §Phase 3). Ordering the benchmark *before* W0 was a logical inversion.
...
- **Phase 2b — before ANY Revert enable:** **L8 revert outer-txn atomicity** (revert is per-record best-effort today; unsafe to enable regardless of A/B). Then re-run the #4273 benchmark on the W0-corrected main (≥3× 5k execute + a same-write-shape/different-txn-boundary control) — the real measurement.
- **Phase 3 — recovery scale (after W0 trusted + re-measured benchmark + #4224/#4274 ratified):** **L6 ‖ L7**, but they **must reconcile ownership of the async engine** (build it once — #4274 Option-B and #4224 async overlap).
...
**To safely enable the EXISTING destructive capabilities (not extensions): at minimum W0-1 trust bundle 3–5 pw + Revert outer-txn atomicity 2–4 pw** — i.e. "the only unbuilt piece is W0" is inaccurate; Revert-atomicity is also a must-before-enable. **Full remaining line ~20–32 dev pw** (Option-B; +6–10 Option-A): safety-gate tool fixes ~3–5 (mostly built) · W0-1 bundle 3–5 · **Revert-atomicity 2–4 (must-before-Revert)** · base-wide+retention+async 7–12 (shared surface) · T-state 4–6 · edge 2–4 · O-2 ~0.
