...
| **D-10（三次订正）** | **原始 user-ID 渲染残留 — 二订正所记 3 处新址全部处置完毕（class 本身不宣告闭合，理由见状态列）**。二订正记「3 处新址」：`TemplateDetailView.vue:140`/`:340` + `ApprovalNewView.vue:956`。本次三订正轮在**新分支**（`claude/approval-rawid-choice-and-mechanical-census-20260819`，从 fresh `origin/main` 切出，HEAD 含 #5016 `6ae6304f17`）逐处机械重核：**`TemplateDetailView.vue:140`/`:340` 两处早已闭合**——不是本轮改的，是 #5016 本身就已把它们改成 `{{ visibilityScopeIdsDisplay(template.visibilityScope) }}` / `{{ legacyAssigneeIdsDisplay((node.config as any).assigneeType, (node.config as any).assigneeIds) }}`，无 `.ids.join`/`assigneeIds?.join` 残留；**二订正把这两处记为「仍 OPEN」是对旧 head（`62dbc69c76`）的核实，未随 #5016 落地而更新，属过期记录，非本轮引入的新事实**。第三处 `ApprovalNewView.vue:956`（`choiceOptionLabel()` 的自选下拉 `option.name?.trim() \|\| option.id`——requester_choice 提交时选审批人 picker，REQUESTER 可见，非管理员限定）**本次订正轮在本分支修复**：改为与 `ApprovalUserPicker.vue` `optionLabel`/`isUnidentifiable` 同构的 `成员 N` 序数回退 + 不可辨识选项 disable（`isChoiceOptionUnidentifiable`），并新增提交时守卫（`firstUnidentifiableChoiceNode`，镜像减签 disable+submit-guard 姿态）——挂载态判别负例+正例见 `apps/web/tests/approvalNewView.spec.ts`「Lock-1 §K2」块，机械 mutation 已验证两层测试（挂载负例 + 下方枚举守卫）均会因 revert 到 `\|\| option.id` 而转红。**同轮另发现一处二订正未列举的相邻缺口**（`apps/web/src/views/approval/ApprovalMetricsView.vue:167`，按发起人汇总——后端 `keyExpr = requester_snapshot->>'id'`，`name` 为空时旧代码回退到该原始 requester user id；admin-gated 但仍是原始内部 id 触达渲染）：已在本分支同修，回退为 `发起人 N` 序数（`requesterDisplayLabel`）。**枚举守卫本身重建**：`apps/web/tests/approval-member-identity-coverage-enumeration.spec.ts` 从手写 SITES 清单重建为源码派生的机械 pattern 普查（`readdirSync` 遍历 `src/approvals/**` + `src/views/approval/**`，6 类正则 pattern 各配真实 positive-control + allowlist 过期检测 + 非空扫描断言 + scope-leak sweep），未登记的新命中一律转红——这正是能在下一次同类回归时机械抓住、而非依赖人工记起来登记的机制。**成员 N 身份不可辨识残留**（二订正记的「另一处」，原文：无生产者写 `assigneeName`，三处标签在生产环境永远是序数，管理员无法辨识「成员 3」是谁，存在撤错审批人风险）：二订正的诊断是**两个不同机制各管一半**，不得混为一谈——「能否辨识」由 #5016 的 `getResolvedUserName`（`/api/approvals/directory/resolve` 批量解析，命中真实姓名的活跃用户返回真名，非序数）负责；「不可辨识时能否被选中/操作」由 disable-when-unidentifiable 负责。二订正指出的缺口是前者：核实当前分支 G1/G3（`assignmentDisplayLabel`/`reducibleAssignees`）等站点均已是 `resolvedName || 成员 N`（resolver 优先，序数只是兜底），并非「回退到序数即视为已解决」——只要目录里该 id 有真实姓名，三处标签会显示真名，非恒为序数；G3/G4/G5 的 disable-when-unidentifiable 是**另一半**（防止对不可辨识者动作），两者合起来才是完整闭合，本轮未见回归。 | ✅ **已在本分支修复并验证**（choiceOptionLabel + ApprovalMetricsView 按发起人汇总 + 枚举守卫重建）；TemplateDetailView 两处系确认 #5016 早已闭合，非本轮新增；PR 详情见对应 PR body。**范围限定**：本次核实只覆盖 `src/approvals/**` + `src/views/approval/**` 两棵树内本 census 6 类 pattern 命中，不等于「所有原始 id 渲染问题」的全域穷尽；下次同类回归的第一道防线是上述枚举守卫，而非人工记忆。 |
...
- **已执行/已 ratify 批次（45 个 PR，见 §0 锚点表「已合并审批切片」行）内的实现切片全部落地**（K6 #4993 = `ffa3a5f595` 为最后一个能力切片；#5004 `6ace2e5a01` 为 test-only residual-hardening）；**该批次内无一个在飞行的实现切片，无未 discharge 的可执行硬化项**。**⚠️ 这不等于「剩余项全部 owner 专属」——先前版本的这句话与 §6.3 / §6.5 / §7.2 自相矛盾，已订正。** 完整 CORE-PARITY 仍需要**尚未开发的代码切片**，至少包括（逐条缺席证据见 §6.3，非穷举列表）：
  ...
  **原始 user-ID 渲染残留** 曾在此列（§6.4 D-10 二订正记「3 处新址」）——D-10 三订正（本报告同一节）核实其中 `TemplateDetailView.vue:140`/`:340` 已由 #5016 闭合、第三处 `ApprovalNewView.vue:956` 已在 `claude/approval-rawid-choice-and-mechanical-census-20260819` 分支修复并配上源码派生的机械枚举守卫，**移出本清单**；范围限定见 D-10 三订正条目本身。

  **上述四项都是代码工作，不是 owner 专属项。** owner 专属项单独集中于第 7 节，两者不得混为一谈。
