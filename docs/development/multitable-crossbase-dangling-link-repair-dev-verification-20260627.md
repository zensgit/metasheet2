...
- **(ii) sheet-delete cascade (companion)** — `DELETE /sheets/:sheetId` now runs, in **one transaction**: `DELETE FROM meta_links WHERE foreign_record_id IN (SELECT id FROM meta_records WHERE sheet_id = $1)` **then** `DELETE FROM meta_sheets …` (no half delete-edges / delete-sheet). This **closes the normal sheet-delete leak**, but is not a hard guarantee: `foreign_record_id` has no FK and the cleanup does not lock the target records, so under strict concurrency a new inbound edge could be inserted between the cleanup and the cascade delete. **repair-on-read (i) remains the definitive safety net** for legacy / direct-SQL / race residue — no user-visible ghost regardless.
